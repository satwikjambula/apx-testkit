 ...
 * metadata: apex-ux-pattern-catalog, catalog-information, for-apex-developers, patterns, reusable-apex-ux-patterns
...
const pageUrl = () => `${APP_BASE.replace(/\/+$/, '')}/home`;
const norm = (s: string) =>
...
test.describe('page 1: Home', () => {
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    await gotoPage(page);
    expect(norm(await page.title())).toBe(norm('Oracle APEX UX Pattern Catalog'));
  });
});
