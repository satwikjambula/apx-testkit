 ...
 * metadata: interactive-report, interactive-report-2
...
const pageUrl = () => `${APP_BASE.replace(/\/+$/, '')}/browse-interactive-report`;
const norm = (s: string) =>
...
test.describe('page 230: Browse – Interactive Report', () => {
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    await gotoPage(page);
    expect(norm(await page.title())).toBe(norm('Browse – Interactive Report'));
  });
});
