 ...
 * metadata: activity-2, details, details-2, item-header, item-summary-details, key-facts, metric-cards, status-list, summary-container
...
const pageUrl = () => `${APP_BASE.replace(/\/+$/, '')}/item-detail-summary`;
const norm = (s: string) =>
...
test.describe('page 310: Item Detail – Summary', () => {
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    await gotoPage(page);
    expect(norm(await page.title())).toBe(norm('Item Detail – Summary'));
  });
});
