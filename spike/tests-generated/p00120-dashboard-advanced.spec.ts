 ...
 * metadata: chart-1, chart-2, chart-3, chart-4, chart-4_1, content-row, content-row-2, dashboard-title, metric-cards, metric-cards_1, stock-chart, time-series
...
const pageUrl = () => `${APP_BASE.replace(/\/+$/, '')}/dashboard-advanced`;
const norm = (s: string) =>
...
test.describe('page 120: Dashboard – Advanced', () => {
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    await gotoPage(page);
    expect(norm(await page.title())).toBe(norm('Dashboard – Advanced'));
  });
});
