 ...
 * metadata: apex-ux-pattern-catalog, patterns, reusable-apex-ux-patterns
...
const pageUrl = () => `${APP_BASE.replace(/\/+$/, '')}/patterns`;
const norm = (s: string) =>
...
test.describe('page 2: Patterns', () => {
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    await gotoPage(page);
    expect(norm(await page.title())).toBe(norm('Patterns'));
  });
});
