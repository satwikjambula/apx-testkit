 ...
 * metadata: apex-ux-pattern-catalog, catalog-updates, pattern-categories, reusable-apex-ux-patterns
...
const pageUrl = () => `${APP_BASE.replace(/\/+$/, '')}/reference`;
const norm = (s: string) =>
...
test.describe('page 3: Reference', () => {
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    await gotoPage(page);
    expect(norm(await page.title())).toBe(norm('Reference'));
  });
});
