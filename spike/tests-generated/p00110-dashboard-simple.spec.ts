 ...
 * metadata: chart-1, chart-2, chart-3, content-row, content-row-2, dashboard-title, metric-cards, time-series
...
const pageUrl = () => `${APP_BASE.replace(/\/+$/, '')}/dashboard-simple`;
const norm = (s: string) =>
...
test.describe('page 110: Dashboard – Simple', () => {
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    await gotoPage(page);
    expect(norm(await page.title())).toBe(norm('Dashboard – Simple'));
  });
});
