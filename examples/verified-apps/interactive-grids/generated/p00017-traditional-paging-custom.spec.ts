/**
 * GENERATED by apx-testgen from pages/p00017-traditional-paging-custom.apx — DO NOT EDIT.
 ...
 * (./p00017-traditional-paging-custom.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, overview-traditional-paging-custom, traditional-paging-custom
 ...
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), overview-traditional-paging-custom (staticContent).
...
import { TraditionalPagingCustomPage } from './p00017-traditional-paging-custom.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 17: Traditional Paging (Custom) [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new TraditionalPagingCustomPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new TraditionalPagingCustomPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Traditional Paging (Custom)'));
  });
