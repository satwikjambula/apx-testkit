/**
 * GENERATED by apx-testgen from pages/p00034-form-with-grid.apx — DO NOT EDIT.
 ...
 * (./p00034-form-with-grid.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, change-control, employees, overview-form-with-grid
 ...
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), change-control (staticContent), overview-form-with-grid (staticContent).
...
import { FormWithGridPage } from './p00034-form-with-grid.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 34: Form with Grid [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new FormWithGridPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new FormWithGridPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Form with Grid'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (2 items, 0 hidden)', async ({ page }) => {
    const po = new FormWithGridPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P34_AUTH', 'P34_JUSTIFICATION']);
  });

  test('apex.item round-trip on P34_AUTH', async ({ page }) => {
    const po = new FormWithGridPage(page);
    await po.goto();
    await po.auth.setValue('apx-testgen');
    expect(await po.auth.getValue()).toBe('apx-testgen');
  });

  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new FormWithGridPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Next', 'Previous', 'Apply Changes']);
  });
