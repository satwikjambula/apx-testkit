/**
 * GENERATED by apx-testgen from pages/p00006-row-header.apx — DO NOT EDIT.
 ...
 * (./p00006-row-header.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, overview-row-header, row-header
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Region types NOT covered by an auto-generated assertion (no verified DOM convention, or a runtime id genuinely unconstructible from static data -- see docs/grammar-assumptions.md "Still open" and ADR-003): breadcrumb (breadcrumb), overview-row-header (staticContent), row-header (interactiveGrid).
...
import { RowHeaderPage } from './p00006-row-header.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 6: Row Header [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new RowHeaderPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new RowHeaderPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Row Header'));
  });
