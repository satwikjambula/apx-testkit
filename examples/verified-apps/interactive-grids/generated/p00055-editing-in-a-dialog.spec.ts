/**
 * GENERATED by apx-testgen from pages/p00055-editing-in-a-dialog.apx — DO NOT EDIT.
 ...
 * (./p00055-editing-in-a-dialog.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: basic-editing, buttons, overview-editing-in-a-dialog
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Region types NOT covered by an auto-generated assertion (no verified DOM convention, or a runtime id genuinely unconstructible from static data -- see docs/grammar-assumptions.md "Still open" and ADR-003): basic-editing (interactiveGrid), buttons (staticContent), overview-editing-in-a-dialog (staticContent).
...
import { EditingInADialogPage } from './p00055-editing-in-a-dialog.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 55: "Editing in a Dialog " [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new EditingInADialogPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new EditingInADialogPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('"Editing in a Dialog "'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 1 hidden)', async ({ page }) => {
    const po = new EditingInADialogPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P55_DEPTNO']);
  });
