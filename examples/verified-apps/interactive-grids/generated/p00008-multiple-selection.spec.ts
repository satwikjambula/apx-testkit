 ...
 * (./p00008-multiple-selection.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, compare-salary, multiple-selection, overview-multiple-selection
 ...
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), overview-multiple-selection (staticContent).
...
import { expectItemsPresent, expectButtonsPresent, ApexChartRegion, ApexInteractiveGridRegion, normalizeTitle, login } from '@apx/testkit';
import { MultipleSelectionPage } from './p00008-multiple-selection.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 8: Multiple Selection [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new MultipleSelectionPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new MultipleSelectionPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Multiple Selection'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 1 hidden)', async ({ page }) => {
    const po = new MultipleSelectionPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P8_SELECTED']);
  });
  ...
  test('every Chart region with a known runtime id resolves a real chart type (1 region)', async ({ page }) => {
    ...
    const charts: Array<[string, string]> = [['barchart', 'bar']];
