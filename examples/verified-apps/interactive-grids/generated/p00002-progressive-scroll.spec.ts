/**
 * GENERATED by apx-testgen from pages/p00002-progressive-scroll.apx — DO NOT EDIT.
 ...
 * (./p00002-progressive-scroll.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, overview-progressive-scroll-paging, progressive-scroll
 ...
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), overview-progressive-scroll-paging (staticContent).
...
import { ProgressiveScrollPage } from './p00002-progressive-scroll.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 2: Progressive Scroll [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ProgressiveScrollPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ProgressiveScrollPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Progressive Scroll'));
  });
