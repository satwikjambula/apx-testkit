/**
 * GENERATED by apx-testgen from pages/p00051-client-validation.apx — DO NOT EDIT.
 ...
 * (./p00051-client-validation.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, client-validation, overview-client-validation
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Region types NOT covered by an auto-generated assertion (no verified DOM convention, or a runtime id genuinely unconstructible from static data -- see docs/grammar-assumptions.md "Still open" and ADR-003): breadcrumb (breadcrumb), client-validation (interactiveGrid), overview-client-validation (staticContent).
...
import { ClientValidationPage } from './p00051-client-validation.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 51: Client Validation [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ClientValidationPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ClientValidationPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Client Validation'));
  });
