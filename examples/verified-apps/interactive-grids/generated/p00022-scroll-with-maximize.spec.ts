/**
 * GENERATED by apx-testgen from pages/p00022-scroll-with-maximize.apx — DO NOT EDIT.
 ...
 * (./p00022-scroll-with-maximize.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, overview-scroll-with-maximize, scroll-with-maximize
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Region types NOT covered by an auto-generated assertion (no verified DOM convention, or a runtime id genuinely unconstructible from static data -- see docs/grammar-assumptions.md "Still open" and ADR-003): breadcrumb (breadcrumb), overview-scroll-with-maximize (staticContent), scroll-with-maximize (interactiveGrid).
...
import { ScrollWithMaximizePage } from './p00022-scroll-with-maximize.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 22: Scroll with Maximize [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ScrollWithMaximizePage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ScrollWithMaximizePage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Scroll with Maximize'));
  });
