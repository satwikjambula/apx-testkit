/**
 * GENERATED by apx-testgen from pages/p00053-advanced-delete.apx — DO NOT EDIT.
 ...
 * (./p00053-advanced-delete.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: advanced-delete, breadcrumb, overview-advanced-delete
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Region types NOT covered by an auto-generated assertion (no verified DOM convention, or a runtime id genuinely unconstructible from static data -- see docs/grammar-assumptions.md "Still open" and ADR-003): advanced-delete (interactiveGrid), breadcrumb (breadcrumb), overview-advanced-delete (staticContent).
...
import { AdvancedDeletePage } from './p00053-advanced-delete.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 53: Advanced Delete [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new AdvancedDeletePage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new AdvancedDeletePage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Advanced Delete'));
  });
