/**
 * GENERATED by apx-testgen from pages/p00007-sequence-row-header.apx — DO NOT EDIT.
 ...
 * (./p00007-sequence-row-header.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, overview-sequence-row-header, sequence-row-header
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Region types NOT covered by an auto-generated assertion (no verified DOM convention, or a runtime id genuinely unconstructible from static data -- see docs/grammar-assumptions.md "Still open" and ADR-003): breadcrumb (breadcrumb), overview-sequence-row-header (staticContent), sequence-row-header (interactiveGrid).
...
import { SequenceRowHeaderPage } from './p00007-sequence-row-header.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 7: Sequence Row Header [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new SequenceRowHeaderPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new SequenceRowHeaderPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Sequence Row Header'));
  });
