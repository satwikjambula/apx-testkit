/**
 * GENERATED by apx-testgen from pages/p00019-scroll-paging.apx — DO NOT EDIT.
 ...
 * (./p00019-scroll-paging.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, overview-scroll-paging, scroll-paging
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Region types NOT covered by an auto-generated assertion (no verified DOM convention, or a runtime id genuinely unconstructible from static data -- see docs/grammar-assumptions.md "Still open" and ADR-003): breadcrumb (breadcrumb), overview-scroll-paging (staticContent), scroll-paging (interactiveGrid).
...
import { ScrollPagingPage } from './p00019-scroll-paging.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 19: Scroll Paging [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ScrollPagingPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ScrollPagingPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Scroll Paging'));
  });
