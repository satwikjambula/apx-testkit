 ...
 * (./p00031-validation.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, overview-validation, validation
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Region types NOT covered by an auto-generated assertion (no verified DOM convention, or a runtime id genuinely unconstructible from static data -- see docs/grammar-assumptions.md "Still open" and ADR-003): breadcrumb (breadcrumb), overview-validation (staticContent), validation (interactiveGrid).
...
import { ValidationPage } from './p00031-validation.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 31: Validation [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ValidationPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ValidationPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Validation'));
  });
