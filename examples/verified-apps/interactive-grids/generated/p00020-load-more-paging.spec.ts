/**
 * GENERATED by apx-testgen from pages/p00020-load-more-paging.apx — DO NOT EDIT.
 ...
 * (./p00020-load-more-paging.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, load-more-paging, overview-load-more-paging
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Region types NOT covered by an auto-generated assertion (no verified DOM convention, or a runtime id genuinely unconstructible from static data -- see docs/grammar-assumptions.md "Still open" and ADR-003): breadcrumb (breadcrumb), load-more-paging (interactiveGrid), overview-load-more-paging (staticContent).
...
import { LoadMorePagingPage } from './p00020-load-more-paging.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 20: Load More Paging [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new LoadMorePagingPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new LoadMorePagingPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Load More Paging'));
  });
