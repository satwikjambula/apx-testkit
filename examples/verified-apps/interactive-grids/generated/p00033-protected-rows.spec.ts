/**
 * GENERATED by apx-testgen from pages/p00033-protected-rows.apx — DO NOT EDIT.
 ...
 * (./p00033-protected-rows.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, overview-protected-rows, protected-rows
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Region types NOT covered by an auto-generated assertion (no verified DOM convention, or a runtime id genuinely unconstructible from static data -- see docs/grammar-assumptions.md "Still open" and ADR-003): breadcrumb (breadcrumb), overview-protected-rows (staticContent), protected-rows (interactiveGrid).
...
import { ProtectedRowsPage } from './p00033-protected-rows.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 33: Protected Rows [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ProtectedRowsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ProtectedRowsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Protected Rows'));
  });
