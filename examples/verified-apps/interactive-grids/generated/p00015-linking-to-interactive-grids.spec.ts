/**
 * GENERATED by apx-testgen from pages/p00015-linking-to-interactive-grids.apx — DO NOT EDIT.
 ...
 * (./p00015-linking-to-interactive-grids.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, container, interactive-grid, overview-linking-to-interactive-grids, report-linking-examples
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * 1 Interactive Grid region(s) SKIPPED -- no htmlDomId set, runtime id genuinely unconstructible from static data (ADR-003 layer 3): interactive-grid.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), container (staticContent), overview-linking-to-interactive-grids (staticContent), report-linking-examples (list).
...
import { LinkingToInteractiveGridsPage } from './p00015-linking-to-interactive-grids.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 15: Linking to Interactive Grids [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new LinkingToInteractiveGridsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new LinkingToInteractiveGridsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Linking to Interactive Grids'));
  });
});
