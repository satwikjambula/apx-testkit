/**
 * GENERATED by apx-testgen from pages/p00004-column-groups.apx — DO NOT EDIT.
 ...
 * (./p00004-column-groups.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, column-groups, overview-column-groups
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Region types NOT covered by an auto-generated assertion (no verified DOM convention, or a runtime id genuinely unconstructible from static data -- see docs/grammar-assumptions.md "Still open" and ADR-003): breadcrumb (breadcrumb), column-groups (interactiveGrid), overview-column-groups (staticContent).
...
import { ColumnGroupsPage } from './p00004-column-groups.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 4: Column Groups [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ColumnGroupsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ColumnGroupsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Column Groups'));
  });
