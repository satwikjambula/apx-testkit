/**
 * GENERATED by apx-testgen from pages/p00014-chart-view.apx — DO NOT EDIT.
 ...
 * (./p00014-chart-view.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: basic-read-only, chart-view, overview-chart-view
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * 1 Interactive Grid region(s) SKIPPED -- no htmlDomId set, runtime id genuinely unconstructible from static data (ADR-003 layer 3): chart-view.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): basic-read-only (breadcrumb), overview-chart-view (staticContent).
...
import { ChartViewPage } from './p00014-chart-view.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 14: Chart View [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ChartViewPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ChartViewPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Chart View'));
  });
