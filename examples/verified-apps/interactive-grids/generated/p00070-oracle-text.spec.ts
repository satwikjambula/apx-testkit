/**
 * GENERATED by apx-testgen from pages/p00070-oracle-text.apx — DO NOT EDIT.
 ...
 * (./p00070-oracle-text.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: oracle-text, oracle-text-index-does-not-exist, oracle-text-is-not-available, overview-oracle-text, people
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * 1 Interactive Grid region(s) SKIPPED -- no htmlDomId set, runtime id genuinely unconstructible from static data (ADR-003 layer 3): people.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): oracle-text (breadcrumb), oracle-text-index-does-not-exist (staticContent), oracle-text-is-not-available (staticContent), overview-oracle-text (staticContent).
...
import { OracleTextPage } from './p00070-oracle-text.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 70: Oracle Text [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new OracleTextPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new OracleTextPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Oracle Text'));
  });
