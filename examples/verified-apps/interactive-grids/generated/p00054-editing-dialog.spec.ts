/**
 * GENERATED by apx-testgen from pages/p00054-editing-dialog.apx — DO NOT EDIT.
 ...
 * (./p00054-editing-dialog.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, department-overview, overview-editing-in-a-dialog
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Region types NOT covered by an auto-generated assertion (no verified DOM convention, or a runtime id genuinely unconstructible from static data -- see docs/grammar-assumptions.md "Still open" and ADR-003): breadcrumb (breadcrumb), department-overview (interactiveGrid), overview-editing-in-a-dialog (staticContent).
...
import { EditingDialogPage } from './p00054-editing-dialog.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 54: Editing (Dialog) [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new EditingDialogPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new EditingDialogPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Editing (Dialog)'));
  });
