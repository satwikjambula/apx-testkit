/**
 * GENERATED by apx-testgen from pages/p00003-basic-reporting.apx — DO NOT EDIT.
 ...
 * (./p00003-basic-reporting.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: basic-read-only, basic-reporting, overview-basic-reporting
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * 1 Interactive Grid region(s) SKIPPED -- no htmlDomId set, runtime id genuinely unconstructible from static data (ADR-003 layer 3): basic-reporting.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): basic-read-only (breadcrumb), overview-basic-reporting (staticContent).
...
import { BasicReportingPage } from './p00003-basic-reporting.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 3: Basic Reporting [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new BasicReportingPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new BasicReportingPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Basic Reporting'));
  });
