/**
 * GENERATED by apx-testgen from pages/p00018-traditional-paging-select-list.apx — DO NOT EDIT.
 ...
 * (./p00018-traditional-paging-select-list.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, overview-traditional-paging-select-list, traditional-paging-select-list
 ...
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), overview-traditional-paging-select-list (staticContent).
...
import { TraditionalPagingSelectListPage } from './p00018-traditional-paging-select-list.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 18: Traditional Paging (Select List) [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new TraditionalPagingSelectListPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new TraditionalPagingSelectListPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Traditional Paging (Select List)'));
  });
