/**
 * GENERATED by apx-testgen from pages/p00021-full-page-with-maximize.apx — DO NOT EDIT.
 ...
 * (./p00021-full-page-with-maximize.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, full-page-with-maximize, overview-full-page-with-maximize
 ...
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), overview-full-page-with-maximize (staticContent).
...
import { FullPageWithMaximizePage } from './p00021-full-page-with-maximize.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 21: Full Page with Maximize [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new FullPageWithMaximizePage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new FullPageWithMaximizePage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Full Page with Maximize'));
  });
