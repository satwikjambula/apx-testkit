/**
 * GENERATED by apx-testgen from pages/p00030-basic-editing.apx — DO NOT EDIT.
 ...
 * (./p00030-basic-editing.page.js), not raw testkit calls, so both stay in sync.
...
import { BasicEditingPage } from './p00030-basic-editing.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 30: Basic Editing [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new BasicEditingPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new BasicEditingPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Basic Editing'));
  });
