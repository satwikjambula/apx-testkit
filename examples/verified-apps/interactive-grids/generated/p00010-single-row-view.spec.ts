/**
 * GENERATED by apx-testgen from pages/p00010-single-row-view.apx — DO NOT EDIT.
 ...
 * (./p00010-single-row-view.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, overview-single-row-view, single-row-view
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Region types NOT covered by an auto-generated assertion (no verified DOM convention, or a runtime id genuinely unconstructible from static data -- see docs/grammar-assumptions.md "Still open" and ADR-003): breadcrumb (breadcrumb), overview-single-row-view (staticContent), single-row-view (interactiveGrid).
...
import { SingleRowViewPage } from './p00010-single-row-view.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 10: Single Row View [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new SingleRowViewPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new SingleRowViewPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Single Row View'));
  });
