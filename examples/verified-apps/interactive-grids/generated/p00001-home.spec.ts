 ...
 * Regions present in metadata: about-this-application, app-name, interactive-grid-examples, oracle-apex-communities-and-resources
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): about-this-application (staticContent), app-name (staticContent), interactive-grid-examples (list), oracle-apex-communities-and-resources (plugin/sampleAppsFooter).
...
test.describe('page 1: Home [requires auth]', () => {
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new HomePage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Sample Interactive Grids'));
  });
