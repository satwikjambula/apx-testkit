/**
 * GENERATED by apx-testgen from pages/p00012-traditional-paging-total-rows.apx — DO NOT EDIT.
 ...
 * (./p00012-traditional-paging-total-rows.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, overview-traditional-paging-total-rows, traditional-paging-total-rows
 ...
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), overview-traditional-paging-total-rows (staticContent).
...
import { TraditionalPagingTotalRowsPage } from './p00012-traditional-paging-total-rows.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 12: Traditional Paging (Total Rows) [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new TraditionalPagingTotalRowsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new TraditionalPagingTotalRowsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Traditional Paging (Total Rows)'));
  });
