 ...
 * (./p00170-add-initiative.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, nomenclature-initiative
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), nomenclature-initiative (staticContent).
...
import { AddInitiativePage } from './p00170-add-initiative.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 170: Add Initiative [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new AddInitiativePage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new AddInitiativePage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Add &NOMENCLATURE_INITIATIVE.'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (8 items, 1 hidden)', async ({ page }) => {
    const po = new AddInitiativePage(page);
    await po.goto();
    await expectItemsPresent(page, ['P170_FOCUS_AREA_ID', 'P170_IMAGE', 'P170_IMAGE_SELECTED', 'P170_IMAGE_SELECTED_DISPLAY', 'P170_INITIATIVE', 'P170_OBJECTIVE', 'P170_SPONSOR_ID', 'P170_TAGS']);
  });

  test('apex.item round-trip on P170_INITIATIVE', async ({ page }) => {
    const po = new AddInitiativePage(page);
    await po.goto();
    await po.initiative.setValue('apx-testgen');
    expect(await po.initiative.getValue()).toBe('apx-testgen');
  });
