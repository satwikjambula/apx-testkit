/**
 * GENERATED by apx-testgen from pages/p10090-release-milestone-types.apx — DO NOT EDIT.
 ...
 * (./p10090-release-milestone-types.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, release-milestone-types, sql
...
import { ReleaseMilestoneTypesPage } from './p10090-release-milestone-types.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 10090: Release Milestone Types [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ReleaseMilestoneTypesPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ReleaseMilestoneTypesPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Release Milestone Types'));
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new ReleaseMilestoneTypesPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Add Release Milestone Type', 'Navigate Up']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new ReleaseMilestoneTypesPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['release-milestone-types']);
  });
});
