 ...
 * (./p10601-resource-type-usage.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, usage
...
import { ResourceTypeUsagePage } from './p10601-resource-type-usage.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 10601: Resource Type Usage [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ResourceTypeUsagePage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ResourceTypeUsagePage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Resource Type Usage'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 1 hidden)', async ({ page }) => {
    const po = new ResourceTypeUsagePage(page);
    await po.goto();
    await expectItemsPresent(page, ['P10601_ID']);
  });
  ...
  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new ResourceTypeUsagePage(page);
    await po.goto();
    await expectRegionsResolve(page, ['usage']);
  });
});
