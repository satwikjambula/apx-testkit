/**
 * GENERATED by apx-testgen from pages/p00048-release-description.apx — DO NOT EDIT.
 ...
 * (./p00048-release-description.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: button-bar, release-description
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): button-bar (staticContent), release-description (dynamicContent).
...
import { ReleaseDescriptionPage } from './p00048-release-description.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 48: Release Description [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ReleaseDescriptionPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ReleaseDescriptionPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Release Description'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 1 hidden)', async ({ page }) => {
    const po = new ReleaseDescriptionPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P48_ID']);
  });

  test('every labeled .apx button exists (1 button)', async ({ page }) => {
    const po = new ReleaseDescriptionPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Close']);
  });
});
