 ...
 * (./p00172-add-initiative-default-tasks.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, default-tasks
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), default-tasks (classicReport).
...
import { AddInitiativeDefaultTasksPage } from './p00172-add-initiative-default-tasks.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 172: Add Initiative - Default Tasks [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new AddInitiativeDefaultTasksPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new AddInitiativeDefaultTasksPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Add &NOMENCLATURE_INITIATIVE. - Default Tasks'));
  });
