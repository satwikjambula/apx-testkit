 ...
 * (./p14010-delete-sample-data.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, delete-sample-data
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), delete-sample-data (classicReport).
...
import { DeleteSampleDataPage } from './p14010-delete-sample-data.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 14010: Delete Sample Data [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new DeleteSampleDataPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new DeleteSampleDataPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Delete Sample Data'));
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new DeleteSampleDataPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Delete', 'Up']);
  });
});
