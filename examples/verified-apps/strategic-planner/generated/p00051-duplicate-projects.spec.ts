/**
 * GENERATED by apx-testgen from pages/p00051-duplicate-projects.apx — DO NOT EDIT.
 ...
 * (./p00051-duplicate-projects.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, duplidate-nomenclature-projects
...
import { DuplicateProjectsPage } from './p00051-duplicate-projects.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 51: Duplicate Projects [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new DuplicateProjectsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new DuplicateProjectsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Duplicate Projects'));
  });
  ...
  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new DuplicateProjectsPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['duplidate-nomenclature-projects']);
  });
});
