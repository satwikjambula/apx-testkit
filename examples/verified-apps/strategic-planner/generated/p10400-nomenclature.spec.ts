/**
 * GENERATED by apx-testgen from pages/p10400-nomenclature.apx — DO NOT EDIT.
 ...
 * (./p10400-nomenclature.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, menubar, nomenclature, sql
 ...
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), menubar (themeTemplateComponent/contentRow), sql (classicReport).
...
import { NomenclaturePage } from './p10400-nomenclature.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 10400: Nomenclature [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new NomenclaturePage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new NomenclaturePage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Nomenclature'));
  });
  ...
  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new NomenclaturePage(page);
    await po.goto();
    await expectRegionsResolve(page, ['nomenclature']);
  });
});
