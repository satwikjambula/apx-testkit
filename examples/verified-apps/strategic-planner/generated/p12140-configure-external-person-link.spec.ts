 ...
 * (./p12140-configure-external-person-link.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, configure-external-person-link
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), configure-external-person-link (staticContent).
...
import { ConfigureExternalPersonLinkPage } from './p12140-configure-external-person-link.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 12140: Configure External Person Link [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ConfigureExternalPersonLinkPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ConfigureExternalPersonLinkPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Configure External Person Link'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (2 items, 0 hidden)', async ({ page }) => {
    const po = new ConfigureExternalPersonLinkPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P12140_EXTERNAL_PERSON_LINK', 'P12140_EXT_PERSON_LINK_DOMAIN']);
  });

  test('apex.item round-trip on P12140_EXTERNAL_PERSON_LINK', async ({ page }) => {
    const po = new ConfigureExternalPersonLinkPage(page);
    await po.goto();
    await po.externalPersonLink.setValue('apx-testgen');
    expect(await po.externalPersonLink.getValue()).toBe('apx-testgen');
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new ConfigureExternalPersonLinkPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Apply Changes', 'Cancel']);
  });
});
