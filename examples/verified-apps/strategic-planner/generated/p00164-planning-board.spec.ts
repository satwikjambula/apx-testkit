/**
 * GENERATED by apx-testgen from pages/p00164-planning-board.apx — DO NOT EDIT.
 ...
 * (./p00164-planning-board.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, menubar, planning-board
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), menubar (themeTemplateComponent/contentRow), planning-board (classicReport).
...
import { PlanningBoardPage } from './p00164-planning-board.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 164: Planning Board [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new PlanningBoardPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new PlanningBoardPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Planning Board'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (13 items, 13 hidden)', async ({ page }) => {
    const po = new PlanningBoardPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P164_AREA_ID', 'P164_COLUMN_COUNT', 'P164_COLUMN_NAMES', 'P164_DRAG_COLUMN_ID', 'P164_DRAG_FOCUS_AREA_ID', 'P164_DRAG_ITEM_ID', 'P164_DROP_COLUMN_ID', 'P164_DROP_FOCUS_AREA_ID', 'P164_DROP_RELEASE', 'P164_INITIATIVE', 'P164_INITIATIVE_ID', 'P164_SR_ABOVE', 'P164_SR_BELOW']);
  });
