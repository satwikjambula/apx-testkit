/**
 * GENERATED by apx-testgen from pages/p14000-about-page-text.apx — DO NOT EDIT.
 ...
 * (./p14000-about-page-text.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: about-page-text, breadcrumb
...
import { AboutPageTextPage } from './p14000-about-page-text.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 14000: About Page Text [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new AboutPageTextPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new AboutPageTextPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('About Page Text'));
  });
  ...
  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new AboutPageTextPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['about-page-text']);
  });
});
