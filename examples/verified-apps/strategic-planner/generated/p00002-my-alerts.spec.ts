 ...
 * (./p00002-my-alerts.page.js), not raw testkit calls, so both stay in sync.
 ...
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), notifications (themeTemplateComponent/contentRow).
...
import { MyAlertsPage } from './p00002-my-alerts.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 2: My Alerts [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new MyAlertsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new MyAlertsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('My Alerts'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 1 hidden)', async ({ page }) => {
    const po = new MyAlertsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P2_NOTIF_TO_DISMISS']);
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new MyAlertsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Close', 'Dismiss All']);
  });
});
