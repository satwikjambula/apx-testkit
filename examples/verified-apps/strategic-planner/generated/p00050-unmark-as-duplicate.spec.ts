/**
 * GENERATED by apx-testgen from pages/p00050-unmark-as-duplicate.apx — DO NOT EDIT.
 ...
 * (./p00050-unmark-as-duplicate.page.js), not raw testkit calls, so both stay in sync.
...
import { UnmarkAsDuplicatePage } from './p00050-unmark-as-duplicate.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 50: Unmark as Duplicate [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new UnmarkAsDuplicatePage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new UnmarkAsDuplicatePage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Not a Duplicate'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (3 items, 1 hidden)', async ({ page }) => {
    const po = new UnmarkAsDuplicatePage(page);
    await po.goto();
    await expectItemsPresent(page, ['P50_DUPLICATE_OF', 'P50_PROJECT', 'P50_PROJECT_ID']);
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new UnmarkAsDuplicatePage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Not a Duplicate']);
  });
});
