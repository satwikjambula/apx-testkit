/**
 * GENERATED by apx-testgen from pages/p16020-email-reporting.apx — DO NOT EDIT.
 ...
 * (./p16020-email-reporting.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, email-reporting
...
import { EmailReportingPage } from './p16020-email-reporting.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 16020: Email Reporting [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new EmailReportingPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new EmailReportingPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Email Reporting'));
  });
  ...
  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new EmailReportingPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['email-reporting']);
  });
});
