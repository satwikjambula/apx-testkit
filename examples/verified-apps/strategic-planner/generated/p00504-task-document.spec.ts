 ...
 * (./p00504-task-document.page.js), not raw testkit calls, so both stay in sync.
...
import { TaskDocumentPage } from './p00504-task-document.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 504: Task Document [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new TaskDocumentPage(page);
  ...
  test('every .apx pageItem exists in DOM/apex.item registry (14 items, 9 hidden)', async ({ page }) => {
    const po = new TaskDocumentPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P504_CREATED', 'P504_CREATED_BY', 'P504_DOCUMENT_BLOB', 'P504_DOCUMENT_CHARSET', 'P504_DOCUMENT_FILENAME', 'P504_DOCUMENT_LASTUPD', 'P504_DOCUMENT_MIMETYPE', 'P504_DOC_DESCRIPTION', 'P504_ID', 'P504_IMPORTANT_YN', 'P504_TAGS', 'P504_TASK_ID', 'P504_UPDATED', 'P504_UPDATED_BY']);
  });

  test('apex.item round-trip on P504_TAGS', async ({ page }) => {
    const po = new TaskDocumentPage(page);
    await po.goto();
