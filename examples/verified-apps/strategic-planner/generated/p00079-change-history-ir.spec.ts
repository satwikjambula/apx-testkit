/**
 * GENERATED by apx-testgen from pages/p00079-change-history-ir.apx — DO NOT EDIT.
 ...
 * (./p00079-change-history-ir.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, change-history
...
import { ChangeHistoryIrPage } from './p00079-change-history-ir.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 79: Change History [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ChangeHistoryIrPage(page);
  ...
  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new ChangeHistoryIrPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['change-history']);
  });
});
