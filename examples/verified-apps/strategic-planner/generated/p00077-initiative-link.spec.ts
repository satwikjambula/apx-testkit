 ...
 * (./p00077-initiative-link.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, initiative-link
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), initiative-link (form).
...
import { InitiativeLinkPage } from './p00077-initiative-link.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 77: Initiative Link [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new InitiativeLinkPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new InitiativeLinkPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Initiative Link'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (5 items, 1 hidden)', async ({ page }) => {
    const po = new InitiativeLinkPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P77_ID', 'P77_IMPORTANT_YN', 'P77_INITIATIVE_ID', 'P77_LINK_NAME', 'P77_LINK_URL']);
  });

  test('apex.item round-trip on P77_LINK_NAME', async ({ page }) => {
    const po = new InitiativeLinkPage(page);
    await po.goto();
    await po.linkName.setValue('apx-testgen');
    expect(await po.linkName.getValue()).toBe('apx-testgen');
  });
