/**
 * GENERATED by apx-testgen from pages/p00025-about-release-dashboard.apx — DO NOT EDIT.
 ...
 * (./p00025-about-release-dashboard.page.js), not raw testkit calls, so both stay in sync.
...
import { AboutReleaseDashboardPage } from './p00025-about-release-dashboard.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 25: About Release Dashboard [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new AboutReleaseDashboardPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new AboutReleaseDashboardPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('About Release Dashboard'));
  });
});
