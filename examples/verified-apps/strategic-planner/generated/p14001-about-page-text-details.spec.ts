 ...
 * (./p14001-about-page-text-details.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: about-page-text-details, buttons
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): about-page-text-details (form), buttons (staticContent).
...
import { AboutPageTextDetailsPage } from './p14001-about-page-text-details.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 14001: About Page Text Details [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new AboutPageTextDetailsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new AboutPageTextDetailsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('About Page Text Details'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (4 items, 1 hidden)', async ({ page }) => {
    const po = new AboutPageTextDetailsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P14001_CONFIGURABLE_CONTENT', 'P14001_CONFIG_ID', 'P14001_DESCRIPTION', 'P14001_ID']);
  });
