 ...
 * (./p00300-release-projects.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, facets, menubar, nomenclature-projects, release, sorting
 ...
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), menubar (themeTemplateComponent/contentRow), nomenclature-projects (themeTemplateComponent/contentRow), release (classicReport), sorting (staticContent).
...
import { ReleaseProjectsPage } from './p00300-release-projects.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 300: release_projects [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ReleaseProjectsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ReleaseProjectsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Release &NOMENCLATURE_PROJECTS.'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (5 items, 4 hidden)', async ({ page }) => {
    const po = new ReleaseProjectsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P300_INITIATIVE_ID', 'P300_ORDER_BY', 'P300_PROJECT_TO_FAVORITE', 'P300_RELEASE', 'P300_RELEASE_ID']);
  });
  ...
  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new ReleaseProjectsPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['facets']);
  });
});
