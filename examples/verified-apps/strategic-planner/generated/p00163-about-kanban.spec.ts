/**
 * GENERATED by apx-testgen from pages/p00163-about-kanban.apx — DO NOT EDIT.
 ...
 * (./p00163-about-kanban.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: kanban-board
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): kanban-board (staticContent).
...
import { AboutKanbanPage } from './p00163-about-kanban.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 163: About Kanban [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new AboutKanbanPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new AboutKanbanPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('About Kanban'));
  });
});
