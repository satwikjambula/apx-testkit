 ...
 * (./p00009-project-contributor.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, project-contributor
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), project-contributor (form).
...
import { ProjectContributorPage } from './p00009-project-contributor.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 9: Project Contributor [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ProjectContributorPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ProjectContributorPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Project Contributor'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (6 items, 1 hidden)', async ({ page }) => {
    const po = new ProjectContributorPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P9_ID', 'P9_PROJECT_ID', 'P9_RESPONSIBILITY', 'P9_RESPONSIBILITY_ID', 'P9_TAGS', 'P9_TEAM_MEMBER_ID']);
  });

  test('apex.item round-trip on P9_RESPONSIBILITY', async ({ page }) => {
    const po = new ProjectContributorPage(page);
    await po.goto();
    await po.responsibility.setValue('apx-testgen');
    expect(await po.responsibility.getValue()).toBe('apx-testgen');
  });
