/**
 * GENERATED by apx-testgen from pages/p00097-most-viewed-projects.apx — DO NOT EDIT.
 ...
 * (./p00097-most-viewed-projects.page.js), not raw testkit calls, so both stay in sync.
...
import { MostViewedProjectsPage } from './p00097-most-viewed-projects.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 97: Most Viewed Projects [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new MostViewedProjectsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new MostViewedProjectsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Most Viewed &NOMENCLATURE_PROJECTS.'));
  });
