 ...
 * (./p10111-project-status.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, nomenclature-project-status
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), nomenclature-project-status (form).
...
import { ProjectStatusPage } from './p10111-project-status.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 10111: Project Status [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ProjectStatusPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ProjectStatusPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('&NOMENCLATURE_PROJECT. Status'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (9 items, 5 hidden)', async ({ page }) => {
    const po = new ProjectStatusPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P10111_CREATED', 'P10111_CREATED_BY', 'P10111_DISPLAY_SEQ', 'P10111_ID', 'P10111_INCLUDE_YN', 'P10111_STATIC_ID', 'P10111_STATUS', 'P10111_UPDATED', 'P10111_UPDATED_BY']);
  });

  test('apex.item round-trip on P10111_STATIC_ID', async ({ page }) => {
    const po = new ProjectStatusPage(page);
    await po.goto();
    await po.staticId.setValue('apx-testgen');
    expect(await po.staticId.getValue()).toBe('apx-testgen');
  });

  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new ProjectStatusPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Add &NOMENCLATURE_PROJECT. Status', 'Delete', 'Apply Changes']);
  });
});
