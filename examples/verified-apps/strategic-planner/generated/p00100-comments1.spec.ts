 ...
 * (./p00100-comments1.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, comment-report, comments, post-comment-button-container
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), comment-report (classicReport), comments (staticContent), post-comment-button-container (staticContent).
...
import { Comments1Page } from './p00100-comments1.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 100: Comments [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new Comments1Page(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new Comments1Page(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Comments'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (4 items, 1 hidden)', async ({ page }) => {
    const po = new Comments1Page(page);
    await po.goto();
    await expectItemsPresent(page, ['P100_COMMENT', 'P100_IMAGE_REF_ID', 'P100_PRIVATE_YN', 'P100_PROJECT_ID']);
  });

  test('every labeled .apx button exists (3 buttons)', async ({ page }) => {
    const po = new Comments1Page(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Post Comment', 'View Project']);
  });
});
