 ...
 * (./p10062-task-status.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, task-status
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), task-status (form).
...
import { TaskStatusPage } from './p10062-task-status.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 10062: Task Status [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new TaskStatusPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new TaskStatusPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Task Status'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (12 items, 5 hidden)', async ({ page }) => {
    const po = new TaskStatusPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P10062_CREATED', 'P10062_CREATED_BY', 'P10062_DISPLAY_SEQ', 'P10062_ID', 'P10062_INCLUDE_YN', 'P10062_INDICATES_COMPLETE_YN', 'P10062_IS_DEFAULT_YN', 'P10062_STATIC_ID', 'P10062_STATUS', 'P10062_TASK_TYPE_ID', 'P10062_UPDATED', 'P10062_UPDATED_BY']);
  });

  test('apex.item round-trip on P10062_STATIC_ID', async ({ page }) => {
    const po = new TaskStatusPage(page);
    await po.goto();
  ...
  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new TaskStatusPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Add Task Status', 'Delete', 'Apply Changes']);
  });
});
