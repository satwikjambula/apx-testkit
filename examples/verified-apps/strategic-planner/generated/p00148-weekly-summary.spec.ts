/**
 * GENERATED by apx-testgen from pages/p00148-weekly-summary.apx — DO NOT EDIT.
 ...
 * (./p00148-weekly-summary.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, buttons, hidden-stuff, weekly-summary, weekly-summary-content
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), buttons (staticContent), hidden-stuff (staticContent), weekly-summary (staticContent), weekly-summary-content (dynamicContent).
...
import { WeeklySummaryPage } from './p00148-weekly-summary.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 148: Weekly Summary [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new WeeklySummaryPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new WeeklySummaryPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Weekly Summary'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (4 items, 4 hidden)', async ({ page }) => {
    const po = new WeeklySummaryPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P148_EMAIL', 'P148_FIRST_NAME', 'P148_ID', 'P148_LAST_NAME']);
  });
