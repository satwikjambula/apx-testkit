/**
 * GENERATED by apx-testgen from pages/p10050-application-log.apx — DO NOT EDIT.
 ...
 * (./p10050-application-log.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: application-log, breadcrumb
...
import { ApplicationLogPage } from './p10050-application-log.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 10050: Application Log [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ApplicationLogPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ApplicationLogPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Application Log'));
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new ApplicationLogPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Purge Log', 'Up']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new ApplicationLogPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['application-log']);
  });
});
