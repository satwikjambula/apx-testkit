 ...
 * (./p00064-change-history.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, button-bar, nomenclature-projects, search, search-results
 ...
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), button-bar (staticContent), nomenclature-projects (classicReport), search-results (classicReport).
...
import { ChangeHistoryPage } from './p00064-change-history.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 64: Project Change History [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ChangeHistoryPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ChangeHistoryPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Change History'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (3 items, 3 hidden)', async ({ page }) => {
    const po = new ChangeHistoryPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P64_PROJECT_FI', 'P64_PROJECT_ID', 'P64_PROJECT_PN']);
  });

  test('every labeled .apx button exists (3 buttons)', async ({ page }) => {
    const po = new ChangeHistoryPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Refresh', 'Tag History', 'Navigate Up']);
  });
