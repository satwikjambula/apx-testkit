/**
 * GENERATED by apx-testgen from pages/p00140-activity-quick-look1.apx — DO NOT EDIT.
 ...
 * (./p00140-activity-quick-look1.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: activity, associated-nomenclature-initiative, associated-nomenclature-project
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): activity (classicReport), associated-nomenclature-initiative (classicReport), associated-nomenclature-project (classicReport).
...
import { ActivityQuickLook1Page } from './p00140-activity-quick-look1.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 140: Activity Quick Look [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ActivityQuickLook1Page(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ActivityQuickLook1Page(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Activity Quick Look'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 1 hidden)', async ({ page }) => {
    const po = new ActivityQuickLook1Page(page);
    await po.goto();
    await expectItemsPresent(page, ['P140_ID']);
  });
});
