 ...
 * (./p00112-initiative-milestones.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, details, details-2, filter, menubar, report-results
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), details (staticContent), details-2 (classicReport), filter (staticContent), menubar (themeTemplateComponent/contentRow), report-results (classicReport).
...
import { InitiativeMilestonesPage } from './p00112-initiative-milestones.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 112: Initiative Milestones [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new InitiativeMilestonesPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new InitiativeMilestonesPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('&NOMENCLATURE_INITIATIVE. Milestones'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (11 items, 3 hidden)', async ({ page }) => {
    const po = new InitiativeMilestonesPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P112_FOCUS_AREA_ID', 'P112_INITIATIVE', 'P112_INITIATIVE_ID', 'P112_INIT_FOCUS_AREA_ID', 'P112_MAX_PERCENT', 'P112_MILESTONE_TYPE_ID', 'P112_MIN_PERCENT', 'P112_PROJECT', 'P112_PROJECT_GROUP_ID', 'P112_TAG', 'P112_TARGET']);
  });

  test('apex.item round-trip on P112_PROJECT', async ({ page }) => {
    const po = new InitiativeMilestonesPage(page);
    await po.goto();
    await po.project.setValue('apx-testgen');
    expect(await po.project.getValue()).toBe('apx-testgen');
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new InitiativeMilestonesPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Run Report', 'Navigate Up']);
  });
});
