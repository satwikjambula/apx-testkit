 ...
 * (./p00057-initiative-focus-area-change-history.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, button-bar, details, search, search-results
 ...
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), button-bar (staticContent), details (classicReport), search-results (classicReport).
...
import { InitiativeFocusAreaChangeHistoryPage } from './p00057-initiative-focus-area-change-history.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 57: Initiative Focus Area Change History [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new InitiativeFocusAreaChangeHistoryPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new InitiativeFocusAreaChangeHistoryPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Initiative Focus Area Change History'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (2 items, 2 hidden)', async ({ page }) => {
    const po = new InitiativeFocusAreaChangeHistoryPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P57_INIT_FOCUS_AREA', 'P57_INIT_FOCUS_AREA_ID']);
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new InitiativeFocusAreaChangeHistoryPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Refresh', 'Navigate Up']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new InitiativeFocusAreaChangeHistoryPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['search']);
  });
});
