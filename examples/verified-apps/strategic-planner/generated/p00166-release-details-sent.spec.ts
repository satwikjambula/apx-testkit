 ...
 * (./p00166-release-details-sent.page.js), not raw testkit calls, so both stay in sync.
...
import { ReleaseDetailsSentPage } from './p00166-release-details-sent.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 166: Release Details Sent [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ReleaseDetailsSentPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ReleaseDetailsSentPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Release Details Sent'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 1 hidden)', async ({ page }) => {
    const po = new ReleaseDetailsSentPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P166_ID']);
  });
});
