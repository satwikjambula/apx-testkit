 ...
 * (./p00026-ai-summaries.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: ai-summaries, buttons
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): ai-summaries (classicReport), buttons (staticContent).
...
import { AiSummariesPage } from './p00026-ai-summaries.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 26: AI Summaries [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new AiSummariesPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new AiSummariesPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('AI &NOMENCLATURE_PROJECT. Summaries'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 1 hidden)', async ({ page }) => {
    const po = new AiSummariesPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P26_PROJECT_ID']);
  });

  test('every labeled .apx button exists (3 buttons)', async ({ page }) => {
    const po = new AiSummariesPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Generate Full Summary', 'Generate Update', 'Info']);
  });
});
