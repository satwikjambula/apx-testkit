 ...
 * (./p00040-ifa-link.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, link
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), link (form).
...
import { IfaLinkPage } from './p00040-ifa-link.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 40: Link [requires auth]', () => {
  ...
  test('every .apx pageItem exists in DOM/apex.item registry (9 items, 6 hidden)', async ({ page }) => {
    const po = new IfaLinkPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P40_CREATED', 'P40_CREATED_BY', 'P40_ID', 'P40_IMPORTANT_YN', 'P40_INIT_FOCUS_AREA_ID', 'P40_LINK_NAME', 'P40_LINK_URL', 'P40_UPDATED', 'P40_UPDATED_BY']);
  });

  test('apex.item round-trip on P40_LINK_NAME', async ({ page }) => {
    const po = new IfaLinkPage(page);
    await po.goto();
