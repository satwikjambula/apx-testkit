/**
 * GENERATED by apx-testgen from pages/p16010-job-reporting.apx — DO NOT EDIT.
 ...
 * (./p16010-job-reporting.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, job-reporting
...
import { JobReportingPage } from './p16010-job-reporting.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 16010: Job Reporting [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new JobReportingPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new JobReportingPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Job Reporting'));
  });
  ...
  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new JobReportingPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['job-reporting']);
  });
});
