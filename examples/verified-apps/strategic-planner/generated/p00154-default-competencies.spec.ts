/**
 * GENERATED by apx-testgen from pages/p00154-default-competencies.apx — DO NOT EDIT.
 ...
 * (./p00154-default-competencies.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, default-competencies, sql
...
import { DefaultCompetenciesPage } from './p00154-default-competencies.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 154: Default Competencies [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new DefaultCompetenciesPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new DefaultCompetenciesPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Default Competencies'));
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new DefaultCompetenciesPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Add Competency', 'Up']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new DefaultCompetenciesPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['default-competencies']);
  });
});
