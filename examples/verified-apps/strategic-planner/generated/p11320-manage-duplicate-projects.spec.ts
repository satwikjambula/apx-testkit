/**
 * GENERATED by apx-testgen from pages/p11320-manage-duplicate-projects.apx — DO NOT EDIT.
 ...
 * (./p11320-manage-duplicate-projects.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, duplicate-nomenclature-projects
...
import { ManageDuplicateProjectsPage } from './p11320-manage-duplicate-projects.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 11320: Manage Duplicate Projects [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ManageDuplicateProjectsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ManageDuplicateProjectsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Manage Duplicate &NOMENCLATURE_PROJECTS.'));
  });
  ...
  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new ManageDuplicateProjectsPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['duplicate-nomenclature-projects']);
  });
});
