 ...
 * (./p00076-delete-release.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: associated-with-this-release, buttons, release
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): associated-with-this-release (classicReport), buttons (staticContent), release (form).
...
import { DeleteReleasePage } from './p00076-delete-release.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 76: Delete Release [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new DeleteReleasePage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new DeleteReleasePage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Delete Release'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (2 items, 0 hidden)', async ({ page }) => {
    const po = new DeleteReleasePage(page);
    await po.goto();
    await expectItemsPresent(page, ['P76_ID', 'P76_RELEASE_OWNER_ID']);
  });
