/**
 * GENERATED by apx-testgen from pages/p00158-comments-ir.apx — DO NOT EDIT.
 ...
 * (./p00158-comments-ir.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, nomenclature-project-comments
...
import { CommentsIrPage } from './p00158-comments-ir.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 158: Comments Interactive Report [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new CommentsIrPage(page);
  ...
  test('every labeled .apx button exists (3 buttons)', async ({ page }) => {
    const po = new CommentsIrPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Filter Report', 'Reset', 'Navigate Up']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new CommentsIrPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['nomenclature-project-comments']);
  });
});
