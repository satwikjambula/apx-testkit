/**
 * GENERATED by apx-testgen from pages/p10061-task-statuses.apx — DO NOT EDIT.
 ...
 * (./p10061-task-statuses.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, sql, task-statuses
...
import { TaskStatusesPage } from './p10061-task-statuses.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 10061: Task Statuses [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new TaskStatusesPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new TaskStatusesPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Task Statuses'));
  });

  test('every labeled .apx button exists (3 buttons)', async ({ page }) => {
    const po = new TaskStatusesPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Add Task Status', 'Reset', 'Navigate Up']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new TaskStatusesPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['task-statuses']);
  });
});
