/**
 * GENERATED by apx-testgen from pages/p10515-release-milestone-default-tags.apx — DO NOT EDIT.
 ...
 * (./p10515-release-milestone-default-tags.page.js), not raw testkit calls, so both stay in sync.
...
import { ReleaseMilestoneDefaultTagsPage } from './p10515-release-milestone-default-tags.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 10515: Release Milestone Default Tags [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ReleaseMilestoneDefaultTagsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ReleaseMilestoneDefaultTagsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Release Milestone Default Tags'));
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new ReleaseMilestoneDefaultTagsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Add Release Milestone Default Tag', 'Navigate Up']);
  });
