/**
 * GENERATED by apx-testgen from pages/p16000-error-log.apx — DO NOT EDIT.
 ...
 * (./p16000-error-log.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, error-log
...
import { ErrorLogPage } from './p16000-error-log.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 16000: Error Log [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ErrorLogPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ErrorLogPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Error Log'));
  });
  ...
  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new ErrorLogPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['error-log']);
  });
});
