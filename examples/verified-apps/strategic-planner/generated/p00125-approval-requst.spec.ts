 ...
 * (./p00125-approval-requst.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: approval-history, p125-approval-type-review, project, subsequent-approvers
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): approval-history (classicReport), p125-approval-type-review (staticContent), project (classicReport), subsequent-approvers (classicReport).
...
import { ApprovalRequstPage } from './p00125-approval-requst.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 125: Approval Request [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ApprovalRequstPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ApprovalRequstPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Approval Request'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (5 items, 4 hidden)', async ({ page }) => {
    const po = new ApprovalRequstPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P125_APPROVAL_TYPE', 'P125_COMMENTS', 'P125_PROJECT_APPROVAL_CHAIN_ID', 'P125_PROJECT_APPROVAL_ID', 'P125_WF_TASK_ID']);
  });

  test('apex.item round-trip on P125_COMMENTS', async ({ page }) => {
    const po = new ApprovalRequstPage(page);
    await po.goto();
  ...
  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new ApprovalRequstPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Approve', 'Reject', 'Request Clarification', 'Withdraw Request']);
  });
});
