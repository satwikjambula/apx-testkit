 ...
 * (./p04015-initiative-approval-entry.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, existing-initiative-approval-chain, initiative-approval-entry
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), existing-initiative-approval-chain (classicReport), initiative-approval-entry (form).
...
import { InitiativeApprovalEntryPage } from './p04015-initiative-approval-entry.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 4015: Initiative Approval Entry [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new InitiativeApprovalEntryPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new InitiativeApprovalEntryPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Initiative Approval Entry'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (10 items, 3 hidden)', async ({ page }) => {
    const po = new InitiativeApprovalEntryPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P4015_ACTIVE_YN', 'P4015_ALTERNATE_END_DATE', 'P4015_ALTERNATE_START_DATE', 'P4015_ALTERNATE_TEAM_MEMBER_ID', 'P4015_APPROVAL_SEQ', 'P4015_ID', 'P4015_INITIATIVE_APPROVAL_ID', 'P4015_INIT_ALTERNATE_END_DATE', 'P4015_INIT_ALTERNATE_START_DATE', 'P4015_TEAM_MEMBER_ID']);
  });
