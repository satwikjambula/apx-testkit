/**
 * GENERATED by apx-testgen from pages/p00014-release-change-history-old.apx — DO NOT EDIT.
 ...
 * (./p00014-release-change-history-old.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, button-bar, release, search, search-results
 ...
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), button-bar (staticContent), release (classicReport), search-results (classicReport).
...
import { ReleaseChangeHistoryOldPage } from './p00014-release-change-history-old.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 14: Release Change History [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ReleaseChangeHistoryOldPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ReleaseChangeHistoryOldPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Release Change History'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (2 items, 2 hidden)', async ({ page }) => {
    const po = new ReleaseChangeHistoryOldPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P14_RELEASE', 'P14_RELEASE_ID']);
  });
