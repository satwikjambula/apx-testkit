/**
 * GENERATED by apx-testgen from pages/p15021-default-team-member-tag.apx — DO NOT EDIT.
 ...
 * (./p15021-default-team-member-tag.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, default-team-member-tag
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), default-team-member-tag (form).
...
import { DefaultTeamMemberTagPage } from './p15021-default-team-member-tag.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 15021: Default Team Member Tag [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new DefaultTeamMemberTagPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new DefaultTeamMemberTagPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Default Team Member Tag'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (4 items, 1 hidden)', async ({ page }) => {
    const po = new DefaultTeamMemberTagPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P15021_DESCRIPTION', 'P15021_DISPLAY_SEQUENCE', 'P15021_ID', 'P15021_TAG']);
  });

  test('apex.item round-trip on P15021_DESCRIPTION', async ({ page }) => {
    const po = new DefaultTeamMemberTagPage(page);
    await po.goto();
