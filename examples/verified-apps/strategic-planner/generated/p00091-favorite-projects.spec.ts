/**
 * GENERATED by apx-testgen from pages/p00091-favorite-projects.apx — DO NOT EDIT.
 ...
 * (./p00091-favorite-projects.page.js), not raw testkit calls, so both stay in sync.
...
import { FavoriteProjectsPage } from './p00091-favorite-projects.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 91: Favorite Projects [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new FavoriteProjectsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new FavoriteProjectsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Favorite &NOMENCLATURE_PROJECTS.'));
  });
