/**
 * GENERATED by apx-testgen from pages/p00152-group-member.apx — DO NOT EDIT.
 ...
 * (./p00152-group-member.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, group-member
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), group-member (form).
...
import { GroupMemberPage } from './p00152-group-member.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 152: Group Member [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new GroupMemberPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new GroupMemberPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Group Member'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (6 items, 1 hidden)', async ({ page }) => {
    const po = new GroupMemberPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P152_ASSIGNMENT', 'P152_FULL_TIME_YN', 'P152_GROUP_ID', 'P152_GROUP_LEADER_YN', 'P152_ID', 'P152_TEAM_MEMBER_ID']);
  });

  test('apex.item round-trip on P152_ASSIGNMENT', async ({ page }) => {
    const po = new GroupMemberPage(page);
    await po.goto();
    await po.assignment.setValue('apx-testgen');
    expect(await po.assignment.getValue()).toBe('apx-testgen');
  });

  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new GroupMemberPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Add Group Member', 'Delete', 'Apply Changes']);
  });
});
