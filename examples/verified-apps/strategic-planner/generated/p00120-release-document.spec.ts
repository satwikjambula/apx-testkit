 ...
 * (./p00120-release-document.page.js), not raw testkit calls, so both stay in sync.
...
import { ReleaseDocumentPage } from './p00120-release-document.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 120: Release Document [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ReleaseDocumentPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ReleaseDocumentPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Release Document'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (14 items, 10 hidden)', async ({ page }) => {
    const po = new ReleaseDocumentPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P120_CREATED', 'P120_CREATED_BY', 'P120_DOCUMENT_BLOB', 'P120_DOCUMENT_CHARSET', 'P120_DOCUMENT_FILENAME', 'P120_DOCUMENT_LASTUPD', 'P120_DOCUMENT_MIMETYPE', 'P120_DOC_DESCRIPTION', 'P120_ID', 'P120_IMPORTANT_YN', 'P120_RELEASE_ID', 'P120_TAGS', 'P120_UPDATED', 'P120_UPDATED_BY']);
  });

  test('apex.item round-trip on P120_TAGS', async ({ page }) => {
    const po = new ReleaseDocumentPage(page);
    await po.goto();
