 ...
 * (./p10087-notification.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, notification
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), notification (form).
...
import { NotificationPage } from './p10087-notification.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 10087: Notification [requires auth]', () => {
  ...
  test('every .apx pageItem exists in DOM/apex.item registry (5 items, 1 hidden)', async ({ page }) => {
    const po = new NotificationPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P10087_DESCRIPTION', 'P10087_ID', 'P10087_IS_ACTIVE_YN', 'P10087_NAME', 'P10087_STATIC_ID']);
  });

  test('apex.item round-trip on P10087_DESCRIPTION', async ({ page }) => {
    const po = new NotificationPage(page);
    await po.goto();
