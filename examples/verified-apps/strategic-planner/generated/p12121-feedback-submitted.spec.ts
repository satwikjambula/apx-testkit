/**
 * GENERATED by apx-testgen from pages/p12121-feedback-submitted.apx — DO NOT EDIT.
 ...
 * (./p12121-feedback-submitted.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: feedback-submitted
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): feedback-submitted (staticContent).
...
import { FeedbackSubmittedPage } from './p12121-feedback-submitted.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 12121: Feedback Submitted [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new FeedbackSubmittedPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new FeedbackSubmittedPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Feedback Submitted'));
  });
