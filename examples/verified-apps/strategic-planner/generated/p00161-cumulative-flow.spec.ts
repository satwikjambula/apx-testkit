 ...
 * (./p00161-cumulative-flow.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: cumulative-flow
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * 1 chart region(s) SKIPPED -- no htmlDomId set, runtime id genuinely unconstructible from static data (ADR-003 layer 3): cumulative-flow.
...
import { CumulativeFlowPage } from './p00161-cumulative-flow.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 161: Cumulative Flow [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new CumulativeFlowPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new CumulativeFlowPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Cumulative Flow'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (4 items, 1 hidden)', async ({ page }) => {
    const po = new CumulativeFlowPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P161_FOCUS_AREA', 'P161_INITIATIVE_ID', 'P161_RESOLUTION', 'P161_SCOPE']);
  });
});
