/**
 * GENERATED by apx-testgen from pages/p00084-project-details-sent.apx — DO NOT EDIT.
 ...
 * (./p00084-project-details-sent.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: details-sent
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): details-sent (dynamicContent).
...
import { ProjectDetailsSentPage } from './p00084-project-details-sent.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 84: Project Details Sent [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ProjectDetailsSentPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ProjectDetailsSentPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('&NOMENCLATURE_PROJECT. Details Sent'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 1 hidden)', async ({ page }) => {
    const po = new ProjectDetailsSentPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P84_ID']);
  });
});
