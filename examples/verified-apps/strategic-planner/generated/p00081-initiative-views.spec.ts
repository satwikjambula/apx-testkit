/**
 * GENERATED by apx-testgen from pages/p00081-initiative-views.apx — DO NOT EDIT.
 ...
 * (./p00081-initiative-views.page.js), not raw testkit calls, so both stay in sync.
...
import { InitiativeViewsPage } from './p00081-initiative-views.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 81: Initiative Views [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new InitiativeViewsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new InitiativeViewsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('&NOMENCLATURE_INITIATIVE. Views'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 0 hidden)', async ({ page }) => {
    const po = new InitiativeViewsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P81_ORDER_BY']);
  });
