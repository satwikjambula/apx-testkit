/**
 * GENERATED by apx-testgen from pages/p00072-page-views-by-user.apx — DO NOT EDIT.
 ...
 * (./p00072-page-views-by-user.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, filter, page-performance
 ...
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), filter (staticContent).
...
import { PageViewsByUserPage } from './p00072-page-views-by-user.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 72: Page Views by User [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new PageViewsByUserPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new PageViewsByUserPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Page Views by User'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 0 hidden)', async ({ page }) => {
    const po = new PageViewsByUserPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P72_SINCE']);
  });

  test('every labeled .apx button exists (5 buttons)', async ({ page }) => {
    const po = new PageViewsByUserPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Go', 'Page Performance', 'Page Views', 'Refresh', 'Up']);
  });
