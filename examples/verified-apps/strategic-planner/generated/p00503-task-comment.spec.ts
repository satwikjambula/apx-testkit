 ...
 * (./p00503-task-comment.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, task-comment
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), task-comment (form).
...
import { TaskCommentPage } from './p00503-task-comment.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 503: Task Comment [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new TaskCommentPage(page);
  ...
  test('every .apx pageItem exists in DOM/apex.item registry (5 items, 2 hidden)', async ({ page }) => {
    const po = new TaskCommentPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P503_BODY', 'P503_ID', 'P503_NEW_IMAGE_REF_ID', 'P503_PRIVATE_YN', 'P503_TASK_ID']);
  });
