 ...
 * (./p10150-initiative-focus-areas.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, initiative-focus-areas, sql
...
import { InitiativeFocusAreasPage } from './p10150-initiative-focus-areas.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 10150: Initiative Focus Areas [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new InitiativeFocusAreasPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new InitiativeFocusAreasPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Initiative Focus Areas'));
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new InitiativeFocusAreasPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Add Focus Area', 'Up']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new InitiativeFocusAreasPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['initiative-focus-areas']);
  });
});
