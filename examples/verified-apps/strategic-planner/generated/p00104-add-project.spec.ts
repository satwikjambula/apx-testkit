 ...
 * (./p00104-add-project.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: button-bar, content
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): button-bar (staticContent), content (staticContent).
...
import { AddProjectPage } from './p00104-add-project.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 104: Add Project [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new AddProjectPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new AddProjectPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Add Project'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (5 items, 3 hidden)', async ({ page }) => {
    const po = new AddProjectPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P104_AREA', 'P104_DEFAULT_AREA', 'P104_DEFAULT_INITIATIVE', 'P104_INITIATIVE', 'P104_RELEASE_ID']);
  });
