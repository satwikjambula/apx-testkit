 ...
 * (./p00069-release-images.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, facets, images, release
 ...
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), release (classicReport).
...
import { ReleaseImagesPage } from './p00069-release-images.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 69: Release Images [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ReleaseImagesPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ReleaseImagesPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('&P69_RELEASE. - Images'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (2 items, 2 hidden)', async ({ page }) => {
    const po = new ReleaseImagesPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P69_RELEASE', 'P69_RELEASE_ID']);
  });
