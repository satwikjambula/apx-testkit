/**
 * GENERATED by apx-testgen from pages/p00175-add-initiative-confirm.apx — DO NOT EDIT.
 ...
 * (./p00175-add-initiative-confirm.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, the-following-nomenclature-initiative-will-be-created
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), the-following-nomenclature-initiative-will-be-created (dynamicContent).
...
import { AddInitiativeConfirmPage } from './p00175-add-initiative-confirm.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 175: Add Initiative - Confirm [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new AddInitiativeConfirmPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new AddInitiativeConfirmPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Add Initiative - Confirm'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 1 hidden)', async ({ page }) => {
    const po = new AddInitiativeConfirmPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P175_NEW_INITIATIVE_ID']);
  });

  test('every labeled .apx button exists (3 buttons)', async ({ page }) => {
    const po = new AddInitiativeConfirmPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Create', 'Previous']);
  });
});
