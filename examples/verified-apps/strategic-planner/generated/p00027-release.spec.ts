 ...
 * (./p00027-release.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, release
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), release (form).
...
import { ReleasePage } from './p00027-release.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 27: Release [requires auth]', () => {
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ReleasePage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Release'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (11 items, 1 hidden)', async ({ page }) => {
    const po = new ReleasePage(page);
    await po.goto();
    await expectItemsPresent(page, ['P27_DESCRIPTION', 'P27_ID', 'P27_RELEASE', 'P27_RELEASE_COMPLETED', 'P27_RELEASE_MANAGEMENT_URL', 'P27_RELEASE_OPEN_COMPLETED', 'P27_RELEASE_OPEN_DATE', 'P27_RELEASE_OWNER_ID', 'P27_RELEASE_TARGET_DATE', 'P27_RELEASE_TRAIN', 'P27_RELEASE_TYPE']);
  });

  test('apex.item round-trip on P27_RELEASE', async ({ page }) => {
    const po = new ReleasePage(page);
    await po.goto();
    await po.release.setValue('apx-testgen');
    expect(await po.release.getValue()).toBe('apx-testgen');
  });
