 ...
 * (./p00052-un-archive-nomenclature-project.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: archive, button-container
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): archive (staticContent), button-container (staticContent).
...
import { UnArchiveNomenclatureProjectPage } from './p00052-un-archive-nomenclature-project.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 52: Un-Archive &NOMENCLATURE_PROJECT. [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new UnArchiveNomenclatureProjectPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new UnArchiveNomenclatureProjectPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Restore &NOMENCLATURE_PROJECT.'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (2 items, 1 hidden)', async ({ page }) => {
    const po = new UnArchiveNomenclatureProjectPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P52_PROJECT', 'P52_PROJECT_ID']);
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new UnArchiveNomenclatureProjectPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Restore &NOMENCLATURE_PROJECT.']);
  });
});
