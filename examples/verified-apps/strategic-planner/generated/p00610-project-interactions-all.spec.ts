/**
 * GENERATED by apx-testgen from pages/p00610-project-interactions-all.apx — DO NOT EDIT.
 ...
 * (./p00610-project-interactions-all.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, button-bar, search, search-results
 ...
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), button-bar (staticContent), search-results (classicReport).
...
import { ProjectInteractionsAllPage } from './p00610-project-interactions-all.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 610: Project Interactions [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ProjectInteractionsAllPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ProjectInteractionsAllPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('&NOMENCLATURE_PROJECT. Interactions'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 1 hidden)', async ({ page }) => {
    const po = new ProjectInteractionsAllPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P610_PROJECT_ID']);
  });
