 ...
 * (./p10110-project-statuses.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, nomenclature-project-statuses, sql
...
import { ProjectStatusesPage } from './p10110-project-statuses.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 10110: Project Statuses [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ProjectStatusesPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ProjectStatusesPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('&NOMENCLATURE_PROJECT. Statuses'));
  });

  test('every labeled .apx button exists (3 buttons)', async ({ page }) => {
    const po = new ProjectStatusesPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Add &NOMENCLATURE_PROJECT. Status', 'Reset', 'Navigate Up']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new ProjectStatusesPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['nomenclature-project-statuses']);
  });
});
