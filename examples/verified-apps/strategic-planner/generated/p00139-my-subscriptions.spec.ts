 ...
 * (./p00139-my-subscriptions.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, notification-subscriptions
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), notification-subscriptions (classicReport).
...
import { MySubscriptionsPage } from './p00139-my-subscriptions.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 139: My Subscriptions [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new MySubscriptionsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new MySubscriptionsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('My Subscriptions'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (3 items, 3 hidden)', async ({ page }) => {
    const po = new MySubscriptionsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P139_FREQUENCY', 'P139_NOTIFICATION_ID', 'P139_RELEASE_ID']);
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new MySubscriptionsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Manage My Subscriptions', 'Navigate Up']);
  });
});
