/**
 * GENERATED by apx-testgen from pages/p12120-feedback.apx — DO NOT EDIT.
 ...
 * (./p12120-feedback.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, form-on-feedback
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), form-on-feedback (staticContent).
...
import { FeedbackPage } from './p12120-feedback.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 12120: Feedback [requires auth]', () => {
  ...
  test('every .apx pageItem exists in DOM/apex.item registry (4 items, 2 hidden)', async ({ page }) => {
    const po = new FeedbackPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P12120_APP_ID', 'P12120_FEEDBACK', 'P12120_PAGE_ID', 'P12120_RATING']);
  });

  test('apex.item round-trip on P12120_FEEDBACK', async ({ page }) => {
    const po = new FeedbackPage(page);
    await po.goto();
