 ...
 * (./p10014-add-multiple-users-step-2.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, exceptions, hidden-items, no-valid-users-exist-page-info, p10014-valid-count-users-to-add, valid-users-exist-page-info, wizard-container
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), exceptions (classicReport), hidden-items (staticContent), no-valid-users-exist-page-info (staticContent), p10014-valid-count-users-to-add (listView), valid-users-exist-page-info (plSqlDynamicContent), wizard-container (staticContent).
...
import { AddMultipleUsersStep2Page } from './p10014-add-multiple-users-step-2.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 10014: Add Multiple Users - Step 2 [requires auth]', () => {
  ...
  test('every .apx pageItem exists in DOM/apex.item registry (3 items, 3 hidden)', async ({ page }) => {
    const po = new AddMultipleUsersStep2Page(page);
    await po.goto();
    await expectItemsPresent(page, ['P10014_INVALID_COUNT', 'P10014_ROLE', 'P10014_VALID_COUNT']);
  });
