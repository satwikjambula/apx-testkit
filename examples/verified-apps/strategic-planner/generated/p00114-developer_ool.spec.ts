/**
 * GENERATED by apx-testgen from pages/p00114-developer_ool.apx — DO NOT EDIT.
 ...
 * (./p00114-developer_ool.page.js), not raw testkit calls, so both stay in sync.
...
import { DeveloperOolPage } from './p00114-developer_ool.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 114: Developer_Pool [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new DeveloperOolPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new DeveloperOolPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Developer Pool'));
  });
