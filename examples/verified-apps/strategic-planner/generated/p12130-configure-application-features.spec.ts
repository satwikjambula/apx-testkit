/**
 * GENERATED by apx-testgen from pages/p12130-configure-application-features.apx — DO NOT EDIT.
 ...
 * (./p12130-configure-application-features.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, configure-application-features
...
import { ConfigureApplicationFeaturesPage } from './p12130-configure-application-features.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 12130: Configure Application Features [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ConfigureApplicationFeaturesPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ConfigureApplicationFeaturesPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Configure Application Features'));
  });

  test('every labeled .apx button exists (3 buttons)', async ({ page }) => {
    const po = new ConfigureApplicationFeaturesPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Apply Changes', 'Reset', 'Navigate Up']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new ConfigureApplicationFeaturesPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['configure-application-features']);
  });
});
