 ...
 * (./p00056-ifa-document.page.js), not raw testkit calls, so both stay in sync.
...
import { IfaDocumentPage } from './p00056-ifa-document.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 56: Initiative Focus Area Document [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new IfaDocumentPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new IfaDocumentPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Document'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (14 items, 10 hidden)', async ({ page }) => {
    const po = new IfaDocumentPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P56_CREATED', 'P56_CREATED_BY', 'P56_DOCUMENT_BLOB', 'P56_DOCUMENT_CHARSET', 'P56_DOCUMENT_FILENAME', 'P56_DOCUMENT_LASTUPD', 'P56_DOCUMENT_MIMETYPE', 'P56_DOC_DESCRIPTION', 'P56_ID', 'P56_IMPORTANT_YN', 'P56_INIT_FOCUS_AREA_ID', 'P56_TAGS', 'P56_UPDATED', 'P56_UPDATED_BY']);
  });

  test('apex.item round-trip on P56_TAGS', async ({ page }) => {
    const po = new IfaDocumentPage(page);
    await po.goto();
