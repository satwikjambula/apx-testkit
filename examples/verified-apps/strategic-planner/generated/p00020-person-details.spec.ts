 ...
 * (./p00020-person-details.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, user
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), user (form).
...
import { PersonDetailsPage } from './p00020-person-details.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 20: Person Details [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new PersonDetailsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new PersonDetailsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('&NOMENCLATURE_USER. Details'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (18 items, 5 hidden)', async ({ page }) => {
    const po = new PersonDetailsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P20_COMMENT_NOTIF_PREF', 'P20_COMPETENCIES', 'P20_COUNTRY_ID', 'P20_EMAIL', 'P20_FIRST_NAME', 'P20_ID', 'P20_INITIALS', 'P20_IS_CURRENT_YN', 'P20_LAST_NAME', 'P20_LOCATION', 'P20_NOTIFICATION_PREF', 'P20_PHOTO', 'P20_PHOTO_CHARSET', 'P20_PHOTO_FILENAME', 'P20_PHOTO_LASTUPD', 'P20_PHOTO_MIMETYPE', 'P20_SCREEN_NAME', 'P20_TAGS']);
  });

  test('apex.item round-trip on P20_EMAIL', async ({ page }) => {
    const po = new PersonDetailsPage(page);
    await po.goto();
  ...
  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new PersonDetailsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Add &NOMENCLATURE_USER.', 'Delete', 'Apply Changes']);
  });
});
