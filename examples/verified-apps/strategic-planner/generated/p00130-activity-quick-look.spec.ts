 ...
 * (./p00130-activity-quick-look.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: activity-content-row
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): activity-content-row (themeTemplateComponent/contentRow).
...
import { ActivityQuickLookPage } from './p00130-activity-quick-look.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 130: Activity Quick Look [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ActivityQuickLookPage(page);
  ...
  test('every .apx pageItem exists in DOM/apex.item registry (4 items, 1 hidden)', async ({ page }) => {
    const po = new ActivityQuickLookPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P130_INCLUDE_FUTURE', 'P130_INCLUDE_PAST', 'P130_PROJECT_ID', 'P130_PROJECT_NAME']);
  });
});
