 ...
 * (./p10810-priority.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, nomenclature-project-priority
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), nomenclature-project-priority (form).
...
import { PriorityPage } from './p10810-priority.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 10810: Priority [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new PriorityPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new PriorityPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('&NOMENCLATURE_PROJECT. Priority'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (4 items, 1 hidden)', async ({ page }) => {
    const po = new PriorityPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P10810_DESCRIPTION', 'P10810_ID', 'P10810_IS_DEFAULT_YN', 'P10810_PRIORITY']);
  });

  test('apex.item round-trip on P10810_DESCRIPTION', async ({ page }) => {
    const po = new PriorityPage(page);
    await po.goto();
