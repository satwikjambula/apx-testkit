 ...
 * (./p00115-what-is-release-milestones.page.js), not raw testkit calls, so both stay in sync.
...
import { WhatIsReleaseMilestonesPage } from './p00115-what-is-release-milestones.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 115: What is Release Milestones [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new WhatIsReleaseMilestonesPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new WhatIsReleaseMilestonesPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Release Milestones'));
  });

  test('every labeled .apx button exists (3 buttons)', async ({ page }) => {
    const po = new WhatIsReleaseMilestonesPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['About &NOMENCLATURE_PROJECTS.', 'About Releases', 'About &NOMENCLATURE_USERS.']);
  });
});
