/**
 * GENERATED by apx-testgen from pages/p10500-project-default-tags.apx — DO NOT EDIT.
 ...
 * (./p10500-project-default-tags.page.js), not raw testkit calls, so both stay in sync.
...
import { ProjectDefaultTagsPage } from './p10500-project-default-tags.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 10500: Project Default Tags [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ProjectDefaultTagsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ProjectDefaultTagsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('&NOMENCLATURE_PROJECT. Default Tags'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 0 hidden)', async ({ page }) => {
    const po = new ProjectDefaultTagsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P10500_ADD_TAG']);
  });

  test('apex.item round-trip on P10500_ADD_TAG', async ({ page }) => {
    const po = new ProjectDefaultTagsPage(page);
    await po.goto();
