/**
 * GENERATED by apx-testgen from pages/p00055-cross-release-calendar.apx — DO NOT EDIT.
 ...
 * (./p00055-cross-release-calendar.page.js), not raw testkit calls, so both stay in sync.
 ...
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), button-bar (staticContent), release (classicReport), search-results (calendar).
...
import { CrossReleaseCalendarPage } from './p00055-cross-release-calendar.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 55: Cross-Release Calendar [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new CrossReleaseCalendarPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new CrossReleaseCalendarPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Cross-Release Milestone Calendar'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (2 items, 2 hidden)', async ({ page }) => {
    const po = new CrossReleaseCalendarPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P55_RELEASE', 'P55_RELEASE_ID']);
  });
