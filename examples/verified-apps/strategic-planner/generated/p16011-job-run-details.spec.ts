 ...
 * (./p16011-job-run-details.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: job-run-details
...
import { JobRunDetailsPage } from './p16011-job-run-details.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 16011: Job Run Details [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new JobRunDetailsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new JobRunDetailsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Job Run Details'));
  });
  ...
  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new JobRunDetailsPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['job-run-details']);
  });
});
