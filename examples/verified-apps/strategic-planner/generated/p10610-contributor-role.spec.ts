/**
 * GENERATED by apx-testgen from pages/p10610-contributor-role.apx — DO NOT EDIT.
 ...
 * (./p10610-contributor-role.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, contributor-role
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), contributor-role (form).
...
import { ContributorRolePage } from './p10610-contributor-role.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 10610: Contributor Role [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ContributorRolePage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ContributorRolePage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Contributor Role'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (4 items, 1 hidden)', async ({ page }) => {
    const po = new ContributorRolePage(page);
    await po.goto();
    await expectItemsPresent(page, ['P10610_ID', 'P10610_IS_DEFAULT_YN', 'P10610_RESOURCE_DESCRIPTION', 'P10610_RESOURCE_TYPE']);
  });

  test('apex.item round-trip on P10610_RESOURCE_DESCRIPTION', async ({ page }) => {
    const po = new ContributorRolePage(page);
    await po.goto();
    await po.resourceDescription.setValue('apx-testgen');
    expect(await po.resourceDescription.getValue()).toBe('apx-testgen');
  });
