/**
 * GENERATED by apx-testgen from pages/p00141-documents1.apx — DO NOT EDIT.
 ...
 * (./p00141-documents1.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, documents
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), documents (appTemplateComponent/contentRowSimple).
...
import { Documents1Page } from './p00141-documents1.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 141: Documents [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new Documents1Page(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new Documents1Page(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Documents'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (2 items, 2 hidden)', async ({ page }) => {
    const po = new Documents1Page(page);
    await po.goto();
    await expectItemsPresent(page, ['P141_ID', 'P141_PROJECT_ID']);
  });

  test('every labeled .apx button exists (3 buttons)', async ({ page }) => {
    const po = new Documents1Page(page);
    await po.goto();
    await expectButtonsPresent(page, ['Add Document', 'Cancel', 'View Project']);
  });
});
