/**
 * GENERATED by apx-testgen from pages/p00149-manage-my-subscriptions.apx — DO NOT EDIT.
 ...
 * (./p00149-manage-my-subscriptions.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, info, notification-subscriptions
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), info (staticContent), notification-subscriptions (classicReport).
...
import { ManageMySubscriptionsPage } from './p00149-manage-my-subscriptions.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 149: Manage My Subscriptions [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ManageMySubscriptionsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ManageMySubscriptionsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Manage My Subscriptions'));
  });
