 ...
 * (./p00126-clarification-requested.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: approval-history, p126-approval-type-review, project
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): approval-history (classicReport), p126-approval-type-review (staticContent), project (classicReport).
...
import { ClarificationRequestedPage } from './p00126-clarification-requested.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 126: Clarification Requested [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ClarificationRequestedPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ClarificationRequestedPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Clarification Requested'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (5 items, 4 hidden)', async ({ page }) => {
    const po = new ClarificationRequestedPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P126_APPROVAL_TYPE', 'P126_COMMENTS', 'P126_PROJECT_APPROVAL_CHAIN_ID', 'P126_PROJECT_APPROVAL_ID', 'P126_WF_TASK_ID']);
  });

  test('apex.item round-trip on P126_COMMENTS', async ({ page }) => {
    const po = new ClarificationRequestedPage(page);
    await po.goto();
  ...
  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new ClarificationRequestedPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Submit', 'Withdraw Review']);
  });
});
