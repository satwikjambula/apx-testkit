 ...
 * (./p12124-feedback1.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, form-items-region
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), form-items-region (staticContent).
...
import { Feedback1Page } from './p12124-feedback1.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 12124: Feedback [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new Feedback1Page(page);
  ...
  test('every .apx pageItem exists in DOM/apex.item registry (8 items, 1 hidden)', async ({ page }) => {
    const po = new Feedback1Page(page);
    await po.goto();
    await expectItemsPresent(page, ['P12124_FEEDBACK', 'P12124_FEEDBACK_STATUS', 'P12124_FILED', 'P12124_ID', 'P12124_PAGE_ID', 'P12124_RATING_ICON', 'P12124_RESPONSE', 'P12124_USER_AGENT']);
  });

  test('apex.item round-trip on P12124_RESPONSE', async ({ page }) => {
    const po = new Feedback1Page(page);
    await po.goto();
    await po.response.setValue('apx-testgen');
    expect(await po.response.getValue()).toBe('apx-testgen');
  });
