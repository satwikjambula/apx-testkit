 ...
 * (./p00118-what-are-release-comments.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: about, button-container
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): about (staticContent), button-container (staticContent).
...
import { WhatAreReleaseCommentsPage } from './p00118-what-are-release-comments.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 118: What are Release Comments [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new WhatAreReleaseCommentsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new WhatAreReleaseCommentsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Release Comments'));
  });

  test('every labeled .apx button exists (3 buttons)', async ({ page }) => {
    const po = new WhatAreReleaseCommentsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['What is a &NOMENCLATURE_PROJECT.?', 'What is a Release?', 'What is a &NOMENCLATURE_USER.?']);
  });
});
