/**
 * GENERATED by apx-testgen from pages/p00103-groups.apx — DO NOT EDIT.
 ...
 * (./p00103-groups.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, groups, menubar
...
import { GroupsPage } from './p00103-groups.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 103: Groups [requires auth]', () => {
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new GroupsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Groups'));
  });
  ...
  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new GroupsPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['groups']);
  });
});
