/**
 * GENERATED by apx-testgen from pages/p10100-project-sizes.apx — DO NOT EDIT.
 ...
 * (./p10100-project-sizes.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, project-size, sql
...
import { ProjectSizesPage } from './p10100-project-sizes.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 10100: Project Size [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ProjectSizesPage(page);
  ...
  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new ProjectSizesPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Add Size', 'Up']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new ProjectSizesPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['project-size']);
  });
});
