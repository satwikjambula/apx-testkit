 ...
 * (./p00098-task-quick-look.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, comments, documents, links, p98-task-type, tags
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), comments (classicReport), documents (classicReport), links (classicReport), p98-task-type (classicReport), tags (classicReport).
...
import { TaskQuickLookPage } from './p00098-task-quick-look.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 98: Task Quick Look [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new TaskQuickLookPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new TaskQuickLookPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Quick Look'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (3 items, 3 hidden)', async ({ page }) => {
    const po = new TaskQuickLookPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P98_PROJECT_ID', 'P98_TASK_ID', 'P98_TASK_TYPE']);
  });

  test('every labeled .apx button exists (6 buttons)', async ({ page }) => {
    const po = new TaskQuickLookPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Close', 'View Details', 'Edit', 'Edit', 'Edit', 'History']);
  });
});
