/**
 * GENERATED by apx-testgen from pages/p10820-help.apx — DO NOT EDIT.
 ...
 * (./p10820-help.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: about-this-application, breadcrumb, menubar, percent-complete, terminology, terminology-2
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): about-this-application (dynamicContent), breadcrumb (breadcrumb), menubar (themeTemplateComponent/contentRow), percent-complete (staticContent), terminology (staticContent), terminology-2 (staticContent).
...
import { HelpPage } from './p10820-help.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 10820: About this App [requires auth]', () => {
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new HelpPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('About this App'));
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new HelpPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Navigate Up', 'View Percent Complete Scales']);
  });
});
