/**
 * GENERATED by apx-testgen from pages/p04000-workflow-console.apx — DO NOT EDIT.
 ...
 * (./p04000-workflow-console.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, report
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), report (themeTemplateComponent/contentRow).
...
import { WorkflowConsolePage } from './p04000-workflow-console.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 4000: Workflow Console [requires auth]', () => {
  ...
  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 0 hidden)', async ({ page }) => {
    const po = new WorkflowConsolePage(page);
    await po.goto();
    await expectItemsPresent(page, ['P4000_SORT_BY']);
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new WorkflowConsolePage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Resume All Suspended', 'Navigate Up']);
  });
});
