 ...
 * (./p00012-project-document.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, nomenclature-project-document
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), nomenclature-project-document (form).
...
import { ProjectDocumentPage } from './p00012-project-document.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 12: Project Document [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ProjectDocumentPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ProjectDocumentPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('&NOMENCLATURE_PROJECT. Document'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (10 items, 5 hidden)', async ({ page }) => {
    const po = new ProjectDocumentPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P12_DOCUMENT_BLOB', 'P12_DOCUMENT_CHARSET', 'P12_DOCUMENT_FILENAME', 'P12_DOCUMENT_LASTUPD', 'P12_DOCUMENT_MIMETYPE', 'P12_DOC_DESCRIPTION', 'P12_ID', 'P12_IMPORTANT_YN', 'P12_PROJECT_ID', 'P12_TAGS']);
  });

  test('apex.item round-trip on P12_TAGS', async ({ page }) => {
    const po = new ProjectDocumentPage(page);
    await po.goto();
