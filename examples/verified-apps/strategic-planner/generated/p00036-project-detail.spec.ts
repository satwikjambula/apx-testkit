 ...
 * (./p00036-project-detail.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: activity, approvals, attributes, buttons, links, milestones, related, reviews, tags, tasks
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): activity (classicReport), approvals (classicReport), attributes (classicReport), buttons (staticContent), links (classicReport), milestones (classicReport), related (classicReport), reviews (classicReport), tags (classicReport), tasks (classicReport).
...
import { ProjectDetailPage } from './p00036-project-detail.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 36: Project Detail [requires auth]', () => {
  ...
  test('every .apx pageItem exists in DOM/apex.item registry (4 items, 4 hidden)', async ({ page }) => {
    const po = new ProjectDetailPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P36_ID', 'P36_LOG', 'P36_PERMALINK', 'P36_PROJECT_NAME']);
  });

  test('every labeled .apx button exists (14 buttons)', async ({ page }) => {
    const po = new ProjectDetailPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Add Activity', 'Add Link', 'Add Milestone', 'Add Review', 'Add Task', 'Close', 'Comments', 'Description', 'Documents', 'Edit', 'Edit', 'View History', 'View Change History', 'View Details']);
  });
});
