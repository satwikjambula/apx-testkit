/**
 * GENERATED by apx-testgen from pages/p00119-project-groups1.apx — DO NOT EDIT.
 ...
 * (./p00119-project-groups1.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: activity-content-row, breadcrumb, comments, comments-report, current-activity, menubar, nomenclature-projects, rds
 ...
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): activity-content-row (themeTemplateComponent/contentRow), breadcrumb (breadcrumb), comments (staticContent), comments-report (classicReport), current-activity (staticContent), menubar (themeTemplateComponent/contentRow), rds (regionDisplaySelector).
...
import { ProjectGroups1Page } from './p00119-project-groups1.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 119: Project Groups [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ProjectGroups1Page(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ProjectGroups1Page(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('&NOMENCLATURE_PROJECT. Groups'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (2 items, 2 hidden)', async ({ page }) => {
    const po = new ProjectGroups1Page(page);
    await po.goto();
    await expectItemsPresent(page, ['P119_PROJECT_GROUP', 'P119_PROJECT_GROUP_ID']);
  });
  ...
  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new ProjectGroups1Page(page);
    await po.goto();
    await expectRegionsResolve(page, ['nomenclature-projects']);
  });
});
