 ...
 * (./p00124-history.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, changes, history
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), changes (classicReport), history (classicReport).
...
import { HistoryPage } from './p00124-history.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 124: Change History [requires auth]', () => {
  ...
  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 1 hidden)', async ({ page }) => {
    const po = new HistoryPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P124_ID']);
  });
