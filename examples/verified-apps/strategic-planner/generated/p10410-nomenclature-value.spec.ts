 ...
 * (./p10410-nomenclature-value.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, nomenclature
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), nomenclature (form).
...
import { NomenclatureValuePage } from './p10410-nomenclature-value.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 10410: Nomenclature Value [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new NomenclatureValuePage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new NomenclatureValuePage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Nomenclature Value'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (4 items, 2 hidden)', async ({ page }) => {
    const po = new NomenclatureValuePage(page);
    await po.goto();
    await expectItemsPresent(page, ['P10410_DEFAULT_ORDER', 'P10410_DEFAULT_VALUE', 'P10410_ID', 'P10410_STATIC_ID']);
  });

  test('apex.item round-trip on P10410_DEFAULT_VALUE', async ({ page }) => {
    const po = new NomenclatureValuePage(page);
    await po.goto();
    await po.defaultValue.setValue('apx-testgen');
    expect(await po.defaultValue.getValue()).toBe('apx-testgen');
  });
