 ...
 * (./p10790-team-member-notifications.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, team-member-notifications
...
import { TeamMemberNotificationsPage } from './p10790-team-member-notifications.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 10790: Team Member Notifications [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new TeamMemberNotificationsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new TeamMemberNotificationsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Team Member Notifications'));
  });
  ...
  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new TeamMemberNotificationsPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['team-member-notifications']);
  });
});
