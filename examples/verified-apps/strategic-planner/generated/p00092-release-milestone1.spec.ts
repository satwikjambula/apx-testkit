 ...
 * (./p00092-release-milestone1.page.js), not raw testkit calls, so both stay in sync.
...
import { ReleaseMilestone1Page } from './p00092-release-milestone1.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 92: Release Milestone [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ReleaseMilestone1Page(page);
  ...
  test('every .apx pageItem exists in DOM/apex.item registry (11 items, 3 hidden)', async ({ page }) => {
    const po = new ReleaseMilestone1Page(page);
    await po.goto();
    await expectItemsPresent(page, ['P92_ID', 'P92_MILESTONE_COMPLETED_YN', 'P92_MILESTONE_DATE', 'P92_MILESTONE_DESCRIPTION', 'P92_MILESTONE_NAME', 'P92_MILESTONE_START_DATE', 'P92_MILESTONE_TYPE_ID', 'P92_ORIGINAL_MILESTONE_DATE', 'P92_RELEASE_ID', 'P92_TAGS', 'P92_UPPER_MILESTONE_NAME']);
  });

  test('apex.item round-trip on P92_MILESTONE_NAME', async ({ page }) => {
    const po = new ReleaseMilestone1Page(page);
    await po.goto();
    await po.milestoneName.setValue('apx-testgen');
    expect(await po.milestoneName.getValue()).toBe('apx-testgen');
  });

  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new ReleaseMilestone1Page(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Add Milestone', 'Delete', 'Apply Changes']);
  });
});
