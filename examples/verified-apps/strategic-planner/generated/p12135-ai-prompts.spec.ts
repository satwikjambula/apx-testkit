/**
 * GENERATED by apx-testgen from pages/p12135-ai-prompts.apx — DO NOT EDIT.
 ...
 * (./p12135-ai-prompts.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, prompts
...
import { AiPromptsPage } from './p12135-ai-prompts.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 12135: AI Prompts [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new AiPromptsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new AiPromptsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('AI Prompts'));
  });
  ...
  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new AiPromptsPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['prompts']);
  });
});
