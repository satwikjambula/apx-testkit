 ...
 * (./p04001-workflow-details.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: activities, activity-audit, buttons, history, parameters, subject, variables, workflow-diagram
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): activities (themeTemplateComponent/contentRow), activity-audit (themeTemplateComponent/comments), buttons (staticContent), history (themeTemplateComponent/comments), parameters (themeTemplateComponent/contentRow), subject (themeTemplateComponent/contentRow), variables (themeTemplateComponent/contentRow), workflow-diagram (workflowDiagram).
...
import { WorkflowDetailsPage } from './p04001-workflow-details.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 4001: Workflow Details [requires auth]', () => {
  ...
  test('every .apx pageItem exists in DOM/apex.item registry (3 items, 3 hidden)', async ({ page }) => {
    const po = new WorkflowDetailsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P4001_ACTIVITY_ID', 'P4001_PARENT_WORKFLOW_ID', 'P4001_WORKFLOW_ID']);
  });

  test('every labeled .apx button exists (7 buttons)', async ({ page }) => {
    const po = new WorkflowDetailsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Close', 'Close', 'Resume', 'Retry', 'Suspend', 'Terminate', 'To Parent Workflow']);
  });
});
