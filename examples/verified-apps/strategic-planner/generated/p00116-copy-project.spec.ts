 ...
 * (./p00116-copy-project.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: button-container, copy
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): button-container (staticContent), copy (staticContent).
...
import { CopyProjectPage } from './p00116-copy-project.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 116: Copy &NOMENCLATURE_PROJECT. [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new CopyProjectPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new CopyProjectPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Copy &NOMENCLATURE_PROJECT.'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (4 items, 2 hidden)', async ({ page }) => {
    const po = new CopyProjectPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P116_NEW_PROJECT_ID', 'P116_NEW_PROJECT_NAME', 'P116_PROJECT', 'P116_PROJECT_ID']);
  });

  test('apex.item round-trip on P116_NEW_PROJECT_NAME', async ({ page }) => {
    const po = new CopyProjectPage(page);
    await po.goto();
    await po.newProjectName.setValue('apx-testgen');
    expect(await po.newProjectName.getValue()).toBe('apx-testgen');
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new CopyProjectPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Copy']);
  });
});
