 ...
 * (./p00127-default-completeness-scale.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, initiative, status-scale
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), initiative (form), status-scale (classicReport).
...
import { DefaultCompletenessScalePage } from './p00127-default-completeness-scale.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 127: Default Completeness Scale [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new DefaultCompletenessScalePage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new DefaultCompletenessScalePage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Default Completeness Scale'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (3 items, 1 hidden)', async ({ page }) => {
    const po = new DefaultCompletenessScalePage(page);
    await po.goto();
    await expectItemsPresent(page, ['P127_ID', 'P127_INITIATIVE', 'P127_STATUS_SCALE']);
  });
