 ...
 * (./p00066-area-initiatives.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: about, additional-attriubtes, area-details, breadcrumb, contextual-information, faceted-search, menubar, nomenclature-initiatives, sort-container
 ...
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): about (staticContent), additional-attriubtes (classicReport), area-details (staticContent), breadcrumb (breadcrumb), contextual-information (classicReport), menubar (themeTemplateComponent/contentRow), nomenclature-initiatives (themeTemplateComponent/contentRow), sort-container (staticContent).
...
import { AreaInitiativesPage } from './p00066-area-initiatives.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 66: &P66_AREA. &NOMENCLATURE_INITIATIVES. [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new AreaInitiativesPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new AreaInitiativesPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('&P66_AREA. &NOMENCLATURE_INITIATIVES.'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (4 items, 2 hidden)', async ({ page }) => {
    const po = new AreaInitiativesPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P66_AREA', 'P66_AREA_ID', 'P66_IMAGE', 'P66_ORDER_BY']);
  });
  ...
  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new AreaInitiativesPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['faceted-search']);
  });
});
