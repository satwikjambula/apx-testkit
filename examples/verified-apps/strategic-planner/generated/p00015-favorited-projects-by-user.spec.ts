/**
 * GENERATED by apx-testgen from pages/p00015-favorited-projects-by-user.apx — DO NOT EDIT.
 ...
 * (./p00015-favorited-projects-by-user.page.js), not raw testkit calls, so both stay in sync.
...
import { FavoritedProjectsByUserPage } from './p00015-favorited-projects-by-user.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 15: Favorited Projects by User [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new FavoritedProjectsByUserPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new FavoritedProjectsByUserPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Favorited &NOMENCLATURE_PROJECTS. by User'));
  });
