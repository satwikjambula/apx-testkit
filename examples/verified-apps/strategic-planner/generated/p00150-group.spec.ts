 ...
 * (./p00150-group.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, group
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), group (form).
...
import { GroupPage } from './p00150-group.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 150: Group [requires auth]', () => {
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new GroupPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Group'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (4 items, 1 hidden)', async ({ page }) => {
    const po = new GroupPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P150_DESCRIPTION', 'P150_GROUP_NAME', 'P150_GROUP_TAG', 'P150_ID']);
  });
