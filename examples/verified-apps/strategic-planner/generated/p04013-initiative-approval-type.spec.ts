 ...
 * (./p04013-initiative-approval-type.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: approvers, buttons, nomenclature-initiative-approval-type
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): approvers (classicReport), buttons (staticContent), nomenclature-initiative-approval-type (form).
...
import { InitiativeApprovalTypePage } from './p04013-initiative-approval-type.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 4013: Initiative Approval Type [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new InitiativeApprovalTypePage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new InitiativeApprovalTypePage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('&NOMENCLATURE_INITIATIVE. Approval Type'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (4 items, 1 hidden)', async ({ page }) => {
    const po = new InitiativeApprovalTypePage(page);
    await po.goto();
    await expectItemsPresent(page, ['P4013_ACTIVE_YN', 'P4013_APPROVAL_TYPE_ID', 'P4013_ID', 'P4013_INITIATIVE_ID']);
  });
