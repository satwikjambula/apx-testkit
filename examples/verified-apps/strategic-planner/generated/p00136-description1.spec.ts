 ...
 * (./p00136-description1.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, p136-project-name
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), p136-project-name (dynamicContent).
...
import { Description1Page } from './p00136-description1.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 136: Description [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new Description1Page(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new Description1Page(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('&NOMENCLATURE_PROJECT. Desription'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (3 items, 3 hidden)', async ({ page }) => {
    const po = new Description1Page(page);
    await po.goto();
    await expectItemsPresent(page, ['P136_ID', 'P136_LOG', 'P136_PROJECT_NAME']);
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new Description1Page(page);
    await po.goto();
    await expectButtonsPresent(page, ['Close', 'View &NOMENCLATURE_PROJECT.']);
  });
});
