 ...
 * (./p04016-initiative-approval-chain1.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: add-new-entry, buttons, initiative, initiative-approval-chain
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): add-new-entry (staticContent), buttons (staticContent), initiative (staticContent), initiative-approval-chain (classicReport).
...
import { InitiativeApprovalChain1Page } from './p04016-initiative-approval-chain1.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 4016: Initiative Approval Chain [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new InitiativeApprovalChain1Page(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new InitiativeApprovalChain1Page(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Initiative Approval Chain'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (7 items, 4 hidden)', async ({ page }) => {
    const po = new InitiativeApprovalChain1Page(page);
    await po.goto();
    await expectItemsPresent(page, ['P4016_APPROVAL_COUNT', 'P4016_APPROVAL_SEQ', 'P4016_DELETE_PROCEED_YN', 'P4016_ID_TO_DELETE', 'P4016_INITIATIVE_APPROVAL_ID', 'P4016_OTHER_APPROVERS_COUNT', 'P4016_TEAM_MEMBER_ID']);
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new InitiativeApprovalChain1Page(page);
    await po.goto();
    await expectButtonsPresent(page, ['Add Approver', 'Cancel']);
  });
});
