 ...
 * (./p00110-activity2.page.js), not raw testkit calls, so both stay in sync.
...
import { Activity2Page } from './p00110-activity2.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 110: Activity [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new Activity2Page(page);
  ...
  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new Activity2Page(page);
    await po.goto();
    await expectButtonsPresent(page, ['About &NOMENCLATURE_PROJECTS.', 'About &NOMENCLATURE_USERS.', 'About &NOMENCLATURE_AREAS.', 'About &NOMENCLATURE_INITIATIVES.']);
  });
});
