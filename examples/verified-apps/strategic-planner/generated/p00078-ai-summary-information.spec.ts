/**
 * GENERATED by apx-testgen from pages/p00078-ai-summary-information.apx — DO NOT EDIT.
 ...
 * (./p00078-ai-summary-information.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: information
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): information (dynamicContent).
...
import { AiSummaryInformationPage } from './p00078-ai-summary-information.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 78: AI Summary Information [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new AiSummaryInformationPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new AiSummaryInformationPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('AI Summary Information'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 1 hidden)', async ({ page }) => {
    const po = new AiSummaryInformationPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P78_TYPE']);
  });
});
