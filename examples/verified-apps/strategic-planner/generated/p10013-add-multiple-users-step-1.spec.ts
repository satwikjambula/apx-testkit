/**
 * GENERATED by apx-testgen from pages/p10013-add-multiple-users-step-1.apx — DO NOT EDIT.
 ...
 * (./p10013-add-multiple-users-step-1.page.js), not raw testkit calls, so both stay in sync.
...
import { AddMultipleUsersStep1Page } from './p10013-add-multiple-users-step-1.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 10013: Add Multiple Users - Step 1 [requires auth]', () => {
  ...
  test('every .apx pageItem exists in DOM/apex.item registry (2 items, 0 hidden)', async ({ page }) => {
    const po = new AddMultipleUsersStep1Page(page);
    await po.goto();
    await expectItemsPresent(page, ['P10013_PRELIM_USERS', 'P10013_ROLE']);
  });

  test('apex.item round-trip on P10013_PRELIM_USERS', async ({ page }) => {
    const po = new AddMultipleUsersStep1Page(page);
    await po.goto();
