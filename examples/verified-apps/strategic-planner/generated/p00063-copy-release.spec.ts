 ...
 * (./p00063-copy-release.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, milestones, release
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), milestones (classicReport), release (staticContent).
...
import { CopyReleasePage } from './p00063-copy-release.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 63: Copy Release [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new CopyReleasePage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new CopyReleasePage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Copy Release'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (5 items, 2 hidden)', async ({ page }) => {
    const po = new CopyReleasePage(page);
    await po.goto();
    await expectItemsPresent(page, ['P63_COPY_FROM_RELEASE_ID', 'P63_NEW_RELEASE_ID', 'P63_OPEN_DATE', 'P63_PUSH_DAYS', 'P63_RELEASE']);
  });

  test('apex.item round-trip on P63_RELEASE', async ({ page }) => {
    const po = new CopyReleasePage(page);
    await po.goto();
