 ...
 * (./p00095-initiative-default-tasks.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, initiative-tasks, report
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), initiative-tasks (staticContent), report (classicReport).
...
import { InitiativeDefaultTasksPage } from './p00095-initiative-default-tasks.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 95: Initiative Default Tasks [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new InitiativeDefaultTasksPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new InitiativeDefaultTasksPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Initiative Default Tasks'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 0 hidden)', async ({ page }) => {
    const po = new InitiativeDefaultTasksPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P95_INITIATIVE_ID']);
  });
