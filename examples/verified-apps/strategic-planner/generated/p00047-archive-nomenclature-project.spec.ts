 ...
 * (./p00047-archive-nomenclature-project.page.js), not raw testkit calls, so both stay in sync.
...
import { ArchiveNomenclatureProjectPage } from './p00047-archive-nomenclature-project.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 47: Archive &NOMENCLATURE_PROJECT. [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ArchiveNomenclatureProjectPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ArchiveNomenclatureProjectPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Archive &NOMENCLATURE_PROJECT.'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (2 items, 1 hidden)', async ({ page }) => {
    const po = new ArchiveNomenclatureProjectPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P47_PROJECT', 'P47_PROJECT_ID']);
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new ArchiveNomenclatureProjectPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Archive Project', 'Cancel']);
  });
});
