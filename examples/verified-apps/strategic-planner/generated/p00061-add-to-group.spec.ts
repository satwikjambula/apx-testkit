 ...
 * (./p00061-add-to-group.page.js), not raw testkit calls, so both stay in sync.
...
import { AddToGroupPage } from './p00061-add-to-group.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 61: Add to Group [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new AddToGroupPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new AddToGroupPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Add to Group'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (6 items, 1 hidden)', async ({ page }) => {
    const po = new AddToGroupPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P61_ASSIGNMENT', 'P61_FULL_TIME_YN', 'P61_GROUP_ID', 'P61_GROUP_LEADER_YN', 'P61_ID', 'P61_TEAM_MEMBER_ID']);
  });

  test('apex.item round-trip on P61_ASSIGNMENT', async ({ page }) => {
    const po = new AddToGroupPage(page);
    await po.goto();
