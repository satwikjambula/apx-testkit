/**
 * GENERATED by apx-testgen from pages/p00093-project-exceptions.apx — DO NOT EDIT.
 ...
 * (./p00093-project-exceptions.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: contents
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): contents (dynamicContent).
...
import { ProjectExceptionsPage } from './p00093-project-exceptions.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 93: Project Exceptions [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ProjectExceptionsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ProjectExceptionsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Project Exceptions'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 0 hidden)', async ({ page }) => {
    const po = new ProjectExceptionsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P93_PROJECT_ID']);
  });
});
