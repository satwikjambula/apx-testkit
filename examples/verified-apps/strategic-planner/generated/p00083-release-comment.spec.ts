/**
 * GENERATED by apx-testgen from pages/p00083-release-comment.apx — DO NOT EDIT.
 ...
 * (./p00083-release-comment.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, release-comment
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), release-comment (form).
...
import { ReleaseCommentPage } from './p00083-release-comment.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 83: Release Comment [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ReleaseCommentPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ReleaseCommentPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Release Comment'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (4 items, 2 hidden)', async ({ page }) => {
    const po = new ReleaseCommentPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P83_BODY', 'P83_ID', 'P83_NEW_IMAGE_REF_ID', 'P83_PRIVATE_YN']);
  });
