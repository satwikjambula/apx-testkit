/**
 * GENERATED by apx-testgen from pages/p00082-tag-history.apx — DO NOT EDIT.
 ...
 * (./p00082-tag-history.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: tag-changes
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): tag-changes (classicReport).
...
import { TagHistoryPage } from './p00082-tag-history.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 82: Tag History [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new TagHistoryPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new TagHistoryPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Tag History'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 1 hidden)', async ({ page }) => {
    const po = new TagHistoryPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P82_ID']);
  });
});
