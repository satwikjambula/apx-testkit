/**
 * GENERATED by apx-testgen from pages/p04010-approval-types.apx — DO NOT EDIT.
 ...
 * (./p04010-approval-types.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: approval-types, breadcrumb
...
import { ApprovalTypesPage } from './p04010-approval-types.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 4010: Approval Types [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ApprovalTypesPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ApprovalTypesPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Approval Types'));
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new ApprovalTypesPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Add Approval Type', 'Navigate Up']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new ApprovalTypesPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['approval-types']);
  });
});
