/**
 * GENERATED by apx-testgen from pages/p10700-manage-activity-types.apx — DO NOT EDIT.
 ...
 * (./p10700-manage-activity-types.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: activity-types, breadcrumb, sql
...
import { ManageActivityTypesPage } from './p10700-manage-activity-types.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 10700: Activity Types [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ManageActivityTypesPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ManageActivityTypesPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Activity Types'));
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new ManageActivityTypesPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Add Activity Type', 'Navigate Up']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new ManageActivityTypesPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['activity-types']);
  });
});
