/**
 * GENERATED by apx-testgen from pages/p00021-initiatives.apx — DO NOT EDIT.
 ...
 * (./p00021-initiatives.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, faceted-search, items, menubar, nomenclature-initiatives
 ...
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), items (staticContent), menubar (themeTemplateComponent/contentRow), nomenclature-initiatives (themeTemplateComponent/contentRow).
...
import { InitiativesPage } from './p00021-initiatives.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 21: Initiatives [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new InitiativesPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new InitiativesPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('&NOMENCLATURE_INITIATIVES.'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (2 items, 1 hidden)', async ({ page }) => {
    const po = new InitiativesPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P21_DEFAULT_AREA_ID', 'P21_ORDER_BY']);
  });
