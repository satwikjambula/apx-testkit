/**
 * GENERATED by apx-testgen from pages/p00165-ai-release-summaries.apx — DO NOT EDIT.
 ...
 * (./p00165-ai-release-summaries.page.js), not raw testkit calls, so both stay in sync.
...
import { AiReleaseSummariesPage } from './p00165-ai-release-summaries.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 165: AI Release Summaries [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new AiReleaseSummariesPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new AiReleaseSummariesPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('AI Release Summaries'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 1 hidden)', async ({ page }) => {
    const po = new AiReleaseSummariesPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P165_RELEASE_ID']);
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new AiReleaseSummariesPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Generate Summary', 'Info']);
  });
});
