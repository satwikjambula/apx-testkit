/**
 * GENERATED by apx-testgen from pages/p10011-user-report.apx — DO NOT EDIT.
 ...
 * (./p10011-user-report.page.js), not raw testkit calls, so both stay in sync.
...
import { UserReportPage } from './p10011-user-report.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 10011: Users [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new UserReportPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new UserReportPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Users'));
  });

  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new UserReportPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Add Multiple Users', 'Add User', 'Reset', 'Navigate Up']);
  });
