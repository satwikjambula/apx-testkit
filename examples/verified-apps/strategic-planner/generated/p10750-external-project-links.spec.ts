/**
 * GENERATED by apx-testgen from pages/p10750-external-project-links.apx — DO NOT EDIT.
 ...
 * (./p10750-external-project-links.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, external-nomenclature-project-links, sql
...
import { ExternalProjectLinksPage } from './p10750-external-project-links.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 10750: External Project Links [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ExternalProjectLinksPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ExternalProjectLinksPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('External &NOMENCLATURE_PROJECT. Links'));
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new ExternalProjectLinksPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Add Link', 'Navigate Up']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new ExternalProjectLinksPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['external-nomenclature-project-links']);
  });
});
