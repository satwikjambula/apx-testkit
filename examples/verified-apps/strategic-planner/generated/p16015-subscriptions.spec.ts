 ...
 * (./p16015-subscriptions.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, subscriptions
...
import { SubscriptionsPage } from './p16015-subscriptions.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 16015: Subscriptions [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new SubscriptionsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new SubscriptionsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Subscriptions'));
  });

  test('every labeled .apx button exists (5 buttons)', async ({ page }) => {
    const po = new SubscriptionsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Add New', 'Apply Changes', 'Information', 'Reset', 'Navigate Up']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new SubscriptionsPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['subscriptions']);
  });
});
