 ...
 * (./p00013-initiative-document.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, document
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), document (form).
...
import { InitiativeDocumentPage } from './p00013-initiative-document.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 13: Initiative Document [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new InitiativeDocumentPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new InitiativeDocumentPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Initiative Document'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (14 items, 10 hidden)', async ({ page }) => {
    const po = new InitiativeDocumentPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P13_CREATED', 'P13_CREATED_BY', 'P13_DOCUMENT_BLOB', 'P13_DOCUMENT_CHARSET', 'P13_DOCUMENT_FILENAME', 'P13_DOCUMENT_LASTUPD', 'P13_DOCUMENT_MIMETYPE', 'P13_DOC_DESCRIPTION', 'P13_ID', 'P13_IMPORTANT_YN', 'P13_INITIATIVE_ID', 'P13_TAGS', 'P13_UPDATED', 'P13_UPDATED_BY']);
  });

  test('apex.item round-trip on P13_TAGS', async ({ page }) => {
    const po = new InitiativeDocumentPage(page);
    await po.goto();
    await po.tags.setValue('apx-testgen');
    expect(await po.tags.getValue()).toBe('apx-testgen');
  });

  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new InitiativeDocumentPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Add Document', 'Delete', 'Apply Changes']);
  });
});
