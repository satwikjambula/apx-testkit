 ...
 * (./p00128-favorite.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: button-bar, favorite
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): button-bar (staticContent), favorite (staticContent).
...
import { FavoritePage } from './p00128-favorite.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 128: Favorite [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new FavoritePage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new FavoritePage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Favorite'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (3 items, 1 hidden)', async ({ page }) => {
    const po = new FavoritePage(page);
    await po.goto();
    await expectItemsPresent(page, ['P128_FAVORITE', 'P128_PROJECT', 'P128_PROJECT_ID']);
  });
