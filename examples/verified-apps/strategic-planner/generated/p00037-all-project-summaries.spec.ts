 ...
 * (./p00037-all-project-summaries.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, summaries
...
import { AllProjectSummariesPage } from './p00037-all-project-summaries.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 37: All Project Summaries [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new AllProjectSummariesPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new AllProjectSummariesPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('All &NOMENCLATURE_PROJECT. Summaries'));
  });
  ...
  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new AllProjectSummariesPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['summaries']);
  });
});
