/**
 * GENERATED by apx-testgen from pages/p00045-archived-nomenclature-projects.apx — DO NOT EDIT.
 ...
 * (./p00045-archived-nomenclature-projects.page.js), not raw testkit calls, so both stay in sync.
...
import { ArchivedNomenclatureProjectsPage } from './p00045-archived-nomenclature-projects.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 45: Archived &NOMENCLATURE_PROJECTS. [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ArchivedNomenclatureProjectsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ArchivedNomenclatureProjectsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Archived &NOMENCLATURE_PROJECTS.'));
  });
