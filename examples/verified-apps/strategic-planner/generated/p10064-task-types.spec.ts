/**
 * GENERATED by apx-testgen from pages/p10064-task-types.apx — DO NOT EDIT.
 ...
 * (./p10064-task-types.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, sql, task-types
...
import { TaskTypesPage } from './p10064-task-types.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 10064: Task Types [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new TaskTypesPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new TaskTypesPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Task Types'));
  });

  test('every labeled .apx button exists (3 buttons)', async ({ page }) => {
    const po = new TaskTypesPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Add Task Type', 'Reset', 'Navigate Up']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new TaskTypesPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['task-types']);
  });
});
