/**
 * GENERATED by apx-testgen from pages/p00080-project-views.apx — DO NOT EDIT.
 ...
 * (./p00080-project-views.page.js), not raw testkit calls, so both stay in sync.
...
import { ProjectViewsPage } from './p00080-project-views.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 80: Project Views [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ProjectViewsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ProjectViewsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('&NOMENCLATURE_PROJECT. Views'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 0 hidden)', async ({ page }) => {
    const po = new ProjectViewsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P80_ORDER_BY']);
  });
