/**
 * GENERATED by apx-testgen from pages/p00090-project-group.apx — DO NOT EDIT.
 ...
 * (./p00090-project-group.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, nomenclature-project
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), nomenclature-project (form).
...
import { ProjectGroupPage } from './p00090-project-group.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 90: Project Group [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ProjectGroupPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ProjectGroupPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('&NOMENCLATURE_PROJECT. Group'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (4 items, 1 hidden)', async ({ page }) => {
    const po = new ProjectGroupPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P90_GROUP_DESCRIPTION', 'P90_GROUP_NAME', 'P90_ID', 'P90_IS_ACTIVE_YN']);
  });

  test('apex.item round-trip on P90_GROUP_DESCRIPTION', async ({ page }) => {
    const po = new ProjectGroupPage(page);
    await po.goto();
    await po.groupDescription.setValue('apx-testgen');
    expect(await po.groupDescription.getValue()).toBe('apx-testgen');
  });
