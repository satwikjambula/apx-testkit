 ...
 * (./p00060-task-images.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, facets, images, task
 ...
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), task (classicReport).
...
import { TaskImagesPage } from './p00060-task-images.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 60: Task Images [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new TaskImagesPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new TaskImagesPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('&P60_TASK_NAME. - Images'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (2 items, 2 hidden)', async ({ page }) => {
    const po = new TaskImagesPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P60_TASK_ID', 'P60_TASK_NAME']);
  });
