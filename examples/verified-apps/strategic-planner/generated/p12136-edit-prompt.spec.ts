 ...
 * (./p12136-edit-prompt.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, edit-prompt
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), edit-prompt (form).
...
import { EditPromptPage } from './p12136-edit-prompt.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 12136: Edit Prompt [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new EditPromptPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new EditPromptPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Edit Prompt'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (4 items, 1 hidden)', async ({ page }) => {
    const po = new EditPromptPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P12136_DESCRIPTION', 'P12136_ID', 'P12136_PROMPT', 'P12136_STATIC_ID']);
  });

  test('apex.item round-trip on P12136_PROMPT', async ({ page }) => {
    const po = new EditPromptPage(page);
    await po.goto();
    await po.prompt.setValue('apx-testgen');
    expect(await po.prompt.getValue()).toBe('apx-testgen');
  });
