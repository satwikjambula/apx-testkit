 ...
 * (./p00506-task-link.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, task-link
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), task-link (form).
...
import { TaskLinkPage } from './p00506-task-link.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 506: Task Link [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new TaskLinkPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new TaskLinkPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Link'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (9 items, 5 hidden)', async ({ page }) => {
    const po = new TaskLinkPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P506_CREATED', 'P506_CREATED_BY', 'P506_ID', 'P506_IMPORTANT_YN', 'P506_LINK_NAME', 'P506_LINK_URL', 'P506_TASK_ID', 'P506_UPDATED', 'P506_UPDATED_BY']);
  });

  test('apex.item round-trip on P506_LINK_NAME', async ({ page }) => {
    const po = new TaskLinkPage(page);
    await po.goto();
