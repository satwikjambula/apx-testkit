/**
 * GENERATED by apx-testgen from pages/p00043-countries.apx — DO NOT EDIT.
 ...
 * (./p00043-countries.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, countries, sql
...
import { CountriesPage } from './p00043-countries.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 43: Countries [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new CountriesPage(page);
  ...
  test('every labeled .apx button exists (3 buttons)', async ({ page }) => {
    const po = new CountriesPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Add Country', 'Reset', 'Navigate Up']);
  });
