 ...
 * (./p00023-projects.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, content-row, faceted-search, items, menubar
 ...
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), content-row (themeTemplateComponent/contentRow), items (staticContent), menubar (themeTemplateComponent/contentRow).
...
import { ProjectsPage } from './p00023-projects.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 23: Projects [requires auth]', () => {
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ProjectsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('&NOMENCLATURE_PROJECTS.'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (7 items, 6 hidden)', async ({ page }) => {
    const po = new ProjectsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P23_FAV1', 'P23_FAV2', 'P23_FAV3', 'P23_FOCUS_AREA_ID', 'P23_INITIATIVE_ID', 'P23_ORDER_BY', 'P23_PROJECT_TO_FAVORITE']);
  });
