 ...
 * (./p10101-project-size.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, project-size
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), project-size (form).
...
import { ProjectSizePage } from './p10101-project-size.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 10101: Project Size [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ProjectSizePage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ProjectSizePage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('&NOMENCLATURE_PROJECT. Size'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (5 items, 1 hidden)', async ({ page }) => {
    const po = new ProjectSizePage(page);
    await po.goto();
    await expectItemsPresent(page, ['P10101_EFFORT_DAYS', 'P10101_ID', 'P10101_INCLUDE_YN', 'P10101_PROJECT_SIZE', 'P10101_SIZE_DESCRIPTION']);
  });

  test('apex.item round-trip on P10101_PROJECT_SIZE', async ({ page }) => {
    const po = new ProjectSizePage(page);
    await po.goto();
    await po.projectSize.setValue('apx-testgen');
    expect(await po.projectSize.getValue()).toBe('apx-testgen');
  });
