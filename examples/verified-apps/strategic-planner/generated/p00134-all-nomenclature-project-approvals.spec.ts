/**
 * GENERATED by apx-testgen from pages/p00134-all-nomenclature-project-approvals.apx — DO NOT EDIT.
 ...
 * (./p00134-all-nomenclature-project-approvals.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: all-project-approvals, breadcrumb
...
import { AllNomenclatureProjectApprovalsPage } from './p00134-all-nomenclature-project-approvals.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 134: All Project Approvals [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new AllNomenclatureProjectApprovalsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new AllNomenclatureProjectApprovalsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('All &NOMENCLATURE_PROJECT. Approvals'));
  });
  ...
  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new AllNomenclatureProjectApprovalsPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['all-project-approvals']);
  });
});
