/**
 * GENERATED by apx-testgen from pages/p00031-links-from-comments.apx — DO NOT EDIT.
 ...
 * (./p00031-links-from-comments.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: button-bar, links
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): button-bar (staticContent), links (dynamicContent).
...
import { LinksFromCommentsPage } from './p00031-links-from-comments.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 31: Links from Comments [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new LinksFromCommentsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new LinksFromCommentsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Links from Comments'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 1 hidden)', async ({ page }) => {
    const po = new LinksFromCommentsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P31_ID']);
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new LinksFromCommentsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Add Links', 'Cancel']);
  });
});
