 ...
 * (./p00089-project-views-by-user.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, items, search, search-results
 ...
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), items (staticContent), search-results (classicReport).
...
import { ProjectViewsByUserPage } from './p00089-project-views-by-user.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 89: Project Views by User [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ProjectViewsByUserPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ProjectViewsByUserPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('&NOMENCLATURE_PROJECT. Views by User'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (5 items, 4 hidden)', async ({ page }) => {
    const po = new ProjectViewsByUserPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P89_CURRENT_MONTH', 'P89_FIRST_MONTH', 'P89_ORDER_BY', 'P89_SECOND_MONTH', 'P89_THIRD_MONTH']);
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new ProjectViewsByUserPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Reset', 'Navigate Up']);
  });
