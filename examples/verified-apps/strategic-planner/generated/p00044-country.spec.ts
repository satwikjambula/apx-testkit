 ...
 * (./p00044-country.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, country
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), country (form).
...
import { CountryPage } from './p00044-country.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 44: Country [requires auth]', () => {
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new CountryPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Country'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (9 items, 1 hidden)', async ({ page }) => {
    const po = new CountryPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P44_COUNTRY_CODE', 'P44_COUNTRY_CODE3', 'P44_COUNTRY_NAME', 'P44_DISPLAY_YN', 'P44_ID', 'P44_LAT', 'P44_LON', 'P44_QUICK_PICK_YN', 'P44_REGION']);
  });

  test('apex.item round-trip on P44_COUNTRY_CODE', async ({ page }) => {
    const po = new CountryPage(page);
    await po.goto();
    await po.countryCode.setValue('apx-testgen');
    expect(await po.countryCode.getValue()).toBe('apx-testgen');
  });
