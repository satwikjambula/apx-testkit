 ...
 * (./p00032-description.page.js), not raw testkit calls, so both stay in sync.
...
import { DescriptionPage } from './p00032-description.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 32: Description [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new DescriptionPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new DescriptionPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Description'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (3 items, 1 hidden)', async ({ page }) => {
    const po = new DescriptionPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P32_DESCRIPTION', 'P32_ID', 'P32_PROJECT']);
  });
