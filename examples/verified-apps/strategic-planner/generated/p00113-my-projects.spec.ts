 ...
 * (./p00113-my-projects.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, content-row, filter, menubar, sort, user
 ...
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), content-row (themeTemplateComponent/contentRow), menubar (themeTemplateComponent/contentRow), sort (staticContent), user (staticContent).
...
import { MyProjectsPage } from './p00113-my-projects.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 113: My Projects [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new MyProjectsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new MyProjectsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('My &NOMENCLATURE_PROJECTS.'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (3 items, 1 hidden)', async ({ page }) => {
    const po = new MyProjectsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P113_ORDER_BY', 'P113_PROJECT_TO_FAVORITE', 'P113_USER']);
  });
  ...
  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new MyProjectsPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['filter']);
  });
});
