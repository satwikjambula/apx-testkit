/**
 * GENERATED by apx-testgen from pages/p00132-my-approval-requests.apx — DO NOT EDIT.
 ...
 * (./p00132-my-approval-requests.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: more-info-pending, pending-requests
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): more-info-pending (classicReport), pending-requests (classicReport).
...
import { MyApprovalRequestsPage } from './p00132-my-approval-requests.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 132: My Approval Requests [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new MyApprovalRequestsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new MyApprovalRequestsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('My Approval Requests'));
  });
});
