/**
 * GENERATED by apx-testgen from pages/p10016-people-that-are-not-users.apx — DO NOT EDIT.
 ...
 * (./p10016-people-that-are-not-users.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, people
...
import { PeopleThatAreNotUsersPage } from './p10016-people-that-are-not-users.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 10016: People that are not Users [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new PeopleThatAreNotUsersPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new PeopleThatAreNotUsersPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('&NOMENCLATURE_USERS. that are not Users'));
  });
  ...
  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new PeopleThatAreNotUsersPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['people']);
  });
});
