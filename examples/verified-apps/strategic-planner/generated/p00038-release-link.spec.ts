 ...
 * (./p00038-release-link.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, release-link
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), release-link (form).
...
import { ReleaseLinkPage } from './p00038-release-link.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 38: Release Link [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ReleaseLinkPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ReleaseLinkPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Release Link'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (5 items, 1 hidden)', async ({ page }) => {
    const po = new ReleaseLinkPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P38_ID', 'P38_IMPORTANT_YN', 'P38_LINK_NAME', 'P38_LINK_URL', 'P38_RELEASE_ID']);
  });

  test('apex.item round-trip on P38_LINK_NAME', async ({ page }) => {
    const po = new ReleaseLinkPage(page);
    await po.goto();
