/**
 * GENERATED by apx-testgen from pages/p00157-images-in-comments.apx — DO NOT EDIT.
 ...
 * (./p00157-images-in-comments.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, images-in-comments
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * 1 chart region(s) SKIPPED -- no htmlDomId set, runtime id genuinely unconstructible from static data (ADR-003 layer 3): images-in-comments.
...
import { ImagesInCommentsPage } from './p00157-images-in-comments.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 157: Images in Comments [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ImagesInCommentsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ImagesInCommentsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Images in Comments'));
  });
