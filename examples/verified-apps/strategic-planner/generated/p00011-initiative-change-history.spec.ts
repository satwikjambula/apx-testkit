 ...
 * (./p00011-initiative-change-history.page.js), not raw testkit calls, so both stay in sync.
...
import { InitiativeChangeHistoryPage } from './p00011-initiative-change-history.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 11: Initiative Change History [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new InitiativeChangeHistoryPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new InitiativeChangeHistoryPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('&NOMENCLATURE_INITIATIVE. Change History'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 1 hidden)', async ({ page }) => {
    const po = new InitiativeChangeHistoryPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P11_INITIATIVE_ID']);
  });
