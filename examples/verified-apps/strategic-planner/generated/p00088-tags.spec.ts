 ...
 * (./p00088-tags.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, context, tag-buttons, tags
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), context (staticContent), tag-buttons (staticContent), tags (classicReport).
...
import { TagsPage } from './p00088-tags.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 88: Tags [requires auth]', () => {
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new TagsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('&NOMENCLATURE_PROJECT. Tags'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (4 items, 2 hidden)', async ({ page }) => {
    const po = new TagsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P88_ADD_TAG', 'P88_PROJECT', 'P88_PROJECT_ID', 'P88_TAG_TO_REMOVE']);
  });

  test('apex.item round-trip on P88_ADD_TAG', async ({ page }) => {
    const po = new TagsPage(page);
    await po.goto();
  ...
  test('every labeled .apx button exists (3 buttons)', async ({ page }) => {
    const po = new TagsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Add Tags', 'Cancel', 'View Project']);
  });
});
