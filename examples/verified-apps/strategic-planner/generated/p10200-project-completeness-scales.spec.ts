/**
 * GENERATED by apx-testgen from pages/p10200-project-completeness-scales.apx — DO NOT EDIT.
 ...
 * (./p10200-project-completeness-scales.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, information, project-scales
 ...
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), information (staticContent).
...
import { ProjectCompletenessScalesPage } from './p10200-project-completeness-scales.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 10200: Project Completeness Scales [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ProjectCompletenessScalesPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ProjectCompletenessScalesPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('&NOMENCLATURE_PROJECT. Completeness Scales'));
  });
  ...
  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new ProjectCompletenessScalesPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['project-scales']);
  });
});
