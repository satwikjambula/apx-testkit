/**
 * GENERATED by apx-testgen from pages/p00171-add-initiative-completeness-scale.apx — DO NOT EDIT.
 ...
 * (./p00171-add-initiative-completeness-scale.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, completeness-scale, status-scale-report
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), completeness-scale (staticContent), status-scale-report (classicReport).
...
import { AddInitiativeCompletenessScalePage } from './p00171-add-initiative-completeness-scale.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 171: Add Initiative - Completeness Scale [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new AddInitiativeCompletenessScalePage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new AddInitiativeCompletenessScalePage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Add &NOMENCLATURE_INITIATIVE. - Completeness Scale'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 0 hidden)', async ({ page }) => {
    const po = new AddInitiativeCompletenessScalePage(page);
    await po.goto();
    await expectItemsPresent(page, ['P171_STATUS_SCALE']);
  });

  test('every labeled .apx button exists (3 buttons)', async ({ page }) => {
    const po = new AddInitiativeCompletenessScalePage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Next', 'Previous']);
  });
});
