/**
 * GENERATED by apx-testgen from pages/p00074-people.apx — DO NOT EDIT.
 ...
 * (./p00074-people.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, faceted-search, filters, menubar, users
 ...
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), filters (staticContent), menubar (themeTemplateComponent/contentRow), users (themeTemplateComponent/contentRow).
...
import { PeoplePage } from './p00074-people.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 74: People [requires auth]', () => {
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new PeoplePage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('&NOMENCLATURE_USERS.'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 0 hidden)', async ({ page }) => {
    const po = new PeoplePage(page);
    await po.goto();
    await expectItemsPresent(page, ['P74_ORDER_BY']);
  });
