 ...
 * (./p00049-mark-as-dupliate.page.js), not raw testkit calls, so both stay in sync.
...
import { MarkAsDupliatePage } from './p00049-mark-as-dupliate.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 49: Mark as Dupliate [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new MarkAsDupliatePage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new MarkAsDupliatePage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Mark as Dupliate'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (3 items, 1 hidden)', async ({ page }) => {
    const po = new MarkAsDupliatePage(page);
    await po.goto();
    await expectItemsPresent(page, ['P49_DUPLICATE_OF', 'P49_PROJECT', 'P49_PROJECT_ID']);
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new MarkAsDupliatePage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Mark as Duplicate']);
  });
});
