 ...
 * (./p10010-configure-access-control.page.js), not raw testkit calls, so both stay in sync.
...
import { ConfigureAccessControlPage } from './p10010-configure-access-control.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 10010: Configure Access Control [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ConfigureAccessControlPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ConfigureAccessControlPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Configure Access Control'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 0 hidden)', async ({ page }) => {
    const po = new ConfigureAccessControlPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P10010_ALLOW_OTHER_USERS']);
  });
