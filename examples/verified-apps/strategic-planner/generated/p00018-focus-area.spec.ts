 ...
 * (./p00018-focus-area.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, nomenclature-area
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), nomenclature-area (form).
...
import { FocusAreaPage } from './p00018-focus-area.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 18: Focus Area [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new FocusAreaPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new FocusAreaPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('&NOMENCLATURE_AREA.'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (15 items, 9 hidden)', async ({ page }) => {
    const po = new FocusAreaPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P18_AREA', 'P18_CREATED', 'P18_CREATED_BY', 'P18_DESCRIPTION', 'P18_HIDDEN_BY_DEFAULT_YN', 'P18_ID', 'P18_IMAGE', 'P18_IMAGE_LAST_UPDATED', 'P18_IMAGE_MIMETYPE', 'P18_IMAGE_NAME', 'P18_OWNER_ID', 'P18_STATUS_SCALE', 'P18_TAGS', 'P18_UPDATED', 'P18_UPDATED_BY']);
  });

  test('apex.item round-trip on P18_AREA', async ({ page }) => {
    const po = new FocusAreaPage(page);
    await po.goto();
    await po.area.setValue('apx-testgen');
    expect(await po.area.getValue()).toBe('apx-testgen');
  });

  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new FocusAreaPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Add &NOMENCLATURE_AREA.', 'Delete', 'Apply Changes']);
  });
});
