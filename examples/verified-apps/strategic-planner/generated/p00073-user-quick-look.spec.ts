 ...
 * (./p00073-user-quick-look.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: active-approvals, associated-groups, button-container, current-and-future-nomenclature-project-activities, nomenclature-project-associations-excludes-completed, nomenclature-user-details, not-a-team-member, open-reviews
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): active-approvals (classicReport), associated-groups (classicReport), button-container (staticContent), current-and-future-nomenclature-project-activities (themeTemplateComponent/contentRow), nomenclature-project-associations-excludes-completed (classicReport), nomenclature-user-details (classicReport), not-a-team-member (staticContent), open-reviews (classicReport).
...
import { UserQuickLookPage } from './p00073-user-quick-look.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 73: User Quick Look [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new UserQuickLookPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new UserQuickLookPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('&NOMENCLATURE_USER. Quick Look'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (4 items, 2 hidden)', async ({ page }) => {
    const po = new UserQuickLookPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P73_EXT_LINK', 'P73_PHOTO', 'P73_PHOTO_PLACEHOLDER', 'P73_TEAM_MEMBER_ID']);
  });

  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new UserQuickLookPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Edit (Admin)', 'External Directory', 'Manage My Profile', 'View Details']);
  });
});
