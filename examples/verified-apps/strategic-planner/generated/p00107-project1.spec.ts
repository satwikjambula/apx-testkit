 ...
 * (./p00107-project1.page.js), not raw testkit calls, so both stay in sync.
...
import { Project1Page } from './p00107-project1.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 107: Project [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new Project1Page(page);
  ...
  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new Project1Page(page);
    await po.goto();
    await expectButtonsPresent(page, ['About &NOMENCLATURE_INITIATIVES.', 'About Releases', 'About Activities', 'About &NOMENCLATURE_AREAS.']);
  });
});
