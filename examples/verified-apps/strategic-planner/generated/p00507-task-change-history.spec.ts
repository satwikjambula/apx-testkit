 ...
 * (./p00507-task-change-history.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, button-bar, details, search, search-results, task
 ...
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), button-bar (staticContent), details (staticContent), search-results (classicReport), task (classicReport).
...
import { TaskChangeHistoryPage } from './p00507-task-change-history.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 507: Task Change History [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new TaskChangeHistoryPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new TaskChangeHistoryPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Task Change History'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (2 items, 2 hidden)', async ({ page }) => {
    const po = new TaskChangeHistoryPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P507_PROJECT_ID', 'P507_TASK_ID']);
  });
