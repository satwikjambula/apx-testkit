 ...
 * (./p00070-project-groups.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, project-groups
...
import { ProjectGroupsPage } from './p00070-project-groups.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 70: Project Groups [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ProjectGroupsPage(page);
  ...
  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new ProjectGroupsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Add Group', 'Navigate Up']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new ProjectGroupsPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['project-groups']);
  });
});
