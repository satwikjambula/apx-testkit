/**
 * GENERATED by apx-testgen from pages/p10520-team-member-default-tags.apx — DO NOT EDIT.
 ...
 * (./p10520-team-member-default-tags.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, default-tags, sql
 ...
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), sql (classicReport).
...
import { TeamMemberDefaultTagsPage } from './p10520-team-member-default-tags.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 10520: Team Member Default Tags [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new TeamMemberDefaultTagsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new TeamMemberDefaultTagsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Team Member Default Tags'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 0 hidden)', async ({ page }) => {
    const po = new TeamMemberDefaultTagsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P10520_ADD_TAG']);
  });

  test('apex.item round-trip on P10520_ADD_TAG', async ({ page }) => {
    const po = new TeamMemberDefaultTagsPage(page);
    await po.goto();
    await po.addTag.setValue('apx-testgen');
    expect(await po.addTag.getValue()).toBe('apx-testgen');
  });

  test('every labeled .apx button exists (3 buttons)', async ({ page }) => {
    const po = new TeamMemberDefaultTagsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Add Tag', 'Reset', 'Navigate Up']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new TeamMemberDefaultTagsPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['default-tags']);
  });
});
