/**
 * GENERATED by apx-testgen from pages/p00008-releases.apx — DO NOT EDIT.
 ...
 * (./p00008-releases.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, faceted-search, items, menubar, releases
 ...
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), items (staticContent), menubar (themeTemplateComponent/contentRow), releases (themeTemplateComponent/contentRow).
...
import { ReleasesPage } from './p00008-releases.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 8: Releases [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ReleasesPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ReleasesPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Releases'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 0 hidden)', async ({ page }) => {
    const po = new ReleasesPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P8_ORDER_BY']);
  });
