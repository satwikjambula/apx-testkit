/**
 * GENERATED by apx-testgen from pages/p00600-project-interactions.apx — DO NOT EDIT.
 ...
 * (./p00600-project-interactions.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: button-bar, nomenclature-project-interactions
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): button-bar (staticContent), nomenclature-project-interactions (classicReport).
...
import { ProjectInteractionsPage } from './p00600-project-interactions.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 600: Project Interactions [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ProjectInteractionsPage(page);
  ...
  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 1 hidden)', async ({ page }) => {
    const po = new ProjectInteractionsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P600_PROJECT_ID']);
  });

  test('every labeled .apx button exists (1 button)', async ({ page }) => {
    const po = new ProjectInteractionsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Details']);
  });
});
