 ...
 * (./p00075-current-activity.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, current-activity, filter, menubar
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), current-activity (classicReport), filter (staticContent), menubar (themeTemplateComponent/contentRow).
...
import { CurrentActivityPage } from './p00075-current-activity.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 75: Current Activity [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new CurrentActivityPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new CurrentActivityPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Current Activity'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 0 hidden)', async ({ page }) => {
    const po = new CurrentActivityPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P75_RELEASE']);
  });
