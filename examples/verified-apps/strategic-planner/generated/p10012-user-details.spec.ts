 ...
 * (./p10012-user-details.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, form-on-manage-user-access
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), form-on-manage-user-access (form).
...
import { UserDetailsPage } from './p10012-user-details.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 10012: User Details [requires auth]', () => {
  ...
  test('every .apx pageItem exists in DOM/apex.item registry (5 items, 3 hidden)', async ({ page }) => {
    const po = new UserDetailsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P10012_APPLICATION_ID', 'P10012_ID', 'P10012_OLD_ROLE_NAME', 'P10012_ROLE_IDS', 'P10012_USER_NAME']);
  });

  test('apex.item round-trip on P10012_USER_NAME', async ({ page }) => {
    const po = new UserDetailsPage(page);
    await po.goto();
  ...
  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new UserDetailsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Add User', 'Remove All Access', 'Apply Changes']);
  });
});
