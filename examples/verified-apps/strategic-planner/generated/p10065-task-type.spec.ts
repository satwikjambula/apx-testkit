 ...
 * (./p10065-task-type.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, task-type, used-as-default-for
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), task-type (form), used-as-default-for (classicReport).
...
import { TaskTypePage } from './p10065-task-type.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 10065: Task Type [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new TaskTypePage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new TaskTypePage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Task Type'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (12 items, 5 hidden)', async ({ page }) => {
    const po = new TaskTypePage(page);
    await po.goto();
    await expectItemsPresent(page, ['P10065_CREATED', 'P10065_CREATED_BY', 'P10065_DESCRIPTION', 'P10065_DISPLAY_SEQ', 'P10065_ID', 'P10065_INCLUDE_YN', 'P10065_PARENT_TYPE_ID', 'P10065_STATIC_ID', 'P10065_TASK_NAME_REQ_YN', 'P10065_TASK_TYPE', 'P10065_UPDATED', 'P10065_UPDATED_BY']);
  });

  test('apex.item round-trip on P10065_DESCRIPTION', async ({ page }) => {
    const po = new TaskTypePage(page);
    await po.goto();
  ...
  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new TaskTypePage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Add Task Type', 'Delete', 'Apply Changes']);
  });
});
