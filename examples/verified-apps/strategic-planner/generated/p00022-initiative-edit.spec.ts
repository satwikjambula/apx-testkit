 ...
 * (./p00022-initiative-edit.page.js), not raw testkit calls, so both stay in sync.
 ...
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), nomenclature-initiative (form).
...
import { InitiativeEditPage } from './p00022-initiative-edit.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 22: Initiative [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new InitiativeEditPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new InitiativeEditPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('&NOMENCLATURE_INITIATIVE.'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (8 items, 1 hidden)', async ({ page }) => {
    const po = new InitiativeEditPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P22_FOCUS_AREA_ID', 'P22_HIDDEN_BY_DEFAULT_YN', 'P22_ID', 'P22_IMAGE', 'P22_INITIATIVE', 'P22_OBJECTIVE', 'P22_SPONSOR_ID', 'P22_TAGS']);
  });

  test('apex.item round-trip on P22_INITIATIVE', async ({ page }) => {
    const po = new InitiativeEditPage(page);
    await po.goto();
