 ...
 * (./p00010-related-project.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: audit-info, buttons, related-project
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): audit-info (dynamicContent), buttons (staticContent), related-project (form).
...
import { RelatedProjectPage } from './p00010-related-project.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 10: Related Project [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new RelatedProjectPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new RelatedProjectPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Related &NOMENCLATURE_PROJECT.'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (10 items, 7 hidden)', async ({ page }) => {
    const po = new RelatedProjectPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P10_CREATED', 'P10_CREATED_BY', 'P10_HEADER_INFO', 'P10_ID', 'P10_PROJECT_ID', 'P10_PROJECT_RELATION', 'P10_RELATED_PROJECT_ID', 'P10_RELATION_TYPE', 'P10_UPDATED', 'P10_UPDATED_BY']);
  });

  test('apex.item round-trip on P10_PROJECT_RELATION', async ({ page }) => {
    const po = new RelatedProjectPage(page);
    await po.goto();
    await po.projectRelation.setValue('apx-testgen');
    expect(await po.projectRelation.getValue()).toBe('apx-testgen');
  });
