 ...
 * (./p11310-archived-project.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: archived-nomenclature-project, buttons
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): archived-nomenclature-project (form), buttons (staticContent).
...
import { ArchivedProjectPage } from './p11310-archived-project.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 11310: Archived project [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ArchivedProjectPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ArchivedProjectPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Archived &NOMENCLATURE_PROJECT.'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (6 items, 0 hidden)', async ({ page }) => {
    const po = new ArchivedProjectPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P11310_CREATED', 'P11310_CREATED_BY', 'P11310_ID', 'P11310_PROJECT', 'P11310_UPDATED', 'P11310_UPDATED_BY']);
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new ArchivedProjectPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Delete']);
  });
});
