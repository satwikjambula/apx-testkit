 ...
 * (./p00108-release1.page.js), not raw testkit calls, so both stay in sync.
...
import { Release1Page } from './p00108-release1.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 108: Release [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new Release1Page(page);
  ...
  test('every labeled .apx button exists (3 buttons)', async ({ page }) => {
    const po = new Release1Page(page);
    await po.goto();
    await expectButtonsPresent(page, ['About Release Milestones', 'About &NOMENCLATURE_PROJECTS.', 'About &NOMENCLATURE_USERS.']);
  });
});
