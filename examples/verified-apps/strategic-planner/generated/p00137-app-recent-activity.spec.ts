/**
 * GENERATED by apx-testgen from pages/p00137-app-recent-activity.apx — DO NOT EDIT.
 ...
 * (./p00137-app-recent-activity.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: app-recent-activity-24-hours, filter
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): app-recent-activity-24-hours (classicReport), filter (staticContent).
...
import { AppRecentActivityPage } from './p00137-app-recent-activity.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 137: App Recent Activity [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new AppRecentActivityPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new AppRecentActivityPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('App Recent Activity'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 0 hidden)', async ({ page }) => {
    const po = new AppRecentActivityPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P137_DAYS']);
  });
});
