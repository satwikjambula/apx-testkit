 ...
 * (./p10760-external-system-link.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, external-system-link
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), external-system-link (form).
...
import { ExternalSystemLinkPage } from './p10760-external-system-link.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 10760: External System Link [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ExternalSystemLinkPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ExternalSystemLinkPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('External System Link'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (8 items, 1 hidden)', async ({ page }) => {
    const po = new ExternalSystemLinkPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P10760_DESCRIPTION', 'P10760_EVAULATION_SEQUENCE', 'P10760_EXTERNAL_SYSTEM_NAME', 'P10760_ID', 'P10760_IS_ACTIVE_YN', 'P10760_LINK_PATTERN', 'P10760_REQUIRED_INITIATIVE_ID', 'P10760_TICKET_ID_REGEX']);
  });

  test('apex.item round-trip on P10760_DESCRIPTION', async ({ page }) => {
    const po = new ExternalSystemLinkPage(page);
    await po.goto();
