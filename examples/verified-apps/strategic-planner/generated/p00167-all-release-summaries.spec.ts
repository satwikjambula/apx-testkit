/**
 * GENERATED by apx-testgen from pages/p00167-all-release-summaries.apx — DO NOT EDIT.
 ...
 * (./p00167-all-release-summaries.page.js), not raw testkit calls, so both stay in sync.
...
import { AllReleaseSummariesPage } from './p00167-all-release-summaries.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 167: All Release Summaries [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new AllReleaseSummariesPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new AllReleaseSummariesPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('All Release Summaries'));
  });
