/**
 * GENERATED by apx-testgen from pages/p00096-involved-people.apx — DO NOT EDIT.
 ...
 * (./p00096-involved-people.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: users
...
import { InvolvedPeoplePage } from './p00096-involved-people.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 96: Involved People [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new InvolvedPeoplePage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new InvolvedPeoplePage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Involved People'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 1 hidden)', async ({ page }) => {
    const po = new InvolvedPeoplePage(page);
    await po.goto();
    await expectItemsPresent(page, ['P96_ID']);
  });
});
