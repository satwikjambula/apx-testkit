/**
 * GENERATED by apx-testgen from pages/p00202-release-dashboard.apx — DO NOT EDIT.
 ...
 * (./p00202-release-dashboard.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, dashboard, menubar, nomenclature-projects-by-owner, nomenclature-projects-by-priority, nomenclature-projects-by-size, release
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), dashboard (staticContent), menubar (themeTemplateComponent/contentRow), nomenclature-projects-by-owner (classicReport), nomenclature-projects-by-priority (classicReport), nomenclature-projects-by-size (classicReport), release (classicReport).
...
import { ReleaseDashboardPage } from './p00202-release-dashboard.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 202: Release Dashboard [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ReleaseDashboardPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ReleaseDashboardPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Release Dashboard'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (2 items, 2 hidden)', async ({ page }) => {
    const po = new ReleaseDashboardPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P202_RELEASE', 'P202_RELEASE_ID']);
  });
