 ...
 * (./p00153-user-project-exceptions.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, buttons, hidden-stuff, nomenclature-project-exceptions, project-exceptions-content
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), buttons (staticContent), hidden-stuff (staticContent), nomenclature-project-exceptions (staticContent), project-exceptions-content (dynamicContent).
...
import { UserProjectExceptionsPage } from './p00153-user-project-exceptions.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 153: Project Exceptions [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new UserProjectExceptionsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new UserProjectExceptionsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('&NOMENCLATURE_PROJECT. Exceptions'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (4 items, 4 hidden)', async ({ page }) => {
    const po = new UserProjectExceptionsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P153_EMAIL', 'P153_FIRST_NAME', 'P153_ID', 'P153_LAST_NAME']);
  });

  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new UserProjectExceptionsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Email to me', 'Subscribe me', 'Unsubscribe', 'Navigate Up']);
  });
});
