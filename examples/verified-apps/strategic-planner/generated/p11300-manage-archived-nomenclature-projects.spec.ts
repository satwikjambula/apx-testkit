/**
 * GENERATED by apx-testgen from pages/p11300-manage-archived-nomenclature-projects.apx — DO NOT EDIT.
 ...
 * (./p11300-manage-archived-nomenclature-projects.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: archived-nomenclature-projects, breadcrumb
...
import { ManageArchivedNomenclatureProjectsPage } from './p11300-manage-archived-nomenclature-projects.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 11300: Manage Archived Projects [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ManageArchivedNomenclatureProjectsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ManageArchivedNomenclatureProjectsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Manage Archived &NOMENCLATURE_PROJECTS.'));
  });
  ...
  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new ManageArchivedNomenclatureProjectsPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['archived-nomenclature-projects']);
  });
});
