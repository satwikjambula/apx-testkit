/**
 * GENERATED by apx-testgen from pages/p10770-home-page-messages.apx — DO NOT EDIT.
 ...
 * (./p10770-home-page-messages.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, home-page-messages
...
import { HomePageMessagesPage } from './p10770-home-page-messages.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 10770: Home Page Messages [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new HomePageMessagesPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new HomePageMessagesPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Home Page Messages'));
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new HomePageMessagesPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Add Message', 'Navigate Up']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new HomePageMessagesPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['home-page-messages']);
  });
});
