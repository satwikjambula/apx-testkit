/**
 * GENERATED by apx-testgen from pages/p00007-comments-filter-report.apx — DO NOT EDIT.
 ...
 * (./p00007-comments-filter-report.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, facets, nomenclature-project-comments
 ...
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), nomenclature-project-comments (classicReport).
...
import { CommentsFilterReportPage } from './p00007-comments-filter-report.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 7: Comments Filter Report [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new CommentsFilterReportPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new CommentsFilterReportPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Comments Filter Report'));
  });

  test('every labeled .apx button exists (3 buttons)', async ({ page }) => {
    const po = new CommentsFilterReportPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Interactive Report', 'Reset', 'Navigate Up']);
  });
