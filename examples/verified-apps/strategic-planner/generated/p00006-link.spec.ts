 ...
 * (./p00006-link.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, nomenclature-project-link
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), nomenclature-project-link (form).
...
import { LinkPage } from './p00006-link.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 6: Link [requires auth]', () => {
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new LinkPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('&NOMENCLATURE_PROJECT. Link'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (5 items, 1 hidden)', async ({ page }) => {
    const po = new LinkPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P6_ID', 'P6_IMPORTANT_YN', 'P6_LINK_NAME', 'P6_LINK_URL', 'P6_PROJECT_ID']);
  });

  test('apex.item round-trip on P6_LINK_NAME', async ({ page }) => {
    const po = new LinkPage(page);
    await po.goto();
