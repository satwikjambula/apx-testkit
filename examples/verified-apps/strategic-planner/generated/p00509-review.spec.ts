 ...
 * (./p00509-review.page.js), not raw testkit calls, so both stay in sync.
...
import { ReviewPage } from './p00509-review.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 509: Review [requires auth]', () => {
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ReviewPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Review'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (17 items, 8 hidden)', async ({ page }) => {
    const po = new ReviewPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P509_CREATED', 'P509_CREATED_BY', 'P509_DESCRIPTION', 'P509_ID', 'P509_IMPACT', 'P509_OLD_OWNER_ID', 'P509_OWNER_ID', 'P509_PROJECT_ID', 'P509_START_DATE', 'P509_STATUS_ID', 'P509_STATUS_LAST_CHANGED_ON', 'P509_TAGS', 'P509_TARGET_COMPLETE', 'P509_TASK_SUB_TYPE_ID', 'P509_TASK_TYPE_ID', 'P509_UPDATED', 'P509_UPDATED_BY']);
  });

  test('apex.item round-trip on P509_DESCRIPTION', async ({ page }) => {
    const po = new ReviewPage(page);
    await po.goto();
