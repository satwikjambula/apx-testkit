/**
 * GENERATED by apx-testgen from pages/p00054-add-multiple-people.apx — DO NOT EDIT.
 ...
 * (./p00054-add-multiple-people.page.js), not raw testkit calls, so both stay in sync.
...
import { AddMultiplePeoplePage } from './p00054-add-multiple-people.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 54: Add Multiple &NOMENCLATURE_USERS. [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new AddMultiplePeoplePage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new AddMultiplePeoplePage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Add Multiple &NOMENCLATURE_USERS.'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 0 hidden)', async ({ page }) => {
    const po = new AddMultiplePeoplePage(page);
    await po.goto();
    await expectItemsPresent(page, ['P54_PRELIM_USERS']);
  });

  test('apex.item round-trip on P54_PRELIM_USERS', async ({ page }) => {
    const po = new AddMultiplePeoplePage(page);
    await po.goto();
