 ...
 * (./p10151-initiative-focus-area.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, initiative-focus-area
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), initiative-focus-area (form).
...
import { InitiativeFocusAreaPage } from './p10151-initiative-focus-area.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 10151: Initiative Focus Area [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new InitiativeFocusAreaPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new InitiativeFocusAreaPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Initiative Focus Area'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (11 items, 5 hidden)', async ({ page }) => {
    const po = new InitiativeFocusAreaPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P10151_ACTIVE_YN', 'P10151_CREATED', 'P10151_CREATED_BY', 'P10151_DESCRIPTION', 'P10151_DEVELOPMENT_OWNER_ID', 'P10151_DISPLAY_SEQUENCE', 'P10151_FOCUS_AREA', 'P10151_ID', 'P10151_INITIATIVE_ID', 'P10151_UPDATED', 'P10151_UPDATED_BY']);
  });

  test('apex.item round-trip on P10151_DESCRIPTION', async ({ page }) => {
    const po = new InitiativeFocusAreaPage(page);
    await po.goto();
  ...
  test('every labeled .apx button exists (5 buttons)', async ({ page }) => {
    const po = new InitiativeFocusAreaPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Add', 'Add and Add Another', 'Delete', 'Apply Changes']);
  });
});
