 ...
 * (./p00087-initiative-comment.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, initiative-comment
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), initiative-comment (form).
...
import { InitiativeCommentPage } from './p00087-initiative-comment.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 87: Initiative Comment [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new InitiativeCommentPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new InitiativeCommentPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('&NOMENCLATURE_INITIATIVE. Comment'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (4 items, 2 hidden)', async ({ page }) => {
    const po = new InitiativeCommentPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P87_BODY', 'P87_ID', 'P87_NEW_IMAGE_REF_ID', 'P87_PRIVATE_YN']);
  });
