 ...
 * (./p00058-add-multiple-people-step-2.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, exceptions, hidden-items, no-valid-users-exist-page-info, p58-valid-count-users-to-add, valid-users-exist-page-info, wizard-container
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), exceptions (classicReport), hidden-items (staticContent), no-valid-users-exist-page-info (staticContent), p58-valid-count-users-to-add (listView), valid-users-exist-page-info (plSqlDynamicContent), wizard-container (staticContent).
...
import { AddMultiplePeopleStep2Page } from './p00058-add-multiple-people-step-2.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 58: Add Multiple &NOMENCLATURE_USERS - Step 2 [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new AddMultiplePeopleStep2Page(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new AddMultiplePeopleStep2Page(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Add Multiple &NOMENCLATURE_USERS. - Step 2'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (2 items, 2 hidden)', async ({ page }) => {
    const po = new AddMultiplePeopleStep2Page(page);
    await po.goto();
    await expectItemsPresent(page, ['P58_INVALID_COUNT', 'P58_VALID_COUNT']);
  });
