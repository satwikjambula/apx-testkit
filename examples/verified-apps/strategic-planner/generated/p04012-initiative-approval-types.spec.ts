/**
 * GENERATED by apx-testgen from pages/p04012-initiative-approval-types.apx — DO NOT EDIT.
 ...
 * (./p04012-initiative-approval-types.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, nomenclature-initiative-approval-types
...
import { InitiativeApprovalTypesPage } from './p04012-initiative-approval-types.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 4012: Initiative Approval Types [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new InitiativeApprovalTypesPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new InitiativeApprovalTypesPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('&NOMENCLATURE_INITIATIVE. Approval Types'));
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new InitiativeApprovalTypesPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Add Type', 'Navigate Up']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new InitiativeApprovalTypesPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['nomenclature-initiative-approval-types']);
  });
});
