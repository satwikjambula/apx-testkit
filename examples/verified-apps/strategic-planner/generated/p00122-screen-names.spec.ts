/**
 * GENERATED by apx-testgen from pages/p00122-screen-names.apx — DO NOT EDIT.
 ...
 * (./p00122-screen-names.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, screen-names
...
import { ScreenNamesPage } from './p00122-screen-names.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 122: Screen Names [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ScreenNamesPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ScreenNamesPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Screen Names'));
  });
  ...
  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new ScreenNamesPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['screen-names']);
  });
});
