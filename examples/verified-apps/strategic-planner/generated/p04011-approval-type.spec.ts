 ...
 * (./p04011-approval-type.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: approval-type, buttons, existing-types
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): approval-type (form), buttons (staticContent), existing-types (classicReport).
...
import { ApprovalTypePage } from './p04011-approval-type.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 4011: Approval Type [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ApprovalTypePage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ApprovalTypePage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Approval Type'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (5 items, 1 hidden)', async ({ page }) => {
    const po = new ApprovalTypePage(page);
    await po.goto();
    await expectItemsPresent(page, ['P4011_ACTIVE_YN', 'P4011_APPROVAL_TYPE', 'P4011_DESCRIPTION', 'P4011_DISPLAY_SEQ', 'P4011_ID']);
  });

  test('apex.item round-trip on P4011_APPROVAL_TYPE', async ({ page }) => {
    const po = new ApprovalTypePage(page);
    await po.goto();
    await po.approvalType.setValue('apx-testgen');
    expect(await po.approvalType.getValue()).toBe('apx-testgen');
  });
