 ...
 * (./p10600-contributor-roles-admin.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, resource-types, sql
...
import { ContributorRolesAdminPage } from './p10600-contributor-roles-admin.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 10600: Contributor Roles [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ContributorRolesAdminPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ContributorRolesAdminPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Contributor Roles'));
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new ContributorRolesAdminPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Add Role', 'Navigate Up']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new ContributorRolesAdminPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['resource-types']);
  });
});
