/**
 * GENERATED by apx-testgen from pages/p10516-release-milestone-default-tag.apx — DO NOT EDIT.
 ...
 * (./p10516-release-milestone-default-tag.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, release-milestone-default-tag
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), release-milestone-default-tag (form).
...
import { ReleaseMilestoneDefaultTagPage } from './p10516-release-milestone-default-tag.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 10516: Release Milestone Default Tag [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ReleaseMilestoneDefaultTagPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ReleaseMilestoneDefaultTagPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Release Milestone Default Tag'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (4 items, 1 hidden)', async ({ page }) => {
    const po = new ReleaseMilestoneDefaultTagPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P10516_DESCRIPTION', 'P10516_DISPLAY_SEQUENCE', 'P10516_ID', 'P10516_TAG']);
  });

  test('apex.item round-trip on P10516_DESCRIPTION', async ({ page }) => {
    const po = new ReleaseMilestoneDefaultTagPage(page);
    await po.goto();
