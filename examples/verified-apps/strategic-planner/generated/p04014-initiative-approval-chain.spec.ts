/**
 * GENERATED by apx-testgen from pages/p04014-initiative-approval-chain.apx — DO NOT EDIT.
 ...
 * (./p04014-initiative-approval-chain.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, initiative-approval-chain
...
import { InitiativeApprovalChainPage } from './p04014-initiative-approval-chain.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 4014: Initiative Approval Chain [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new InitiativeApprovalChainPage(page);
  ...
  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new InitiativeApprovalChainPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Add Entry', 'Navigate Up']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new InitiativeApprovalChainPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['initiative-approval-chain']);
  });
});
