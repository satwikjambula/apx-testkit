 ...
 * (./p10074-notifications.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, notifications, sql
...
import { NotificationsPage } from './p10074-notifications.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 10074: Notifications [requires auth]', () => {
  ...
  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new NotificationsPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['notifications']);
  });
});
