/**
 * GENERATED by apx-testgen from pages/p00106-initiativeabout.apx — DO NOT EDIT.
 ...
 * (./p00106-initiativeabout.page.js), not raw testkit calls, so both stay in sync.
...
import { InitiativeaboutPage } from './p00106-initiativeabout.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 106: Initiative [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new InitiativeaboutPage(page);
  ...
  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new InitiativeaboutPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['About &NOMENCLATURE_PROJECTS.', 'About Releases', 'About Activities', 'About &NOMENCLATURE_AREAS.']);
  });
});
