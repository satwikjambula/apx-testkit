 ...
 * (./p00035-ifa-comment.page.js), not raw testkit calls, so both stay in sync.
...
import { IfaCommentPage } from './p00035-ifa-comment.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 35: Edit Initiative Focus Area Comment [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new IfaCommentPage(page);
  ...
  test('every .apx pageItem exists in DOM/apex.item registry (4 items, 2 hidden)', async ({ page }) => {
    const po = new IfaCommentPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P35_BODY', 'P35_ID', 'P35_NEW_IMAGE_REF_ID', 'P35_PRIVATE_YN']);
  });
