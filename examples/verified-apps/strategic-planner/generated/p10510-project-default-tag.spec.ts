 ...
 * (./p10510-project-default-tag.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, nomenclature-project-default-tag
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), nomenclature-project-default-tag (form).
...
import { ProjectDefaultTagPage } from './p10510-project-default-tag.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 10510: Project Default Tag [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ProjectDefaultTagPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ProjectDefaultTagPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('&NOMENCLATURE_PROJECT. Default Tag'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (4 items, 1 hidden)', async ({ page }) => {
    const po = new ProjectDefaultTagPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P10510_DESCRIPTION', 'P10510_DISPLAY_SEQUENCE', 'P10510_ID', 'P10510_TAG']);
  });

  test('apex.item round-trip on P10510_DESCRIPTION', async ({ page }) => {
    const po = new ProjectDefaultTagPage(page);
    await po.goto();
