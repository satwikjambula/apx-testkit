/**
 * GENERATED by apx-testgen from pages/p00046-image-gallery.apx — DO NOT EDIT.
 ...
 * (./p00046-image-gallery.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, facets, image-gallery
...
import { ImageGalleryPage } from './p00046-image-gallery.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 46: Image Gallery [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ImageGalleryPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ImageGalleryPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Image Gallery'));
  });
  ...
  test('every interactiveReport/cards/facetedSearch region resolves (2 regions)', async ({ page }) => {
    const po = new ImageGalleryPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['facets', 'image-gallery']);
  });
});
