 ...
 * (./p00068-ifa-images.page.js), not raw testkit calls, so both stay in sync.
...
import { IfaImagesPage } from './p00068-ifa-images.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 68: Initiative Focus Area Images [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new IfaImagesPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new IfaImagesPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('&P68_FOCUS_AREA. - Images'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (2 items, 2 hidden)', async ({ page }) => {
    const po = new IfaImagesPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P68_FOCUS_AREA', 'P68_INIT_FOCUS_AREA_ID']);
  });
