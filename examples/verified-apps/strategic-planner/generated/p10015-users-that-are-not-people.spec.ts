 ...
 * (./p10015-users-that-are-not-people.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, users
...
import { UsersThatAreNotPeoplePage } from './p10015-users-that-are-not-people.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 10015: Users that are not People [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new UsersThatAreNotPeoplePage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new UsersThatAreNotPeoplePage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Users that are not &NOMENCLATURE_USERS.'));
  });
  ...
  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new UsersThatAreNotPeoplePage(page);
    await po.goto();
    await expectRegionsResolve(page, ['users']);
  });
});
