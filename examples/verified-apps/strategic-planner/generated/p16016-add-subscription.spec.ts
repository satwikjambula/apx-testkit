 ...
 * (./p16016-add-subscription.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, subscriptions
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), subscriptions (classicReport).
...
import { AddSubscriptionPage } from './p16016-add-subscription.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 16016: Add Subscription [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new AddSubscriptionPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new AddSubscriptionPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Add Subscription'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (5 items, 1 hidden)', async ({ page }) => {
    const po = new AddSubscriptionPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P16016_FREQUENCY', 'P16016_NOTIFICATION_ID', 'P16016_RELEASE_EXCEPTIONS_ID', 'P16016_RELEASE_ID', 'P16016_TEAM_MEMBER_ID']);
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new AddSubscriptionPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Add Subscription', 'Cancel']);
  });
});
