/**
 * GENERATED by apx-testgen from pages/p16017-about-subscriptions.apx — DO NOT EDIT.
 ...
 * (./p16017-about-subscriptions.page.js), not raw testkit calls, so both stay in sync.
...
import { AboutSubscriptionsPage } from './p16017-about-subscriptions.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 16017: About Subscriptions [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new AboutSubscriptionsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new AboutSubscriptionsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('About Subscriptions'));
  });
});
