/**
 * GENERATED by apx-testgen from pages/p10791-notification-email-contents.apx — DO NOT EDIT.
 ...
 * (./p10791-notification-email-contents.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: notification-email-contents
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): notification-email-contents (classicReport).
...
import { NotificationEmailContentsPage } from './p10791-notification-email-contents.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 10791: Notification Email Contents [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new NotificationEmailContentsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new NotificationEmailContentsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Notification Email Contents'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 1 hidden)', async ({ page }) => {
    const po = new NotificationEmailContentsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P10791_ID']);
  });
});
