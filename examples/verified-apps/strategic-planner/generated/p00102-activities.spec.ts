/**
 * GENERATED by apx-testgen from pages/p00102-activities.apx — DO NOT EDIT.
 ...
 * (./p00102-activities.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: activities, breadcrumb, buttons-container, faceted-search, menubar
 ...
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): activities (themeTemplateComponent/contentRow), breadcrumb (breadcrumb), buttons-container (staticContent), menubar (themeTemplateComponent/contentRow).
...
import { ActivitiesPage } from './p00102-activities.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 102: Activities [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ActivitiesPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ActivitiesPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Activities'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 0 hidden)', async ({ page }) => {
    const po = new ActivitiesPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P102_ORDER_BY']);
  });
