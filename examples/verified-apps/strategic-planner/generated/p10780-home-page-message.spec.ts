 ...
 * (./p10780-home-page-message.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, home-page-message
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), home-page-message (form).
...
import { HomePageMessagePage } from './p10780-home-page-message.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 10780: Home Page Message [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new HomePageMessagePage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new HomePageMessagePage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Home Page Message'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (7 items, 1 hidden)', async ({ page }) => {
    const po = new HomePageMessagePage(page);
    await po.goto();
    await expectItemsPresent(page, ['P10780_FROM_DATE', 'P10780_ID', 'P10780_IS_ACTIVE_YN', 'P10780_NOTIFICATION_DESC', 'P10780_NOTIFICATION_NAME', 'P10780_REQUIRED_USER_TAG', 'P10780_TO_DATE']);
  });

  test('apex.item round-trip on P10780_NOTIFICATION_DESC', async ({ page }) => {
    const po = new HomePageMessagePage(page);
    await po.goto();
    await po.notificationDesc.setValue('apx-testgen');
    expect(await po.notificationDesc.getValue()).toBe('apx-testgen');
  });
