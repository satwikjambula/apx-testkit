 ...
 * (./p00203-release-calendar.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, button-bar, menubar, release, search, search-results
 ...
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), button-bar (staticContent), menubar (themeTemplateComponent/contentRow), release (classicReport), search-results (calendar).
...
import { ReleaseCalendarPage } from './p00203-release-calendar.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 203: Release Calendar [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ReleaseCalendarPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ReleaseCalendarPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Release Calendar'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (2 items, 2 hidden)', async ({ page }) => {
    const po = new ReleaseCalendarPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P203_RELEASE', 'P203_RELEASE_ID']);
  });

  test('every labeled .apx button exists (3 buttons)', async ({ page }) => {
    const po = new ReleaseCalendarPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Report View', 'Reset', 'Navigate Up']);
  });
