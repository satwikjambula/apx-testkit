 ...
 * (./p00162-control-chart.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: control-chart
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * 1 chart region(s) SKIPPED -- no htmlDomId set, runtime id genuinely unconstructible from static data (ADR-003 layer 3): control-chart.
...
import { ControlChartPage } from './p00162-control-chart.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 162: Control Chart [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ControlChartPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ControlChartPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Control Chart'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (6 items, 1 hidden)', async ({ page }) => {
    const po = new ControlChartPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P162_FOCUS_AREA', 'P162_INITIATIVE_ID', 'P162_IN_PROGRESS_FROM', 'P162_IN_PROGRESS_TO', 'P162_PRIORITY', 'P162_PROJECT_SIZE']);
  });
});
