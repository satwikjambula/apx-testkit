/**
 * GENERATED by apx-testgen from pages/p00099-past-due-ir.apx — DO NOT EDIT.
 ...
 * (./p00099-past-due-ir.page.js), not raw testkit calls, so both stay in sync.
...
import { PastDueIrPage } from './p00099-past-due-ir.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 99: Past Due Milestones, Reviews and Tasks [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new PastDueIrPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new PastDueIrPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Past Due Milestones, Reviews and Tasks'));
  });
