/**
 * GENERATED by apx-testgen from pages/p00159-project-contributors-ir.apx — DO NOT EDIT.
 ...
 * (./p00159-project-contributors-ir.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, search-results
...
import { ProjectContributorsIrPage } from './p00159-project-contributors-ir.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 159: Project Contributors [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ProjectContributorsIrPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ProjectContributorsIrPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('&NOMENCLATURE_PROJECT. Contributors'));
  });
