/**
 * GENERATED by apx-testgen from pages/p10800-priorities.apx — DO NOT EDIT.
 ...
 * (./p10800-priorities.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, nomenclature-project-priorities, sql
...
import { PrioritiesPage } from './p10800-priorities.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 10800: Priorities [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new PrioritiesPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new PrioritiesPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('&NOMENCLATURE_PROJECT. Priorities'));
  });

  test('every labeled .apx button exists (3 buttons)', async ({ page }) => {
    const po = new PrioritiesPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Add Priority', 'Reset', 'Navigate Up']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new PrioritiesPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['nomenclature-project-priorities']);
  });
});
