 ...
 * (./p00039-page-views.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, page-performance
...
import { PageViewsPage } from './p00039-page-views.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 39: Page Views [requires auth]', () => {
  ...
  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new PageViewsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Page Performance', 'Refresh', 'Up', 'User Interactions']);
  });
