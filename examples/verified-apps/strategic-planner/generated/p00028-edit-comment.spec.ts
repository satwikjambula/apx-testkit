 ...
 * (./p00028-edit-comment.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, edit-comment
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), edit-comment (form).
...
import { EditCommentPage } from './p00028-edit-comment.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 28: Edit Comment [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new EditCommentPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new EditCommentPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Edit Comment'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (6 items, 3 hidden)', async ({ page }) => {
    const po = new EditCommentPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P28_AUTHOR_ID', 'P28_BODY', 'P28_ID', 'P28_NEW_IMAGE_REF_ID', 'P28_PRIVATE_YN', 'P28_PROJECT_ID']);
  });
