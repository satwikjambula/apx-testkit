 ...
 * (./p00155-default-competency.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, default-competency
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), default-competency (form).
...
import { DefaultCompetencyPage } from './p00155-default-competency.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 155: Default Competency [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new DefaultCompetencyPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new DefaultCompetencyPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Default Competency'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (3 items, 1 hidden)', async ({ page }) => {
    const po = new DefaultCompetencyPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P155_COMPETENCY', 'P155_DESCRIPTION', 'P155_ID']);
  });

  test('apex.item round-trip on P155_COMPETENCY', async ({ page }) => {
    const po = new DefaultCompetencyPage(page);
    await po.goto();
    await po.competency.setValue('apx-testgen');
    expect(await po.competency.getValue()).toBe('apx-testgen');
  });

  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new DefaultCompetencyPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Add Competency', 'Delete', 'Apply Changes']);
  });
});
