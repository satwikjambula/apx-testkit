 ...
 * (./p00501-task.page.js), not raw testkit calls, so both stay in sync.
...
import { TaskPage } from './p00501-task.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 501: Task [requires auth]', () => {
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new TaskPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Task'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (18 items, 8 hidden)', async ({ page }) => {
    const po = new TaskPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P501_CHILD_COUNT', 'P501_CREATED', 'P501_CREATED_BY', 'P501_DESCRIPTION', 'P501_ID', 'P501_OLD_OWNER_ID', 'P501_OWNER_ID', 'P501_PROJECT_ID', 'P501_START_DATE', 'P501_STATUS_ID', 'P501_STATUS_LAST_CHANGED_ON', 'P501_TAGS', 'P501_TARGET_COMPLETE', 'P501_TASK', 'P501_TASK_SUB_TYPE_ID', 'P501_TASK_TYPE_ID', 'P501_UPDATED', 'P501_UPDATED_BY']);
  });

  test('apex.item round-trip on P501_DESCRIPTION', async ({ page }) => {
    const po = new TaskPage(page);
    await po.goto();
