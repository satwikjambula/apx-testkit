/**
 * GENERATED by apx-testgen from pages/p00071-page-performance.apx — DO NOT EDIT.
 ...
 * (./p00071-page-performance.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, page-performance, timeframe
 ...
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), timeframe (staticContent).
...
import { PagePerformancePage } from './p00071-page-performance.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 71: Page Performance [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new PagePerformancePage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new PagePerformancePage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Page Performance'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 0 hidden)', async ({ page }) => {
    const po = new PagePerformancePage(page);
    await po.goto();
    await expectItemsPresent(page, ['P71_TIMEFRAME']);
  });

  test('every labeled .apx button exists (5 buttons)', async ({ page }) => {
    const po = new PagePerformancePage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Page Views', 'Refresh', 'Set', 'Up', 'User Interactions']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new PagePerformancePage(page);
    await po.goto();
    await expectRegionsResolve(page, ['page-performance']);
  });
});
