 ...
 * (./p00121-request-approval.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: approval-chain, button-container, request-approval
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): approval-chain (classicReport), button-container (staticContent), request-approval (staticContent).
...
import { RequestApprovalPage } from './p00121-request-approval.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 121: Request Approval [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new RequestApprovalPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new RequestApprovalPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Request Approval'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (8 items, 5 hidden)', async ({ page }) => {
    const po = new RequestApprovalPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P121_APPROVAL_TYPE', 'P121_APPROVAL_TYPE_ID', 'P121_APP_NAME', 'P121_JUSTIFICATION', 'P121_PROJECT', 'P121_PROJECT_APPROVAL_ID', 'P121_PROJECT_ID', 'P121_PROJECT_URL']);
  });

  test('apex.item round-trip on P121_JUSTIFICATION', async ({ page }) => {
    const po = new RequestApprovalPage(page);
    await po.goto();
    await po.justification.setValue('apx-testgen');
    expect(await po.justification.getValue()).toBe('apx-testgen');
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new RequestApprovalPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Request Approval']);
  });
});
