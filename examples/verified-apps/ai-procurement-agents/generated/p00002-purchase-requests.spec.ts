/**
 * GENERATED by apx-testgen from pages/p00002-purchase-requests.apx — DO NOT EDIT.
 ...
 * (./p00002-purchase-requests.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: purchase-requests
...
import { PurchaseRequestsPage } from './p00002-purchase-requests.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 2: Purchase Requests [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new PurchaseRequestsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new PurchaseRequestsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Purchase Requests'));
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new PurchaseRequestsPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['purchase-requests']);
  });
});
