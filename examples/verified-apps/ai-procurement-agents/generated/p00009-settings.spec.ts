/**
 * GENERATED by apx-testgen from pages/p00009-settings.apx — DO NOT EDIT.
 ...
 * (./p00009-settings.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: app-settings
...
import { SettingsPage } from './p00009-settings.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 9: Settings [requires auth]', () => {
  ...
  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new SettingsPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['app-settings']);
  });
});
