 ...
 * Regions present in metadata: dashboard-metrics, recent-agent-activity
...
test.describe('page 1: Dashboard [requires auth]', () => {
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new HomePage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('AI Procurement Agents'));
  });

  test('every interactiveReport/cards/facetedSearch region resolves (2 regions)', async ({ page }) => {
    const po = new HomePage(page);
    await po.goto();
    await expectRegionsResolve(page, ['dashboard-metrics', 'recent-agent-activity']);
  });
});
