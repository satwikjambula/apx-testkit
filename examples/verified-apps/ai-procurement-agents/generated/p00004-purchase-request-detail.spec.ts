/**
 * GENERATED by apx-testgen from pages/p00004-purchase-request-detail.apx — DO NOT EDIT.
 ...
 * (./p00004-purchase-request-detail.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: request-summary, ai-recommendation, latest-tool-calls
...
import { PurchaseRequestDetailPage } from './p00004-purchase-request-detail.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 4: Purchase Request Detail [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new PurchaseRequestDetailPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new PurchaseRequestDetailPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Purchase Request Detail'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 1 hidden)', async ({ page }) => {
    const po = new PurchaseRequestDetailPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P4_PURCHASE_REQUEST_ID']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (3 regions)', async ({ page }) => {
    const po = new PurchaseRequestDetailPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['request-summary', 'ai-recommendation', 'latest-tool-calls']);
  });
});
