/**
 * GENERATED by apx-testgen from pages/p00006-approval-inbox.apx — DO NOT EDIT.
 ...
 * (./p00006-approval-inbox.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: approval-inbox
...
import { ApprovalInboxPage } from './p00006-approval-inbox.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 6: Approval Inbox [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ApprovalInboxPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ApprovalInboxPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Approval Inbox'));
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new ApprovalInboxPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['approval-inbox']);
  });
});
