/**
 * GENERATED by apx-testgen from pages/p00007-policies.apx — DO NOT EDIT.
 ...
 * (./p00007-policies.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: policies
...
import { PoliciesPage } from './p00007-policies.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 7: Policies [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new PoliciesPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new PoliciesPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Policies'));
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new PoliciesPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['policies']);
  });
});
