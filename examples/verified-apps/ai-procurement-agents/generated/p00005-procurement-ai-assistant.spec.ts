 ...
 * (./p00005-procurement-ai-assistant.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: assistant-host
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): assistant-host (staticContent).
...
import { ProcurementAiAssistantPage } from './p00005-procurement-ai-assistant.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 5: Procurement AI Assistant [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ProcurementAiAssistantPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ProcurementAiAssistantPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Procurement AI Assistant'));
  });

  test('every labeled .apx button exists (1 button)', async ({ page }) => {
    const po = new ProcurementAiAssistantPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Open Assistant']);
  });
});
