/**
 * GENERATED by apx-testgen from pages/p00008-agent-runs-audit.apx — DO NOT EDIT.
 ...
 * (./p00008-agent-runs-audit.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: agent-runs, tool-calls
...
import { AgentRunsAuditPage } from './p00008-agent-runs-audit.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 8: Agent Runs Audit [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new AgentRunsAuditPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new AgentRunsAuditPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Agent Runs Audit'));
  });

  test('every interactiveReport/cards/facetedSearch region resolves (2 regions)', async ({ page }) => {
    const po = new AgentRunsAuditPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['agent-runs', 'tool-calls']);
  });
});
