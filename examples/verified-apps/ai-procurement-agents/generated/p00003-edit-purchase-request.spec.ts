/**
 * GENERATED by apx-testgen from pages/p00003-edit-purchase-request.apx — DO NOT EDIT.
 ...
 * (./p00003-edit-purchase-request.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: request-form, request-lines
 ...
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): request-form (form).
...
import { EditPurchaseRequestPage } from './p00003-edit-purchase-request.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 3: Create / Edit Purchase Request [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new EditPurchaseRequestPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new EditPurchaseRequestPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Create / Edit Purchase Request'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 1 hidden)', async ({ page }) => {
    const po = new EditPurchaseRequestPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P3_PURCHASE_REQUEST_ID']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new EditPurchaseRequestPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['request-lines']);
  });
});
