 ...
 * Regions present in metadata: car, drive, driving-with-apex
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): car (staticContent), drive (staticContent), driving-with-apex (staticContent).
...
test.describe('page 1: ホーム [requires auth]', () => {
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new HomePage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Driving with APEX'));
  });
});
