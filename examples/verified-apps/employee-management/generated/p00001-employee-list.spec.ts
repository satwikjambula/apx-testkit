 ...
 * (./p00001-employee-list.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, employee-report
...
import { EmployeeListPage } from './p00001-employee-list.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 1: Employee List [requires auth]', () => {
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new EmployeeListPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Employee List'));
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new EmployeeListPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Reset', '従業員登録']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new EmployeeListPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['employee-report']);
  });
});
