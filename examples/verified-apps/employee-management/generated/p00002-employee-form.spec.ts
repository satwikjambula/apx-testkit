 ...
 * (./p00002-employee-form.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, employee-form
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), employee-form (form).
...
import { EmployeeFormPage } from './p00002-employee-form.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 2: Employee Form [requires auth]', () => {
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new EmployeeFormPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Employee Form'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (8 items, 0 hidden)', async ({ page }) => {
    const po = new EmployeeFormPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P2_COMM', 'P2_DEPTNO', 'P2_EMPNO', 'P2_ENAME', 'P2_HIREDATE', 'P2_JOB', 'P2_MGR', 'P2_SAL']);
  });

  test('apex.item round-trip on P2_ENAME', async ({ page }) => {
    const po = new EmployeeFormPage(page);
    await po.goto();
  ...
  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new EmployeeFormPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['キャンセル', '従業員削除', '従業員情報更新', '従業員登録']);
  });
});
