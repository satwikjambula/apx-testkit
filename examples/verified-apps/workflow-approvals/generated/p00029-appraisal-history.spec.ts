/**
 * GENERATED by apx-testgen from pages/p00029-appraisal-history.apx — DO NOT EDIT.
 ...
 * (./p00029-appraisal-history.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: about-this-page, appraisal-history, breadcrumb
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Region types NOT covered by an auto-generated assertion (no verified DOM convention, or a runtime id genuinely unconstructible from static data -- see docs/grammar-assumptions.md "Still open" and ADR-003): about-this-page (staticContent), appraisal-history (classicReport), breadcrumb (breadcrumb).
...
import { AppraisalHistoryPage } from './p00029-appraisal-history.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 29: Appraisal History [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new AppraisalHistoryPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new AppraisalHistoryPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Appraisal History'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 0 hidden)', async ({ page }) => {
    const po = new AppraisalHistoryPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P29_SHOW_FOR_SUBORDINATES']);
  });
});
