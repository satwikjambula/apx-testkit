/**
 * GENERATED by apx-testgen from pages/p00009-approvers.apx — DO NOT EDIT.
 ...
 * (./p00009-approvers.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: about-this-page, approvers, new, salary-change-approvers, tips
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * 1 Interactive Grid region(s) SKIPPED -- no htmlDomId set, runtime id genuinely unconstructible from static data (ADR-003 layer 3): approvers.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): about-this-page (staticContent), new (staticContent), salary-change-approvers (breadcrumb), tips (staticContent).
...
import { ApproversPage } from './p00009-approvers.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 9: Approvers [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ApproversPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ApproversPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Approvers'));
  });

  test('every labeled .apx button exists (1 button)', async ({ page }) => {
    const po = new ApproversPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Test Salary Approver Assignment...']);
  });
});
