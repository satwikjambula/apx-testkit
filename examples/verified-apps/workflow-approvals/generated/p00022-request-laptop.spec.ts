 ...
 * (./p00022-request-laptop.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, request-laptop
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Region types NOT covered by an auto-generated assertion (no verified DOM convention, or a runtime id genuinely unconstructible from static data -- see docs/grammar-assumptions.md "Still open" and ADR-003): buttons (staticContent), request-laptop (form).
...
import { RequestLaptopPage } from './p00022-request-laptop.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 22: Request Laptop [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new RequestLaptopPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new RequestLaptopPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Request Laptop'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (5 items, 2 hidden)', async ({ page }) => {
    const po = new RequestLaptopPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P22_EMPNO', 'P22_ID', 'P22_LAPTOP_TYPE', 'P22_MINIMUM_DATE', 'P22_NEED_BY']);
  });
