 ...
 * (./p00005-my-requests.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: about-this-page, breadcrumb, initiated-by-me-smart-filters, requests-you-made
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): about-this-page (staticContent), breadcrumb (breadcrumb), initiated-by-me-smart-filters (smartFilters), requests-you-made (themeTemplateComponent/contentRow).
...
import { MyRequestsPage } from './p00005-my-requests.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 5: My Requests [requires auth]', () => {
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new MyRequestsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('My Requests'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (3 items, 1 hidden)', async ({ page }) => {
    const po = new MyRequestsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P5_SHOW_EXPIRED', 'P5_SORT_BY', 'P5_TASK_ID']);
  });
});
