 ...
 * (./p00019-workflow-status.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: about-this-page, breadcrumb, my-workflows-report, my-workflows-smart-filters
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): about-this-page (staticContent), breadcrumb (breadcrumb), my-workflows-report (themeTemplateComponent/contentRow), my-workflows-smart-filters (smartFilters).
...
import { WorkflowStatusPage } from './p00019-workflow-status.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 19: Workflow Status [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new WorkflowStatusPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new WorkflowStatusPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Workflow Status'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (2 items, 1 hidden)', async ({ page }) => {
    const po = new WorkflowStatusPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P19_SORT_BY', 'P19_WORKFLOW_ID']);
  });
});
