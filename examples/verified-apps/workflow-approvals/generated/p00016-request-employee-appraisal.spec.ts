/**
 * GENERATED by apx-testgen from pages/p00016-request-employee-appraisal.apx — DO NOT EDIT.
 ...
 * (./p00016-request-employee-appraisal.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, request-appraisal-input
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Region types NOT covered by an auto-generated assertion (no verified DOM convention, or a runtime id genuinely unconstructible from static data -- see docs/grammar-assumptions.md "Still open" and ADR-003): buttons (staticContent), request-appraisal-input (form).
...
import { RequestEmployeeAppraisalPage } from './p00016-request-employee-appraisal.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 16: "Request Employee Appraisal " [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new RequestEmployeeAppraisalPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new RequestEmployeeAppraisalPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Request Appraisal for'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (7 items, 5 hidden)', async ({ page }) => {
    const po = new RequestEmployeeAppraisalPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P16_APPRAISAL_DATE', 'P16_APPRAISAL_PERIOD', 'P16_EMPNO', 'P16_ENAME', 'P16_ID', 'P16_PERIOD_END', 'P16_PERIOD_START']);
  });
