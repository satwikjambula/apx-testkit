 ...
 * (./p00004-request-salary-change.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, request-salary-change
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Region types NOT covered by an auto-generated assertion (no verified DOM convention, or a runtime id genuinely unconstructible from static data -- see docs/grammar-assumptions.md "Still open" and ADR-003): buttons (staticContent), request-salary-change (form).
...
import { RequestSalaryChangePage } from './p00004-request-salary-change.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 4: Request Salary Change [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new RequestSalaryChangePage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new RequestSalaryChangePage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Request Salary Change'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (5 items, 2 hidden)', async ({ page }) => {
    const po = new RequestSalaryChangePage(page);
    await po.goto();
    await expectItemsPresent(page, ['P4_EMPNO', 'P4_ENAME', 'P4_JOB', 'P4_PROPOSED_SALARY', 'P4_SAL']);
  });
