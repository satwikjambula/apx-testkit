 ...
 * (./p00011-email-settings.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: about-this-page, email-settings
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Region types NOT covered by an auto-generated assertion (no verified DOM convention, or a runtime id genuinely unconstructible from static data -- see docs/grammar-assumptions.md "Still open" and ADR-003): about-this-page (staticContent), email-settings (breadcrumb).
...
import { EmailSettingsPage } from './p00011-email-settings.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 11: Email Settings [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new EmailSettingsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new EmailSettingsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Email Settings'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 0 hidden)', async ({ page }) => {
    const po = new EmailSettingsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P11_DEMO_NOTIFICATION_EMAIL']);
  });

  test('apex.item round-trip on P11_DEMO_NOTIFICATION_EMAIL', async ({ page }) => {
    const po = new EmailSettingsPage(page);
    await po.goto();
    await po.demoNotificationEmail.setValue('apx-testgen');
    expect(await po.demoNotificationEmail.getValue()).toBe('apx-testgen');
  });
