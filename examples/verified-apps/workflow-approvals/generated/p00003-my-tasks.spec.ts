 ...
 * (./p00003-my-tasks.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: about-this-page, breadcrumb, my-tasks-smart-filters, tasks-to-approve
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): about-this-page (staticContent), breadcrumb (breadcrumb), my-tasks-smart-filters (smartFilters), tasks-to-approve (themeTemplateComponent/contentRow).
...
import { MyTasksPage } from './p00003-my-tasks.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 3: My Tasks [requires auth]', () => {
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new MyTasksPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('My Tasks'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (3 items, 1 hidden)', async ({ page }) => {
    const po = new MyTasksPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P3_SHOW_EXPIRED', 'P3_SORT_BY', 'P3_TASK_ID']);
  });
});
