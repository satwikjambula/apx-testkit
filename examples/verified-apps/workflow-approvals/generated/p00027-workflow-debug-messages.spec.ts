/**
 * GENERATED by apx-testgen from pages/p00027-workflow-debug-messages.apx — DO NOT EDIT.
 ...
 * (./p00027-workflow-debug-messages.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: about-this-page, breadcrumb, workflow-debug-messages
...
import { WorkflowDebugMessagesPage } from './p00027-workflow-debug-messages.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 27: Workflow Debug Messages [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new WorkflowDebugMessagesPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new WorkflowDebugMessagesPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Workflow Debug Messages'));
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new WorkflowDebugMessagesPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['workflow-debug-messages']);
  });
});
