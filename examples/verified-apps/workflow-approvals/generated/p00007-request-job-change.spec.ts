 ...
 * (./p00007-request-job-change.page.js), not raw testkit calls, so both stay in sync.
...
import { RequestJobChangePage } from './p00007-request-job-change.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 7: Request Job Change [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new RequestJobChangePage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new RequestJobChangePage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Request Job Change'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (4 items, 1 hidden)', async ({ page }) => {
    const po = new RequestJobChangePage(page);
    await po.goto();
    await expectItemsPresent(page, ['P7_EMPNO', 'P7_ENAME', 'P7_JOB', 'P7_PROPOSED_JOB']);
  });
