/**
 * GENERATED by apx-testgen from pages/p00023-laptop-order-management.apx — DO NOT EDIT.
 ...
 * (./p00023-laptop-order-management.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: about-this-page, breadcrumb, laptop-order-management
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Region types NOT covered by an auto-generated assertion (no verified DOM convention, or a runtime id genuinely unconstructible from static data -- see docs/grammar-assumptions.md "Still open" and ADR-003): about-this-page (staticContent), breadcrumb (breadcrumb), laptop-order-management (interactiveGrid).
...
import { LaptopOrderManagementPage } from './p00023-laptop-order-management.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 23: Laptop Order Management [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new LaptopOrderManagementPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new LaptopOrderManagementPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Laptop Order Management'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 0 hidden)', async ({ page }) => {
    const po = new LaptopOrderManagementPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P23_SHOW_DELIVERED']);
  });
});
