/**
 * GENERATED by apx-testgen from pages/p00008-employees.apx — DO NOT EDIT.
 ...
 * (./p00008-employees.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: about-this-page, breadcrumb, employees, form-region
 ...
 * Region types NOT covered by an auto-generated assertion (no verified DOM convention, or a runtime id genuinely unconstructible from static data -- see docs/grammar-assumptions.md "Still open" and ADR-003): about-this-page (staticContent), breadcrumb (breadcrumb), form-region (staticContent).
...
import { EmployeesPage } from './p00008-employees.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 8: Employees [requires auth]', () => {
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new EmployeesPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Employees'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (2 items, 0 hidden)', async ({ page }) => {
    const po = new EmployeesPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P8_ORDER_BY', 'P8_SHOW_ONLY_SUBORDINATES']);
  });
