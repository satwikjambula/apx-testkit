 ...
 * (./p00006-task-administration.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: about-this-page, admin-tasks-smart-filters, breadcrumb, task-you-can-administer
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): about-this-page (staticContent), admin-tasks-smart-filters (smartFilters), breadcrumb (breadcrumb), task-you-can-administer (themeTemplateComponent/contentRow).
...
import { TaskAdministrationPage } from './p00006-task-administration.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 6: Task Administration [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new TaskAdministrationPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new TaskAdministrationPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Task Administration'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (3 items, 1 hidden)', async ({ page }) => {
    const po = new TaskAdministrationPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P6_SHOW_EXPIRED', 'P6_SORT_BY', 'P6_TASK_ID']);
  });
});
