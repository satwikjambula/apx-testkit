 ...
 * (./p00028-request-bank-account-change.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, request-job-change
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Region types NOT covered by an auto-generated assertion (no verified DOM convention, or a runtime id genuinely unconstructible from static data -- see docs/grammar-assumptions.md "Still open" and ADR-003): buttons (staticContent), request-job-change (form).
...
import { RequestBankAccountChangePage } from './p00028-request-bank-account-change.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 28: Request Bank Account Change [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new RequestBankAccountChangePage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new RequestBankAccountChangePage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Request Bank Account Change'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (4 items, 1 hidden)', async ({ page }) => {
    const po = new RequestBankAccountChangePage(page);
    await po.goto();
    await expectItemsPresent(page, ['P28_BANK_ACCOUNT', 'P28_EMPNO', 'P28_ENAME', 'P28_NEW_BANK_ACCOUNT']);
  });

  test('apex.item round-trip on P28_NEW_BANK_ACCOUNT', async ({ page }) => {
    const po = new RequestBankAccountChangePage(page);
    await po.goto();
    await po.newBankAccount.setValue('apx-testgen');
    expect(await po.newBankAccount.getValue()).toBe('apx-testgen');
  });
