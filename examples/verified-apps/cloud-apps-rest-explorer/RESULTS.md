# cloud-apps-rest-explorer — verification results
...
repository, `26.1` branch, `utility-apps/` directory (UPL-1.0).
...
- `staticContent`: 12
- `interactiveReport`: 4
- `themeTemplateComponent/contentRow`: 3
- `smartFilters`: 1
- `tree`: 1
- `classicReport`: 1
...
in `raw` bags, nothing lost (ADR-001): `action`, `column`, `computation`, `filter`, `pageGroup`, `process`, `savedReport`

## Generation

`@apx/testgen` produced **4** page object + smoke spec pairs
(3 marked skip: auth required). See `generated/` for
the actual files.
...
**0 added, 0 removed, 0 changed, 4 unchanged** — confirms zero spurious differences.
