 ...
 * (./p00002-manage-catalogs.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: pillars
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): pillars (themeTemplateComponent/contentRow).
...
import { ManageCatalogsPage } from './p00002-manage-catalogs.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 2: Manage Catalogs [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ManageCatalogsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ManageCatalogsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Manage Catalogs'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (6 items, 5 hidden)', async ({ page }) => {
    const po = new ManageCatalogsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P2_BACKGROUND_CONTEXT', 'P2_ERROR_MESSAGE', 'P2_RELEASE', 'P2_SELECTED_ACTION', 'P2_SELECTED_PILLAR', 'P2_SELECTED_RELEASE']);
  });
});
