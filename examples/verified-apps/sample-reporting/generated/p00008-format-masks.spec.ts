/**
 * GENERATED by apx-testgen from pages/p00008-format-masks.apx — DO NOT EDIT.
 ...
 * (./p00008-format-masks.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: about-this-page, breadcrumb, container, format-mask-example-report, sql-source
...
import { FormatMasksPage } from './p00008-format-masks.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 8: Format Masks [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new FormatMasksPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new FormatMasksPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Format Masks'));
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new FormatMasksPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['format_mask']);
  });
});
