 ...
 * (./p00003-classic-report.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: about-this-page, breadcrumb, button-bar, classic-report, sql-source
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): about-this-page (staticContent), breadcrumb (breadcrumb), button-bar (staticContent), classic-report (classicReport), sql-source (plugin/sourceDisplay).
...
import { ClassicReportPage } from './p00003-classic-report.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 3: Classic Report [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ClassicReportPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ClassicReportPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Classic Report'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 0 hidden)', async ({ page }) => {
    const po = new ClassicReportPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P3_STATUS']);
  });
