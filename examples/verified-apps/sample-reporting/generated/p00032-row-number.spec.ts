/**
 * GENERATED by apx-testgen from pages/p00032-row-number.apx — DO NOT EDIT.
 ...
 * (./p00032-row-number.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: about-this-page, breadcrumb, container, row-number, sql-source
...
import { RowNumberPage } from './p00032-row-number.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 32: ROW_NUMBER [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new RowNumberPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new RowNumberPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('ROW_NUMBER'));
  });
