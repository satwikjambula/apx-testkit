/**
 * GENERATED by apx-testgen from pages/p00041-cards.apx — DO NOT EDIT.
 ...
 * (./p00041-cards.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: about-this-page, breadcrumb, cards
...
import { CardsPage } from './p00041-cards.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 41: Cards [requires auth]', () => {
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new CardsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Cards'));
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new CardsPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['cards_report']);
  });
});
