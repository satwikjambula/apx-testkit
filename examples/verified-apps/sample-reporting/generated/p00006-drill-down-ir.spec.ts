/**
 * GENERATED by apx-testgen from pages/p00006-drill-down-ir.apx — DO NOT EDIT.
 ...
 * (./p00006-drill-down-ir.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: about-this-page, breadcrumb, container, projects, sql-source
...
import { DrillDownIrPage } from './p00006-drill-down-ir.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 6: Drill Down IR [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new DrillDownIrPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new DrillDownIrPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Drill Down Interactive Report'));
  });
