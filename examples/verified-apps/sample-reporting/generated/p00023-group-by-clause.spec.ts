 ...
 * (./p00023-group-by-clause.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: about-this-page, breadcrumb, group-by-clause, sql-source
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): about-this-page (staticContent), breadcrumb (breadcrumb), group-by-clause (classicReport), sql-source (plugin/sourceDisplay).
...
import { GroupByClausePage } from './p00023-group-by-clause.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 23: Group By Clause [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new GroupByClausePage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new GroupByClausePage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Group By Clause'));
  });
});
