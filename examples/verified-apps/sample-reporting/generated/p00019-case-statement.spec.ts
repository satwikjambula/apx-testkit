 ...
 * (./p00019-case-statement.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: about-this-page, breadcrumb, case-statements, container, sql-source
...
import { CaseStatementPage } from './p00019-case-statement.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 19: CASE Statement [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new CaseStatementPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new CaseStatementPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('CASE Statement'));
  });
