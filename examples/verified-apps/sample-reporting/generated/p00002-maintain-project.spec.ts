 ...
 * (./p00002-maintain-project.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, maintain-project
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), maintain-project (staticContent).
...
import { MaintainProjectPage } from './p00002-maintain-project.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 2: Maintain Project [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new MaintainProjectPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new MaintainProjectPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Maintain Project'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (10 items, 2 hidden)', async ({ page }) => {
    const po = new MaintainProjectPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P2_ASSIGNED_TO', 'P2_BUDGET', 'P2_COST', 'P2_END_DATE', 'P2_ID', 'P2_PROJECT', 'P2_ROWID', 'P2_START_DATE', 'P2_STATUS', 'P2_TASK_NAME']);
  });

  test('apex.item round-trip on P2_ASSIGNED_TO', async ({ page }) => {
    const po = new MaintainProjectPage(page);
    await po.goto();
