/**
 * GENERATED by apx-testgen from pages/p00025-pivot-syntax.apx — DO NOT EDIT.
 ...
 * (./p00025-pivot-syntax.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: about-this-page, breadcrumb, pivot-query-syntax, sql-source
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): about-this-page (staticContent), breadcrumb (breadcrumb), pivot-query-syntax (classicReport), sql-source (plugin/sourceDisplay).
...
import { PivotSyntaxPage } from './p00025-pivot-syntax.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 25: Pivot Syntax [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new PivotSyntaxPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new PivotSyntaxPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Pivot Syntax'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 1 hidden)', async ({ page }) => {
    const po = new PivotSyntaxPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P25_OPERATOR']);
  });
});
