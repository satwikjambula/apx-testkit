/**
 * GENERATED by apx-testgen from pages/p00012-custom-buttons.apx — DO NOT EDIT.
 ...
 * (./p00012-custom-buttons.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: about-this-page, breadcrumb, container, interactive-report, sql-source
...
import { CustomButtonsPage } from './p00012-custom-buttons.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 12: Custom Buttons [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new CustomButtonsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new CustomButtonsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Custom Buttons'));
  });
