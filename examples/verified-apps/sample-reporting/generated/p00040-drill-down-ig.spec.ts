/**
 * GENERATED by apx-testgen from pages/p00040-drill-down-ig.apx — DO NOT EDIT.
 ...
 * (./p00040-drill-down-ig.page.js), not raw testkit calls, so both stay in sync.
...
import { DrillDownIgPage } from './p00040-drill-down-ig.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 40: Drill Down IG [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new DrillDownIgPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new DrillDownIgPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Drill Down IG'));
  });
  ...
  test('every Interactive Grid region with a known runtime id resolves a current view (1 region)', async ({ page }) => {
    const po = new DrillDownIgPage(page);
    await po.goto();
    for (const id of ['projects_report']) {
