/**
 * GENERATED by apx-testgen from pages/p00035-pipelined-functions.apx — DO NOT EDIT.
 ...
 * (./p00035-pipelined-functions.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: about-this-page, breadcrumb, container, package-source, pipelined-functions, sql-source
 ...
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): about-this-page (staticContent), breadcrumb (breadcrumb), container (staticContent), package-source (plSqlDynamicContent), sql-source (plugin/sourceDisplay).
...
import { PipelinedFunctionsPage } from './p00035-pipelined-functions.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 35: Pipelined Functions [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new PipelinedFunctionsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new PipelinedFunctionsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Pipelined Functions'));
  });
  ...
  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new PipelinedFunctionsPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['QUERY']);
  });
});
