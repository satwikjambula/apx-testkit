/**
 * GENERATED by apx-testgen from pages/p00038-report-from-collection.apx — DO NOT EDIT.
 ...
 * (./p00038-report-from-collection.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: about-this-page, before-header-page-process, breadcrumb, container, interactive-report-on-collection, sql-source
 ...
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): about-this-page (staticContent), before-header-page-process (plSqlDynamicContent), breadcrumb (breadcrumb), container (staticContent), sql-source (plugin/sourceDisplay).
...
import { ReportFromCollectionPage } from './p00038-report-from-collection.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 38: Report from Collection [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ReportFromCollectionPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ReportFromCollectionPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Report from Collection'));
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new ReportFromCollectionPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Recreate Collection', 'Reset']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new ReportFromCollectionPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['REPORT1']);
  });
});
