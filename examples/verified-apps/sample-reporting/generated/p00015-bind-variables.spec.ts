/**
 * GENERATED by apx-testgen from pages/p00015-bind-variables.apx — DO NOT EDIT.
 ...
 * (./p00015-bind-variables.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: about-this-page, breadcrumb, button-bar, container, projects, sql-source
 ...
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): about-this-page (staticContent), breadcrumb (breadcrumb), button-bar (staticContent), container (staticContent), sql-source (plugin/sourceDisplay).
...
import { BindVariablesPage } from './p00015-bind-variables.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 15: Bind Variables [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new BindVariablesPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new BindVariablesPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Bind Variables'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 0 hidden)', async ({ page }) => {
    const po = new BindVariablesPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P15_PROJECT']);
  });
  ...
  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new BindVariablesPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['projects_report']);
  });
});
