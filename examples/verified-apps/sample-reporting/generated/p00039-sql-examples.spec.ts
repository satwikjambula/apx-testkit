/**
 * GENERATED by apx-testgen from pages/p00039-sql-examples.apx — DO NOT EDIT.
 ...
 * (./p00039-sql-examples.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, sql-syntax-examples
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), sql-syntax-examples (list).
...
import { SqlExamplesPage } from './p00039-sql-examples.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 39: SQL Examples [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new SqlExamplesPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new SqlExamplesPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('SQL Examples'));
  });
});
