/**
 * GENERATED by apx-testgen from pages/p00028-regular-expressions.apx — DO NOT EDIT.
 ...
 * (./p00028-regular-expressions.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: about-this-page, breadcrumb, container, query-use-regex-functions, sql-source
 ...
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): about-this-page (staticContent), breadcrumb (breadcrumb), container (staticContent), sql-source (plugin/sourceDisplay).
...
import { RegularExpressionsPage } from './p00028-regular-expressions.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 28: Regular Expressions [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new RegularExpressionsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new RegularExpressionsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Regular Expressions'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 1 hidden)', async ({ page }) => {
    const po = new RegularExpressionsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P28_FUNCTION']);
  });
  ...
  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new RegularExpressionsPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['regex_report']);
  });
});
