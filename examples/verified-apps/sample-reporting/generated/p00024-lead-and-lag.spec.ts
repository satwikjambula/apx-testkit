/**
 * GENERATED by apx-testgen from pages/p00024-lead-and-lag.apx — DO NOT EDIT.
 ...
 * (./p00024-lead-and-lag.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: about-this-page, breadcrumb, container, lead-and-lag, sql-source
...
import { LeadAndLagPage } from './p00024-lead-and-lag.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 24: LEAD and LAG [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new LeadAndLagPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new LeadAndLagPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('LEAD and LAG'));
  });
