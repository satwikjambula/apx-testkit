 ...
 * (./p00020-top-n-queries.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: about-this-page, breadcrumb, container, query, sql-source
...
import { TopNQueriesPage } from './p00020-top-n-queries.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 20: Top N Queries [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new TopNQueriesPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new TopNQueriesPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Top N Queries'));
  });
  ...
  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new TopNQueriesPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['topn_report']);
  });
});
