 ...
 * (./p00013-non-tabular-templates.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: about-this-page, breadcrumb, button-bar, comment-bubbles, sql-source
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): about-this-page (staticContent), breadcrumb (breadcrumb), button-bar (staticContent), comment-bubbles (classicReport), sql-source (plugin/sourceDisplay).
...
import { NonTabularTemplatesPage } from './p00013-non-tabular-templates.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 13: Non-Tabular Templates [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new NonTabularTemplatesPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new NonTabularTemplatesPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Non-Tabular Templates'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (2 items, 1 hidden)', async ({ page }) => {
    const po = new NonTabularTemplatesPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P13_ROWS', 'P13_STATUS']);
  });
