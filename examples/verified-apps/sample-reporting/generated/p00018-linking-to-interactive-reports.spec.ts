 ...
 * (./p00018-linking-to-interactive-reports.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: about-this-page, breadcrumb, container, developer-supplied-query, interactive-report, interactive-report-query-based-on-current-user-s-session, report-linking-examples
...
import { LinkingToInteractiveReportsPage } from './p00018-linking-to-interactive-reports.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 18: Linking to Interactive Reports [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new LinkingToInteractiveReportsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new LinkingToInteractiveReportsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Linking to Interactive Reports'));
  });
