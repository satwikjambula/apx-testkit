 ...
 * (./p00029-soundex.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: about-this-page, breadcrumb, button-bar, soundex-query, sql-source
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): about-this-page (staticContent), breadcrumb (breadcrumb), button-bar (staticContent), soundex-query (classicReport), sql-source (plugin/sourceDisplay).
...
import { SoundexPage } from './p00029-soundex.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 29: Soundex [requires auth]', () => {
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new SoundexPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Soundex'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (2 items, 1 hidden)', async ({ page }) => {
    const po = new SoundexPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P29_FUNCTION', 'P29_NAME_SEARCH']);
  });

  test('apex.item round-trip on P29_NAME_SEARCH', async ({ page }) => {
    const po = new SoundexPage(page);
    await po.goto();
    await po.nameSearch.setValue('apx-testgen');
    expect(await po.nameSearch.getValue()).toBe('apx-testgen');
  });

  test('every labeled .apx button exists (1 button)', async ({ page }) => {
    const po = new SoundexPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Reset Report']);
  });
});
