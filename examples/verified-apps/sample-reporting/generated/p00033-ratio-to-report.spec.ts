/**
 * GENERATED by apx-testgen from pages/p00033-ratio-to-report.apx — DO NOT EDIT.
 ...
 * (./p00033-ratio-to-report.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: about-this-page, breadcrumb, container, ratio-to-report, sql-source
...
import { RatioToReportPage } from './p00033-ratio-to-report.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 33: RATIO_TO_REPORT [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new RatioToReportPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new RatioToReportPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('RATIO_TO_REPORT'));
  });
