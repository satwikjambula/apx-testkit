 ...
 * (./p00005-filtering.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: about-this-page, filter, filtered-data, filtering, sql-source
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): about-this-page (staticContent), filter (staticContent), filtered-data (classicReport), filtering (breadcrumb), sql-source (plugin/sourceDisplay).
...
import { FilteringPage } from './p00005-filtering.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 5: Filtering [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new FilteringPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new FilteringPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Filtering'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (4 items, 0 hidden)', async ({ page }) => {
    const po = new FilteringPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P5_ASSIGNEE', 'P5_ROWS', 'P5_SEARCH', 'P5_STATUS']);
  });

  test('apex.item round-trip on P5_ASSIGNEE', async ({ page }) => {
    const po = new FilteringPage(page);
    await po.goto();
    await po.assignee.setValue('apx-testgen');
    expect(await po.assignee.getValue()).toBe('apx-testgen');
  });
