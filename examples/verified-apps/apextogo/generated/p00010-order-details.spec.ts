/**
 * GENERATED by apx-testgen from pages/p00010-order-details.apx — DO NOT EDIT.
 ...
 * (./p00010-order-details.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: actions, order-details, order-summary, order-total
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): actions (staticContent), order-details (classicReport), order-summary (themeTemplateComponent/contentRow), order-total (classicReport).
...
import { OrderDetailsPage } from './p00010-order-details.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 10: Order Details [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new OrderDetailsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new OrderDetailsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Order Details'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 1 hidden)', async ({ page }) => {
    const po = new OrderDetailsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P10_ORDER_ID']);
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new OrderDetailsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Go Back', 'Invite a friend']);
  });
});
