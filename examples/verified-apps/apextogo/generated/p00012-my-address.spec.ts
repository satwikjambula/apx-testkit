 ...
 * (./p00012-my-address.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: actions, location-map
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Region types NOT covered by an auto-generated assertion (no verified DOM convention, or a runtime id genuinely unconstructible from static data -- see docs/grammar-assumptions.md "Still open" and ADR-003): actions (staticContent), location-map (map).
...
import { MyAddressPage } from './p00012-my-address.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 12: My Address [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new MyAddressPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new MyAddressPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('My Address'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (2 items, 2 hidden)', async ({ page }) => {
    const po = new MyAddressPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P12_LATITUDE', 'P12_LONGITUDE']);
  });
