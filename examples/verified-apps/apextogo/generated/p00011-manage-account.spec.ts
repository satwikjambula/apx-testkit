 ...
 * (./p00011-manage-account.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: actions
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Region types NOT covered by an auto-generated assertion (no verified DOM convention, or a runtime id genuinely unconstructible from static data -- see docs/grammar-assumptions.md "Still open" and ADR-003): actions (staticContent).
...
import { ManageAccountPage } from './p00011-manage-account.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 11: Manage Account [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ManageAccountPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ManageAccountPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Manage Account'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (5 items, 0 hidden)', async ({ page }) => {
    const po = new ManageAccountPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P11_EMAIL', 'P11_LASTNAME', 'P11_NAME', 'P11_PHONE_NUMBER', 'P11_PROFILE_PIC']);
  });
  ...
  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new ManageAccountPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Account', 'Save']);
  });
});
