 ...
 * (./p00014-support.page.js), not raw testkit calls, so both stay in sync.
...
import { SupportPage } from './p00014-support.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 14: Support [requires auth]', () => {
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new SupportPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Support'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 0 hidden)', async ({ page }) => {
    const po = new SupportPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P14_FEEDBACK']);
  });

  test('apex.item round-trip on P14_FEEDBACK', async ({ page }) => {
    const po = new SupportPage(page);
    await po.goto();
  ...
  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new SupportPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Account', 'Send']);
  });
});
