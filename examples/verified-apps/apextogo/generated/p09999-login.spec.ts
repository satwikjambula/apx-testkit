...
test.describe('page 9999: Sign In', () => {
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new LoginPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('APEXToGo - Sign In'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 0 hidden)', async ({ page }) => {
    const po = new LoginPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P9999_USERNAME']);
  });
