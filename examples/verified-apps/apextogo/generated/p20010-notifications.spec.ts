 ...
 * (./p20010-notifications.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: push-notifications, push-notifications-not-supported
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Region types NOT covered by an auto-generated assertion (no verified DOM convention, or a runtime id genuinely unconstructible from static data -- see docs/grammar-assumptions.md "Still open" and ADR-003): push-notifications (staticContent), push-notifications-not-supported (staticContent).
...
import { NotificationsPage } from './p20010-notifications.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 20010: Notifications [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new NotificationsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new NotificationsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Notifications'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 0 hidden)', async ({ page }) => {
    const po = new NotificationsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P20010_ENABLE_PUSH']);
  });

  test('every labeled .apx button exists (1 button)', async ({ page }) => {
    const po = new NotificationsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Account']);
  });
});
