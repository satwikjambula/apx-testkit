 ...
 * Region types NOT covered by an auto-generated assertion (no verified DOM convention, or a runtime id genuinely unconstructible from static data -- see docs/grammar-assumptions.md "Still open" and ADR-003): actions (staticContent), settings (list).
...
test.describe('page 20000: Account [requires auth]', () => {
  ...
  test('every labeled .apx button exists (1 button)', async ({ page }) => {
    const po = new AccountPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Sign Out']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (2 regions)', async ({ page }) => {
    const po = new AccountPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['user-info', 'user-info-2']);
  });
});
