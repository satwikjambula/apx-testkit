/**
 * GENERATED by apx-testgen from pages/p00006-all-restaurants.apx — DO NOT EDIT.
 ...
 * (./p00006-all-restaurants.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: all-restaurants
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Region types NOT covered by an auto-generated assertion (no verified DOM convention, or a runtime id genuinely unconstructible from static data -- see docs/grammar-assumptions.md "Still open" and ADR-003): all-restaurants (themeTemplateComponent/contentRow).
...
import { AllRestaurantsPage } from './p00006-all-restaurants.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 6: All Restaurants [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new AllRestaurantsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new AllRestaurantsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('All Restaurants'));
  });
});
