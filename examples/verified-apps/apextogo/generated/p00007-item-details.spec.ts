 ...
 * Region types NOT covered by an auto-generated assertion (no verified DOM convention, or a runtime id genuinely unconstructible from static data -- see docs/grammar-assumptions.md "Still open" and ADR-003): actions (staticContent), quantity-container (staticContent).
...
test.describe('page 7: Item Details [requires auth]', () => {
  ...
  test('every labeled .apx button exists (6 buttons)', async ({ page }) => {
    const po = new ItemDetailsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['+', 'Add to Cart', 'Remove from Cart', 'Update Cart', 'Share', '-']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new ItemDetailsPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['item-information']);
  });
});
