 ...
 * (./p00005-restaurant.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: restaurant-information, restaurant-items
 ...
 * Region types NOT covered by an auto-generated assertion (no verified DOM convention, or a runtime id genuinely unconstructible from static data -- see docs/grammar-assumptions.md "Still open" and ADR-003): restaurant-items (themeTemplateComponent/contentRow).
...
import { RestaurantPage } from './p00005-restaurant.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 5: Restaurant [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new RestaurantPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new RestaurantPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Restaurant'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 1 hidden)', async ({ page }) => {
    const po = new RestaurantPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P5_RESTAURANT_ID']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new RestaurantPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['restaurant-information']);
  });
});
