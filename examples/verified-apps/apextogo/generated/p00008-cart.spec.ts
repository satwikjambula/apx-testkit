 ...
 * (./p00008-cart.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: actions, cart, total
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Region types NOT covered by an auto-generated assertion (no verified DOM convention, or a runtime id genuinely unconstructible from static data -- see docs/grammar-assumptions.md "Still open" and ADR-003): actions (staticContent), cart (themeTemplateComponent/contentRow), total (classicReport).
...
import { CartPage } from './p00008-cart.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 8: Cart [requires auth]', () => {
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new CartPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Cart'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (2 items, 2 hidden)', async ({ page }) => {
    const po = new CartPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P8_ORDER_ID', 'P8_SHOPPING_CART_ITEMS']);
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new CartPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Checkout', 'Clear Cart']);
  });
});
