 ...
 * (./p00023-sales-history-interactive-grid.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, sales-history-interactive-grid, timer
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * 1 Interactive Grid region(s) SKIPPED -- no htmlDomId set, runtime id genuinely unconstructible from static data (ADR-003 layer 3): sales-history-interactive-grid.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), timer (dynamicContent).
...
import { SalesHistoryInteractiveGridPage } from './p00023-sales-history-interactive-grid.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 23: Sales History Interactive Grid [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new SalesHistoryInteractiveGridPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new SalesHistoryInteractiveGridPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Sales History Interactive Grid'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 0 hidden)', async ({ page }) => {
    const po = new SalesHistoryInteractiveGridPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P23_NAVIGATION']);
  });
