/**
 * GENERATED by apx-testgen from pages/p00033-transaction-log.apx — DO NOT EDIT.
 ...
 * (./p00033-transaction-log.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, transaction-generation-log
...
import { TransactionLogPage } from './p00033-transaction-log.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 33: Transaction Log [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new TransactionLogPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new TransactionLogPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Transaction Log'));
  });
  ...
  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new TransactionLogPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['transaction-generation-log']);
  });
});
