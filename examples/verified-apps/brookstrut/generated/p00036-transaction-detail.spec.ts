 ...
 * (./p00036-transaction-detail.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: transaction-detail, transaction-line-items
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Region types NOT covered by an auto-generated assertion (no verified DOM convention, or a runtime id genuinely unconstructible from static data -- see docs/grammar-assumptions.md "Still open" and ADR-003): transaction-detail (classicReport), transaction-line-items (classicReport).
...
import { TransactionDetailPage } from './p00036-transaction-detail.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 36: Transaction Detail [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new TransactionDetailPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new TransactionDetailPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Transaction Detail'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (3 items, 0 hidden)', async ({ page }) => {
    const po = new TransactionDetailPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P36_TRANSACTION_DATE', 'P36_TRANSACTION_ENTRY_ID', 'P36_TRANSACTION_ID']);
  });
});
