/**
 * GENERATED by apx-testgen from pages/p00045-sales-history-smart-search-w-menu-actions.apx — DO NOT EDIT.
...
import { SalesHistorySmartSearchWMenuActionsPage } from './p00045-sales-history-smart-search-w-menu-actions.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 45: Sales History Smart Search w Menu Actions [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new SalesHistorySmartSearchWMenuActionsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new SalesHistorySmartSearchWMenuActionsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Sales History Smart Search w Menu Actions'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 0 hidden)', async ({ page }) => {
    const po = new SalesHistorySmartSearchWMenuActionsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P45_NAVIGATION']);
  });
