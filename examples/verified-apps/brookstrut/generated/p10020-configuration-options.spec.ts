 ...
 * (./p10020-configuration-options.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, configuration-options
...
import { ConfigurationOptionsPage } from './p10020-configuration-options.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 10020: Configuration Options [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ConfigurationOptionsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ConfigurationOptionsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Configuration Options'));
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new ConfigurationOptionsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Apply Changes', 'Reset']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new ConfigurationOptionsPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['configuration-options']);
  });
});
