 ...
 * (./p00011-products.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, items
...
import { ProductsPage } from './p00011-products.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 11: Products [requires auth]', () => {
  ...
  test('every labeled .apx button exists (3 buttons)', async ({ page }) => {
    const po = new ProductsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Add Store Item', 'Reset', 'Up']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new ProductsPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['items']);
  });
});
