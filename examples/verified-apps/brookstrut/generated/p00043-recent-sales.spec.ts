/**
 * GENERATED by apx-testgen from pages/p00043-recent-sales.apx — DO NOT EDIT.
 ...
 * (./p00043-recent-sales.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, recent-sales
...
import { RecentSalesPage } from './p00043-recent-sales.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 43: Recent Sales [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new RecentSalesPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new RecentSalesPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Recent Sales'));
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new RecentSalesPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['recent-sales']);
  });
});
