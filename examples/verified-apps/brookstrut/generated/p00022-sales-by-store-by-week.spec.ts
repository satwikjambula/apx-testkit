/**
 * GENERATED by apx-testgen from pages/p00022-sales-by-store-by-week.apx — DO NOT EDIT.
 ...
 * (./p00022-sales-by-store-by-week.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, sales-by-region-by-week
...
import { SalesByStoreByWeekPage } from './p00022-sales-by-store-by-week.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 22: Sales by Store by Week [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new SalesByStoreByWeekPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new SalesByStoreByWeekPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Sales by Store by Week'));
  });
  ...
  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new SalesByStoreByWeekPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['sales-by-region-by-week']);
  });
});
