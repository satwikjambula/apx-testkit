/**
 * GENERATED by apx-testgen from pages/p00029-generate-transaction.apx — DO NOT EDIT.
 ...
 * (./p00029-generate-transaction.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, transaction-details
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Region types NOT covered by an auto-generated assertion (no verified DOM convention, or a runtime id genuinely unconstructible from static data -- see docs/grammar-assumptions.md "Still open" and ADR-003): breadcrumb (breadcrumb), transaction-details (classicReport).
...
import { GenerateTransactionPage } from './p00029-generate-transaction.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 29: Generate Transaction [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new GenerateTransactionPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new GenerateTransactionPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Generate Transaction'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 1 hidden)', async ({ page }) => {
    const po = new GenerateTransactionPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P29_TRANSACTION_ID']);
  });

  test('every labeled .apx button exists (3 buttons)', async ({ page }) => {
    const po = new GenerateTransactionPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Generate Transaction', 'Reset', 'Up']);
  });
});
