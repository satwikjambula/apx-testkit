/**
 * GENERATED by apx-testgen from pages/p00034-transaction-summary-by-minute.apx — DO NOT EDIT.
 ...
 * (./p00034-transaction-summary-by-minute.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, transaction-generation-log-summarized-by-minute
...
import { TransactionSummaryByMinutePage } from './p00034-transaction-summary-by-minute.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 34: Transaction Summary by Minute [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new TransactionSummaryByMinutePage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new TransactionSummaryByMinutePage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Transaction Summary by Minute'));
  });
  ...
  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new TransactionSummaryByMinutePage(page);
    await po.goto();
    await expectRegionsResolve(page, ['transaction-generation-log-summarized-by-minute']);
  });
});
