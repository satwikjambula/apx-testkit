 ...
 * (./p00013-sales-history-classic.page.js), not raw testkit calls, so both stay in sync.
 ...
 * Region types NOT covered by an auto-generated assertion (no verified DOM convention, or a runtime id genuinely unconstructible from static data -- see docs/grammar-assumptions.md "Still open" and ADR-003): breadcrumb (breadcrumb), facets (staticContent), sales-history (classicReport), timer (dynamicContent).
...
import { SalesHistoryClassicPage } from './p00013-sales-history-classic.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 13: Sales History Classic [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new SalesHistoryClassicPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new SalesHistoryClassicPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Sales History - Classic'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 0 hidden)', async ({ page }) => {
    const po = new SalesHistoryClassicPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P13_NAVIGATION']);
  });
