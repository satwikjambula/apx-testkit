/**
 * GENERATED by apx-testgen from pages/p00009-region.apx — DO NOT EDIT.
 ...
 * (./p00009-region.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttonbar, regions
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Region types NOT covered by an auto-generated assertion (no verified DOM convention, or a runtime id genuinely unconstructible from static data -- see docs/grammar-assumptions.md "Still open" and ADR-003): buttonbar (staticContent), regions (staticContent).
...
import { RegionPage } from './p00009-region.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 9: Region [requires auth]', () => {
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new RegionPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Region'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (2 items, 1 hidden)', async ({ page }) => {
    const po = new RegionPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P9_ID', 'P9_REGION_NAME']);
  });

  test('apex.item round-trip on P9_REGION_NAME', async ({ page }) => {
    const po = new RegionPage(page);
    await po.goto();
    await po.regionName.setValue('apx-testgen');
    expect(await po.regionName.getValue()).toBe('apx-testgen');
  });

  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new RegionPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Add Region', 'Delete', 'Apply Changes']);
  });
});
