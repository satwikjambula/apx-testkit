 ...
 * (./p00015-region-stores.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, region-performance, stores, timing
 ...
 * Region types NOT covered by an auto-generated assertion (no verified DOM convention, or a runtime id genuinely unconstructible from static data -- see docs/grammar-assumptions.md "Still open" and ADR-003): breadcrumb (breadcrumb), stores (classicReport), timing (dynamicContent).
...
import { RegionStoresPage } from './p00015-region-stores.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 15: Region Stores [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new RegionStoresPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new RegionStoresPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Region Stores'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (2 items, 2 hidden)', async ({ page }) => {
    const po = new RegionStoresPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P15_REGION_ID', 'P15_REGION_NAME']);
  });

  test('every labeled .apx button exists (3 buttons)', async ({ page }) => {
    const po = new RegionStoresPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Edit', 'Refresh', 'Up']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new RegionStoresPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['region-performance']);
  });
});
