 ...
 * (./p00007-store.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, store
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Region types NOT covered by an auto-generated assertion (no verified DOM convention, or a runtime id genuinely unconstructible from static data -- see docs/grammar-assumptions.md "Still open" and ADR-003): breadcrumb (breadcrumb), store (staticContent).
...
import { StorePage } from './p00007-store.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 7: Store [requires auth]', () => {
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new StorePage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Store'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (12 items, 3 hidden)', async ({ page }) => {
    const po = new StorePage(page);
    await po.goto();
    await expectItemsPresent(page, ['P7_ID', 'P7_REGION_ID', 'P7_STORE_ADDRESS', 'P7_STORE_CITY', 'P7_STORE_ITEMS', 'P7_STORE_LAT', 'P7_STORE_LNG', 'P7_STORE_NAME', 'P7_STORE_OPEN_DATE', 'P7_STORE_STATE', 'P7_STORE_TYPE', 'P7_STORE_ZIP']);
  });

  test('apex.item round-trip on P7_STORE_ADDRESS', async ({ page }) => {
    const po = new StorePage(page);
    await po.goto();
    await po.storeAddress.setValue('apx-testgen');
    expect(await po.storeAddress.getValue()).toBe('apx-testgen');
  });

  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new StorePage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Add Store', 'Delete', 'Apply Changes']);
  });
});
