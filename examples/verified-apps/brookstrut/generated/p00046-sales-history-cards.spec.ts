/**
 * GENERATED by apx-testgen from pages/p00046-sales-history-cards.apx — DO NOT EDIT.
 ...
 * (./p00046-sales-history-cards.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, facets, sales-history, smart-search, timer
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), facets (staticContent), sales-history (classicReport), smart-search (smartFilters), timer (dynamicContent).
...
import { SalesHistoryCardsPage } from './p00046-sales-history-cards.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 46: Sales History Cards [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new SalesHistoryCardsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new SalesHistoryCardsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Sales History Cards'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 0 hidden)', async ({ page }) => {
    const po = new SalesHistoryCardsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P46_NAVIGATION']);
  });
