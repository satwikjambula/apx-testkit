/**
 * GENERATED by apx-testgen from pages/p00006-stores-report-content-row.apx — DO NOT EDIT.
 ...
 * (./p00006-stores-report-content-row.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, facets, stores
 ...
 * Region types NOT covered by an auto-generated assertion (no verified DOM convention, or a runtime id genuinely unconstructible from static data -- see docs/grammar-assumptions.md "Still open" and ADR-003): breadcrumb (breadcrumb), stores (themeTemplateComponent/contentRow).
...
import { StoresReportContentRowPage } from './p00006-stores-report-content-row.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 6: Stores Report  - Content Row [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new StoresReportContentRowPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new StoresReportContentRowPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Stores  - Content Row'));
  });

  test('every labeled .apx button exists (3 buttons)', async ({ page }) => {
    const po = new StoresReportContentRowPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Add Store', 'Reset', 'Up']);
  });
