/**
 * GENERATED by apx-testgen from pages/p00005-sales.apx — DO NOT EDIT.
 ...
 * (./p00005-sales.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, sales-dashboard, sales-over-time
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * 1 chart region(s) SKIPPED -- no htmlDomId set, runtime id genuinely unconstructible from static data (ADR-003 layer 3): sales-over-time.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), sales-dashboard (classicReport).
...
import { SalesPage } from './p00005-sales.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 5: Sales [requires auth]', () => {
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new SalesPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Sales'));
  });

  test('every labeled .apx button exists (3 buttons)', async ({ page }) => {
    const po = new SalesPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Generate Transaction', 'Refresh', 'Up']);
  });
});
