 ...
 * (./p00003-store-performance.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, store-performance, store-products
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Region types NOT covered by an auto-generated assertion (no verified DOM convention, or a runtime id genuinely unconstructible from static data -- see docs/grammar-assumptions.md "Still open" and ADR-003): breadcrumb (breadcrumb), store-performance (classicReport), store-products (classicReport).
...
import { StorePerformancePage } from './p00003-store-performance.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 3: Store [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new StorePerformancePage(page);
  ...
  test('every .apx pageItem exists in DOM/apex.item registry (2 items, 2 hidden)', async ({ page }) => {
    const po = new StorePerformancePage(page);
    await po.goto();
    await expectItemsPresent(page, ['P3_ID', 'P3_STORE_NAME']);
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new StorePerformancePage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Edit Store', 'Up']);
  });
});
