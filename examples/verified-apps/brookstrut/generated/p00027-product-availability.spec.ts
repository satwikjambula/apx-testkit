/**
 * GENERATED by apx-testgen from pages/p00027-product-availability.apx — DO NOT EDIT.
 ...
 * (./p00027-product-availability.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, product-availability
...
import { ProductAvailabilityPage } from './p00027-product-availability.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 27: Product Availability [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ProductAvailabilityPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ProductAvailabilityPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Product Availability'));
  });
  ...
  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new ProductAvailabilityPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['product-availability']);
  });
});
