/**
 * GENERATED by apx-testgen from pages/p00032-table-counts.apx — DO NOT EDIT.
 ...
 * (./p00032-table-counts.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, row-counts
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Region types NOT covered by an auto-generated assertion (no verified DOM convention, or a runtime id genuinely unconstructible from static data -- see docs/grammar-assumptions.md "Still open" and ADR-003): breadcrumb (breadcrumb), row-counts (classicReport).
...
import { TableCountsPage } from './p00032-table-counts.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 32: Table Counts [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new TableCountsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new TableCountsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Table Counts'));
  });
