/**
 * GENERATED by apx-testgen from pages/p10010-theme-style-selection.apx — DO NOT EDIT.
 ...
 * (./p10010-theme-style-selection.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, configure-appearance
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Region types NOT covered by an auto-generated assertion (no verified DOM convention, or a runtime id genuinely unconstructible from static data -- see docs/grammar-assumptions.md "Still open" and ADR-003): buttons (staticContent), configure-appearance (staticContent).
...
import { ThemeStyleSelectionPage } from './p10010-theme-style-selection.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 10010: Theme Style Selection [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ThemeStyleSelectionPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ThemeStyleSelectionPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Theme Style Selection'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (2 items, 0 hidden)', async ({ page }) => {
    const po = new ThemeStyleSelectionPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P10010_DESKTOP_THEME_STYLE_ID', 'P10010_END_USER_STYLE']);
  });
