 ...
 * (./p00030-transaction.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, generate-one-transaction-on-page-load, show-last-generation
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), generate-one-transaction-on-page-load (staticContent), show-last-generation (plSqlDynamicContent).
...
import { TransactionPage } from './p00030-transaction.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 30: Transaction [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new TransactionPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new TransactionPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Transaction'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (2 items, 1 hidden)', async ({ page }) => {
    const po = new TransactionPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P30_AUTOREFRESH', 'P30_TRANSACTION_ID']);
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new TransactionPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Refresh Page', 'Up']);
  });
});
