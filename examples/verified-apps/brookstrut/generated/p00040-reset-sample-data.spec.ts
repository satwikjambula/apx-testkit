/**
 * GENERATED by apx-testgen from pages/p00040-reset-sample-data.apx — DO NOT EDIT.
 ...
 * (./p00040-reset-sample-data.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, manage-sample-data
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Region types NOT covered by an auto-generated assertion (no verified DOM convention, or a runtime id genuinely unconstructible from static data -- see docs/grammar-assumptions.md "Still open" and ADR-003): breadcrumb (breadcrumb), manage-sample-data (list).
...
import { ResetSampleDataPage } from './p00040-reset-sample-data.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 40: Reset Sample Data [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ResetSampleDataPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ResetSampleDataPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Reset Sample Data'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 1 hidden)', async ({ page }) => {
    const po = new ResetSampleDataPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P40_SIZE']);
  });
