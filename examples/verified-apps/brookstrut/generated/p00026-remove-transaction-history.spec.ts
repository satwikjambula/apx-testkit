 ...
 * (./p00026-remove-transaction-history.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, tranaction-count
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Region types NOT covered by an auto-generated assertion (no verified DOM convention, or a runtime id genuinely unconstructible from static data -- see docs/grammar-assumptions.md "Still open" and ADR-003): breadcrumb (breadcrumb), tranaction-count (dynamicContent).
...
import { RemoveTransactionHistoryPage } from './p00026-remove-transaction-history.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 26: Remove Transaction History [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new RemoveTransactionHistoryPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new RemoveTransactionHistoryPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Remove Transaction History'));
  });

  test('every labeled .apx button exists (3 buttons)', async ({ page }) => {
    const po = new RemoveTransactionHistoryPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Remove History', 'Up']);
  });
});
