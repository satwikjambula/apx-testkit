/**
 * GENERATED by apx-testgen from pages/p00018-sales-by-product.apx — DO NOT EDIT.
 ...
 * (./p00018-sales-by-product.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, sales-by-product, search
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Region types NOT covered by an auto-generated assertion (no verified DOM convention, or a runtime id genuinely unconstructible from static data -- see docs/grammar-assumptions.md "Still open" and ADR-003): breadcrumb (breadcrumb), sales-by-product (classicReport), search (smartFilters).
...
import { SalesByProductPage } from './p00018-sales-by-product.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 18: Sales by Product [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new SalesByProductPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new SalesByProductPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Sales by Product'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (2 items, 1 hidden)', async ({ page }) => {
    const po = new SalesByProductPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P18_ID', 'P18_PROCUCT_NAME']);
  });
