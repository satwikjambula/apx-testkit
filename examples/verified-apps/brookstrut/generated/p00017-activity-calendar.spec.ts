 ...
 * (./p00017-activity-calendar.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: activity-calendar, activity-calendar-2
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Region types NOT covered by an auto-generated assertion (no verified DOM convention, or a runtime id genuinely unconstructible from static data -- see docs/grammar-assumptions.md "Still open" and ADR-003): activity-calendar (calendar), activity-calendar-2 (breadcrumb).
...
import { ActivityCalendarPage } from './p00017-activity-calendar.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 17: Activity Calendar [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ActivityCalendarPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ActivityCalendarPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Activity Calendar'));
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new ActivityCalendarPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Refresh', 'Up']);
  });
});
