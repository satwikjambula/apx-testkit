 ...
 * (./p00041-load-data.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, hidden-items, results, summary
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Region types NOT covered by an auto-generated assertion (no verified DOM convention, or a runtime id genuinely unconstructible from static data -- see docs/grammar-assumptions.md "Still open" and ADR-003): breadcrumb (breadcrumb), hidden-items (staticContent), results (classicReport), summary (classicReport).
...
import { LoadDataPage } from './p00041-load-data.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 41: Load Data [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new LoadDataPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new LoadDataPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Load Data'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (5 items, 5 hidden)', async ({ page }) => {
    const po = new LoadDataPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P41_DAYS', 'P41_LOAD_TIME', 'P41_ORDERS', 'P41_ROWS', 'P41_SIZE']);
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new LoadDataPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Reload Data', 'Up']);
  });
});
