/**
 * GENERATED by apx-testgen from pages/p00025-sales-by-product-and-store-by-week.apx — DO NOT EDIT.
 ...
 * (./p00025-sales-by-product-and-store-by-week.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, sales-by-product-region-by-week
...
import { SalesByProductAndStoreByWeekPage } from './p00025-sales-by-product-and-store-by-week.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 25: Sales by Product and Store by Week [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new SalesByProductAndStoreByWeekPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new SalesByProductAndStoreByWeekPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Sales by Product and Store by Week'));
  });
  ...
  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new SalesByProductAndStoreByWeekPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['sales-by-product-region-by-week']);
  });
});
