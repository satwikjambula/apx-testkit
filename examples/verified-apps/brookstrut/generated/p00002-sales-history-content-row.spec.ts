/**
 * GENERATED by apx-testgen from pages/p00002-sales-history-content-row.apx — DO NOT EDIT.
 ...
 * (./p00002-sales-history-content-row.page.js), not raw testkit calls, so both stay in sync.
...
import { SalesHistoryContentRowPage } from './p00002-sales-history-content-row.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 2: Sales History Content Row [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new SalesHistoryContentRowPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new SalesHistoryContentRowPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Sales History Content Row'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 0 hidden)', async ({ page }) => {
    const po = new SalesHistoryContentRowPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P2_NAVIGATION']);
  });
