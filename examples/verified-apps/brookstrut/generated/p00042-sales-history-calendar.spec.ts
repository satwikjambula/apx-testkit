/**
 * GENERATED by apx-testgen from pages/p00042-sales-history-calendar.apx — DO NOT EDIT.
 ...
 * (./p00042-sales-history-calendar.page.js), not raw testkit calls, so both stay in sync.
 ...
 * Region types NOT covered by an auto-generated assertion (no verified DOM convention, or a runtime id genuinely unconstructible from static data -- see docs/grammar-assumptions.md "Still open" and ADR-003): breadcrumb (breadcrumb), facets (staticContent), sales-history (calendar), timer (dynamicContent).
...
import { SalesHistoryCalendarPage } from './p00042-sales-history-calendar.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 42: Sales History Calendar [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new SalesHistoryCalendarPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new SalesHistoryCalendarPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Sales History Calendar'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 0 hidden)', async ({ page }) => {
    const po = new SalesHistoryCalendarPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P42_NAVIGATION']);
  });
