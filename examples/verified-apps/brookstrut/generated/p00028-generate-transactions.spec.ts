 ...
 * (./p00028-generate-transactions.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, generate-transactions
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Region types NOT covered by an auto-generated assertion (no verified DOM convention, or a runtime id genuinely unconstructible from static data -- see docs/grammar-assumptions.md "Still open" and ADR-003): breadcrumb (breadcrumb), generate-transactions (list).
...
import { GenerateTransactionsPage } from './p00028-generate-transactions.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 28: Generate Transactions [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new GenerateTransactionsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new GenerateTransactionsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Generate Transactions'));
  });
