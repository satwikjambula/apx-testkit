/**
 * GENERATED by apx-testgen from pages/p00038-sales-history-content-row-with-menu.apx — DO NOT EDIT.
 ...
 * (./p00038-sales-history-content-row-with-menu.page.js), not raw testkit calls, so both stay in sync.
 ...
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), facets (staticContent), sales-history (themeTemplateComponent/contentRow), timer (dynamicContent).
...
import { SalesHistoryContentRowWithMenuPage } from './p00038-sales-history-content-row-with-menu.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 38: Sales History Content Row with Menu [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new SalesHistoryContentRowWithMenuPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new SalesHistoryContentRowWithMenuPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Sales History Content Row with Menu'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 0 hidden)', async ({ page }) => {
    const po = new SalesHistoryContentRowWithMenuPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P38_NAVIGATION']);
  });
  ...
  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new SalesHistoryContentRowWithMenuPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['facets-2']);
  });
});
