 ...
 * (./p00008-store-regions.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, store-regions
...
import { StoreRegionsPage } from './p00008-store-regions.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 8: Store Regions [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new StoreRegionsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new StoreRegionsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Store Regions'));
  });

  test('every labeled .apx button exists (3 buttons)', async ({ page }) => {
    const po = new StoreRegionsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Add Region', 'Reset', 'Up']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new StoreRegionsPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['store-regions']);
  });
});
