 ...
 * (./p00012-product.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: button-bar, product
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Region types NOT covered by an auto-generated assertion (no verified DOM convention, or a runtime id genuinely unconstructible from static data -- see docs/grammar-assumptions.md "Still open" and ADR-003): button-bar (staticContent), product (staticContent).
...
import { ProductPage } from './p00012-product.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 12: Product [requires auth]', () => {
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ProductPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Product'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (5 items, 1 hidden)', async ({ page }) => {
    const po = new ProductPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P12_ID', 'P12_ITEM_DESC', 'P12_ITEM_NAME', 'P12_ITEM_TYPE', 'P12_MSRP']);
  });

  test('apex.item round-trip on P12_ITEM_DESC', async ({ page }) => {
    const po = new ProductPage(page);
    await po.goto();
    await po.itemDesc.setValue('apx-testgen');
    expect(await po.itemDesc.getValue()).toBe('apx-testgen');
  });

  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new ProductPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Add Store Item', 'Delete', 'Apply Changes']);
  });
});
