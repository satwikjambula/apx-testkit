/**
 * GENERATED by apx-testgen from pages/p00021-event-log.apx — DO NOT EDIT.
 ...
 * (./p00021-event-log.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, event-log
...
import { EventLogPage } from './p00021-event-log.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 21: Event Log [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new EventLogPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new EventLogPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Event Log'));
  });
  ...
  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new EventLogPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['event-log']);
  });
});
