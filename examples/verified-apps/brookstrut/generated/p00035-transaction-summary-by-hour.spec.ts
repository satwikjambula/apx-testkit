/**
 * GENERATED by apx-testgen from pages/p00035-transaction-summary-by-hour.apx — DO NOT EDIT.
 ...
 * (./p00035-transaction-summary-by-hour.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, transaction-generation-log-summarized-by-hour
...
import { TransactionSummaryByHourPage } from './p00035-transaction-summary-by-hour.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 35: Transaction Summary by Hour [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new TransactionSummaryByHourPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new TransactionSummaryByHourPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Transaction Summary by Hour'));
  });
  ...
  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new TransactionSummaryByHourPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['transaction-generation-log-summarized-by-hour']);
  });
});
