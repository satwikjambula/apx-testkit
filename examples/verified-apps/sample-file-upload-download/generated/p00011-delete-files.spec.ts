/**
 * GENERATED by apx-testgen from pages/p00011-delete-files.apx — DO NOT EDIT.
 ...
 * (./p00011-delete-files.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, delete-files
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), delete-files (staticContent).
...
import { DeleteFilesPage } from './p00011-delete-files.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 11: Delete Files [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new DeleteFilesPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new DeleteFilesPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Delete Files'));
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new DeleteFilesPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Delete Files']);
  });
});
