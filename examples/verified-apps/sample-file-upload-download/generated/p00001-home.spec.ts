 ...
 * Regions present in metadata: about-this-application, additional-oracle-apex-communities-and-resources, app-name, breadcrumbs, footer, project-summary, project-summary-sql-source, recent-files, recent-files-sql-source
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): about-this-application (staticContent), additional-oracle-apex-communities-and-resources (staticContent), app-name (staticContent), breadcrumbs (breadcrumb), footer (plugin/sampleAppsFooter), project-summary (classicReport), project-summary-sql-source (plugin/sourceDisplay), recent-files (classicReport), recent-files-sql-source (plugin/sourceDisplay).
...
test.describe('page 1: Home [requires auth]', () => {
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new HomePage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Sample File Upload and Download'));
  });

  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new HomePage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Add File', 'Add Project', 'View Files', 'View Projects']);
  });
});
