/**
 * GENERATED by apx-testgen from pages/p00010-manage-example-data.apx — DO NOT EDIT.
 ...
 * (./p00010-manage-example-data.page.js), not raw testkit calls, so both stay in sync.
 ...
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), manage-sample-data (staticContent).
...
import { ManageExampleDataPage } from './p00010-manage-example-data.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 10: Manage Example Data [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ManageExampleDataPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ManageExampleDataPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Manage Example Data'));
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new ManageExampleDataPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Recreate']);
  });
});
