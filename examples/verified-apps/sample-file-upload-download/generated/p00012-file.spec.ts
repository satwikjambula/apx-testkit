 ...
 * (./p00012-file.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: application-features-demonstrated-on-this-page, breadcrumb, file
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): application-features-demonstrated-on-this-page (staticContent), breadcrumb (breadcrumb), file (staticContent).
...
import { FilePage } from './p00012-file.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 12: File [requires auth]', () => {
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new FilePage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('File'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (8 items, 4 hidden)', async ({ page }) => {
    const po = new FilePage(page);
    await po.goto();
    await expectItemsPresent(page, ['P12_DOWNLOAD_LINK', 'P12_FILENAME', 'P12_FILE_BLOB', 'P12_FILE_CHARSET', 'P12_FILE_COMMENTS', 'P12_FILE_MIMETYPE', 'P12_ID', 'P12_PROJECT_ID']);
  });

  test('apex.item round-trip on P12_FILE_COMMENTS', async ({ page }) => {
    const po = new FilePage(page);
    await po.goto();
  ...
  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new FilePage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Add File', 'Delete', 'Apply Changes']);
  });
});
