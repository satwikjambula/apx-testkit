 ...
 * (./p00003-file-list.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: about-this-page, application-features-demonstrated-on-this-page, breadcrumb, container, display-download-links-in-an-interactive-report, files, include-file-size-in-a-report-using-the-filesize-format-mask, page-title, sql-source
 ...
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): about-this-page (staticContent), application-features-demonstrated-on-this-page (staticContent), breadcrumb (breadcrumb), container (staticContent), display-download-links-in-an-interactive-report (staticContent), include-file-size-in-a-report-using-the-filesize-format-mask (staticContent), page-title (staticContent), sql-source (plugin/sourceDisplay).
...
import { FileListPage } from './p00003-file-list.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 3: Files [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new FileListPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new FileListPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Files'));
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new FileListPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Add File', 'Reset']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new FileListPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['files_report']);
  });
});
