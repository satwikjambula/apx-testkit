 ...
 * (./p00015-line.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: about-this-page, breadcrumb, line-chart-data-labels-line-styles, line-chart-data-labels-line-styles-information, line-chart-declarative-font-formatting, line-chart-declarative-font-formatting-information, line-chart-reference-object-via-javascript-customizations, line-chart-reference-object-via-javascript-information, line-chart-show-hide-labels-via-dynamic-action, line-chart-show-hide-labels-via-dynamic-action-information, line-chart-time-axis, line-chart-time-axis-information, region-display-selector
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Chart type-check emitted for 4 region(s) with a known runtime id (htmlDomId set).
 * 1 chart region(s) SKIPPED -- no htmlDomId set, runtime id genuinely unconstructible from static data (ADR-003 layer 3): line-chart-reference-object-via-javascript-customizations.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): about-this-page (staticContent), breadcrumb (breadcrumb), line-chart-data-labels-line-styles-information (staticContent), line-chart-declarative-font-formatting-information (staticContent), line-chart-reference-object-via-javascript-information (staticContent), line-chart-show-hide-labels-via-dynamic-action-information (staticContent), line-chart-time-axis-information (staticContent), region-display-selector (regionDisplaySelector).
...
import { LinePage } from './p00015-line.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 15: Line [requires auth]', () => {
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new LinePage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Line'));
  });

  test('every labeled .apx button exists (10 buttons)', async ({ page }) => {
    const po = new LinePage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Area', 'Bar', 'Combo', 'Hide Labels', 'Horizontal', 'Line', 'Line with Area', 'Reset Zoom', 'Show Labels', 'Vertical']);
  });

  test('every Chart region with a known runtime id resolves a real chart type (4 regions)', async ({ page }) => {
    const po = new LinePage(page);
    await po.goto();
    ...
    const charts: Array<[string, string]> = [['lineChart', 'line'], ['lineChartAxes', 'line'], ['lineChartLabels', 'line'], ['line_chart', 'line']];
