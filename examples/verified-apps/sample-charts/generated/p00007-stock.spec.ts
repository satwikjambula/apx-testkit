/**
 * GENERATED by apx-testgen from pages/p00007-stock.apx — DO NOT EDIT.
 ...
 * (./p00007-stock.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: about-this-page, breadcrumb, dummy-corp-stock-chart, information
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * 1 chart region(s) SKIPPED -- no htmlDomId set, runtime id genuinely unconstructible from static data (ADR-003 layer 3): dummy-corp-stock-chart.
...
import { StockPage } from './p00007-stock.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 7: Stock [requires auth]', () => {
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new StockPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Stock'));
  });
});
