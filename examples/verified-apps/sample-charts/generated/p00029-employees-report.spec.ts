/**
 * GENERATED by apx-testgen from pages/p00029-employees-report.apx — DO NOT EDIT.
 ...
 * (./p00029-employees-report.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: about-this-page, breadcrumb, employees-report
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Region types NOT covered by an auto-generated assertion (no verified DOM convention, or a runtime id genuinely unconstructible from static data -- see docs/grammar-assumptions.md "Still open" and ADR-003): about-this-page (staticContent), breadcrumb (breadcrumb), employees-report (interactiveGrid).
...
import { EmployeesReportPage } from './p00029-employees-report.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 29: Employees  Report [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new EmployeesReportPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new EmployeesReportPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Employees  Report'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 1 hidden)', async ({ page }) => {
    const po = new EmployeesReportPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P29_DEPTNO']);
  });
});
