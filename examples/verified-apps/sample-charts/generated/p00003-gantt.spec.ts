 ...
 * 2 chart region(s) SKIPPED -- no htmlDomId set, runtime id genuinely unconstructible from static data (ADR-003 layer 3): gantt-chart, tasks-per-employee-not-hierarchical.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): about-this-page (staticContent), breadcrumbs (breadcrumb), gantt-controls (staticContent), multiple-tasks-information (staticContent), multiple-tasks-per-row (staticContent), region-display-selector (regionDisplaySelector), single-task-information (staticContent).
...
test.describe('page 3: Gantt [requires auth]', () => {
  ...
  test('every Chart region with a known runtime id resolves a real chart type (1 region)', async ({ page }) => {
    ...
    const charts: Array<[string, string]> = [['gantt', 'gantt']];
