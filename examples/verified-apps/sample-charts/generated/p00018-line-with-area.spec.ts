/**
 * GENERATED by apx-testgen from pages/p00018-line-with-area.apx — DO NOT EDIT.
 ...
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): about-this-page (staticContent), breadcrumb (breadcrumb), custom-tooltip-labels-information (staticContent), data-labels-line-styles-information (staticContent), fill-gaps-and-sort-information (staticContent), line-with-area-chart-stacked-percent-information (staticContent), region-display-selector (regionDisplaySelector).
...
import { LineWithAreaPage } from './p00018-line-with-area.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 18: Line with Area [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new LineWithAreaPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new LineWithAreaPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Line with Area'));
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new LineWithAreaPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Horizontal', 'Vertical']);
  });
  ...
  test('every Chart region with a known runtime id resolves a real chart type (4 regions)', async ({ page }) => {
    ...
    const charts: Array<[string, string]> = [['tooltips', 'lineWithArea'], ['lineChart', 'lineWithArea'], ['mixedLineChart', 'lineWithArea'], ['linePercentChart', 'lineWithArea']];
