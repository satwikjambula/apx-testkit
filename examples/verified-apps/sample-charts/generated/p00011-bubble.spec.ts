/**
 * GENERATED by apx-testgen from pages/p00011-bubble.apx — DO NOT EDIT.
 ...
 * (./p00011-bubble.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: about-this-page, breadcrumb, bubble, filter, information, region-display-selector
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * 1 chart region(s) SKIPPED -- no htmlDomId set, runtime id genuinely unconstructible from static data (ADR-003 layer 3): bubble.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): about-this-page (staticContent), breadcrumb (breadcrumb), filter (staticContent), information (staticContent), region-display-selector (regionDisplaySelector).
...
import { BubblePage } from './p00011-bubble.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 11: Bubble [requires auth]', () => {
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new BubblePage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Bubble'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 0 hidden)', async ({ page }) => {
    const po = new BubblePage(page);
    await po.goto();
    await expectItemsPresent(page, ['P11_POINT']);
  });
});
