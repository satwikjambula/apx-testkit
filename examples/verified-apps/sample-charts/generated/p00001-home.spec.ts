 ...
 * Regions present in metadata: about-sample-charts, example-charts, footer, sample-charts
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): about-sample-charts (staticContent), example-charts (list), footer (plugin/sampleAppsFooter), sample-charts (staticContent).
...
test.describe('page 1: Sample Charts [requires auth]', () => {
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new HomePage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Sample Charts'));
  });
});
