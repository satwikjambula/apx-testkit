/**
 * GENERATED by apx-testgen from pages/p00008-scatter.apx — DO NOT EDIT.
 ...
 * (./p00008-scatter.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: about-this-page, breadcrumb, new, scatter-dynamic-y-axis-min-max-information, scatter-dynamic-y-axis-min-max-via-javascript-code, scatter-percent-format, scatter-percent-format-information
 ...
 * 1 chart region(s) SKIPPED -- no htmlDomId set, runtime id genuinely unconstructible from static data (ADR-003 layer 3): scatter-percent-format.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): about-this-page (staticContent), breadcrumb (breadcrumb), new (regionDisplaySelector), scatter-dynamic-y-axis-min-max-information (staticContent), scatter-percent-format-information (staticContent).
...
import { ScatterPage } from './p00008-scatter.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 8: Scatter [requires auth]', () => {
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ScatterPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Scatter'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (2 items, 0 hidden)', async ({ page }) => {
    const po = new ScatterPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P8_Y_MAX', 'P8_Y_MIN']);
  });

  test('every labeled .apx button exists (1 button)', async ({ page }) => {
    const po = new ScatterPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Set']);
  });
  ...
  test('every Chart region with a known runtime id resolves a real chart type (1 region)', async ({ page }) => {
    ...
    const charts: Array<[string, string]> = [['scatterChart', 'scatter']];
