 ...
 * (./p00001-basic-collections.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: about-this-page, apex-collections-view, breadcrumb, footer, tip
 ...
 * Region types NOT covered by an auto-generated assertion (no verified DOM convention, or a runtime id genuinely unconstructible from static data -- see docs/grammar-assumptions.md "Still open" and ADR-003): about-this-page (staticContent), breadcrumb (breadcrumb), footer (plugin/sampleAppsFooter), tip (staticContent).
...
import { BasicCollectionsPage } from './p00001-basic-collections.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 1: Basic Collections [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new BasicCollectionsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new BasicCollectionsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Basic Collections'));
  });

  test('every labeled .apx button exists (3 buttons)', async ({ page }) => {
    const po = new BasicCollectionsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Create Collection', 'Modify Collection', 'Reset']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new BasicCollectionsPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['apex-collections-view']);
  });
});
