 ...
 * (./p00007-editemp.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, collection-member
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Region types NOT covered by an auto-generated assertion (no verified DOM convention, or a runtime id genuinely unconstructible from static data -- see docs/grammar-assumptions.md "Still open" and ADR-003): buttons (staticContent), collection-member (staticContent).
...
import { EditempPage } from './p00007-editemp.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 7: Add/Edit Collection Member [requires auth]', () => {
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new EditempPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Add/Edit Collection Member'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (11 items, 1 hidden)', async ({ page }) => {
    const po = new EditempPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P7_COMM', 'P7_DEPTNO', 'P7_EMPNO', 'P7_ENAME', 'P7_HIREDATE', 'P7_JOB', 'P7_MGR', 'P7_SAL', 'P7_SEQ', 'P7_STATUS', 'P7_XMLTYPE']);
  });

  test('apex.item round-trip on P7_ENAME', async ({ page }) => {
    const po = new EditempPage(page);
    await po.goto();
  ...
  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new EditempPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Add Member', 'Cancel', 'Delete Member', 'Update Member']);
  });
});
