...
import { EmpPage } from './p00006-emp.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 6: Data Synchronization [requires auth]', () => {
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new EmpPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Data Synchronization'));
  });

  test('every labeled .apx button exists (3 buttons)', async ({ page }) => {
    const po = new EmpPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Add Member', 'Apply Collection', '"Populate Collection from EBA_DEMO_CS_EMP "']);
  });
});
