 ...
 * (./p00002-create-collection.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, collection
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Region types NOT covered by an auto-generated assertion (no verified DOM convention, or a runtime id genuinely unconstructible from static data -- see docs/grammar-assumptions.md "Still open" and ADR-003): buttons (staticContent), collection (staticContent).
...
import { CreateCollectionPage } from './p00002-create-collection.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 2: Create Collection [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new CreateCollectionPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new CreateCollectionPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Create Collection'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 0 hidden)', async ({ page }) => {
    const po = new CreateCollectionPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P2_NAME']);
  });
  ...
  test('every labeled .apx button exists (3 buttons)', async ({ page }) => {
    const po = new CreateCollectionPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Create Collection', 'Create/Replace Collection']);
  });
});
