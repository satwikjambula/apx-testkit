/**
 * GENERATED by apx-testgen from pages/p00012-sample-collections-api-examples.apx — DO NOT EDIT.
...
import { SampleCollectionsApiExamplesPage } from './p00012-sample-collections-api-examples.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 12: Sample Collections - API Examples [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new SampleCollectionsApiExamplesPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new SampleCollectionsApiExamplesPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Sample Collections - API Syntax'));
  });
});
