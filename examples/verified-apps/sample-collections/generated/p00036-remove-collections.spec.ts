 ...
 * (./p00036-remove-collections.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breacdcrumb, collection-members-in-current-application, remove-collections
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breacdcrumb (breadcrumb), collection-members-in-current-application (classicReport), remove-collections (staticContent).
...
import { RemoveCollectionsPage } from './p00036-remove-collections.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 36: Remove Collections [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new RemoveCollectionsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new RemoveCollectionsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Remove Collections'));
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new RemoveCollectionsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Remove Collections']);
  });
});
