 ...
 * (./p00019-customer-product-use.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, customer-product-use
...
import { CustomerProductUsePage } from './p00019-customer-product-use.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 19: Customer Product Use [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new CustomerProductUsePage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new CustomerProductUsePage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Customer Product Use'));
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new CustomerProductUsePage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Reset']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new CustomerProductUsePage(page);
    await po.goto();
    await expectRegionsResolve(page, ['CUSTOMERS_PRODUCTS']);
  });
});
