 ...
 * (./p00086-referenceability.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, button-bar, by-asset-commitment, by-phase, by-recruitment-owner, customer-summary, region-display-selector
 ...
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), button-bar (staticContent), by-recruitment-owner (plugin/html5BarChart), customer-summary (plugin/badgeList), region-display-selector (regionDisplaySelector).
...
import { ReferenceabilityPage } from './p00086-referenceability.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 86: Referenceability [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ReferenceabilityPage(page);
  ...
  test('every .apx pageItem exists in DOM/apex.item registry (2 items, 0 hidden)', async ({ page }) => {
    const po = new ReferenceabilityPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P86_CATEGORY', 'P86_GEO']);
  });
