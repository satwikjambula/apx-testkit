 ...
 * (./p00097-update.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, update
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), update (staticContent).
...
import { UpdatePage } from './p00097-update.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 97: Update [requires auth]', () => {
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new UpdatePage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Update'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (5 items, 2 hidden)', async ({ page }) => {
    const po = new UpdatePage(page);
    await po.goto();
    await expectItemsPresent(page, ['P97_CONTACT_ID', 'P97_CUSTOMER_ID', 'P97_ID', 'P97_NOTE', 'P97_NOTE_DATE']);
  });

  test('apex.item round-trip on P97_NOTE', async ({ page }) => {
    const po = new UpdatePage(page);
    await po.goto();
  ...
  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new UpdatePage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Add Update', 'Delete', 'Apply Changes']);
  });
});
