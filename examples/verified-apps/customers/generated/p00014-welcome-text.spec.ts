 ...
 * (./p00014-welcome-text.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, welcome-text
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), welcome-text (staticContent).
...
import { WelcomeTextPage } from './p00014-welcome-text.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 14: Welcome Text [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new WelcomeTextPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new WelcomeTextPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Welcome Text'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 0 hidden)', async ({ page }) => {
    const po = new WelcomeTextPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P14_WELCOME_TEXT']);
  });

  test('apex.item round-trip on P14_WELCOME_TEXT', async ({ page }) => {
    const po = new WelcomeTextPage(page);
    await po.goto();
    await po.welcomeText.setValue('apx-testgen');
    expect(await po.welcomeText.getValue()).toBe('apx-testgen');
  });
