 ...
 * (./p00096-customer-referenceability.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, referenceability
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), referenceability (staticContent).
...
import { CustomerReferenceabilityPage } from './p00096-customer-referenceability.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 96: Customer Referenceability [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new CustomerReferenceabilityPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new CustomerReferenceabilityPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Customer Referenceability'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (5 items, 1 hidden)', async ({ page }) => {
    const po = new CustomerReferenceabilityPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P96_ID', 'P96_REFERENCABLE', 'P96_REFERENCE_PHASE_ID', 'P96_REFERENCE_TYPES', 'P96_REF_RECRUITMENT_OWNER']);
  });

  test('apex.item round-trip on P96_REF_RECRUITMENT_OWNER', async ({ page }) => {
    const po = new CustomerReferenceabilityPage(page);
    await po.goto();
    await po.refRecruitmentOwner.setValue('apx-testgen');
    expect(await po.refRecruitmentOwner.getValue()).toBe('apx-testgen');
  });
