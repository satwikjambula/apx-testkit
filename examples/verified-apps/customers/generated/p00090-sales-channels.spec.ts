/**
 * GENERATED by apx-testgen from pages/p00090-sales-channels.apx — DO NOT EDIT.
 ...
 * (./p00090-sales-channels.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, sales-channels
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), sales-channels (staticContent).
...
import { SalesChannelsPage } from './p00090-sales-channels.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 90: Sales Channels [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new SalesChannelsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new SalesChannelsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Sales Channels'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (3 items, 1 hidden)', async ({ page }) => {
    const po = new SalesChannelsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P90_CHANNEL', 'P90_DESCRIPTION', 'P90_ID']);
  });

  test('apex.item round-trip on P90_CHANNEL', async ({ page }) => {
    const po = new SalesChannelsPage(page);
    await po.goto();
    await po.channel.setValue('apx-testgen');
    expect(await po.channel.getValue()).toBe('apx-testgen');
  });

  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new SalesChannelsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Add Sales Channel', 'Delete', 'Apply Changes']);
  });
});
