/**
 * GENERATED by apx-testgen from pages/p00123-acl-features.apx — DO NOT EDIT.
 ...
 * (./p00123-acl-features.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: acl-features
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * 1 Interactive Grid region(s) SKIPPED -- no htmlDomId set, runtime id genuinely unconstructible from static data (ADR-003 layer 3): acl-features.
...
import { AclFeaturesPage } from './p00123-acl-features.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 123: ACL Features [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new AclFeaturesPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new AclFeaturesPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('ACL Features'));
  });
