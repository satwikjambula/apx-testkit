 ...
 * (./p00120-manage-feedback.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, update-feedback
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), update-feedback (staticContent).
...
import { ManageFeedbackPage } from './p00120-manage-feedback.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 120: Manage Feedback [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ManageFeedbackPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ManageFeedbackPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Manage Feedback'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (7 items, 1 hidden)', async ({ page }) => {
    const po = new ManageFeedbackPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P120_APPLICATION_ID', 'P120_FEEDBACK', 'P120_ID', 'P120_PAGE_ID', 'P120_RESPONSE', 'P120_STATUS', 'P120_TYPE_ID']);
  });

  test('apex.item round-trip on P120_RESPONSE', async ({ page }) => {
    const po = new ManageFeedbackPage(page);
    await po.goto();
