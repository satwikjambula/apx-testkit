 ...
 * (./p00037-customer-contact.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, customer-contact
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), customer-contact (staticContent).
...
import { CustomerContactPage } from './p00037-customer-contact.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 37: Customer Contact [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new CustomerContactPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new CustomerContactPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Customer Contact'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (20 items, 3 hidden)', async ({ page }) => {
    const po = new CustomerContactPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P37_ADDRESS_LINE1', 'P37_ADDRESS_LINE2', 'P37_CELL_PHONE', 'P37_CITY', 'P37_COMPANY', 'P37_CONTACT_TYPE_ID', 'P37_COUNTRY', 'P37_CUSTOMER_COMPANY', 'P37_CUSTOMER_ID', 'P37_CUSTOMER_LINK', 'P37_EMAIL', 'P37_FAX', 'P37_ID', 'P37_NAME', 'P37_NOTES', 'P37_PHONE', 'P37_STATE', 'P37_TAGS', 'P37_TITLE', 'P37_ZIP']);
  });

  test('apex.item round-trip on P37_ADDRESS_LINE1', async ({ page }) => {
    const po = new CustomerContactPage(page);
    await po.goto();
  ...
  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new CustomerContactPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Add Contact', 'Delete', 'Apply Changes']);
  });
});
