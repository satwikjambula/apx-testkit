/**
 * GENERATED by apx-testgen from pages/p00074-add-multiple-users2.apx — DO NOT EDIT.
 ...
 * (./p00074-add-multiple-users2.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: bulk-load-users-step, buttons, exceptions, hidden-items, no-valid-users-exist-page-info, users-to-be-added, valid-users-exist-page-info
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): bulk-load-users-step (staticContent), buttons (staticContent), exceptions (classicReport), hidden-items (staticContent), no-valid-users-exist-page-info (staticContent), users-to-be-added (listView), valid-users-exist-page-info (plSqlDynamicContent).
...
import { AddMultipleUsers2Page } from './p00074-add-multiple-users2.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 74: Add Multiple Users [requires auth]', () => {
  ...
  test('every .apx pageItem exists in DOM/apex.item registry (3 items, 3 hidden)', async ({ page }) => {
    const po = new AddMultipleUsers2Page(page);
    await po.goto();
    await expectItemsPresent(page, ['P74_INVALID_COUNT', 'P74_ROLE', 'P74_VALID_COUNT']);
  });
