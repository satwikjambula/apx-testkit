 ...
 * (./p00094-product-reference-statuses.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: reference-statuses
...
import { ProductReferenceStatusesPage } from './p00094-product-reference-statuses.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 94: Product Reference Statuses [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ProductReferenceStatusesPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ProductReferenceStatusesPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Product Reference Statuses'));
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new ProductReferenceStatusesPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Add Status', 'Reset']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new ProductReferenceStatusesPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['reference-statuses']);
  });
});
