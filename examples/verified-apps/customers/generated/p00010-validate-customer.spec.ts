/**
 * GENERATED by apx-testgen from pages/p00010-validate-customer.apx — DO NOT EDIT.
 ...
 * (./p00010-validate-customer.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, form-items, introduction-text, previous-verifications, validate-account
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), form-items (staticContent), introduction-text (staticContent), previous-verifications (classicReport), validate-account (staticContent).
...
import { ValidateCustomerPage } from './p00010-validate-customer.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 10: Validate Customer [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ValidateCustomerPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ValidateCustomerPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Validate Customer'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (3 items, 1 hidden)', async ({ page }) => {
    const po = new ValidateCustomerPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P10_ID', 'P10_I_VALIDATE', 'P10_VERIFICATION_COMMENT']);
  });

  test('apex.item round-trip on P10_VERIFICATION_COMMENT', async ({ page }) => {
    const po = new ValidateCustomerPage(page);
    await po.goto();
  ...
  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new ValidateCustomerPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'I verify customer content is accurate']);
  });
});
