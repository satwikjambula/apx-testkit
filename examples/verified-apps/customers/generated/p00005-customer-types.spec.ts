 ...
 * (./p00005-customer-types.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: customer-types
...
import { CustomerTypesPage } from './p00005-customer-types.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 5: Customer Types [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new CustomerTypesPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new CustomerTypesPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Customer Types'));
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new CustomerTypesPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Add Type Code', 'Reset']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new CustomerTypesPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['customer-types']);
  });
});
