 ...
 * (./p00113-product-family-details.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, product-family-details
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), product-family-details (staticContent).
...
import { ProductFamilyDetailsPage } from './p00113-product-family-details.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 113: Product Family Details [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ProductFamilyDetailsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ProductFamilyDetailsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Product Family Details'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (4 items, 1 hidden)', async ({ page }) => {
    const po = new ProductFamilyDetailsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P113_DESCRIPTION', 'P113_ID', 'P113_NAME', 'P113_PRODUCTS']);
  });

  test('apex.item round-trip on P113_DESCRIPTION', async ({ page }) => {
    const po = new ProductFamilyDetailsPage(page);
    await po.goto();
  ...
  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new ProductFamilyDetailsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Create Product Family', 'Delete', 'Apply Changes']);
  });
});
