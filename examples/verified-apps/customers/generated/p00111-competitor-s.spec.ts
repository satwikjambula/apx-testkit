 ...
 * (./p00111-competitor-s.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, competitors, select-competitors
 ...
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), competitors (staticContent).
...
import { CompetitorSPage } from './p00111-competitor-s.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 111: Competitor(s) [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new CompetitorSPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new CompetitorSPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Competitor(s)'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (2 items, 2 hidden)', async ({ page }) => {
    const po = new CompetitorSPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P111_CUSTOMER_ID', 'P111_REFRESHED']);
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new CompetitorSPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Update Competitor(s)']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new CompetitorSPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['competitorsIRR']);
  });
});
