 ...
 * (./p00110-implementation-partner-s.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, implementation-partner, select-implementation-partners
 ...
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), implementation-partner (staticContent).
...
import { ImplementationPartnerSPage } from './p00110-implementation-partner-s.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 110: Implementation Partner(s) [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ImplementationPartnerSPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ImplementationPartnerSPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Implementation Partner(s)'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (2 items, 2 hidden)', async ({ page }) => {
    const po = new ImplementationPartnerSPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P110_CUSTOMER_ID', 'P110_REFRESHED']);
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new ImplementationPartnerSPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Update Partner(s)']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new ImplementationPartnerSPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['partnersIRR']);
  });
});
