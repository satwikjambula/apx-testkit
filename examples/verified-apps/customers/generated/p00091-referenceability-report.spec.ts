/**
 * GENERATED by apx-testgen from pages/p00091-referenceability-report.apx — DO NOT EDIT.
 ...
 * (./p00091-referenceability-report.page.js), not raw testkit calls, so both stay in sync.
...
import { ReferenceabilityReportPage } from './p00091-referenceability-report.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 91: Referenceability Report [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ReferenceabilityReportPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ReferenceabilityReportPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Referencability Report'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 0 hidden)', async ({ page }) => {
    const po = new ReferenceabilityReportPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P91_CATEGORY']);
  });
