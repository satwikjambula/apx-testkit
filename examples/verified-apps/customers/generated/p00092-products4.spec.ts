 ...
 * (./p00092-products4.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, button-bar, customer-counts-by-product
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * 1 chart region(s) SKIPPED -- no htmlDomId set, runtime id genuinely unconstructible from static data (ADR-003 layer 3): customer-counts-by-product.
...
import { Products4Page } from './p00092-products4.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 92: Products [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new Products4Page(page);
  ...
  test('every .apx pageItem exists in DOM/apex.item registry (2 items, 0 hidden)', async ({ page }) => {
    const po = new Products4Page(page);
    await po.goto();
    await expectItemsPresent(page, ['P92_CATEGORY', 'P92_GEO']);
  });
