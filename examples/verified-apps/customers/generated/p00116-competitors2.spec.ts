/**
 * GENERATED by apx-testgen from pages/p00116-competitors2.apx — DO NOT EDIT.
 ...
 * (./p00116-competitors2.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, competitors
...
import { Competitors2Page } from './p00116-competitors2.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 116: Competitors [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new Competitors2Page(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new Competitors2Page(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Competitors'));
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new Competitors2Page(page);
    await po.goto();
    await expectButtonsPresent(page, ['Create Competitor', 'Reset']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new Competitors2Page(page);
    await po.goto();
    await expectRegionsResolve(page, ['COMP']);
  });
});
