 ...
 * (./p00056-activity-details.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: activity-details, buttons
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): activity-details (staticContent), buttons (staticContent).
...
import { ActivityDetailsPage } from './p00056-activity-details.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 56: Activity Details [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ActivityDetailsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ActivityDetailsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Activity Details'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (11 items, 1 hidden)', async ({ page }) => {
    const po = new ActivityDetailsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P56_ACTIVITY_DATE', 'P56_DESCRIPTION', 'P56_ID', 'P56_LOCATION', 'P56_NAME', 'P56_OWNER', 'P56_REFERENCES', 'P56_SET_DATE', 'P56_SET_LOCATION', 'P56_SET_OWNER', 'P56_TYPE_ID']);
  });

  test('apex.item round-trip on P56_DESCRIPTION', async ({ page }) => {
    const po = new ActivityDetailsPage(page);
    await po.goto();
