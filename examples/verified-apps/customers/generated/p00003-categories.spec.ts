/**
 * GENERATED by apx-testgen from pages/p00003-categories.apx — DO NOT EDIT.
 ...
 * (./p00003-categories.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: categories
...
import { CategoriesPage } from './p00003-categories.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 3: Categories [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new CategoriesPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new CategoriesPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Categories'));
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new CategoriesPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Add Category', 'Reset']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new CategoriesPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['categories']);
  });
});
