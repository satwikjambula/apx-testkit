/**
 * GENERATED by apx-testgen from pages/p00121-email-log.apx — DO NOT EDIT.
 ...
 * (./p00121-email-log.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: status-update-requests
...
import { EmailLogPage } from './p00121-email-log.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 121: Email Log [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new EmailLogPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new EmailLogPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Email Log'));
  });
  ...
  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new EmailLogPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['status-update-requests']);
  });
});
