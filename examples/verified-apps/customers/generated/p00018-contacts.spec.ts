 ...
 * (./p00018-contacts.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, contacts
...
import { ContactsPage } from './p00018-contacts.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 18: Contacts [requires auth]', () => {
  ...
  test('every labeled .apx button exists (3 buttons)', async ({ page }) => {
    const po = new ContactsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Add Contact', 'Reset', 'Upload Contacts Data']);
  });
