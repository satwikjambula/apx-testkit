/**
 * GENERATED by apx-testgen from pages/p00108-competitors.apx — DO NOT EDIT.
 ...
 * (./p00108-competitors.page.js), not raw testkit calls, so both stay in sync.
...
import { CompetitorsPage } from './p00108-competitors.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 108: Competitors [requires auth]', () => {
  ...
  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new CompetitorsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Add Competitor', 'Reset']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new CompetitorsPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['competitors']);
  });
});
