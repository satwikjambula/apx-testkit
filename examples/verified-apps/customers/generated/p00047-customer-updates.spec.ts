/**
 * GENERATED by apx-testgen from pages/p00047-customer-updates.apx — DO NOT EDIT.
 ...
 * (./p00047-customer-updates.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, updates
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), updates (classicReport).
...
import { CustomerUpdatesPage } from './p00047-customer-updates.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 47: Customer Updates [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new CustomerUpdatesPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new CustomerUpdatesPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Customer Updates'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 1 hidden)', async ({ page }) => {
    const po = new CustomerUpdatesPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P47_ID']);
  });
