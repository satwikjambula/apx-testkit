/**
 * GENERATED by apx-testgen from pages/p00119-review-feedback.apx — DO NOT EDIT.
 ...
 * (./p00119-review-feedback.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: feedback
...
import { ReviewFeedbackPage } from './p00119-review-feedback.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 119: Review Feedback [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ReviewFeedbackPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ReviewFeedbackPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Review Feedback'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 1 hidden)', async ({ page }) => {
    const po = new ReviewFeedbackPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P119_EMAIL_FROM_ADDRESS']);
  });
  ...
  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new ReviewFeedbackPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['feedback']);
  });
});
