 ...
 * (./p00114-activity-attachment.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: activity-attachment, buttons
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): activity-attachment (staticContent), buttons (staticContent).
...
import { ActivityAttachmentPage } from './p00114-activity-attachment.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 114: Activity Attachment [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ActivityAttachmentPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ActivityAttachmentPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Activity Attachment'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (6 items, 2 hidden)', async ({ page }) => {
    const po = new ActivityAttachmentPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P114_ACTIVITY_ID', 'P114_FILE_BLOB', 'P114_FILE_COMMENTS', 'P114_FILE_MIMETYPE', 'P114_FILE_NAME', 'P114_ID']);
  });

  test('apex.item round-trip on P114_FILE_COMMENTS', async ({ page }) => {
    const po = new ActivityAttachmentPage(page);
    await po.goto();
