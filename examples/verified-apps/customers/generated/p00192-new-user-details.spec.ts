/**
 * GENERATED by apx-testgen from pages/p00192-new-user-details.apx — DO NOT EDIT.
 ...
 * (./p00192-new-user-details.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, form-items, information, user-details
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), form-items (staticContent), information (plSqlDynamicContent), user-details (staticContent).
...
import { NewUserDetailsPage } from './p00192-new-user-details.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 192: New User Details [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new NewUserDetailsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new NewUserDetailsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('New User Details'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (3 items, 1 hidden)', async ({ page }) => {
    const po = new NewUserDetailsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P192_ACCESS_LEVEL_ID', 'P192_SEQUENCE', 'P192_USERNAME']);
  });

  test('apex.item round-trip on P192_USERNAME', async ({ page }) => {
    const po = new NewUserDetailsPage(page);
    await po.goto();
