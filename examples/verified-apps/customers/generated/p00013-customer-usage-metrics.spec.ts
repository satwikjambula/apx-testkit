 ...
 * (./p00013-customer-usage-metrics.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, customer-usage-metrics, hidden-items
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), customer-usage-metrics (calendar), hidden-items (staticContent).
...
import { CustomerUsageMetricsPage } from './p00013-customer-usage-metrics.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 13: Customer Usage Metrics [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new CustomerUsageMetricsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new CustomerUsageMetricsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Customer Usage Metrics'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (4 items, 4 hidden)', async ({ page }) => {
    const po = new CustomerUsageMetricsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P13_CALENDAR_DATE', 'P13_CALENDAR_END_DATE', 'P13_CALENDAR_TYPE', 'P13_ID']);
  });

  test('every labeled .apx button exists (1 button)', async ({ page }) => {
    const po = new CustomerUsageMetricsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Return to Customer']);
  });
});
