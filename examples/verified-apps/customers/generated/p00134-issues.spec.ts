/**
 * GENERATED by apx-testgen from pages/p00134-issues.apx — DO NOT EDIT.
 ...
 * (./p00134-issues.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, issues
...
import { IssuesPage } from './p00134-issues.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 134: Issues [requires auth]', () => {
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new IssuesPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Issues'));
  });
  ...
  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new IssuesPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['issues']);
  });
});
