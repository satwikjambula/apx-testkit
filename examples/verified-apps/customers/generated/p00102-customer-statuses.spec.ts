/**
 * GENERATED by apx-testgen from pages/p00102-customer-statuses.apx — DO NOT EDIT.
 ...
 * (./p00102-customer-statuses.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: customer-statuses
...
import { CustomerStatusesPage } from './p00102-customer-statuses.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 102: Customer Statuses [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new CustomerStatusesPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new CustomerStatusesPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Customer Statuses'));
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new CustomerStatusesPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Add Status Code', 'Reset']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new CustomerStatusesPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['customer-statuses']);
  });
});
