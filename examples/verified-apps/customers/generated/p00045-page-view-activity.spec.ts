/**
 * GENERATED by apx-testgen from pages/p00045-page-view-activity.apx — DO NOT EDIT.
 ...
 * (./p00045-page-view-activity.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: activity, breadcrumb
...
import { PageViewActivityPage } from './p00045-page-view-activity.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 45: Page View Activity [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new PageViewActivityPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new PageViewActivityPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Page View Activity'));
  });
  ...
  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new PageViewActivityPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['activity']);
  });
});
