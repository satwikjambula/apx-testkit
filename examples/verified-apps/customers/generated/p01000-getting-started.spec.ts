 ...
 * Regions present in metadata: access-control, acl-level, application-options, buttons, feature-configuration, first-time-setup-completed, introduction-wizard, sample-data, users, users-2, welcome-to-application-title
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): access-control (staticContent), acl-level (staticContent), application-options (staticContent), buttons (staticContent), feature-configuration (classicReport), first-time-setup-completed (staticContent), introduction-wizard (staticContent), sample-data (staticContent), users (staticContent), users-2 (classicReport), welcome-to-application-title (staticContent).
...
test.describe('page 1000: Getting Started [requires auth]', () => {
  ...
  test('every .apx pageItem exists in DOM/apex.item registry (3 items, 0 hidden)', async ({ page }) => {
    const po = new GettingStartedPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P1000_ACL_CONFIG', 'P1000_LOAD_SAMPLE_YN', 'P1000_USERNAME_FORMAT']);
  });
