/**
 * GENERATED by apx-testgen from pages/p00128-data-load-results2.apx — DO NOT EDIT.
 ...
 * (./p00128-data-load-results2.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, data-load-process-train, data-load-results, failed-records, records-changed-by-another-user
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), data-load-process-train (list), data-load-results (staticContent), failed-records (classicReport), records-changed-by-another-user (classicReport).
...
import { DataLoadResults2Page } from './p00128-data-load-results2.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 128: Data Load Results [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new DataLoadResults2Page(page);
  ...
  test('every .apx pageItem exists in DOM/apex.item registry (4 items, 0 hidden)', async ({ page }) => {
    const po = new DataLoadResults2Page(page);
    await po.goto();
    await expectItemsPresent(page, ['P128_ERROR_COUNT', 'P128_INSERT_COUNT', 'P128_REVIEW_COUNT', 'P128_UPDATE_COUNT']);
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new DataLoadResults2Page(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Finish']);
  });
});
