/**
 * GENERATED by apx-testgen from pages/p00106-implementation-partners.apx — DO NOT EDIT.
 ...
 * (./p00106-implementation-partners.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, implementation-partners
...
import { ImplementationPartnersPage } from './p00106-implementation-partners.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 106: Implementation Partners [requires auth]', () => {
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ImplementationPartnersPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Implementation Partners'));
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new ImplementationPartnersPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Create', 'Reset']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new ImplementationPartnersPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['implementation-partners']);
  });
});
