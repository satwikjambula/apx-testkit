/**
 * GENERATED by apx-testgen from pages/p00115-partners.apx — DO NOT EDIT.
 ...
 * (./p00115-partners.page.js), not raw testkit calls, so both stay in sync.
...
import { PartnersPage } from './p00115-partners.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 115: Partners [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new PartnersPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new PartnersPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Partners'));
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new PartnersPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Create Partner', 'Reset']);
  });
