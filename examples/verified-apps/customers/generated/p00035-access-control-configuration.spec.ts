 ...
 * (./p00035-access-control-configuration.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: about-access-control, access-control-configuration, access-control-configuration-2
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): about-access-control (plSqlDynamicContent), access-control-configuration (staticContent), access-control-configuration-2 (breadcrumb).
...
import { AccessControlConfigurationPage } from './p00035-access-control-configuration.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 35: Access Control Configuration [requires auth]', () => {
  ...
  test('every .apx pageItem exists in DOM/apex.item registry (3 items, 1 hidden)', async ({ page }) => {
    const po = new AccessControlConfigurationPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P35_ACCESS_CONTROL_SCOPE', 'P35_AC_ENABLED', 'P35_HIDDEN_AC_ENABLED']);
  });
