/**
 * GENERATED by apx-testgen from pages/p00022-updates.apx — DO NOT EDIT.
 ...
 * (./p00022-updates.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: activities, updates
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): activities (breadcrumb), updates (calendar).
...
import { UpdatesPage } from './p00022-updates.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 22: Updates [requires auth]', () => {
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new UpdatesPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Updates'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (2 items, 2 hidden)', async ({ page }) => {
    const po = new UpdatesPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P22_CALENDAR_DATE', 'P22_CALENDAR_TYPE']);
  });
});
