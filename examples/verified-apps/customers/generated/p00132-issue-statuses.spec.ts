/**
 * GENERATED by apx-testgen from pages/p00132-issue-statuses.apx — DO NOT EDIT.
 ...
 * (./p00132-issue-statuses.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: issue-statuses
...
import { IssueStatusesPage } from './p00132-issue-statuses.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 132: Issue Statuses [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new IssueStatusesPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new IssueStatusesPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Issue Statuses'));
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new IssueStatusesPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Add Issue Status', 'Reset']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new IssueStatusesPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['issue-statuses']);
  });
});
