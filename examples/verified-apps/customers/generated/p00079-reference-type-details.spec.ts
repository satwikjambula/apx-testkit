 ...
 * (./p00079-reference-type-details.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: audit-information, buttons, reference-type-details
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): audit-information (staticContent), buttons (staticContent), reference-type-details (staticContent).
...
import { ReferenceTypeDetailsPage } from './p00079-reference-type-details.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 79: Reference Type Details [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ReferenceTypeDetailsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ReferenceTypeDetailsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Reference Type Details'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (7 items, 1 hidden)', async ({ page }) => {
    const po = new ReferenceTypeDetailsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P79_CREATED', 'P79_CREATED_BY', 'P79_ID', 'P79_IS_ACTIVE', 'P79_REFERENCE_TYPE', 'P79_UPDATED', 'P79_UPDATED_BY']);
  });

  test('apex.item round-trip on P79_REFERENCE_TYPE', async ({ page }) => {
    const po = new ReferenceTypeDetailsPage(page);
    await po.goto();
    await po.referenceType.setValue('apx-testgen');
    expect(await po.referenceType.getValue()).toBe('apx-testgen');
  });

  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new ReferenceTypeDetailsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Create Reference Type', 'Delete', 'Apply Changes']);
  });
});
