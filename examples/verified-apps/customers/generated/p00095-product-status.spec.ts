 ...
 * (./p00095-product-status.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, product-status
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), product-status (staticContent).
...
import { ProductStatusPage } from './p00095-product-status.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 95: Product Status [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ProductStatusPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ProductStatusPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Product Status'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (4 items, 1 hidden)', async ({ page }) => {
    const po = new ProductStatusPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P95_DESCRIPTION', 'P95_ID', 'P95_IS_CURRENT_YN', 'P95_STATUS']);
  });

  test('apex.item round-trip on P95_DESCRIPTION', async ({ page }) => {
    const po = new ProductStatusPage(page);
    await po.goto();
  ...
  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new ProductStatusPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Add Status', 'Delete', 'Apply Changes']);
  });
});
