 ...
 * (./p00009-contact-type-details.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: audit-information, buttons, contact-type-details
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): audit-information (staticContent), buttons (staticContent), contact-type-details (staticContent).
...
import { ContactTypeDetailsPage } from './p00009-contact-type-details.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 9: Contact Type Details [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ContactTypeDetailsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ContactTypeDetailsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Contact Type Details'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (7 items, 1 hidden)', async ({ page }) => {
    const po = new ContactTypeDetailsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P9_CONTACT_TYPE', 'P9_CREATED', 'P9_CREATED_BY', 'P9_ID', 'P9_IS_ACTIVE', 'P9_UPDATED', 'P9_UPDATED_BY']);
  });

  test('apex.item round-trip on P9_CONTACT_TYPE', async ({ page }) => {
    const po = new ContactTypeDetailsPage(page);
    await po.goto();
    await po.contactType.setValue('apx-testgen');
    expect(await po.contactType.getValue()).toBe('apx-testgen');
  });

  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new ContactTypeDetailsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Add Contact Type', 'Delete', 'Apply Changes']);
  });
});
