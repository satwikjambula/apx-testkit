 ...
 * (./p00004-category-details.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: audit-information, buttons, category-details
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): audit-information (staticContent), buttons (staticContent), category-details (staticContent).
...
import { CategoryDetailsPage } from './p00004-category-details.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 4: Category Details [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new CategoryDetailsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new CategoryDetailsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Category Details'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (8 items, 1 hidden)', async ({ page }) => {
    const po = new CategoryDetailsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P4_CATEGORY', 'P4_CREATED', 'P4_CREATED_BY', 'P4_DESCRIPTION', 'P4_ID', 'P4_IS_ACTIVE', 'P4_UPDATED', 'P4_UPDATED_BY']);
  });

  test('apex.item round-trip on P4_CATEGORY', async ({ page }) => {
    const po = new CategoryDetailsPage(page);
    await po.goto();
    await po.category.setValue('apx-testgen');
    expect(await po.category.getValue()).toBe('apx-testgen');
  });

  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new CategoryDetailsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Add Category', 'Delete', 'Apply Changes']);
  });
});
