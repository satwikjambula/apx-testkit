 ...
 * (./p00054-activity-type.page.js), not raw testkit calls, so both stay in sync.
 ...
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): activity-type (staticContent), buttons (staticContent).
...
import { ActivityTypePage } from './p00054-activity-type.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 54: Activity Type [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ActivityTypePage(page);
  ...
  test('every .apx pageItem exists in DOM/apex.item registry (3 items, 1 hidden)', async ({ page }) => {
    const po = new ActivityTypePage(page);
    await po.goto();
    await expectItemsPresent(page, ['P54_ID', 'P54_NAME', 'P54_REFERENCES']);
  });

  test('apex.item round-trip on P54_NAME', async ({ page }) => {
    const po = new ActivityTypePage(page);
    await po.goto();
  ...
  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new ActivityTypePage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Add Activity Type', 'Delete', 'Apply Changes']);
  });
});
