 ...
 * (./p00133-issue-status.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, form-on-eba-cust-issue-statuses
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), form-on-eba-cust-issue-statuses (staticContent).
...
import { IssueStatusPage } from './p00133-issue-status.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 133: Issue Status [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new IssueStatusPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new IssueStatusPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Issue Status'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (4 items, 1 hidden)', async ({ page }) => {
    const po = new IssueStatusPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P133_DETAILS', 'P133_ID', 'P133_NAME', 'P133_OPEN_YN']);
  });

  test('apex.item round-trip on P133_DETAILS', async ({ page }) => {
    const po = new IssueStatusPage(page);
    await po.goto();
  ...
  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new IssueStatusPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Add Issue Status', 'Delete', 'Apply Changes']);
  });
});
