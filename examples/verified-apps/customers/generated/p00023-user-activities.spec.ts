/**
 * GENERATED by apx-testgen from pages/p00023-user-activities.apx — DO NOT EDIT.
 ...
 * (./p00023-user-activities.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: top-users, user-activity-status-report
 ...
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): user-activity-status-report (breadcrumb).
...
import { UserActivitiesPage } from './p00023-user-activities.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 23: User Activities [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new UserActivitiesPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new UserActivitiesPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('User Activities'));
  });
