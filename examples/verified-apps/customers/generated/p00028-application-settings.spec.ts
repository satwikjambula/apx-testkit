 ...
 * (./p00028-application-settings.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, rename-application
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), rename-application (staticContent).
...
import { ApplicationSettingsPage } from './p00028-application-settings.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 28: Application Settings [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ApplicationSettingsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ApplicationSettingsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Application Settings'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (2 items, 0 hidden)', async ({ page }) => {
    const po = new ApplicationSettingsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P28_APPLICATION_TITLE', 'P28_FEEDBACK_RECIPIENTS']);
  });

  test('apex.item round-trip on P28_APPLICATION_TITLE', async ({ page }) => {
    const po = new ApplicationSettingsPage(page);
    await po.goto();
