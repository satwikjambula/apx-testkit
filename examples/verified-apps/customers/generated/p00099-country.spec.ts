 ...
 * (./p00099-country.page.js), not raw testkit calls, so both stay in sync.
 ...
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), country (staticContent).
...
import { CountryPage } from './p00099-country.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 99: Country [requires auth]', () => {
  ...
  test('every .apx pageItem exists in DOM/apex.item registry (5 items, 1 hidden)', async ({ page }) => {
    const po = new CountryPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P99_COUNTRY_CODE', 'P99_COUNTRY_NAME', 'P99_DISPLAY_YN', 'P99_ID', 'P99_REGION']);
  });

  test('apex.item round-trip on P99_COUNTRY_CODE', async ({ page }) => {
    const po = new CountryPage(page);
    await po.goto();
  ...
  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new CountryPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Add Country', 'Delete', 'Apply Changes']);
  });
});
