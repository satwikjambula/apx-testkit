/**
 * GENERATED by apx-testgen from pages/p00036-notification.apx — DO NOT EDIT.
 ...
 * (./p00036-notification.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, notifications
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), notifications (staticContent).
...
import { NotificationPage } from './p00036-notification.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 36: Notification [requires auth]', () => {
  ...
  test('every .apx pageItem exists in DOM/apex.item registry (7 items, 1 hidden)', async ({ page }) => {
    const po = new NotificationPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P36_DISPLAY_FROM', 'P36_DISPLAY_SEQUENCE', 'P36_DISPLAY_UNTIL', 'P36_ID', 'P36_NOTIFICATION_DESCRIPTION', 'P36_NOTIFICATION_NAME', 'P36_NOTIFICATION_TYPE']);
  });

  test('apex.item round-trip on P36_DISPLAY_SEQUENCE', async ({ page }) => {
    const po = new NotificationPage(page);
    await po.goto();
