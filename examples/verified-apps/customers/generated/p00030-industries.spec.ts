 ...
 * (./p00030-industries.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, industry
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), industry (staticContent).
...
import { IndustriesPage } from './p00030-industries.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 30: Industries [requires auth]', () => {
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new IndustriesPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Industries'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (2 items, 1 hidden)', async ({ page }) => {
    const po = new IndustriesPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P30_ID', 'P30_INDUSTRY_NAME']);
  });

  test('apex.item round-trip on P30_INDUSTRY_NAME', async ({ page }) => {
    const po = new IndustriesPage(page);
    await po.goto();
  ...
  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new IndustriesPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Add Industry', 'Delete', 'Apply Changes']);
  });
});
