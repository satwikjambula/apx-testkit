/**
 * GENERATED by apx-testgen from pages/p00029-recent-activities.apx — DO NOT EDIT.
 ...
 * (./p00029-recent-activities.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: customer-updates, customer-updates-2
 ...
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): customer-updates-2 (breadcrumb).
...
import { RecentActivitiesPage } from './p00029-recent-activities.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 29: Recent Activities [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new RecentActivitiesPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new RecentActivitiesPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Recent Activities'));
  });
  ...
  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new RecentActivitiesPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['customer-updates']);
  });
});
