 ...
 * (./p00043-product.page.js), not raw testkit calls, so both stay in sync.
...
import { ProductPage } from './p00043-product.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 43: Product [requires auth]', () => {
  ...
  test('every .apx pageItem exists in DOM/apex.item registry (6 items, 1 hidden)', async ({ page }) => {
    const po = new ProductPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P43_DESCRIPTION', 'P43_ID', 'P43_IS_ACTIVE', 'P43_PRODUCT_FAMILY_ID', 'P43_PRODUCT_NAME', 'P43_TAGS']);
  });

  test('apex.item round-trip on P43_DESCRIPTION', async ({ page }) => {
    const po = new ProductPage(page);
    await po.goto();
