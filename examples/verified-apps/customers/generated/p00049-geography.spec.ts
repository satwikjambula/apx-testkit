/**
 * GENERATED by apx-testgen from pages/p00049-geography.apx — DO NOT EDIT.
 ...
 * (./p00049-geography.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, geography
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), geography (staticContent).
...
import { GeographyPage } from './p00049-geography.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 49: Geography [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new GeographyPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new GeographyPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Geography'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (2 items, 1 hidden)', async ({ page }) => {
    const po = new GeographyPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P49_GEOGRAPHY_NAME', 'P49_ID']);
  });

  test('apex.item round-trip on P49_GEOGRAPHY_NAME', async ({ page }) => {
    const po = new GeographyPage(page);
    await po.goto();
    await po.geographyName.setValue('apx-testgen');
    expect(await po.geographyName.getValue()).toBe('apx-testgen');
  });

  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new GeographyPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Add Geography', 'Delete', 'Apply Changes']);
  });
});
