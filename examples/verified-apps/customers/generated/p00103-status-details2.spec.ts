/**
 * GENERATED by apx-testgen from pages/p00103-status-details2.apx — DO NOT EDIT.
 ...
 * (./p00103-status-details2.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: audit-information, buttons, customer-status-code
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): audit-information (staticContent), buttons (staticContent), customer-status-code (staticContent).
...
import { StatusDetails2Page } from './p00103-status-details2.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 103: Status Details [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new StatusDetails2Page(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new StatusDetails2Page(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Status Details'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (8 items, 1 hidden)', async ({ page }) => {
    const po = new StatusDetails2Page(page);
    await po.goto();
    await expectItemsPresent(page, ['P103_CREATED', 'P103_CREATED_BY', 'P103_DESCRIPTION', 'P103_ID', 'P103_IS_ACTIVE', 'P103_STATUS', 'P103_UPDATED', 'P103_UPDATED_BY']);
  });

  test('apex.item round-trip on P103_DESCRIPTION', async ({ page }) => {
    const po = new StatusDetails2Page(page);
    await po.goto();
  ...
  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new StatusDetails2Page(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Add Status Code', 'Delete', 'Apply Changes']);
  });
});
