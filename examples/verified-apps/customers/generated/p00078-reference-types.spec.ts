/**
 * GENERATED by apx-testgen from pages/p00078-reference-types.apx — DO NOT EDIT.
 ...
 * (./p00078-reference-types.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: reference-types
...
import { ReferenceTypesPage } from './p00078-reference-types.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 78: Reference Types [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ReferenceTypesPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ReferenceTypesPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Reference Types'));
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new ReferenceTypesPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Add Reference Type', 'Reset']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new ReferenceTypesPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['reference-types']);
  });
});
