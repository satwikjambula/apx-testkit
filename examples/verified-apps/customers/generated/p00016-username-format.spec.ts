/**
 * GENERATED by apx-testgen from pages/p00016-username-format.apx — DO NOT EDIT.
 ...
 * (./p00016-username-format.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, username-format-preference
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), username-format-preference (staticContent).
...
import { UsernameFormatPage } from './p00016-username-format.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 16: Username Format [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new UsernameFormatPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new UsernameFormatPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Username Format'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 0 hidden)', async ({ page }) => {
    const po = new UsernameFormatPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P16_USERNAME_FORMAT']);
  });
