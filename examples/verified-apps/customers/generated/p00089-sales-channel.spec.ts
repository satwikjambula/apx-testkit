/**
 * GENERATED by apx-testgen from pages/p00089-sales-channel.apx — DO NOT EDIT.
 ...
 * (./p00089-sales-channel.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: sales-channel
...
import { SalesChannelPage } from './p00089-sales-channel.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 89: Sales Channel [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new SalesChannelPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new SalesChannelPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Sales Channel'));
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new SalesChannelPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Add Sales Channel', 'Reset']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new SalesChannelPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['sales-channel']);
  });
});
