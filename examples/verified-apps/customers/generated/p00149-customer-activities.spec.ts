 ...
 * (./p00149-customer-activities.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, customer-activities
...
import { CustomerActivitiesPage } from './p00149-customer-activities.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 149: Customer Activities [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new CustomerActivitiesPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new CustomerActivitiesPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Customer Activities'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 1 hidden)', async ({ page }) => {
    const po = new CustomerActivitiesPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P149_CUSTOMER_ID']);
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new CustomerActivitiesPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Add Activity', 'Reset']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new CustomerActivitiesPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['customer-activities']);
  });
});
