/**
 * GENERATED by apx-testgen from pages/p00032-customers-by-category.apx — DO NOT EDIT.
 ...
 * (./p00032-customers-by-category.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, button-bar, customers-by-category
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * 1 chart region(s) SKIPPED -- no htmlDomId set, runtime id genuinely unconstructible from static data (ADR-003 layer 3): customers-by-category.
...
import { CustomersByCategoryPage } from './p00032-customers-by-category.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 32: Customers by Category [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new CustomersByCategoryPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new CustomersByCategoryPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Customers by Category'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (2 items, 0 hidden)', async ({ page }) => {
    const po = new CustomersByCategoryPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P32_CATEGORY', 'P32_GEO']);
  });
