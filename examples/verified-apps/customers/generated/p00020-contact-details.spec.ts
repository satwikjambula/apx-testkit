 ...
 * (./p00020-contact-details.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: address-country, contact-details, contact-details-2, social
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): address-country (staticContent), contact-details (staticContent), contact-details-2 (breadcrumb), social (staticContent).
...
import { ContactDetailsPage } from './p00020-contact-details.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 20: Contact Details [requires auth]', () => {
  ...
  test('every .apx pageItem exists in DOM/apex.item registry (23 items, 3 hidden)', async ({ page }) => {
    const po = new ContactDetailsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P20_ADDRESS_LINE1', 'P20_ADDRESS_LINE2', 'P20_CELL_PHONE', 'P20_CITY', 'P20_COMPANY', 'P20_CONTACT_ID', 'P20_CONTACT_TYPE_ID', 'P20_COUNTRY', 'P20_CUSTOMER_ID', 'P20_EMAIL', 'P20_FACEBOOK', 'P20_FAX', 'P20_ID', 'P20_LINKEDIN', 'P20_NAME', 'P20_NOTES', 'P20_PHONE', 'P20_REQUEST', 'P20_STATE', 'P20_TAGS', 'P20_TITLE', 'P20_TWITTER', 'P20_ZIP']);
  });

  test('apex.item round-trip on P20_ADDRESS_LINE1', async ({ page }) => {
    const po = new ContactDetailsPage(page);
    await po.goto();
    await po.addressLine1.setValue('apx-testgen');
    expect(await po.addressLine1.getValue()).toBe('apx-testgen');
  });

  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new ContactDetailsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Add Customer Contact', 'Delete', 'Apply Changes']);
  });
});
