/**
 * GENERATED by apx-testgen from pages/p00008-contact-types.apx — DO NOT EDIT.
 ...
 * (./p00008-contact-types.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: contact-types
...
import { ContactTypesPage } from './p00008-contact-types.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 8: Contact Types [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ContactTypesPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ContactTypesPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Contact Types'));
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new ContactTypesPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Add Contact Type', 'Reset']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new ContactTypesPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['contact-types']);
  });
});
