 ...
 * (./p00007-maintain-update.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: audit-information, breadcrumb, maintain-update
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): audit-information (staticContent), breadcrumb (breadcrumb), maintain-update (staticContent).
...
import { MaintainUpdatePage } from './p00007-maintain-update.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 7: Maintain Update [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new MaintainUpdatePage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new MaintainUpdatePage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Maintain Update'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (10 items, 2 hidden)', async ({ page }) => {
    const po = new MaintainUpdatePage(page);
    await po.goto();
    await expectItemsPresent(page, ['P7_CONTATCT_ID', 'P7_CREATED', 'P7_CREATED_BY', 'P7_CUSTOMER_ID', 'P7_ID', 'P7_NOTE', 'P7_NOTE_DATE', 'P7_SS', 'P7_UPDATED', 'P7_UPDATED_BY']);
  });
