 ...
 * (./p00064-customer-attachment.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, buttons, customer-attachment
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), buttons (staticContent), customer-attachment (staticContent).
...
import { CustomerAttachmentPage } from './p00064-customer-attachment.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 64: Customer Attachment [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new CustomerAttachmentPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new CustomerAttachmentPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Customer Attachment'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (8 items, 3 hidden)', async ({ page }) => {
    const po = new CustomerAttachmentPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P64_CUSTOMER_COMPANY', 'P64_CUSTOMER_ID', 'P64_FILE_BLOB', 'P64_FILE_COMMENTS', 'P64_FILE_MIMETYPE', 'P64_FILE_NAME', 'P64_ID', 'P64_TAGS']);
  });

  test('apex.item round-trip on P64_FILE_COMMENTS', async ({ page }) => {
    const po = new CustomerAttachmentPage(page);
    await po.goto();
