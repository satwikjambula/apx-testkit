/**
 * GENERATED by apx-testgen from pages/p00046-application-activity.apx — DO NOT EDIT.
 ...
 * (./p00046-application-activity.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: application-activity, breadcrumb
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): application-activity (calendar), breadcrumb (breadcrumb).
...
import { ApplicationActivityPage } from './p00046-application-activity.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 46: Application Activity [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ApplicationActivityPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ApplicationActivityPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Application Activity'));
  });
});
