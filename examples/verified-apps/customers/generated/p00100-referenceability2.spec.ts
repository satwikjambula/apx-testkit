/**
 * GENERATED by apx-testgen from pages/p00100-referenceability2.apx — DO NOT EDIT.
 ...
 * (./p00100-referenceability2.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, button-bar, by-asset-commitment, by-internal-contact, by-phase, product-summary, region-display-selector
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * 2 chart region(s) SKIPPED -- no htmlDomId set, runtime id genuinely unconstructible from static data (ADR-003 layer 3): by-asset-commitment, by-phase.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), button-bar (staticContent), by-internal-contact (plugin/html5BarChart), product-summary (plugin/badgeList), region-display-selector (regionDisplaySelector).
...
import { Referenceability2Page } from './p00100-referenceability2.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 100: Referenceability [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new Referenceability2Page(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new Referenceability2Page(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Referenceability'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (2 items, 0 hidden)', async ({ page }) => {
    const po = new Referenceability2Page(page);
    await po.goto();
    await expectItemsPresent(page, ['P100_CATEGORY', 'P100_GEO']);
  });
