/**
 * GENERATED by apx-testgen from pages/p00053-activity-types.apx — DO NOT EDIT.
 ...
 * (./p00053-activity-types.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: activity-types
...
import { ActivityTypesPage } from './p00053-activity-types.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 53: Activity Types [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ActivityTypesPage(page);
  ...
  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new ActivityTypesPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Add Activity Type', 'Reset']);
  });
