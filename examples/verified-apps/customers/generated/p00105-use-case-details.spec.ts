 ...
 * (./p00105-use-case-details.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: audit-information, buttons, customer-use-case-code
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): audit-information (staticContent), buttons (staticContent), customer-use-case-code (staticContent).
...
import { UseCaseDetailsPage } from './p00105-use-case-details.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 105: Use Case Details [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new UseCaseDetailsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new UseCaseDetailsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Use Case Details'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (8 items, 1 hidden)', async ({ page }) => {
    const po = new UseCaseDetailsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P105_CREATED', 'P105_CREATED_BY', 'P105_DESCRIPTION', 'P105_ID', 'P105_IS_ACTIVE', 'P105_UPDATED', 'P105_UPDATED_BY', 'P105_USE_CASE']);
  });
  ...
  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new UseCaseDetailsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Add Use Case Code', 'Delete', 'Apply Changes']);
  });
});
