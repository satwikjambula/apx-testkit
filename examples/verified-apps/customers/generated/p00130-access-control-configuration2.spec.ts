 ...
 * (./p00130-access-control-configuration2.page.js), not raw testkit calls, so both stay in sync.
...
import { AccessControlConfiguration2Page } from './p00130-access-control-configuration2.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 130: Access Control Configuration [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new AccessControlConfiguration2Page(page);
  ...
  test('every .apx pageItem exists in DOM/apex.item registry (4 items, 1 hidden)', async ({ page }) => {
    const po = new AccessControlConfiguration2Page(page);
    await po.goto();
    await expectItemsPresent(page, ['P130_ACCESS_CONTROL_SCOPE', 'P130_AC_ENABLED', 'P130_HIDDEN_AC_ENABLED', 'P130_USERNAME_FORMAT']);
  });
