 ...
 * (./p00088-reference-phase.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, reference-phase
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), reference-phase (staticContent).
...
import { ReferencePhasePage } from './p00088-reference-phase.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 88: Reference Phase [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ReferencePhasePage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ReferencePhasePage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Reference Phase'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (5 items, 1 hidden)', async ({ page }) => {
    const po = new ReferencePhasePage(page);
    await po.goto();
    await expectItemsPresent(page, ['P88_DESCRIPTION', 'P88_DISPLAY_SEQUENCE', 'P88_ID', 'P88_IS_AN_ACTIVE_REFERENCE', 'P88_STATUS']);
  });

  test('apex.item round-trip on P88_DESCRIPTION', async ({ page }) => {
    const po = new ReferencePhasePage(page);
    await po.goto();
  ...
  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new ReferencePhasePage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Add Reference Phase', 'Delete', 'Apply Changes']);
  });
});
