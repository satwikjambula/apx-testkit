/**
 * GENERATED by apx-testgen from pages/p00112-product-families.apx — DO NOT EDIT.
 ...
 * (./p00112-product-families.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: product-families
...
import { ProductFamiliesPage } from './p00112-product-families.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 112: Product Families [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ProductFamiliesPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ProductFamiliesPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Product Families'));
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new ProductFamiliesPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Create Product Family', 'Reset']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new ProductFamiliesPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['product-families']);
  });
});
