 ...
 * (./p00066-link.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, buttons, link
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), buttons (staticContent), link (staticContent).
...
import { LinkPage } from './p00066-link.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 66: Link [requires auth]', () => {
  ...
  test('every .apx pageItem exists in DOM/apex.item registry (8 items, 4 hidden)', async ({ page }) => {
    const po = new LinkPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P66_CUSTOMER_COMPANY', 'P66_CUSTOMER_ID', 'P66_ID', 'P66_LINK', 'P66_LINK_COMMENTS', 'P66_LINK_DESC', 'P66_LINK_TYPE', 'P66_TAGS']);
  });

  test('apex.item round-trip on P66_LINK', async ({ page }) => {
    const po = new LinkPage(page);
    await po.goto();
