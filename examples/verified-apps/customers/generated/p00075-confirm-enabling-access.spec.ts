 ...
 * (./p00075-confirm-enabling-access.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, confirm-enabling-access-control
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), confirm-enabling-access-control (staticContent).
...
import { ConfirmEnablingAccessPage } from './p00075-confirm-enabling-access.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 75: Confirm Enabling Access [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ConfirmEnablingAccessPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ConfirmEnablingAccessPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Confirm Enabling Access'));
  });

  test('every labeled .apx button exists (1 button)', async ({ page }) => {
    const po = new ConfirmEnablingAccessPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Enable Access Control']);
  });
});
