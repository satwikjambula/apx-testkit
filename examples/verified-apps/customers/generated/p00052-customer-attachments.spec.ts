/**
 * GENERATED by apx-testgen from pages/p00052-customer-attachments.apx — DO NOT EDIT.
 ...
 * (./p00052-customer-attachments.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: customer-files, customer-files-2
 ...
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): customer-files (breadcrumb).
...
import { CustomerAttachmentsPage } from './p00052-customer-attachments.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 52: Customer Attachments [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new CustomerAttachmentsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new CustomerAttachmentsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Customer Attachments'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (2 items, 2 hidden)', async ({ page }) => {
    const po = new CustomerAttachmentsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P52_DELETE_FILE_ID', 'P52_ID']);
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new CustomerAttachmentsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Add Attachment', 'Reset']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new CustomerAttachmentsPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['customer-files-2']);
  });
});
