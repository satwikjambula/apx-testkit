/**
 * GENERATED by apx-testgen from pages/p00006-status-details.apx — DO NOT EDIT.
 ...
 * (./p00006-status-details.page.js), not raw testkit calls, so both stay in sync.
...
import { StatusDetailsPage } from './p00006-status-details.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 6: Status Details [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new StatusDetailsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new StatusDetailsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Customer Status Details'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (8 items, 1 hidden)', async ({ page }) => {
    const po = new StatusDetailsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P6_CREATED', 'P6_CREATED_BY', 'P6_DESCRIPTION', 'P6_ID', 'P6_IS_ACTIVE', 'P6_TYPE', 'P6_UPDATED', 'P6_UPDATED_BY']);
  });

  test('apex.item round-trip on P6_DESCRIPTION', async ({ page }) => {
    const po = new StatusDetailsPage(page);
    await po.goto();
  ...
  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new StatusDetailsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Add Type Code', 'Delete', 'Apply Changes']);
  });
});
