 ...
 * (./p00027-industry.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: industry
...
import { IndustryPage } from './p00027-industry.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 27: Industry [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new IndustryPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new IndustryPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Industry'));
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new IndustryPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Add Industry', 'Reset']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new IndustryPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['industry']);
  });
});
