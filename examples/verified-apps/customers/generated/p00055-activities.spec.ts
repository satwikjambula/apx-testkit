/**
 * GENERATED by apx-testgen from pages/p00055-activities.apx — DO NOT EDIT.
 ...
 * (./p00055-activities.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: activities, breadcrumb
...
import { ActivitiesPage } from './p00055-activities.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 55: Activities [requires auth]', () => {
  ...
  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new ActivitiesPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Create Activity', 'Reset']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new ActivitiesPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['activities']);
  });
});
