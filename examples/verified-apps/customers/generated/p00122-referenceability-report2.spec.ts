/**
 * GENERATED by apx-testgen from pages/p00122-referenceability-report2.apx — DO NOT EDIT.
 ...
 * (./p00122-referenceability-report2.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, button-bar, by-asset-commitment
...
import { ReferenceabilityReport2Page } from './p00122-referenceability-report2.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 122: Referenceability Report [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ReferenceabilityReport2Page(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ReferenceabilityReport2Page(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Referenceability Report'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 0 hidden)', async ({ page }) => {
    const po = new ReferenceabilityReport2Page(page);
    await po.goto();
    await expectItemsPresent(page, ['P122_CATEGORY']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new ReferenceabilityReport2Page(page);
    await po.goto();
    await expectRegionsResolve(page, ['by-asset-commitment']);
  });
});
