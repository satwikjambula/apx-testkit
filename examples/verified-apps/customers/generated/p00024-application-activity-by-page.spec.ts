 ...
 * (./p00024-application-activity-by-page.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: application-activity-by-page, breadcrumb, button-bar
...
import { ApplicationActivityByPagePage } from './p00024-application-activity-by-page.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 24: Application Activity by Page [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ApplicationActivityByPagePage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ApplicationActivityByPagePage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Application Activity by Page'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 0 hidden)', async ({ page }) => {
    const po = new ApplicationActivityByPagePage(page);
    await po.goto();
    await expectItemsPresent(page, ['P24_TIMEFRAME']);
  });
  ...
  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new ApplicationActivityByPagePage(page);
    await po.goto();
    await expectRegionsResolve(page, ['application-activity-by-page']);
  });
});
