/**
 * GENERATED by apx-testgen from pages/p00127-data-validation2.apx — DO NOT EDIT.
 ...
 * (./p00127-data-validation2.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, data-load-process-train, data-validation, error
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), data-load-process-train (list), data-validation (classicReport), error (staticContent).
...
import { DataValidation2Page } from './p00127-data-validation2.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 127: Data Validation [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new DataValidation2Page(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new DataValidation2Page(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Data Validation'));
  });

  test('every labeled .apx button exists (3 buttons)', async ({ page }) => {
    const po = new DataValidation2Page(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Load Data', 'Previous']);
  });
});
