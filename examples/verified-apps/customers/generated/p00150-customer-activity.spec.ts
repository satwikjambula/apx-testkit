 ...
 * (./p00150-customer-activity.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, customer-activity
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), customer-activity (staticContent).
...
import { CustomerActivityPage } from './p00150-customer-activity.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 150: Customer Activity [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new CustomerActivityPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new CustomerActivityPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Customer Activity'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (11 items, 4 hidden)', async ({ page }) => {
    const po = new CustomerActivityPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P150_ACTIVITY_DATE', 'P150_ACTIVITY_ID', 'P150_CONTACT_ID', 'P150_CUSTOMER_ID', 'P150_ID', 'P150_LOCATION', 'P150_NOTES', 'P150_OWNER', 'P150_SHOW_DATE', 'P150_SHOW_LOCATION', 'P150_SHOW_OWNER']);
  });

  test('apex.item round-trip on P150_LOCATION', async ({ page }) => {
    const po = new CustomerActivityPage(page);
    await po.goto();
    await po.location.setValue('apx-testgen');
    expect(await po.location.getValue()).toBe('apx-testgen');
  });
