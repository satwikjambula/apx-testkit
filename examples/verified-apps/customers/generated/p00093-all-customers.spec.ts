/**
 * GENERATED by apx-testgen from pages/p00093-all-customers.apx — DO NOT EDIT.
 ...
 * (./p00093-all-customers.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumbs, customers, timezone
 ...
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumbs (breadcrumb), timezone (staticContent).
...
import { AllCustomersPage } from './p00093-all-customers.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 93: All Customers [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new AllCustomersPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new AllCustomersPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('All Customers'));
  });

  test('every labeled .apx button exists (3 buttons)', async ({ page }) => {
    const po = new AllCustomersPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Add Customer', 'Reset', 'Upload Customers']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new AllCustomersPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['CUSTOMERS']);
  });
});
