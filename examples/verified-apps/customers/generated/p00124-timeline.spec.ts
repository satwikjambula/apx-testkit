 ...
 * (./p00124-timeline.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, filter-controls, timeline
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), filter-controls (staticContent), timeline (classicReport).
...
import { TimelinePage } from './p00124-timeline.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 124: Timeline [requires auth]', () => {
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new TimelinePage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Timeline'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (3 items, 0 hidden)', async ({ page }) => {
    const po = new TimelinePage(page);
    await po.goto();
    await expectItemsPresent(page, ['P124_CUSTOMER', 'P124_TIMEFRAME', 'P124_UPDATE_BY']);
  });
