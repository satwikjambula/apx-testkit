 ...
 * (./p00107-implementation-partner-details.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, implementation-partner-details
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), implementation-partner-details (staticContent).
...
import { ImplementationPartnerDetailsPage } from './p00107-implementation-partner-details.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 107: Implementation Partner Details [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ImplementationPartnerDetailsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ImplementationPartnerDetailsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Implementation Partner Details'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (4 items, 1 hidden)', async ({ page }) => {
    const po = new ImplementationPartnerDetailsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P107_DESCRIPTION', 'P107_ID', 'P107_NAME', 'P107_WEBSITE']);
  });

  test('apex.item round-trip on P107_DESCRIPTION', async ({ page }) => {
    const po = new ImplementationPartnerDetailsPage(page);
    await po.goto();
