/**
 * GENERATED by apx-testgen from pages/p00058-user_details.apx — DO NOT EDIT.
 ...
 * (./p00058-user_details.page.js), not raw testkit calls, so both stay in sync.
...
import { UserDetailsPage } from './p00058-user_details.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 58: User Details [requires auth]', () => {
  ...
  test('every .apx pageItem exists in DOM/apex.item registry (4 items, 1 hidden)', async ({ page }) => {
    const po = new UserDetailsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P58_ACCESS_LEVEL_ID', 'P58_ACCOUNT_LOCKED', 'P58_ID', 'P58_USERNAME']);
  });

  test('apex.item round-trip on P58_USERNAME', async ({ page }) => {
    const po = new UserDetailsPage(page);
    await po.goto();
