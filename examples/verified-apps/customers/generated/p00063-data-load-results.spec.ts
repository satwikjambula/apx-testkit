 ...
 * (./p00063-data-load-results.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, data-load-process-train, data-load-results, failed-records
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), data-load-process-train (list), data-load-results (staticContent), failed-records (classicReport).
...
import { DataLoadResultsPage } from './p00063-data-load-results.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 63: Data Load Results [requires auth]', () => {
  ...
  test('every .apx pageItem exists in DOM/apex.item registry (3 items, 0 hidden)', async ({ page }) => {
    const po = new DataLoadResultsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P63_ERROR_COUNT', 'P63_INSERT_COUNT', 'P63_UPDATE_COUNT']);
  });
