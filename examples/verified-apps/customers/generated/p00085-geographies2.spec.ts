/**
 * GENERATED by apx-testgen from pages/p00085-geographies2.apx — DO NOT EDIT.
 ...
 * (./p00085-geographies2.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, button-bar, customers-by-geography
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * 1 chart region(s) SKIPPED -- no htmlDomId set, runtime id genuinely unconstructible from static data (ADR-003 layer 3): customers-by-geography.
...
import { Geographies2Page } from './p00085-geographies2.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 85: Geographies [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new Geographies2Page(page);
  ...
  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 0 hidden)', async ({ page }) => {
    const po = new Geographies2Page(page);
    await po.goto();
    await expectItemsPresent(page, ['P85_CATEGORY']);
  });
