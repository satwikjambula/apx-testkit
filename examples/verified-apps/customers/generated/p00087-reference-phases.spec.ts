/**
 * GENERATED by apx-testgen from pages/p00087-reference-phases.apx — DO NOT EDIT.
 ...
 * (./p00087-reference-phases.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: reference-phases
...
import { ReferencePhasesPage } from './p00087-reference-phases.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 87: Reference Phases [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ReferencePhasesPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ReferencePhasesPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Reference Phases'));
  });

  test('every labeled .apx button exists (3 buttons)', async ({ page }) => {
    const po = new ReferencePhasesPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Add Reference Phase', 'Resequence Display Sequence', 'Reset']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new ReferencePhasesPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['reference-phases']);
  });
});
