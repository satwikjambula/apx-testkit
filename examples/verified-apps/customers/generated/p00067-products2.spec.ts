/**
 * GENERATED by apx-testgen from pages/p00067-products2.apx — DO NOT EDIT.
 ...
 * (./p00067-products2.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, products
...
import { Products2Page } from './p00067-products2.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 67: Products [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new Products2Page(page);
  ...
  test('every .apx pageItem exists in DOM/apex.item registry (2 items, 2 hidden)', async ({ page }) => {
    const po = new Products2Page(page);
    await po.goto();
    await expectItemsPresent(page, ['P67_DELETE_PROD_ID', 'P67_ID']);
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new Products2Page(page);
    await po.goto();
    await expectButtonsPresent(page, ['Add Product', 'Reset']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new Products2Page(page);
    await po.goto();
    await expectRegionsResolve(page, ['PRODUCTS']);
  });
});
