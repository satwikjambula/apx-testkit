 ...
 * Regions present in metadata: sample-terminal-emulator, terminal
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): sample-terminal-emulator (staticContent), terminal (staticContent).
...
test.describe('page 1: ホーム [requires auth]', () => {
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new HomePage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Sample Terminal Emulator'));
  });
});
