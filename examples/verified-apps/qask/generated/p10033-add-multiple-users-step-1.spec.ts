/**
 * GENERATED by apx-testgen from pages/p10033-add-multiple-users-step-1.apx — DO NOT EDIT.
 ...
 * (./p10033-add-multiple-users-step-1.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, note, wizard-container
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), note (dynamicContent), wizard-container (staticContent).
...
import { AddMultipleUsersStep1Page } from './p10033-add-multiple-users-step-1.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 10033: Add Multiple Users - Step 1 [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new AddMultipleUsersStep1Page(page);
  ...
  test('every .apx pageItem exists in DOM/apex.item registry (4 items, 1 hidden)', async ({ page }) => {
    const po = new AddMultipleUsersStep1Page(page);
    await po.goto();
    await expectItemsPresent(page, ['P10033_PRELIM_USERS', 'P10033_RESTRICT_USERS_YN', 'P10033_ROLE', 'P10033_SEND_EMAIL_YN']);
  });

  test('apex.item round-trip on P10033_PRELIM_USERS', async ({ page }) => {
    const po = new AddMultipleUsersStep1Page(page);
    await po.goto();
