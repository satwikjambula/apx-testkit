 ...
 * (./p00100-qanda-sessions.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, button-bar, no-sessions-yet, search, search-results
 ...
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), button-bar (staticContent), no-sessions-yet (staticContent), search-results (themeTemplateComponent/contentRow).
...
import { QandaSessionsPage } from './p00100-qanda-sessions.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 100: Q&A Sessions [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new QandaSessionsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new QandaSessionsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Q&A Sessions'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 1 hidden)', async ({ page }) => {
    const po = new QandaSessionsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P100_SESSION_CNT']);
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new QandaSessionsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['New Session', 'Reset']);
  });
