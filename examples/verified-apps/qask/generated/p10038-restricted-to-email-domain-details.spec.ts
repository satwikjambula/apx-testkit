 ...
 * (./p10038-restricted-to-email-domain-details.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, restricted-to-email-domain-details
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), restricted-to-email-domain-details (form).
...
import { RestrictedToEmailDomainDetailsPage } from './p10038-restricted-to-email-domain-details.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 10038: Restricted To Email Domain Details [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new RestrictedToEmailDomainDetailsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new RestrictedToEmailDomainDetailsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Restricted To Email Domain Details'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (8 items, 6 hidden)', async ({ page }) => {
    const po = new RestrictedToEmailDomainDetailsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P10038_COMMENTS', 'P10038_CREATED_BY', 'P10038_CREATED_ON', 'P10038_EMAIL_DOMAIN', 'P10038_ID', 'P10038_ROW_VERSION', 'P10038_UPDATED_BY', 'P10038_UPDATED_ON']);
  });

  test('apex.item round-trip on P10038_COMMENTS', async ({ page }) => {
    const po = new RestrictedToEmailDomainDetailsPage(page);
    await po.goto();
