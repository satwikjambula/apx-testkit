 ...
 * (./p00116-delete-session.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, p116-session-code-p116-session-name
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), p116-session-code-p116-session-name (classicReport).
...
import { DeleteSessionPage } from './p00116-delete-session.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 116: Delete Session [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new DeleteSessionPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new DeleteSessionPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Delete Session'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (3 items, 3 hidden)', async ({ page }) => {
    const po = new DeleteSessionPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P116_SESSION_CODE', 'P116_SESSION_ID', 'P116_SESSION_NAME']);
  });
