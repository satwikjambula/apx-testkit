 ...
 * (./p00120-session-summary.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: actions-menu, breadcrumb, p120-session-name, participants, participants-report, questions
 ...
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): actions-menu (list), breadcrumb (breadcrumb), p120-session-name (classicReport), participants (staticContent), questions (themeTemplateComponent/contentRow).
...
import { SessionSummaryPage } from './p00120-session-summary.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 120: Session Summary [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new SessionSummaryPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new SessionSummaryPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Session Summary'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (3 items, 3 hidden)', async ({ page }) => {
    const po = new SessionSummaryPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P120_SESSION_CODE', 'P120_SESSION_ID', 'P120_SESSION_NAME']);
  });

  test('every labeled .apx button exists (5 buttons)', async ({ page }) => {
    const po = new SessionSummaryPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Actions Menu', 'Ask Question', 'Close Session', 'Start Session', 'Navigate Up']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new SessionSummaryPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['participants-report']);
  });
});
