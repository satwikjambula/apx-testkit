/**
 * GENERATED by apx-testgen from pages/p10022-application-error-log.apx — DO NOT EDIT.
 ...
 * (./p10022-application-error-log.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: application-error-log, breadcrumb
...
import { ApplicationErrorLogPage } from './p10022-application-error-log.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 10022: Application Error Log [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ApplicationErrorLogPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ApplicationErrorLogPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Application Error Log'));
  });
  ...
  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new ApplicationErrorLogPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['application-error-log']);
  });
});
