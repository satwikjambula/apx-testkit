 ...
 * (./p09995-accept-service-terms.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: accept-service-terms
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): accept-service-terms (staticContent).
...
import { AcceptServiceTermsPage } from './p09995-accept-service-terms.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 9995: Accept Service Terms [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new AcceptServiceTermsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new AcceptServiceTermsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('qAsk Accept Service Terms'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (2 items, 1 hidden)', async ({ page }) => {
    const po = new AcceptServiceTermsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P9995_TERMS', 'P9995_TERMS_ID']);
  });

  test('every labeled .apx button exists (1 button)', async ({ page }) => {
    const po = new AcceptServiceTermsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Accept Terms']);
  });
});
