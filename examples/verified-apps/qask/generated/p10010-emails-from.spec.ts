 ...
 * (./p10010-emails-from.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: qask-emails-from-setting
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): qask-emails-from-setting (staticContent).
...
import { EmailsFromPage } from './p10010-emails-from.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 10010: Emails From [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new EmailsFromPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new EmailsFromPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Emails From Setting'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 0 hidden)', async ({ page }) => {
    const po = new EmailsFromPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P10010_FROM_EMAIL']);
  });

  test('apex.item round-trip on P10010_FROM_EMAIL', async ({ page }) => {
    const po = new EmailsFromPage(page);
    await po.goto();
    await po.fromEmail.setValue('apx-testgen');
    expect(await po.fromEmail.getValue()).toBe('apx-testgen');
  });
