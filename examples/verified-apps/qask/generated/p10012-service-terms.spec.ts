/**
 * GENERATED by apx-testgen from pages/p10012-service-terms.apx — DO NOT EDIT.
 ...
 * (./p10012-service-terms.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, service-terms
...
import { ServiceTermsPage } from './p10012-service-terms.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 10012: Service Terms [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ServiceTermsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ServiceTermsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Service Terms'));
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new ServiceTermsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Add New Terms', 'Up']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new ServiceTermsPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['service-terms']);
  });
});
