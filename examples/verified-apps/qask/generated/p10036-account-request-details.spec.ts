 ...
 * (./p10036-account-request-details.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: account-request-details, buttons
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): account-request-details (form), buttons (staticContent).
...
import { AccountRequestDetailsPage } from './p10036-account-request-details.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 10036: Account Request Details [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new AccountRequestDetailsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new AccountRequestDetailsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Account Request Details'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (14 items, 7 hidden)', async ({ page }) => {
    const po = new AccountRequestDetailsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P10036_ACCEPTED_TERMS_ID', 'P10036_CREATED_BY', 'P10036_CREATED_ON', 'P10036_EMAIL', 'P10036_ID', 'P10036_JUSTIFICATION', 'P10036_REMOTE_ADDR', 'P10036_REQUEST_STATUS', 'P10036_ROW_VERSION', 'P10036_STATUS_REASON', 'P10036_UPDATED_BY', 'P10036_UPDATED_ON', 'P10036_USER_AGENT', 'P10036_X_FORWARDED_FOR']);
  });

  test('apex.item round-trip on P10036_STATUS_REASON', async ({ page }) => {
    const po = new AccountRequestDetailsPage(page);
    await po.goto();
    await po.statusReason.setValue('apx-testgen');
    expect(await po.statusReason.getValue()).toBe('apx-testgen');
  });

  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new AccountRequestDetailsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Approve', 'Cancel', 'Decline', 'Delete']);
  });
});
