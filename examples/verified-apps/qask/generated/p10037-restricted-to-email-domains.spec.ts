 ...
 * (./p10037-restricted-to-email-domains.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, note, restricted-to-email-domains
 ...
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), note (dynamicContent).
...
import { RestrictedToEmailDomainsPage } from './p10037-restricted-to-email-domains.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 10037: Restricted To Email Domains [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new RestrictedToEmailDomainsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new RestrictedToEmailDomainsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Restricted To Email Domains'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (2 items, 2 hidden)', async ({ page }) => {
    const po = new RestrictedToEmailDomainsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P10037_RESTRICT_REQUESTS_YN', 'P10037_RESTRICT_USERS_YN']);
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new RestrictedToEmailDomainsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Add Domain', 'Up']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new RestrictedToEmailDomainsPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['restricted-to-email-domains']);
  });
});
