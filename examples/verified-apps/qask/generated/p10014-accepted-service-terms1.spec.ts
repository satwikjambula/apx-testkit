 ...
 * (./p10014-accepted-service-terms1.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: accepted-service-terms, breadcrumb
...
import { AcceptedServiceTerms1Page } from './p10014-accepted-service-terms1.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 10014: Accepted Service Terms [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new AcceptedServiceTerms1Page(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new AcceptedServiceTerms1Page(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Accepted Service Terms'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 1 hidden)', async ({ page }) => {
    const po = new AcceptedServiceTerms1Page(page);
    await po.goto();
    await expectItemsPresent(page, ['P10014_CURRENT_ST_ID']);
  });
  ...
  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new AcceptedServiceTerms1Page(page);
    await po.goto();
    await expectRegionsResolve(page, ['accepted-service-terms']);
  });
});
