 ...
 * (./p00111-session-details.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, session-details
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), session-details (form).
...
import { SessionDetailsPage } from './p00111-session-details.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 111: Session Details [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new SessionDetailsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new SessionDetailsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Session Details'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (13 items, 8 hidden)', async ({ page }) => {
    const po = new SessionDetailsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P111_CREATED_BY', 'P111_CREATED_ON', 'P111_ID', 'P111_LIVE_YN', 'P111_OWNER', 'P111_PURPOSE', 'P111_RESP_EMAIL_REQUIRED_YN', 'P111_RESP_NAME_REQUIRED_YN', 'P111_ROW_VERSION', 'P111_SESSION_CODE', 'P111_SESSION_NAME', 'P111_UPDATED_BY', 'P111_UPDATED_ON']);
  });

  test('apex.item round-trip on P111_PURPOSE', async ({ page }) => {
    const po = new SessionDetailsPage(page);
    await po.goto();
