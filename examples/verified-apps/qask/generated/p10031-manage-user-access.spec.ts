/**
 * GENERATED by apx-testgen from pages/p10031-manage-user-access.apx — DO NOT EDIT.
 ...
 * (./p10031-manage-user-access.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, manage-user-access, note
...
import { ManageUserAccessPage } from './p10031-manage-user-access.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 10031: Manage User Access [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ManageUserAccessPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ManageUserAccessPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Manage User Access'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (2 items, 2 hidden)', async ({ page }) => {
    const po = new ManageUserAccessPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P10031_RESTRICT_REQUESTS_YN', 'P10031_RESTRICT_USERS_YN']);
  });

  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new ManageUserAccessPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Add Multiple Users', 'Add User', 'Reset', 'Up']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new ManageUserAccessPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['manage-user-access']);
  });
});
