/**
 * GENERATED by apx-testgen from pages/p10011-settings.apx — DO NOT EDIT.
 ...
 * (./p10011-settings.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, settings
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), settings (classicReport).
...
import { SettingsPage } from './p10011-settings.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 10011: Settings [requires auth]', () => {
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new SettingsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Settings'));
  });

  test('every labeled .apx button exists (3 buttons)', async ({ page }) => {
    const po = new SettingsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Apply Changes', 'Up']);
  });
});
