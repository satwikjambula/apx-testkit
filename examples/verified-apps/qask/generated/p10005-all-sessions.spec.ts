/**
 * GENERATED by apx-testgen from pages/p10005-all-sessions.apx — DO NOT EDIT.
 ...
 * (./p10005-all-sessions.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: all-sessions, breadcrumb
...
import { AllSessionsPage } from './p10005-all-sessions.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 10005: All Sessions [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new AllSessionsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new AllSessionsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('All Sessions'));
  });
  ...
  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new AllSessionsPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['all-sessions']);
  });
});
