 ...
 * (./p10020-activity-dashboard.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, filters, hourly-page-events, latest-activity, most-active-pages, recent-errors, top-users
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * 3 chart region(s) SKIPPED -- no htmlDomId set, runtime id genuinely unconstructible from static data (ADR-003 layer 3): hourly-page-events, most-active-pages, top-users.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), filters (staticContent), latest-activity (classicReport), recent-errors (classicReport).
...
import { ActivityDashboardPage } from './p10020-activity-dashboard.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 10020: Activity Dashboard [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ActivityDashboardPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ActivityDashboardPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Activity Dashboard'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 0 hidden)', async ({ page }) => {
    const po = new ActivityDashboardPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P10020_TIMEFRAME']);
  });

  test('every labeled .apx button exists (5 buttons)', async ({ page }) => {
    const po = new ActivityDashboardPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Up', 'View Details', 'View Details', 'View Details', 'View Details']);
  });
});
