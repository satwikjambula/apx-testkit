 ...
 * (./p10035-account-requests.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: account-requests, breadcrumb, note
...
import { AccountRequestsPage } from './p10035-account-requests.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 10035: Account Requests [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new AccountRequestsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new AccountRequestsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Account Requests'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (2 items, 2 hidden)', async ({ page }) => {
    const po = new AccountRequestsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P10035_RESTRICT_REQUESTS_YN', 'P10035_RESTRICT_USERS_YN']);
  });

  test('every labeled .apx button exists (1 button)', async ({ page }) => {
    const po = new AccountRequestsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Up']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new AccountRequestsPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['account-requests']);
  });
});
