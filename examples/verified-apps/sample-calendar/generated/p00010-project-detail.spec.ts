 ...
 * (./p00010-project-detail.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, project-detail
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Region types NOT covered by an auto-generated assertion (no verified DOM convention, or a runtime id genuinely unconstructible from static data -- see docs/grammar-assumptions.md "Still open" and ADR-003): buttons (staticContent), project-detail (staticContent).
...
import { ProjectDetailPage } from './p00010-project-detail.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 10: Project Detail [requires auth]', () => {
  ...
  test('every .apx pageItem exists in DOM/apex.item registry (9 items, 1 hidden)', async ({ page }) => {
    const po = new ProjectDetailPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P10_ASSIGNED_TO', 'P10_BUDGET', 'P10_COST', 'P10_END_DATE', 'P10_ID', 'P10_PROJECT', 'P10_START_DATE', 'P10_STATUS', 'P10_TASK_NAME']);
  });

  test('apex.item round-trip on P10_ASSIGNED_TO', async ({ page }) => {
    const po = new ProjectDetailPage(page);
    await po.goto();
