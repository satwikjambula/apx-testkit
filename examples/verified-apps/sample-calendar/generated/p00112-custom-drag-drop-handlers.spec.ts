/**
 * GENERATED by apx-testgen from pages/p00112-custom-drag-drop-handlers.apx — DO NOT EDIT.
 ...
 * (./p00112-custom-drag-drop-handlers.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: about-this-page, breadcrumb, sessions
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Region types NOT covered by an auto-generated assertion (no verified DOM convention, or a runtime id genuinely unconstructible from static data -- see docs/grammar-assumptions.md "Still open" and ADR-003): about-this-page (staticContent), breadcrumb (breadcrumb), sessions (calendar).
...
import { CustomDragDropHandlersPage } from './p00112-custom-drag-drop-handlers.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 112: Custom Drag & Drop Handlers [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new CustomDragDropHandlersPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new CustomDragDropHandlersPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Custom Drag & Drop Handlers'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 1 hidden)', async ({ page }) => {
    const po = new CustomDragDropHandlersPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P112_DRAGDROP_MSG']);
  });
});
