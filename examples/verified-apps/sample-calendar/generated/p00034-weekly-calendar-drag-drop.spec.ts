 ...
 * (./p00034-weekly-calendar-drag-drop.page.js), not raw testkit calls, so both stay in sync.
...
import { WeeklyCalendarDragDropPage } from './p00034-weekly-calendar-drag-drop.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 34: Weekly Calendar: Drag & Drop [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new WeeklyCalendarDragDropPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new WeeklyCalendarDragDropPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Weekly Calendar: Drag & Drop'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 0 hidden)', async ({ page }) => {
    const po = new WeeklyCalendarDragDropPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P34_SHOW_INACTIVE']);
  });
});
