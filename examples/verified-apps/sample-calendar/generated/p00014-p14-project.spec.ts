 ...
 * (./p00014-p14-project.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, end-date, hidden-items, project-details, start-date
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), end-date (plugin/miniCalendar), hidden-items (staticContent), project-details (staticContent), start-date (plugin/miniCalendar).
...
import { P14ProjectPage } from './p00014-p14-project.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 14: &P14_PROJECT. [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new P14ProjectPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new P14ProjectPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('&P14_PROJECT.'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (9 items, 2 hidden)', async ({ page }) => {
    const po = new P14ProjectPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P14_ASSIGNED_TO', 'P14_BUDGET', 'P14_COST', 'P14_END_DATE', 'P14_ID', 'P14_PROJECT', 'P14_START_DATE', 'P14_STATUS', 'P14_TASK_NAME']);
  });
