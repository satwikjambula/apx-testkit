 ...
 * (./p00073-calendar-client-events.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: about-this-page, breadcrumb, items-container, sessions
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Region types NOT covered by an auto-generated assertion (no verified DOM convention, or a runtime id genuinely unconstructible from static data -- see docs/grammar-assumptions.md "Still open" and ADR-003): about-this-page (staticContent), breadcrumb (breadcrumb), items-container (staticContent), sessions (calendar).
...
import { CalendarClientEventsPage } from './p00073-calendar-client-events.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 73: Calendar Client Events [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new CalendarClientEventsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new CalendarClientEventsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Calendar Client Events'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (4 items, 0 hidden)', async ({ page }) => {
    const po = new CalendarClientEventsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P73_ENDDATE', 'P73_SESSION_TYPE', 'P73_STARTDATE', 'P73_TITLE']);
  });

  test('apex.item round-trip on P73_TITLE', async ({ page }) => {
    const po = new CalendarClientEventsPage(page);
    await po.goto();
  ...
  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new CalendarClientEventsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Save to Database', 'Show in Calendar']);
  });
});
