 ...
 * (./p00035-weekly-calendar-time-format.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: about-this-page, breadcrumb, items-container, sessions-12h, sessions-24h, sessions-default
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): about-this-page (staticContent), breadcrumb (breadcrumb), items-container (staticContent), sessions-12h (calendar), sessions-24h (calendar), sessions-default (calendar).
...
import { WeeklyCalendarTimeFormatPage } from './p00035-weekly-calendar-time-format.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 35: Weekly Calendar: Time format [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new WeeklyCalendarTimeFormatPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new WeeklyCalendarTimeFormatPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Weekly Calendar 12h/24h'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (2 items, 0 hidden)', async ({ page }) => {
    const po = new WeeklyCalendarTimeFormatPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P35_SHOW_INACTIVE', 'P35_SHOW_TIME_FORMAT']);
  });
});
