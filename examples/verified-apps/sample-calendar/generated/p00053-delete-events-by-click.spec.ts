/**
 * GENERATED by apx-testgen from pages/p00053-delete-events-by-click.apx — DO NOT EDIT.
 ...
 * (./p00053-delete-events-by-click.page.js), not raw testkit calls, so both stay in sync.
...
import { DeleteEventsByClickPage } from './p00053-delete-events-by-click.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 53: Delete Events by Click [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new DeleteEventsByClickPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new DeleteEventsByClickPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Delete Events by Click'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (2 items, 1 hidden)', async ({ page }) => {
    const po = new DeleteEventsByClickPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P53_SESSION_ID', 'P53_SHOW_INACTIVE']);
  });
});
