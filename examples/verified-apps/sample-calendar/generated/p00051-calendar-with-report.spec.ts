 ...
 * (./p00051-calendar-with-report.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: about-this-page, breadcrumb, button-bar, tasks, tasks-2
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): about-this-page (staticContent), breadcrumb (breadcrumb), button-bar (staticContent), tasks (calendar), tasks-2 (classicReport).
...
import { CalendarWithReportPage } from './p00051-calendar-with-report.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 51: Calendar with Report [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new CalendarWithReportPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new CalendarWithReportPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Calendar with Report'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (3 items, 2 hidden)', async ({ page }) => {
    const po = new CalendarWithReportPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P51_CAL_END_DATE', 'P51_CAL_START_DATE', 'P51_PROJECTS']);
  });
});
