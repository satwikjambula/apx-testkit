/**
 * GENERATED by apx-testgen from pages/p00092-week-calendar-with-icons.apx — DO NOT EDIT.
 ...
 * (./p00092-week-calendar-with-icons.page.js), not raw testkit calls, so both stay in sync.
...
import { WeekCalendarWithIconsPage } from './p00092-week-calendar-with-icons.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 92: Week Calendar with Icons [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new WeekCalendarWithIconsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new WeekCalendarWithIconsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Week Calendar with Icons'));
  });
});
