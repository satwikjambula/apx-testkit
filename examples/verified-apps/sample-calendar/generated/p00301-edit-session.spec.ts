 ...
 * (./p00301-edit-session.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, edit-session
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Region types NOT covered by an auto-generated assertion (no verified DOM convention, or a runtime id genuinely unconstructible from static data -- see docs/grammar-assumptions.md "Still open" and ADR-003): buttons (staticContent), edit-session (staticContent).
...
import { EditSessionPage } from './p00301-edit-session.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 301: Edit Session [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new EditSessionPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new EditSessionPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Edit Session'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (8 items, 2 hidden)', async ({ page }) => {
    const po = new EditSessionPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P301_END_DATE', 'P301_ID', 'P301_ROW_VERSION_NUMBER', 'P301_SESSION_TYPE', 'P301_SPEAKER', 'P301_START_DATE', 'P301_STATUS', 'P301_TITLE']);
  });

  test('apex.item round-trip on P301_SPEAKER', async ({ page }) => {
    const po = new EditSessionPage(page);
    await po.goto();
    await po.speaker.setValue('apx-testgen');
    expect(await po.speaker.getValue()).toBe('apx-testgen');
  });
