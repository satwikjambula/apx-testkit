 ...
 * (./p00031-monthly-calendar-projects.page.js), not raw testkit calls, so both stay in sync.
...
import { MonthlyCalendarProjectsPage } from './p00031-monthly-calendar-projects.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 31: Monthly Calendar: Projects [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new MonthlyCalendarProjectsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new MonthlyCalendarProjectsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Monthly Calendar: Projects'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 0 hidden)', async ({ page }) => {
    const po = new MonthlyCalendarProjectsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P31_PROJECTS']);
  });
});
