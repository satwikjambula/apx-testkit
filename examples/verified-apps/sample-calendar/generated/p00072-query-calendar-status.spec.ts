 ...
 * (./p00072-query-calendar-status.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: about-this-page, breadcrumb, button-bar, tasks
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Region types NOT covered by an auto-generated assertion (no verified DOM convention, or a runtime id genuinely unconstructible from static data -- see docs/grammar-assumptions.md "Still open" and ADR-003): about-this-page (staticContent), breadcrumb (breadcrumb), button-bar (staticContent), tasks (calendar).
...
import { QueryCalendarStatusPage } from './p00072-query-calendar-status.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 72: Query Calendar Status [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new QueryCalendarStatusPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new QueryCalendarStatusPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Query Calendar Status'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (3 items, 0 hidden)', async ({ page }) => {
    const po = new QueryCalendarStatusPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P72_END_DATE', 'P72_START_DATE', 'P72_VIEW_TYPE']);
  });

  test('apex.item round-trip on P72_END_DATE', async ({ page }) => {
    const po = new QueryCalendarStatusPage(page);
    await po.goto();
    await po.endDate.setValue('apx-testgen');
    expect(await po.endDate.getValue()).toBe('apx-testgen');
  });
});
