/**
 * GENERATED by apx-testgen from pages/p00033-weekly-calendar-edit-sessions.apx — DO NOT EDIT.
 ...
 * (./p00033-weekly-calendar-edit-sessions.page.js), not raw testkit calls, so both stay in sync.
...
import { WeeklyCalendarEditSessionsPage } from './p00033-weekly-calendar-edit-sessions.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 33: Weekly Calendar: Edit Sessions [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new WeeklyCalendarEditSessionsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new WeeklyCalendarEditSessionsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Weekly Calendar: Edit Sessions'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 0 hidden)', async ({ page }) => {
    const po = new WeeklyCalendarEditSessionsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P33_SHOW_INACTIVE']);
  });
});
