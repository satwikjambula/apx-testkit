 ...
 * (./p00003-edit-calendar-entry.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, edit-calendar-entry
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Region types NOT covered by an auto-generated assertion (no verified DOM convention, or a runtime id genuinely unconstructible from static data -- see docs/grammar-assumptions.md "Still open" and ADR-003): buttons (staticContent), edit-calendar-entry (staticContent).
...
import { EditCalendarEntryPage } from './p00003-edit-calendar-entry.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 3: Edit Calendar Entry [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new EditCalendarEntryPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new EditCalendarEntryPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Edit Calendar Entry'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (8 items, 3 hidden)', async ({ page }) => {
    const po = new EditCalendarEntryPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P3_END_DATE', 'P3_END_DATE_TMP', 'P3_ID', 'P3_LASTVIEW', 'P3_PROJECT', 'P3_START_DATE', 'P3_STATUS', 'P3_TASK_NAME']);
  });

  test('apex.item round-trip on P3_TASK_NAME', async ({ page }) => {
    const po = new EditCalendarEntryPage(page);
    await po.goto();
    await po.taskName.setValue('apx-testgen');
    expect(await po.taskName.getValue()).toBe('apx-testgen');
  });

  test('every labeled .apx button exists (3 buttons)', async ({ page }) => {
    const po = new EditCalendarEntryPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Add Event', 'Apply Changes']);
  });
});
