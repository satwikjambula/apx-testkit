/**
 * GENERATED by apx-testgen from pages/p00006-administration.apx — DO NOT EDIT.
 ...
 * (./p00006-administration.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, buttons, manage-sample-data
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Region types NOT covered by an auto-generated assertion (no verified DOM convention, or a runtime id genuinely unconstructible from static data -- see docs/grammar-assumptions.md "Still open" and ADR-003): breadcrumb (breadcrumb), buttons (staticContent), manage-sample-data (staticContent).
...
import { AdministrationPage } from './p00006-administration.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 6: Administration [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new AdministrationPage(page);
  ...
  test('every labeled .apx button exists (3 buttons)', async ({ page }) => {
    const po = new AdministrationPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Reset Data', 'View Report']);
  });
});
