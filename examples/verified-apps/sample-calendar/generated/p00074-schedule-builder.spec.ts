 ...
 * (./p00074-schedule-builder.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: about-this-page, breadcrumb, items-container, my-sessions, sessions
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): about-this-page (staticContent), breadcrumb (breadcrumb), items-container (staticContent), my-sessions (calendar), sessions (calendar).
...
import { ScheduleBuilderPage } from './p00074-schedule-builder.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 74: Schedule Builder [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ScheduleBuilderPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ScheduleBuilderPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Schedule Builder'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (4 items, 1 hidden)', async ({ page }) => {
    const po = new ScheduleBuilderPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P74_ENDDATE', 'P74_MY_SESSION_ID', 'P74_STARTDATE', 'P74_TITLE']);
  });

  test('apex.item round-trip on P74_TITLE', async ({ page }) => {
    const po = new ScheduleBuilderPage(page);
    await po.goto();
    await po.title.setValue('apx-testgen');
    expect(await po.title.getValue()).toBe('apx-testgen');
  });

  test('every labeled .apx button exists (3 buttons)', async ({ page }) => {
    const po = new ScheduleBuilderPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Clear Schedule', 'Save to Database', 'Show in Calendar']);
  });
});
