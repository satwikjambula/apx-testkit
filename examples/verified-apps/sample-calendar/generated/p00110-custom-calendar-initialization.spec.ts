 ...
 * (./p00110-custom-calendar-initialization.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, custom-calendar-initialization
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Region types NOT covered by an auto-generated assertion (no verified DOM convention, or a runtime id genuinely unconstructible from static data -- see docs/grammar-assumptions.md "Still open" and ADR-003): breadcrumb (breadcrumb), custom-calendar-initialization (list).
...
import { CustomCalendarInitializationPage } from './p00110-custom-calendar-initialization.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 110: Custom Calendar Initialization [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new CustomCalendarInitializationPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new CustomCalendarInitializationPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Custom Calendar Initialization'));
  });
});
