/**
 * GENERATED by apx-testgen from pages/p00004-time-line.apx — DO NOT EDIT.
 ...
 * (./p00004-time-line.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: about-this-page, breadcrumb, button-bar, project-tasks
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): about-this-page (staticContent), breadcrumb (breadcrumb), button-bar (staticContent), project-tasks (plugin/timelineAndStatusList).
...
import { TimeLinePage } from './p00004-time-line.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 4: Time Line [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new TimeLinePage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new TimeLinePage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Time Line'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 0 hidden)', async ({ page }) => {
    const po = new TimeLinePage(page);
    await po.goto();
    await expectItemsPresent(page, ['P4_PROJECTS']);
  });
