 ...
 * (./p00054-copy-events-by-click.page.js), not raw testkit calls, so both stay in sync.
...
import { CopyEventsByClickPage } from './p00054-copy-events-by-click.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 54: Copy Events by Click [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new CopyEventsByClickPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new CopyEventsByClickPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Copy Events by Click'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (2 items, 1 hidden)', async ({ page }) => {
    const po = new CopyEventsByClickPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P54_SESSION_ID', 'P54_SHOW_INACTIVE']);
  });
});
