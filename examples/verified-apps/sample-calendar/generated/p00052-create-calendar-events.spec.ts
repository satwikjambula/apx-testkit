 ...
 * (./p00052-create-calendar-events.page.js), not raw testkit calls, so both stay in sync.
...
import { CreateCalendarEventsPage } from './p00052-create-calendar-events.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 52: Create Calendar Events [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new CreateCalendarEventsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new CreateCalendarEventsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Create Calendar Events'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (3 items, 2 hidden)', async ({ page }) => {
    const po = new CreateCalendarEventsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P52_ENDDATE', 'P52_SHOW_INACTIVE', 'P52_STARTDATE']);
  });
});
