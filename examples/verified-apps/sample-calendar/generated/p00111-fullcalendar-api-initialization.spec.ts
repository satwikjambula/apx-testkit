/**
 * GENERATED by apx-testgen from pages/p00111-fullcalendar-api-initialization.apx — DO NOT EDIT.
 ...
 * (./p00111-fullcalendar-api-initialization.page.js), not raw testkit calls, so both stay in sync.
...
import { FullcalendarApiInitializationPage } from './p00111-fullcalendar-api-initialization.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 111: FullCalendar API initialization [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new FullcalendarApiInitializationPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new FullcalendarApiInitializationPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('FullCalendar API initialization'));
  });
});
