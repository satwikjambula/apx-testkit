 ...
 * (./p00071-custom-navigation.page.js), not raw testkit calls, so both stay in sync.
...
import { CustomNavigationPage } from './p00071-custom-navigation.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 71: Custom Navigation [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new CustomNavigationPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new CustomNavigationPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Custom Navigation'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (3 items, 0 hidden)', async ({ page }) => {
    const po = new CustomNavigationPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P71_GOTODATE', 'P71_PROJECTS', 'P71_VIEWTYPE']);
  });

  test('every labeled .apx button exists (3 buttons)', async ({ page }) => {
    const po = new CustomNavigationPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Next Calendar Page', 'Previous Calendar Page', 'Today']);
  });
});
