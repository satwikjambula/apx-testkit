/**
 * GENERATED by apx-testgen from pages/p00013-report2.apx — DO NOT EDIT.
 ...
 * (./p00013-report2.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: about-this-page, breadcrumb, report
...
import { Report2Page } from './p00013-report2.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 13: Report [requires auth]', () => {
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new Report2Page(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Report'));
  });
  ...
  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new Report2Page(page);
    await po.goto();
    await expectRegionsResolve(page, ['report']);
  });
});
