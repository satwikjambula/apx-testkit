/**
 * GENERATED by apx-testgen from pages/p00009-date-reporting.apx — DO NOT EDIT.
 ...
 * (./p00009-date-reporting.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: about-this-page, breadcrumb, container, date-reporting, sql-source
...
import { DateReportingPage } from './p00009-date-reporting.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 9: Date Reporting [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new DateReportingPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new DateReportingPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Date Reporting'));
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new DateReportingPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Add Project', 'Reset']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new DateReportingPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['date_reporting']);
  });
});
