 ...
 * Regions present in metadata: about, dwrole-grant-is-missing, examples, installed-on-oracle-cloud, not-installed-on-oracle-cloud, oci-resource-principal-is-missing, prerequisites, privs, sample-document-generator
 ...
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): about (staticContent), dwrole-grant-is-missing (dynamicContent), installed-on-oracle-cloud (staticContent), not-installed-on-oracle-cloud (staticContent), oci-resource-principal-is-missing (dynamicContent), prerequisites (staticContent), privs (staticContent), sample-document-generator (staticContent).
...
test.describe('page 1: Home [requires auth]', () => {
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new HomePage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Sample DocGen'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (2 items, 2 hidden)', async ({ page }) => {
    const po = new HomePage(page);
    await po.goto();
    await expectItemsPresent(page, ['P1_PRIVS_MISSING_YN', 'P1_STATIC_ID']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new HomePage(page);
    await po.goto();
    await expectRegionsResolve(page, ['examples']);
  });
});
