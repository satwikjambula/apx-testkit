/**
 * GENERATED by apx-testgen from pages/p00186-archival-policy-manager.apx — DO NOT EDIT.
 ...
 * (./p00186-archival-policy-manager.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, current-archival-policy
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), current-archival-policy (classicReport).
...
import { ArchivalPolicyManagerPage } from './p00186-archival-policy-manager.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 186: Archival Policy Manager [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ArchivalPolicyManagerPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ArchivalPolicyManagerPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Archival Policy Manager'));
  });
});
