/**
 * GENERATED by apx-testgen from pages/p00060-output-delivery-manager.apx — DO NOT EDIT.
 ...
 * (./p00060-output-delivery-manager.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, output-delivery-manager
...
import { OutputDeliveryManagerPage } from './p00060-output-delivery-manager.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 60: Output Delivery Manager [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new OutputDeliveryManagerPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new OutputDeliveryManagerPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Output Delivery Manager'));
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new OutputDeliveryManagerPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['output-delivery-manager']);
  });
});
