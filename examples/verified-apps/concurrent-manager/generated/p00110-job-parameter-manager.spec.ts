/**
 * GENERATED by apx-testgen from pages/p00110-job-parameter-manager.apx — DO NOT EDIT.
 ...
 * (./p00110-job-parameter-manager.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, job-parameter-manager
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * 1 Interactive Grid region(s) SKIPPED -- no htmlDomId set, runtime id genuinely unconstructible from static data (ADR-003 layer 3): job-parameter-manager.
...
import { JobParameterManagerPage } from './p00110-job-parameter-manager.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 110: Job Parameter Manager [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new JobParameterManagerPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new JobParameterManagerPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Job Parameter Manager'));
  });
});
