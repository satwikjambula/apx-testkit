 ...
 * (./p00250-user-activity-report.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, user-activity-report
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * 1 Interactive Grid region(s) SKIPPED -- no htmlDomId set, runtime id genuinely unconstructible from static data (ADR-003 layer 3): user-activity-report.
...
import { UserActivityReportPage } from './p00250-user-activity-report.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 250: User Activity Report [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new UserActivityReportPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new UserActivityReportPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('User Activity Report'));
  });
});
