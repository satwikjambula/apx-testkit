 ...
 * (./p00210-system-configuration.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, engine-settings, notification-settings, purge-settings
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), engine-settings (staticContent), notification-settings (staticContent), purge-settings (staticContent).
...
import { SystemConfigurationPage } from './p00210-system-configuration.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 210: System Configuration [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new SystemConfigurationPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new SystemConfigurationPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('System Configuration'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (9 items, 0 hidden)', async ({ page }) => {
    const po = new SystemConfigurationPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P210_DISPATCHER_INTERVAL', 'P210_EMAIL_FROM', 'P210_HEARTBEAT_TIMEOUT', 'P210_LOG_BASE_PATH', 'P210_LOG_RETENTION', 'P210_MAX_GLOBAL_CONCURRENT', 'P210_MAX_RETRIES', 'P210_PURGE_RETENTION', 'P210_SLA_ALERT_PCT']);
  });

  test('apex.item round-trip on P210_EMAIL_FROM', async ({ page }) => {
    const po = new SystemConfigurationPage(page);
    await po.goto();
    await po.emailFrom.setValue('apx-testgen');
    expect(await po.emailFrom.getValue()).toBe('apx-testgen');
  });

  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new SystemConfigurationPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Capture System Trace', 'Retry Failed Notifications', 'Run Zombie Recovery', 'Save Configuration']);
  });
});
