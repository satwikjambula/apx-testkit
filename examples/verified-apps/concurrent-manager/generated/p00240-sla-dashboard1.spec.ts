/**
 * GENERATED by apx-testgen from pages/p00240-sla-dashboard1.apx — DO NOT EDIT.
 ...
 * (./p00240-sla-dashboard1.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breach-trend, cards, sla-at-risk-jobs, sla-compliance-by-job, sla-dashboard, sla-policies-region
 ...
 * 2 chart region(s) SKIPPED -- no htmlDomId set, runtime id genuinely unconstructible from static data (ADR-003 layer 3): breach-trend, sla-compliance-by-job.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): sla-dashboard (breadcrumb), sla-policies-region (classicReport).
...
import { SlaDashboard1Page } from './p00240-sla-dashboard1.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 240: SLA Dashboard [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new SlaDashboard1Page(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new SlaDashboard1Page(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('SLA Dashboard'));
  });

  test('every interactiveReport/cards/facetedSearch region resolves (2 regions)', async ({ page }) => {
    const po = new SlaDashboard1Page(page);
    await po.goto();
    await expectRegionsResolve(page, ['cards', 'sla_at_risk']);
  });
});
