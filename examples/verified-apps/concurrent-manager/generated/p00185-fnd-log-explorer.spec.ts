 ...
 * (./p00185-fnd-log-explorer.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: create-info-banner-region, filters, fnd-log-messages
 ...
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): create-info-banner-region (staticContent), filters (staticContent).
...
import { FndLogExplorerPage } from './p00185-fnd-log-explorer.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 185: FND Log Explorer [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new FndLogExplorerPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new FndLogExplorerPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('FND Log Explorer'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (5 items, 0 hidden)', async ({ page }) => {
    const po = new FndLogExplorerPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P185_DATE_FROM', 'P185_DATE_TO', 'P185_LOG_LEVEL', 'P185_MODULE_FILTER', 'P185_RUN_ID']);
  });

  test('apex.item round-trip on P185_MODULE_FILTER', async ({ page }) => {
    const po = new FndLogExplorerPage(page);
    await po.goto();
    await po.moduleFilter.setValue('apx-testgen');
    expect(await po.moduleFilter.getValue()).toBe('apx-testgen');
  });

  test('every labeled .apx button exists (1 button)', async ({ page }) => {
    const po = new FndLogExplorerPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Filter']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new FndLogExplorerPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['fnd_log_ir']);
  });
});
