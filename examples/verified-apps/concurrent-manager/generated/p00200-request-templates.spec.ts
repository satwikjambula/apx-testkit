/**
 * GENERATED by apx-testgen from pages/p00200-request-templates.apx — DO NOT EDIT.
 ...
 * (./p00200-request-templates.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, request-templates
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * 1 Interactive Grid region(s) SKIPPED -- no htmlDomId set, runtime id genuinely unconstructible from static data (ADR-003 layer 3): request-templates.
...
import { RequestTemplatesPage } from './p00200-request-templates.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 200: Request Templates [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new RequestTemplatesPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new RequestTemplatesPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Request Templates'));
  });
