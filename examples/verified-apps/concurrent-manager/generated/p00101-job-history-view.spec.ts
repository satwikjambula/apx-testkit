/**
 * GENERATED by apx-testgen from pages/p00101-job-history-view.apx — DO NOT EDIT.
 ...
 * (./p00101-job-history-view.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: job-history-view
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * 1 Interactive Grid region(s) SKIPPED -- no htmlDomId set, runtime id genuinely unconstructible from static data (ADR-003 layer 3): job-history-view.
...
import { JobHistoryViewPage } from './p00101-job-history-view.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 101: Job History View [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new JobHistoryViewPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new JobHistoryViewPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Job History View'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 1 hidden)', async ({ page }) => {
    const po = new JobHistoryViewPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P101_JOB_CODE']);
  });
});
