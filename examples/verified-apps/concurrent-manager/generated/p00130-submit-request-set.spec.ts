 ...
 * (./p00130-submit-request-set.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, member-jobs, member-parameters, page-items
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), member-jobs (classicReport), member-parameters (staticContent), page-items (staticContent).
...
import { SubmitRequestSetPage } from './p00130-submit-request-set.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 130: Submit Request Set [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new SubmitRequestSetPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new SubmitRequestSetPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Submit Request Set'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (9 items, 2 hidden)', async ({ page }) => {
    const po = new SubmitRequestSetPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P130_PRIORITY', 'P130_RESUBMIT_ANCHOR', 'P130_RESULT_ID', 'P130_SCHEDULED_TS', 'P130_SCHEDULE_END_TS', 'P130_SCHEDULE_INTERVAL', 'P130_SCHEDULE_TYPE', 'P130_SET_CODE', 'P130_SET_PARAMS_JSON']);
  });

  test('apex.item round-trip on P130_SCHEDULE_INTERVAL', async ({ page }) => {
    const po = new SubmitRequestSetPage(page);
    await po.goto();
  ...
  test('every labeled .apx button exists (1 button)', async ({ page }) => {
    const po = new SubmitRequestSetPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Submit']);
  });
});
