 ...
 * (./p00181-purge-preview-region-report.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, purge-preview-region-report
...
import { PurgePreviewRegionReportPage } from './p00181-purge-preview-region-report.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 181: Purge Preview Region Report [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new PurgePreviewRegionReportPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new PurgePreviewRegionReportPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Purge Preview Region Report'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 1 hidden)', async ({ page }) => {
    const po = new PurgePreviewRegionReportPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P181_CONFIG_ID']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new PurgePreviewRegionReportPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['purge-preview-region-report']);
  });
});
