/**
 * GENERATED by apx-testgen from pages/p00195-email-template-manager.apx — DO NOT EDIT.
 ...
 * (./p00195-email-template-manager.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, email-template-manager
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * 1 Interactive Grid region(s) SKIPPED -- no htmlDomId set, runtime id genuinely unconstructible from static data (ADR-003 layer 3): email-template-manager.
...
import { EmailTemplateManagerPage } from './p00195-email-template-manager.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 195: Email Template Manager [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new EmailTemplateManagerPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new EmailTemplateManagerPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Email Template Manager'));
  });
