/**
 * GENERATED by apx-testgen from pages/p00150-job-queue-monitor.apx — DO NOT EDIT.
 ...
 * (./p00150-job-queue-monitor.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, dbms-scheduler-active-workers, dispatcher-health-strip, dispatcher-health-strip-2, job-queue-monitor, summary-cards-region
 ...
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), dbms-scheduler-active-workers (classicReport), dispatcher-health-strip (dynamicContent), dispatcher-health-strip-2 (staticContent), summary-cards-region (classicReport).
...
import { JobQueueMonitorPage } from './p00150-job-queue-monitor.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 150: Job Queue Monitor [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new JobQueueMonitorPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new JobQueueMonitorPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Job Queue Monitor'));
  });

  test('every Interactive Grid region with a known runtime id resolves a current view (1 region)', async ({ page }) => {
    const po = new JobQueueMonitorPage(page);
    await po.goto();
    for (const id of ['job_monitor']) {
