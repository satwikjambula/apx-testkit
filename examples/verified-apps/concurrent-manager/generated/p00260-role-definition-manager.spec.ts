/**
 * GENERATED by apx-testgen from pages/p00260-role-definition-manager.apx — DO NOT EDIT.
 ...
 * (./p00260-role-definition-manager.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, role-definition-manager
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * 1 Interactive Grid region(s) SKIPPED -- no htmlDomId set, runtime id genuinely unconstructible from static data (ADR-003 layer 3): role-definition-manager.
...
import { RoleDefinitionManagerPage } from './p00260-role-definition-manager.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 260: Role Definition Manager [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new RoleDefinitionManagerPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new RoleDefinitionManagerPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Role Definition Manager'));
  });
});
