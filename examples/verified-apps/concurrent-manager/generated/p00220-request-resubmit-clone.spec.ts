 ...
 * (./p00220-request-resubmit-clone.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, new, param-container
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), new (staticContent), param-container (staticContent).
...
import { RequestResubmitClonePage } from './p00220-request-resubmit-clone.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 220: Request Resubmit / Clone [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new RequestResubmitClonePage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new RequestResubmitClonePage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Request Resubmit / Clone'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (11 items, 2 hidden)', async ({ page }) => {
    const po = new RequestResubmitClonePage(page);
    await po.goto();
    await expectItemsPresent(page, ['P220_JOB_CODE', 'P220_JOB_NAME', 'P220_NEW_REQUEST_ID', 'P220_PARAMS_JSON', 'P220_PRIORITY', 'P220_RESUBMIT_ANCHOR', 'P220_SCHEDULED_TS', 'P220_SCHEDULE_END_TS', 'P220_SCHEDULE_INTERVAL', 'P220_SCHEDULE_TYPE', 'P220_SOURCE_REQUEST_ID']);
  });

  test('apex.item round-trip on P220_SCHEDULE_INTERVAL', async ({ page }) => {
    const po = new RequestResubmitClonePage(page);
    await po.goto();
  ...
  test('every labeled .apx button exists (1 button)', async ({ page }) => {
    const po = new RequestResubmitClonePage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Submit Process']);
  });
});
