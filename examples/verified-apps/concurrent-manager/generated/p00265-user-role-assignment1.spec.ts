/**
 * GENERATED by apx-testgen from pages/p00265-user-role-assignment1.apx — DO NOT EDIT.
 ...
 * (./p00265-user-role-assignment1.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, user-role-assignment
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * 1 Interactive Grid region(s) SKIPPED -- no htmlDomId set, runtime id genuinely unconstructible from static data (ADR-003 layer 3): user-role-assignment.
...
import { UserRoleAssignment1Page } from './p00265-user-role-assignment1.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 265: User Role Assignment [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new UserRoleAssignment1Page(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new UserRoleAssignment1Page(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('User Role Assignment'));
  });
});
