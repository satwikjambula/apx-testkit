/**
 * GENERATED by apx-testgen from pages/p00190-notification-configuration.apx — DO NOT EDIT.
 ...
 * (./p00190-notification-configuration.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, notification-configuration
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * 1 Interactive Grid region(s) SKIPPED -- no htmlDomId set, runtime id genuinely unconstructible from static data (ADR-003 layer 3): notification-configuration.
...
import { NotificationConfigurationPage } from './p00190-notification-configuration.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 190: Notification Configuration [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new NotificationConfigurationPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new NotificationConfigurationPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Notification Configuration'));
  });
});
