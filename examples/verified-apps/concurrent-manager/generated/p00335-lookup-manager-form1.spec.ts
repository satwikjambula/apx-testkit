 ...
 * (./p00335-lookup-manager-form1.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, lookup-values, type-detail
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * 1 Interactive Grid region(s) SKIPPED -- no htmlDomId set, runtime id genuinely unconstructible from static data (ADR-003 layer 3): lookup-values.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), type-detail (staticContent).
...
import { LookupManagerForm1Page } from './p00335-lookup-manager-form1.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 335: Lookup Manager Form [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new LookupManagerForm1Page(page);
  ...
  test('every .apx pageItem exists in DOM/apex.item registry (8 items, 4 hidden)', async ({ page }) => {
    const po = new LookupManagerForm1Page(page);
    await po.goto();
    await expectItemsPresent(page, ['P335_ATTRIBUTES_JSON', 'P335_DESCRIPTION', 'P335_ENABLED_FLAG', 'P335_LOOKUP_TYPE', 'P335_MODE', 'P335_ROLE_ID', 'P335_ROLE_NAME', 'P335_VERSION_NO']);
  });

  test('apex.item round-trip on P335_ATTRIBUTES_JSON', async ({ page }) => {
    const po = new LookupManagerForm1Page(page);
    await po.goto();
    await po.attributesJson.setValue('apx-testgen');
    expect(await po.attributesJson.getValue()).toBe('apx-testgen');
  });

  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new LookupManagerForm1Page(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'CREATE', 'DELETE', 'SAVE']);
  });
});
