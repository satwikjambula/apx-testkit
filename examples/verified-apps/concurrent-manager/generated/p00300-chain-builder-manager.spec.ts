 ...
 * (./p00300-chain-builder-manager.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: hidden-objects, scheduler-chains
 ...
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): hidden-objects (staticContent).
...
import { ChainBuilderManagerPage } from './p00300-chain-builder-manager.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 300: Chain Builder & Manager [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ChainBuilderManagerPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ChainBuilderManagerPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('DBMS_Scheduler Chain Builder'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (2 items, 2 hidden)', async ({ page }) => {
    const po = new ChainBuilderManagerPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P300_CHAIN_NAME', 'P300_STATUS_MSG']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new ChainBuilderManagerPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['scheduler-chains']);
  });
});
