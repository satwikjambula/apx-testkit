/**
 * GENERATED by apx-testgen from pages/p00070-system-audit-trail.apx — DO NOT EDIT.
 ...
 * (./p00070-system-audit-trail.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, system-audit-trail
...
import { SystemAuditTrailPage } from './p00070-system-audit-trail.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 70: System Audit Trail [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new SystemAuditTrailPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new SystemAuditTrailPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('System Audit Trail'));
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new SystemAuditTrailPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['system-audit-trail']);
  });
});
