/**
 * GENERATED by apx-testgen from pages/p00020-workday-calendar-manager.apx — DO NOT EDIT.
 ...
 * (./p00020-workday-calendar-manager.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, workday-calendar-manager
...
import { WorkdayCalendarManagerPage } from './p00020-workday-calendar-manager.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 20: Workday Calendar Manager [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new WorkdayCalendarManagerPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new WorkdayCalendarManagerPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Workday Calendar Manager'));
  });
  ...
  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new WorkdayCalendarManagerPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['workday-calendar-manager']);
  });
});
