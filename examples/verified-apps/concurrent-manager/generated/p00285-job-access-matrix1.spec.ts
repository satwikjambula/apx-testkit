 ...
 * (./p00285-job-access-matrix1.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, job-access-matrix
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * 1 Interactive Grid region(s) SKIPPED -- no htmlDomId set, runtime id genuinely unconstructible from static data (ADR-003 layer 3): job-access-matrix.
...
import { JobAccessMatrix1Page } from './p00285-job-access-matrix1.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 285: Job Access Matrix [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new JobAccessMatrix1Page(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new JobAccessMatrix1Page(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Job Access Matrix'));
  });
});
