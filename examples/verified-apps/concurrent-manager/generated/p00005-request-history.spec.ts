/**
 * GENERATED by apx-testgen from pages/p00005-request-history.apx — DO NOT EDIT.
 ...
 * (./p00005-request-history.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, request-history
...
import { RequestHistoryPage } from './p00005-request-history.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 5: Request History [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new RequestHistoryPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new RequestHistoryPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Request History'));
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new RequestHistoryPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['request-history']);
  });
});
