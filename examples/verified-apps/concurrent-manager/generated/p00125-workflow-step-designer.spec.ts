/**
 * GENERATED by apx-testgen from pages/p00125-workflow-step-designer.apx — DO NOT EDIT.
 ...
 * (./p00125-workflow-step-designer.page.js), not raw testkit calls, so both stay in sync.
...
import { WorkflowStepDesignerPage } from './p00125-workflow-step-designer.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 125: Workflow Step Designer [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new WorkflowStepDesignerPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new WorkflowStepDesignerPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Workflow Step Designer'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 1 hidden)', async ({ page }) => {
    const po = new WorkflowStepDesignerPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P125_SET_CODE']);
  });
});
