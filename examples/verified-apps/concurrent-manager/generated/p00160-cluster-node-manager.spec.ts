/**
 * GENERATED by apx-testgen from pages/p00160-cluster-node-manager.apx — DO NOT EDIT.
 ...
 * (./p00160-cluster-node-manager.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, cards-region, cluster-nodes, manager-control-panel, new
 ...
 * 1 Interactive Grid region(s) SKIPPED -- no htmlDomId set, runtime id genuinely unconstructible from static data (ADR-003 layer 3): cluster-nodes.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), manager-control-panel (staticContent), new (staticContent).
...
import { ClusterNodeManagerPage } from './p00160-cluster-node-manager.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 160: Cluster Nodes [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ClusterNodeManagerPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ClusterNodeManagerPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Cluster Node Manager'));
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new ClusterNodeManagerPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['cards-region']);
  });
});
