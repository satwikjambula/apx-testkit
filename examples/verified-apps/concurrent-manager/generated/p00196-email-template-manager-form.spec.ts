 ...
 * (./p00196-email-template-manager-form.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, email-template-manager-form
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), email-template-manager-form (form).
...
import { EmailTemplateManagerFormPage } from './p00196-email-template-manager-form.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 196: Email Template Manager Form [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new EmailTemplateManagerFormPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new EmailTemplateManagerFormPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Email Template Manager Form'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (10 items, 3 hidden)', async ({ page }) => {
    const po = new EmailTemplateManagerFormPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P195_BODY_TEMPLATE', 'P195_DESCRIPTION', 'P196_CREATED_BY', 'P196_CREATED_TS', 'P196_IS_ENABLED', 'P196_IS_HTML', 'P196_ROWID', 'P196_SUBJECT_TEMPLATE', 'P196_TEMPLATE_CODE', 'P196_TEMPLATE_NAME']);
  });

  test('apex.item round-trip on P195_DESCRIPTION', async ({ page }) => {
    const po = new EmailTemplateManagerFormPage(page);
    await po.goto();
    await po.p195Description.setValue('apx-testgen');
    expect(await po.p195Description.getValue()).toBe('apx-testgen');
  });
