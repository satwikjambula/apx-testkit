/**
 * GENERATED by apx-testgen from pages/p00030-work-shift-manager.apx — DO NOT EDIT.
 ...
 * (./p00030-work-shift-manager.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, work-shift-manager
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * 1 Interactive Grid region(s) SKIPPED -- no htmlDomId set, runtime id genuinely unconstructible from static data (ADR-003 layer 3): work-shift-manager.
...
import { WorkShiftManagerPage } from './p00030-work-shift-manager.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 30: Work Shift Manager [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new WorkShiftManagerPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new WorkShiftManagerPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Work Shift Manager'));
  });
});
