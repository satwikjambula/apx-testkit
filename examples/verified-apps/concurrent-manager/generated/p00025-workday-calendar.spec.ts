/**
 * GENERATED by apx-testgen from pages/p00025-workday-calendar.apx — DO NOT EDIT.
 ...
 * (./p00025-workday-calendar.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, workday-calendar
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * 1 Interactive Grid region(s) SKIPPED -- no htmlDomId set, runtime id genuinely unconstructible from static data (ADR-003 layer 3): workday-calendar.
...
import { WorkdayCalendarPage } from './p00025-workday-calendar.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 25: Workday Calendar [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new WorkdayCalendarPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new WorkdayCalendarPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Workday Calendar'));
  });
});
