 ...
 * (./p00230-execution-timeline.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, timeline-container
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), timeline-container (staticContent).
...
import { ExecutionTimelinePage } from './p00230-execution-timeline.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 230: Execution Timeline [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ExecutionTimelinePage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ExecutionTimelinePage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Execution Timeline'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 1 hidden)', async ({ page }) => {
    const po = new ExecutionTimelinePage(page);
    await po.goto();
    await expectItemsPresent(page, ['P230_REQUEST_ID']);
  });
});
