 ...
 * (./p00120-request-set-builder.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, request-set-builder
...
import { RequestSetBuilderPage } from './p00120-request-set-builder.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 120: Request Set Builder [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new RequestSetBuilderPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new RequestSetBuilderPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Request Set Builder'));
  });
  ...
  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new RequestSetBuilderPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['request-set-builder']);
  });
});
