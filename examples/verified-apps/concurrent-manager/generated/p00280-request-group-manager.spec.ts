/**
 * GENERATED by apx-testgen from pages/p00280-request-group-manager.apx — DO NOT EDIT.
 ...
 * (./p00280-request-group-manager.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, buttons, request-group, request-group-manager
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * 2 Interactive Grid region(s) SKIPPED -- no htmlDomId set, runtime id genuinely unconstructible from static data (ADR-003 layer 3): request-group, request-group-manager.
...
import { RequestGroupManagerPage } from './p00280-request-group-manager.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 280: Request Group Manager [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new RequestGroupManagerPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new RequestGroupManagerPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Request Group Manager'));
  });
