/**
 * GENERATED by apx-testgen from pages/p00330-lookup-manager.apx — DO NOT EDIT.
 ...
 * (./p00330-lookup-manager.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, lookup-manager
...
import { LookupManagerPage } from './p00330-lookup-manager.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 330: Lookup Manager [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new LookupManagerPage(page);
  ...
  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new LookupManagerPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['lookup_types_ir']);
  });
});
