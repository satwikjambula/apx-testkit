/**
 * GENERATED by apx-testgen from pages/p00180-purge-policy-manager.apx — DO NOT EDIT.
 ...
 * (./p00180-purge-policy-manager.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, new, purge-policy-manager
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * 1 Interactive Grid region(s) SKIPPED -- no htmlDomId set, runtime id genuinely unconstructible from static data (ADR-003 layer 3): purge-policy-manager.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), new (staticContent).
...
import { PurgePolicyManagerPage } from './p00180-purge-policy-manager.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 180: Purge Policy Manager [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new PurgePolicyManagerPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new PurgePolicyManagerPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Purge Policy Manager'));
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new PurgePolicyManagerPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Purge Filesystem Logs', 'Run Purge Now']);
  });
});
