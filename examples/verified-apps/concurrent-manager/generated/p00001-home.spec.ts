 ...
 * Regions present in metadata: concurrent-manager, filters, performance-trend, queue-health-gauge, recent-activity, summary-cards
 ...
 * 2 chart region(s) SKIPPED -- no htmlDomId set, runtime id genuinely unconstructible from static data (ADR-003 layer 3): performance-trend, queue-health-gauge.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): concurrent-manager (staticContent), filters (staticContent).
...
test.describe('page 1: Home [requires auth]', () => {
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new HomePage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Concurrent Manager'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (2 items, 0 hidden)', async ({ page }) => {
    const po = new HomePage(page);
    await po.goto();
    await expectItemsPresent(page, ['P1_DATE_RANGE', 'P1_STATUS']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (2 regions)', async ({ page }) => {
    const po = new HomePage(page);
    await po.goto();
    await expectRegionsResolve(page, ['recent_activity', 'summary-cards']);
  });
});
