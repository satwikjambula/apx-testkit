 ...
 * (./p00170-priority-map-configuration.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, priority-map-configuration
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * 1 Interactive Grid region(s) SKIPPED -- no htmlDomId set, runtime id genuinely unconstructible from static data (ADR-003 layer 3): priority-map-configuration.
...
import { PriorityMapConfigurationPage } from './p00170-priority-map-configuration.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 170: Priority Map Configuration [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new PriorityMapConfigurationPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new PriorityMapConfigurationPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Priority Map Configuration'));
  });
});
