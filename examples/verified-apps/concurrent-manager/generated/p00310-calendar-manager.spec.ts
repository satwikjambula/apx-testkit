/**
 * GENERATED by apx-testgen from pages/p00310-calendar-manager.apx — DO NOT EDIT.
 ...
 * (./p00310-calendar-manager.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, buttons, calendar-dates, calendar-manager
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * 2 Interactive Grid region(s) SKIPPED -- no htmlDomId set, runtime id genuinely unconstructible from static data (ADR-003 layer 3): calendar-dates, calendar-manager.
...
import { CalendarManagerPage } from './p00310-calendar-manager.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 310: Calendar Manager [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new CalendarManagerPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new CalendarManagerPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Calendar Manager'));
  });
