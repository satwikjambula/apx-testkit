/**
 * GENERATED by apx-testgen from pages/p00320-request-value-sets.apx — DO NOT EDIT.
 ...
 * (./p00320-request-value-sets.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: request-value-sets
...
import { RequestValueSetsPage } from './p00320-request-value-sets.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 320: Request Value Sets [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new RequestValueSetsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new RequestValueSetsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Request Value Sets'));
  });
  ...
  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new RequestValueSetsPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['request-value-sets']);
  });
});
