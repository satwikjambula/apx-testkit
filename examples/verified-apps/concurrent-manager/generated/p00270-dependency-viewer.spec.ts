/**
 * GENERATED by apx-testgen from pages/p00270-dependency-viewer.apx — DO NOT EDIT.
 ...
 * (./p00270-dependency-viewer.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: dependency-viewer, dependency-viewer-2
 ...
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): dependency-viewer-2 (breadcrumb).
...
import { DependencyViewerPage } from './p00270-dependency-viewer.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 270: Dependency Viewer [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new DependencyViewerPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new DependencyViewerPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Dependency Viewer'));
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new DependencyViewerPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['dependency-viewer']);
  });
});
