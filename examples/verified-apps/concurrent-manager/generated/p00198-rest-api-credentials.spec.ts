 ...
 * (./p00198-rest-api-credentials.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: api-reference, available-endpoints, rest-module-status, test-connection
 ...
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): api-reference (staticContent), rest-module-status (dynamicContent), test-connection (staticContent).
...
import { RestApiCredentialsPage } from './p00198-rest-api-credentials.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 198: REST API Credentials [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new RestApiCredentialsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new RestApiCredentialsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('REST API Credentials'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 1 hidden)', async ({ page }) => {
    const po = new RestApiCredentialsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P198_TEST_RESULT']);
  });

  test('every labeled .apx button exists (1 button)', async ({ page }) => {
    const po = new RestApiCredentialsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Test REST API Connection']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new RestApiCredentialsPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['available-endpoints']);
  });
});
