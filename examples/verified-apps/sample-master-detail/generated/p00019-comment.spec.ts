 ...
 * (./p00019-comment.page.js), not raw testkit calls, so both stay in sync.
 ...
 * Region types NOT covered by an auto-generated assertion (no verified DOM convention, or a runtime id genuinely unconstructible from static data -- see docs/grammar-assumptions.md "Still open" and ADR-003): buttons (staticContent), comment (form).
...
import { CommentPage } from './p00019-comment.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 19: Comment [requires auth]', () => {
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new CommentPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Comment'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (3 items, 2 hidden)', async ({ page }) => {
    const po = new CommentPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P19_COMMENT_TEXT', 'P19_ID', 'P19_PROJECT_ID']);
  });

  test('apex.item round-trip on P19_COMMENT_TEXT', async ({ page }) => {
    const po = new CommentPage(page);
    await po.goto();
