 ...
 * (./p00025-preferences.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, preferences
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Region types NOT covered by an auto-generated assertion (no verified DOM convention, or a runtime id genuinely unconstructible from static data -- see docs/grammar-assumptions.md "Still open" and ADR-003): breadcrumb (breadcrumb), preferences (staticContent).
...
import { PreferencesPage } from './p00025-preferences.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 25: Preferences [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new PreferencesPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new PreferencesPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Preferences'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 0 hidden)', async ({ page }) => {
    const po = new PreferencesPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P25_ENABLE_FEEDBACK']);
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new PreferencesPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Set Preferences', 'Cancel']);
  });
});
