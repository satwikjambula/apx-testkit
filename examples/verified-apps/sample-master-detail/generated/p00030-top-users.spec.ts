/**
 * GENERATED by apx-testgen from pages/p00030-top-users.apx — DO NOT EDIT.
 ...
 * (./p00030-top-users.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, button-bar, top-users
...
import { TopUsersPage } from './p00030-top-users.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 30: Top Users [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new TopUsersPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new TopUsersPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Top Users'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 0 hidden)', async ({ page }) => {
    const po = new TopUsersPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P30_TIMEFRAME']);
  });
  ...
  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new TopUsersPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['top-users']);
  });
});
