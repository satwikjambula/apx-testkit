 ...
 * (./p00013-add-comment.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: add-comment, buttons
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Region types NOT covered by an auto-generated assertion (no verified DOM convention, or a runtime id genuinely unconstructible from static data -- see docs/grammar-assumptions.md "Still open" and ADR-003): add-comment (form), buttons (staticContent).
...
import { AddCommentPage } from './p00013-add-comment.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 13: Add Comment [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new AddCommentPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new AddCommentPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Add Comment'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (3 items, 1 hidden)', async ({ page }) => {
    const po = new AddCommentPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P13_COMMENT_TEXT', 'P13_ID', 'P13_PROJECT_ID']);
  });

  test('apex.item round-trip on P13_COMMENT_TEXT', async ({ page }) => {
    const po = new AddCommentPage(page);
    await po.goto();
  ...
  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new AddCommentPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Add Comment', 'Delete', 'Save']);
  });
});
