 ...
 * (./p00026-manage-sample-data.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, button-region, manage-sample-data
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Region types NOT covered by an auto-generated assertion (no verified DOM convention, or a runtime id genuinely unconstructible from static data -- see docs/grammar-assumptions.md "Still open" and ADR-003): breadcrumb (breadcrumb), button-region (staticContent), manage-sample-data (staticContent).
...
import { ManageSampleDataPage } from './p00026-manage-sample-data.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 26: Manage Sample Data [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ManageSampleDataPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ManageSampleDataPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Manage Sample Data'));
  });

  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new ManageSampleDataPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Load Sample Data', 'Remove Sample Data', 'Reset Data']);
  });
});
