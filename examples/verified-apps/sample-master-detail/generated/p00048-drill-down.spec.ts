/**
 * GENERATED by apx-testgen from pages/p00048-drill-down.apx — DO NOT EDIT.
 ...
 * (./p00048-drill-down.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, overview-drill-down, projects
 ...
 * Region types NOT covered by an auto-generated assertion (no verified DOM convention, or a runtime id genuinely unconstructible from static data -- see docs/grammar-assumptions.md "Still open" and ADR-003): breadcrumb (breadcrumb), overview-drill-down (staticContent).
...
import { DrillDownPage } from './p00048-drill-down.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 48: Drill Down [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new DrillDownPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new DrillDownPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Drill Down'));
  });

  test('every labeled .apx button exists (1 button)', async ({ page }) => {
    const po = new DrillDownPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Create Project']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new DrillDownPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['projects']);
  });
});
