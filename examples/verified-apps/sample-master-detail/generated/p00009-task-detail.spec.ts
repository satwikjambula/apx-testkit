 ...
 * (./p00009-task-detail.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: audit-details, buttons, edit-eba-demo-md-tasks
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Region types NOT covered by an auto-generated assertion (no verified DOM convention, or a runtime id genuinely unconstructible from static data -- see docs/grammar-assumptions.md "Still open" and ADR-003): audit-details (staticContent), buttons (staticContent), edit-eba-demo-md-tasks (form).
...
import { TaskDetailPage } from './p00009-task-detail.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 9: Task Detail [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new TaskDetailPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new TaskDetailPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Task Detail'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (14 items, 1 hidden)', async ({ page }) => {
    const po = new TaskDetailPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P9_ASSIGNEE', 'P9_CREATED', 'P9_CREATED_BY', 'P9_DESCRIPTION', 'P9_END_DATE', 'P9_ID', 'P9_IS_COMPLETE_YN', 'P9_MILESTONE_DUE_DATE', 'P9_MILESTONE_ID', 'P9_NAME', 'P9_PROJECT_ID', 'P9_START_DATE', 'P9_UPDATED', 'P9_UPDATED_BY']);
  });

  test('apex.item round-trip on P9_DESCRIPTION', async ({ page }) => {
    const po = new TaskDetailPage(page);
    await po.goto();
  ...
  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new TaskDetailPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Add Task', 'Delete', 'Save']);
  });
});
