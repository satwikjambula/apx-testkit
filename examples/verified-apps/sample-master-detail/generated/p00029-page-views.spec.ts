/**
 * GENERATED by apx-testgen from pages/p00029-page-views.apx — DO NOT EDIT.
 ...
 * (./p00029-page-views.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, button-bar, page-views
 ...
 * Region types NOT covered by an auto-generated assertion (no verified DOM convention, or a runtime id genuinely unconstructible from static data -- see docs/grammar-assumptions.md "Still open" and ADR-003): breadcrumb (breadcrumb), button-bar (staticContent).
...
import { PageViewsPage } from './p00029-page-views.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 29: Page Views [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new PageViewsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new PageViewsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Page Views'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 0 hidden)', async ({ page }) => {
    const po = new PageViewsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P29_TIMEFRAME']);
  });
  ...
  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new PageViewsPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['page-views']);
  });
});
