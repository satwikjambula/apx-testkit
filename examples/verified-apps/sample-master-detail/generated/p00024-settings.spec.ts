 ...
 * (./p00024-settings.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: administration, breadcrumb, reports
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Region types NOT covered by an auto-generated assertion (no verified DOM convention, or a runtime id genuinely unconstructible from static data -- see docs/grammar-assumptions.md "Still open" and ADR-003): administration (list), breadcrumb (breadcrumb), reports (list).
...
import { SettingsPage } from './p00024-settings.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 24: Administration [requires auth]', () => {
  ...
  test('every .apx pageItem exists in DOM/apex.item registry (3 items, 3 hidden)', async ({ page }) => {
    const po = new SettingsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P24_FEEDBACK_STATUS', 'P24_PAGE_VIEWS', 'P24_USER_COUNT']);
  });
});
