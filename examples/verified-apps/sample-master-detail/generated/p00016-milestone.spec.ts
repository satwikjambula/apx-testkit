 ...
 * (./p00016-milestone.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, milestone-details
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Region types NOT covered by an auto-generated assertion (no verified DOM convention, or a runtime id genuinely unconstructible from static data -- see docs/grammar-assumptions.md "Still open" and ADR-003): buttons (staticContent), milestone-details (form).
...
import { MilestonePage } from './p00016-milestone.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 16: Milestone [requires auth]', () => {
  ...
  test('every .apx pageItem exists in DOM/apex.item registry (5 items, 2 hidden)', async ({ page }) => {
    const po = new MilestonePage(page);
    await po.goto();
    await expectItemsPresent(page, ['P16_DESCRIPTION', 'P16_DUE_DATE', 'P16_ID', 'P16_NAME', 'P16_PROJECT_ID']);
  });

  test('apex.item round-trip on P16_DESCRIPTION', async ({ page }) => {
    const po = new MilestonePage(page);
    await po.goto();
