 ...
 * (./p00005-project-detail.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: audit-details, buttons, edit-eba-demo-md-projects
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Region types NOT covered by an auto-generated assertion (no verified DOM convention, or a runtime id genuinely unconstructible from static data -- see docs/grammar-assumptions.md "Still open" and ADR-003): audit-details (staticContent), buttons (staticContent), edit-eba-demo-md-projects (form).
...
import { ProjectDetailPage } from './p00005-project-detail.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 5: Project Detail [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ProjectDetailPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ProjectDetailPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Project Detail'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (10 items, 1 hidden)', async ({ page }) => {
    const po = new ProjectDetailPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P5_COMPLETED_DATE', 'P5_CREATED', 'P5_CREATED_BY', 'P5_DESCRIPTION', 'P5_ID', 'P5_NAME', 'P5_PROJECT_LEAD', 'P5_STATUS_CD', 'P5_UPDATED', 'P5_UPDATED_BY']);
  });

  test('apex.item round-trip on P5_DESCRIPTION', async ({ page }) => {
    const po = new ProjectDetailPage(page);
    await po.goto();
  ...
  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new ProjectDetailPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Create Project', 'Delete', 'Save']);
  });
});
