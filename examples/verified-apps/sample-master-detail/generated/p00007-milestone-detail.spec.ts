 ...
 * (./p00007-milestone-detail.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: audit-details, buttons, edit-eba-demo-md-milestones
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Region types NOT covered by an auto-generated assertion (no verified DOM convention, or a runtime id genuinely unconstructible from static data -- see docs/grammar-assumptions.md "Still open" and ADR-003): audit-details (staticContent), buttons (staticContent), edit-eba-demo-md-milestones (form).
...
import { MilestoneDetailPage } from './p00007-milestone-detail.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 7: Milestone Detail [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new MilestoneDetailPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new MilestoneDetailPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Milestone Detail'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (9 items, 1 hidden)', async ({ page }) => {
    const po = new MilestoneDetailPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P7_CREATED', 'P7_CREATED_BY', 'P7_DESCRIPTION', 'P7_DUE_DATE', 'P7_ID', 'P7_NAME', 'P7_PROJECT_ID', 'P7_UPDATED', 'P7_UPDATED_BY']);
  });

  test('apex.item round-trip on P7_DESCRIPTION', async ({ page }) => {
    const po = new MilestoneDetailPage(page);
    await po.goto();
  ...
  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new MilestoneDetailPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Add Milestone', 'Delete', 'Save']);
  });
});
