 ...
 * (./p00031-task-details.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, links, master-task, region-display-selector, task-actions, to-dos
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), links (classicReport), master-task (classicReport), region-display-selector (regionDisplaySelector), task-actions (list), to-dos (classicReport).
...
import { TaskDetailsPage } from './p00031-task-details.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 31: Task Details [requires auth]', () => {
  ...
  test('every .apx pageItem exists in DOM/apex.item registry (4 items, 4 hidden)', async ({ page }) => {
    const po = new TaskDetailsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P31_LAST_PAGE', 'P31_PROJECT_ID', 'P31_TASK_ID', 'P31_TASK_NAME']);
  });

  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new TaskDetailsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Add to do', 'Add to do', 'Cancel', 'Edit Task']);
  });
});
