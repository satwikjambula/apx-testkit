 ...
 * (./p00011-report-and-marquee-marquee.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, comments, master-project, milestones, project-actions, region-display-selector, tasks
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Region types NOT covered by an auto-generated assertion (no verified DOM convention, or a runtime id genuinely unconstructible from static data -- see docs/grammar-assumptions.md "Still open" and ADR-003): breadcrumb (breadcrumb), comments (classicReport), master-project (classicReport), milestones (classicReport), project-actions (list), region-display-selector (regionDisplaySelector), tasks (classicReport).
...
import { ReportAndMarqueeMarqueePage } from './p00011-report-and-marquee-marquee.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 11: Report and Marquee - Marquee [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ReportAndMarqueeMarqueePage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ReportAndMarqueeMarqueePage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Report and Marquee - Marquee'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (2 items, 2 hidden)', async ({ page }) => {
    const po = new ReportAndMarqueeMarqueePage(page);
    await po.goto();
    await expectItemsPresent(page, ['P11_PROJECT_ID', 'P11_PROJECT_NAME']);
  });

  test('every labeled .apx button exists (5 buttons)', async ({ page }) => {
    const po = new ReportAndMarqueeMarqueePage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Add comment', 'Add milestone', 'Add task', 'Cancel', 'Edit Project']);
  });
});
