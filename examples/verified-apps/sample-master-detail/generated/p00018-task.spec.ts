 ...
 * (./p00018-task.page.js), not raw testkit calls, so both stay in sync.
 ...
 * Region types NOT covered by an auto-generated assertion (no verified DOM convention, or a runtime id genuinely unconstructible from static data -- see docs/grammar-assumptions.md "Still open" and ADR-003): buttons (staticContent), task (form).
...
import { TaskPage } from './p00018-task.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 18: Task [requires auth]', () => {
  ...
  test('every .apx pageItem exists in DOM/apex.item registry (9 items, 2 hidden)', async ({ page }) => {
    const po = new TaskPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P18_ASSIGNEE', 'P18_DESCRIPTION', 'P18_END_DATE', 'P18_ID', 'P18_IS_COMPLETE_YN', 'P18_MILESTONE_ID', 'P18_NAME', 'P18_PROJECT_ID', 'P18_START_DATE']);
  });

  test('apex.item round-trip on P18_DESCRIPTION', async ({ page }) => {
    const po = new TaskPage(page);
    await po.goto();
  ...
  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new TaskPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Add Task', 'Delete', 'Apply Changes']);
  });
});
