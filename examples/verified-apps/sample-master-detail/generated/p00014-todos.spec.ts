 ...
 * (./p00014-todos.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, eba-demo-md-task-todos, task-details
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * 1 Interactive Grid region(s) SKIPPED -- no htmlDomId set, runtime id genuinely unconstructible from static data (ADR-003 layer 3): eba-demo-md-task-todos.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), task-details (classicReport).
...
import { TodosPage } from './p00014-todos.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 14: ToDos [requires auth]', () => {
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new TodosPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('To Dos'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (2 items, 2 hidden)', async ({ page }) => {
    const po = new TodosPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P14_PROJECT_ID', 'P14_TASK_ID']);
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new TodosPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Save']);
  });
});
