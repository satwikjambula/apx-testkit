/**
 * GENERATED by apx-testgen from pages/p00002-side-by-side.apx — DO NOT EDIT.
 ...
 * (./p00002-side-by-side.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, comments, master-records, milestones, no-record-selected, overview-side-by-side, project-details, region-display-selector, search, tasks
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Region types NOT covered by an auto-generated assertion (no verified DOM convention, or a runtime id genuinely unconstructible from static data -- see docs/grammar-assumptions.md "Still open" and ADR-003): breadcrumb (breadcrumb), comments (classicReport), master-records (classicReport), milestones (classicReport), no-record-selected (staticContent), overview-side-by-side (staticContent), project-details (classicReport), region-display-selector (regionDisplaySelector), search (staticContent), tasks (classicReport).
...
import { SideBySidePage } from './p00002-side-by-side.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 2: Side by Side [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new SideBySidePage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new SideBySidePage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Side by Side'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (2 items, 1 hidden)', async ({ page }) => {
    const po = new SideBySidePage(page);
    await po.goto();
    await expectItemsPresent(page, ['P2_ID', 'P2_SEARCH']);
  });
  ...
  test('every labeled .apx button exists (6 buttons)', async ({ page }) => {
    const po = new SideBySidePage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Add Project', 'Edit Project', 'Add Comment', 'Add Milestone', 'Add Task', 'Reset']);
  });
});
