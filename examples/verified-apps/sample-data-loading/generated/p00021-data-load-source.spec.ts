 ...
 * (./p00021-data-load-source.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: about-this-page, breadcrumb, data-load-source, data-load-wizard-progress, page-instructions
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): about-this-page (staticContent), breadcrumb (breadcrumb), data-load-source (staticContent), data-load-wizard-progress (list), page-instructions (staticContent).
...
import { DataLoadSourcePage } from './p00021-data-load-source.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 21: Data Load Source [requires auth]', () => {
  ...
  test('every .apx pageItem exists in DOM/apex.item registry (13 items, 1 hidden)', async ({ page }) => {
    const po = new DataLoadSourcePage(page);
    await po.goto();
    await expectItemsPresent(page, ['P21_APP_DATE_FORMAT', 'P21_CHAR_SET', 'P21_COPY_PASTE', 'P21_CURRENCY', 'P21_DECIMAL_CHARACTER', 'P21_ENCLOSED_BY', 'P21_FILE_NAME', 'P21_FIRST_ROW', 'P21_GROUP_SEPARATOR', 'P21_IMPORT_FROM', 'P21_SAMPLE_DATA', 'P21_SEPARATOR', 'P21_SHOW_ADV_SETTINGS']);
  });

  test('apex.item round-trip on P21_COPY_PASTE', async ({ page }) => {
    const po = new DataLoadSourcePage(page);
    await po.goto();
