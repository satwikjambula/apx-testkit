 ...
 * (./p00032-load-data-using-pl-sql-api.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, data-loading-sql-statement, employees, failed-rows, load-data, page-instructions
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), data-loading-sql-statement (staticContent), employees (classicReport), failed-rows (classicReport), load-data (staticContent), page-instructions (staticContent).
...
import { LoadDataUsingPlSqlApiPage } from './p00032-load-data-using-pl-sql-api.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 32: Load Data using PL/SQL API [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new LoadDataUsingPlSqlApiPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new LoadDataUsingPlSqlApiPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Load Data using PL/SQL API'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (4 items, 3 hidden)', async ({ page }) => {
    const po = new LoadDataUsingPlSqlApiPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P32_DATA', 'P32_ROWS_FAILED', 'P32_ROWS_LOADED', 'P32_SAMPLE_DATA']);
  });

  test('apex.item round-trip on P32_DATA', async ({ page }) => {
    const po = new LoadDataUsingPlSqlApiPage(page);
    await po.goto();
  ...
  test('every labeled .apx button exists (1 button)', async ({ page }) => {
    const po = new LoadDataUsingPlSqlApiPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Load Data']);
  });
});
