 ...
 * (./p00017-background-load.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: about-this-page, breadcrumb, button-bar, create-job-privilege-is-missing, data-source, loaded-file, preview, upload-a-file
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): about-this-page (staticContent), breadcrumb (breadcrumb), button-bar (staticContent), create-job-privilege-is-missing (staticContent), data-source (staticContent), loaded-file (staticContent), preview (classicReport), upload-a-file (staticContent).
...
import { BackgroundLoadPage } from './p00017-background-load.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 17: Background Load [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new BackgroundLoadPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new BackgroundLoadPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Background Load'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (5 items, 3 hidden)', async ({ page }) => {
    const po = new BackgroundLoadPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P17_ERROR_ROWS', 'P17_FILE', 'P17_FILE_NAME', 'P17_LOAD_EXEC_ID', 'P17_PROCESSED_ROWS']);
  });
