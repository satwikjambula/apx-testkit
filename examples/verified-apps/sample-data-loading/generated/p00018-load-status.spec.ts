 ...
 * (./p00018-load-status.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, load-status, rows-with-errors, sales-table-contents, timer-container
 ...
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), load-status (classicReport), rows-with-errors (classicReport), timer-container (staticContent).
...
import { LoadStatusPage } from './p00018-load-status.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 18: Load Status [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new LoadStatusPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new LoadStatusPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Load Status'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (2 items, 2 hidden)', async ({ page }) => {
    const po = new LoadStatusPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P18_LOADING_STATUS', 'P18_LOAD_EXEC_ID']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new LoadStatusPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['sales']);
  });
});
