/**
 * GENERATED by apx-testgen from pages/p00010-data-loading.apx — DO NOT EDIT.
 ...
 * (./p00010-data-loading.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, data-loading
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), data-loading (list).
...
import { DataLoadingPage } from './p00010-data-loading.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 10: Data Loading [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new DataLoadingPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new DataLoadingPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Data Loading'));
  });
});
