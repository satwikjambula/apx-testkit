 ...
 * (./p00031-pl-sql-parser.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, discovered-columns, information, parsed-data, parsed-data-2, report-source, upload-a-file
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), discovered-columns (classicReport), information (staticContent), parsed-data (staticContent), parsed-data-2 (classicReport), report-source (plugin/sourceDisplay), upload-a-file (staticContent).
...
import { PlSqlParserPage } from './p00031-pl-sql-parser.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 31: PL/SQL Parser [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new PlSqlParserPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new PlSqlParserPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('APEX_DATA_PARSER PL/SQL API'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (2 items, 0 hidden)', async ({ page }) => {
    const po = new PlSqlParserPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P31_FILE', 'P31_XLSX_WORKSHEET']);
  });

  test('every labeled .apx button exists (1 button)', async ({ page }) => {
    const po = new PlSqlParserPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Upload File']);
  });
});
