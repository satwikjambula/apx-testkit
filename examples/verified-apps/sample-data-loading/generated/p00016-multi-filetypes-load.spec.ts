 ...
 * (./p00016-multi-filetypes-load.page.js), not raw testkit calls, so both stay in sync.
...
import { MultiFiletypesLoadPage } from './p00016-multi-filetypes-load.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 16: Multiple File Types Load [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new MultiFiletypesLoadPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new MultiFiletypesLoadPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Multiple File Types Load'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (6 items, 3 hidden)', async ({ page }) => {
    const po = new MultiFiletypesLoadPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P16_ERROR_ROW_COUNT', 'P16_FILE', 'P16_FILE_NAME', 'P16_FILE_TYPE', 'P16_PROCESSED_ROW_COUNT', 'P16_XLSX_WORKSHEET']);
  });
