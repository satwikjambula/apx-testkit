/**
 * GENERATED by apx-testgen from pages/p00003-edit-content.apx — DO NOT EDIT.
 ...
 * (./p00003-edit-content.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, edit-content
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Region types NOT covered by an auto-generated assertion (no verified DOM convention, or a runtime id genuinely unconstructible from static data -- see docs/grammar-assumptions.md "Still open" and ADR-003): buttons (staticContent), edit-content (form).
...
import { EditContentPage } from './p00003-edit-content.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 3: Edit Content [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new EditContentPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new EditContentPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Edit Content'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (3 items, 2 hidden)', async ({ page }) => {
    const po = new EditContentPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P3_BODY', 'P3_ID', 'P3_NEW_IMAGE_REF_ID']);
  });
