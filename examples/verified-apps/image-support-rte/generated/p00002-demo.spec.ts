 ...
 * (./p00002-demo.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, content-form, content-report, image-support-demo
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), content-form (staticContent), content-report (classicReport), image-support-demo (staticContent).
...
import { DemoPage } from './p00002-demo.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 2: Image Support Demo [requires auth]', () => {
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new DemoPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Image Support Demo'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (2 items, 1 hidden)', async ({ page }) => {
    const po = new DemoPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P2_CONTENT', 'P2_IMAGE_REF_ID']);
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new DemoPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Add Content', 'View Interactive Report']);
  });
});
