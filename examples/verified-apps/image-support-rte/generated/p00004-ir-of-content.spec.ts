/**
 * GENERATED by apx-testgen from pages/p00004-ir-of-content.apx — DO NOT EDIT.
 ...
 * (./p00004-ir-of-content.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, interactive-report-of-content
...
import { IrOfContentPage } from './p00004-ir-of-content.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 4: Interactive Report of Content [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new IrOfContentPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new IrOfContentPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Interactive Report of Content'));
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new IrOfContentPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['interactive-report-of-content']);
  });
});
