/**
 * GENERATED by apx-testgen from pages/p09000-get-image.apx — DO NOT EDIT.
 ...
 * (./p09000-get-image.page.js), not raw testkit calls, so both stay in sync.
...
import { GetImagePage } from './p09000-get-image.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 9000: Get Image [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new GetImagePage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new GetImagePage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Get Image'));
  });
});
