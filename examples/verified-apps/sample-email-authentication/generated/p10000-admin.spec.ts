 ...
 * Regions present in metadata: access-control, activity-reports, breadcrumb, column, column-2, user-counts-report, user-interface, user-related-actions
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): access-control (staticContent), activity-reports (list), breadcrumb (breadcrumb), column (staticContent), column-2 (staticContent), user-counts-report (classicReport), user-interface (list), user-related-actions (list).
...
test.describe('page 10000: Administration [requires auth]', () => {
  ...
  test('every labeled .apx button exists (1 button)', async ({ page }) => {
    const po = new AdminPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Add User']);
  });
});
