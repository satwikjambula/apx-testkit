/**
 * GENERATED by apx-testgen from pages/p10026-logged-messages.apx — DO NOT EDIT.
 ...
 * (./p10026-logged-messages.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, logged-messages
...
import { LoggedMessagesPage } from './p10026-logged-messages.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 10026: Logged Messages [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new LoggedMessagesPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new LoggedMessagesPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Logged Messages'));
  });
  ...
  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new LoggedMessagesPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['logged-messages']);
  });
});
