 ...
 * (./p10032-user_details.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, form-on-manage-user-access, verification-tokens
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), form-on-manage-user-access (form), verification-tokens (classicReport).
...
import { UserDetailsPage } from './p10032-user_details.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 10032: User Details [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new UserDetailsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new UserDetailsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('User Details'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (6 items, 3 hidden)', async ({ page }) => {
    const po = new UserDetailsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P10032_APPLICATION_ID', 'P10032_ID', 'P10032_IS_ACCOUNT_LOCKED_YN', 'P10032_ROLE_ID', 'P10032_SEND_EMAIL_YN', 'P10032_USER_NAME']);
  });

  test('apex.item round-trip on P10032_USER_NAME', async ({ page }) => {
    const po = new UserDetailsPage(page);
    await po.goto();
  ...
  test('every labeled .apx button exists (5 buttons)', async ({ page }) => {
    const po = new UserDetailsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Add User', 'Delete', 'Reset Verification', 'Apply Changes']);
  });
});
