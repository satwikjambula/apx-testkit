/**
 * GENERATED by apx-testgen from pages/p00100-protected.apx — DO NOT EDIT.
 ...
 * (./p00100-protected.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: protected-content
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): protected-content (staticContent).
...
import { ProtectedPage } from './p00100-protected.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 100: Protected Page [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ProtectedPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ProtectedPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Protected Page'));
  });
});
