/**
 * GENERATED by apx-testgen from pages/p00002-queue.apx — DO NOT EDIT.
 ...
 * (./p00002-queue.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: selected-queue
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): selected-queue (staticContent).
...
import { QueuePage } from './p00002-queue.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 2: Queue [requires auth]', () => {
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new QueuePage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Queue'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 1 hidden)', async ({ page }) => {
    const po = new QueuePage(page);
    await po.goto();
    await expectItemsPresent(page, ['P2_QUEUE']);
  });
});
