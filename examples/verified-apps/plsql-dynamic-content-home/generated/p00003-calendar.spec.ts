/**
 * GENERATED by apx-testgen from pages/p00003-calendar.apx — DO NOT EDIT.
 ...
 * (./p00003-calendar.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: selected-mode
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): selected-mode (staticContent).
...
import { CalendarPage } from './p00003-calendar.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 3: Calendar [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new CalendarPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new CalendarPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Calendar'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 1 hidden)', async ({ page }) => {
    const po = new CalendarPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P3_MODE']);
  });
});
