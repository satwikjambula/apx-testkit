/**
 * GENERATED by apx-testgen from pages/p00004-26-1-ボタン.apx — DO NOT EDIT.
 ...
 * (./p00004-26-1-ボタン.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: employees, メニュー-ポップアップ
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): employees (classicReport), メニュー-ポップアップ (staticContent).
...
import { 261Page } from './p00004-26-1-ボタン.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 4: 26.1 ボタン [requires auth]', () => {
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new 261Page(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('26.1 ボタン'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 1 hidden)', async ({ page }) => {
    const po = new 261Page(page);
    await po.goto();
    await expectItemsPresent(page, ['P4_JOB']);
  });
