/**
 * GENERATED by apx-testgen from pages/p00003-メニュー-ウィジェット２.apx — DO NOT EDIT.
 ...
 * (./p00003-メニュー-ウィジェット２.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: employees, ジョブのリスト, メニュー-ウィジェット
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): employees (classicReport), ジョブのリスト (staticContent), メニュー-ウィジェット (staticContent).
...
import { Page3Page } from './p00003-メニュー-ウィジェット２.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 3: メニュー・ウィジェット２ [requires auth]', () => {
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new Page3Page(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('メニュー・ウィジェット２'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 1 hidden)', async ({ page }) => {
    const po = new Page3Page(page);
    await po.goto();
    await expectItemsPresent(page, ['P3_JOB']);
  });

  test('every labeled .apx button exists (1 button)', async ({ page }) => {
    const po = new Page3Page(page);
    await po.goto();
    await expectButtonsPresent(page, ['Select Job']);
  });
});
