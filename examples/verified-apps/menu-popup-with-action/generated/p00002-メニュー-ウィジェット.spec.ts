/**
 * GENERATED by apx-testgen from pages/p00002-メニュー-ウィジェット.apx — DO NOT EDIT.
 ...
 * (./p00002-メニュー-ウィジェット.page.js), not raw testkit calls, so both stay in sync.
...
import { Page2Page } from './p00002-メニュー-ウィジェット.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 2: メニュー・ウィジェット [requires auth]', () => {
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new Page2Page(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('メニュー・ウィジェット'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 1 hidden)', async ({ page }) => {
    const po = new Page2Page(page);
    await po.goto();
    await expectItemsPresent(page, ['P2_JOB']);
  });
