 ...
 * Regions present in metadata: chat, employees, employees_1, salary-management-agent
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): chat (staticContent), employees (staticContent), employees_1 (classicReport), salary-management-agent (staticContent).
...
test.describe('page 1: ホーム [requires auth]', () => {
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new HomePage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Salary Management Agent'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 0 hidden)', async ({ page }) => {
    const po = new HomePage(page);
    await po.goto();
    await expectItemsPresent(page, ['P1_ENAME']);
  });
});
