 ...
 * (./p00122-airports-map-circle-search.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: about-this-page, selected-airports, selected-airports-irr, us-airports
 ...
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): about-this-page (staticContent), selected-airports (classicReport), us-airports (map).
...
import { AirportsMapCircleSearchPage } from './p00122-airports-map-circle-search.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 122: "Map Circle Search " [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new AirportsMapCircleSearchPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new AirportsMapCircleSearchPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('"&APP_NAME. - Map Circle Search "'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (3 items, 3 hidden)', async ({ page }) => {
    const po = new AirportsMapCircleSearchPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P122_CIRCLE_GEOJSON', 'P122_LARGE_VISIBLE', 'P122_SMALL_VISIBLE']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new AirportsMapCircleSearchPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['selected-airports-irr']);
  });
});
