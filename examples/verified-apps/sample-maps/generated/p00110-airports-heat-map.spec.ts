/**
 * GENERATED by apx-testgen from pages/p00110-airports-heat-map.apx — DO NOT EDIT.
 ...
 * (./p00110-airports-heat-map.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: about-this-page, state, us-airports
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): about-this-page (staticContent), state (staticContent), us-airports (map).
...
import { AirportsHeatMapPage } from './p00110-airports-heat-map.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 110: Airports Heat Map [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new AirportsHeatMapPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new AirportsHeatMapPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('&APP_NAME. - Airports Heat Map'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 0 hidden)', async ({ page }) => {
    const po = new AirportsHeatMapPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P110_STATE']);
  });
});
