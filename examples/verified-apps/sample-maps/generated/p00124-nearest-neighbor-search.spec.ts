 ...
 * (./p00124-nearest-neighbor-search.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: about-this-page, airports, filters, map
 ...
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): about-this-page (staticContent), filters (staticContent), map (map).
...
import { NearestNeighborSearchPage } from './p00124-nearest-neighbor-search.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 124: Nearest Neighbor Search [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new NearestNeighborSearchPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new NearestNeighborSearchPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('&APP_NAME. - Nearest Neighbor Search'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (7 items, 2 hidden)', async ({ page }) => {
    const po = new NearestNeighborSearchPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P124_AIRPORT_TYPE', 'P124_CLICK_POSITION', 'P124_COMMERCIAL_OPS_MIN', 'P124_MAX_DISTANCE', 'P124_NEIGHBORS', 'P124_NEIGHBOR_COUNT', 'P124_SEARCH_FROM']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new NearestNeighborSearchPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['airports']);
  });
});
