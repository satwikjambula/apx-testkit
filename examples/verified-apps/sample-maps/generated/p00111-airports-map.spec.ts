/**
 * GENERATED by apx-testgen from pages/p00111-airports-map.apx — DO NOT EDIT.
 ...
 * (./p00111-airports-map.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: about-this-page, custom-map-markers
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): about-this-page (staticContent), custom-map-markers (map).
...
import { AirportsMapPage } from './p00111-airports-map.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 111: Airports Map [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new AirportsMapPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new AirportsMapPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('&APP_NAME. - Airports Map'));
  });
});
