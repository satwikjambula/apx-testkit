/**
 * GENERATED by apx-testgen from pages/p00201-legacy-oracle-maps-plug-in.apx — DO NOT EDIT.
 ...
 * (./p00201-legacy-oracle-maps-plug-in.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: about-this-page, us-states-legacy-oracle-maps-plug-in
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): about-this-page (staticContent), us-states-legacy-oracle-maps-plug-in (plugin/legacyOracleHtml5MapsRegion).
...
import { LegacyOracleMapsPlugInPage } from './p00201-legacy-oracle-maps-plug-in.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 201: Legacy Oracle Maps Plug-In [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new LegacyOracleMapsPlugInPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new LegacyOracleMapsPlugInPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('&APP_NAME. - Legacy Oracle Maps Plug-In'));
  });
});
