/**
 * GENERATED by apx-testgen from pages/p00123-map-and-report.apx — DO NOT EDIT.
 ...
 * (./p00123-map-and-report.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: about-this-page, airports, map, no-report, results
 ...
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): about-this-page (staticContent), map (map), no-report (staticContent), results (classicReport).
...
import { MapAndReportPage } from './p00123-map-and-report.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 123: Map and Report [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new MapAndReportPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new MapAndReportPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('&APP_NAME. - Map and Report'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (3 items, 3 hidden)', async ({ page }) => {
    const po = new MapAndReportPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P123_BBOX', 'P123_LARGE_VISIBLE', 'P123_SMALL_VISIBLE']);
  });
