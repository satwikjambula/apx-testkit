/**
 * GENERATED by apx-testgen from pages/p00121-airports-search.apx — DO NOT EDIT.
 ...
 * (./p00121-airports-search.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: about-this-page, airports, map
 ...
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): about-this-page (staticContent), map (map).
...
import { AirportsSearchPage } from './p00121-airports-search.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 121: Airports Search [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new AirportsSearchPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new AirportsSearchPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('&APP_NAME. - Airports Search'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 1 hidden)', async ({ page }) => {
    const po = new AirportsSearchPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P121_ID']);
  });
