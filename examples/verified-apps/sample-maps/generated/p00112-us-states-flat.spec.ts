 ...
 * (./p00112-us-states-flat.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: about-this-page, us-states
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): about-this-page (staticContent), us-states (map).
...
import { UsStatesFlatPage } from './p00112-us-states-flat.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 112: US States (flat) [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new UsStatesFlatPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new UsStatesFlatPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('&APP_NAME. - US States (flat)'));
  });
});
