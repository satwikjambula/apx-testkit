/**
 * GENERATED by apx-testgen from pages/p00114-us-states-with-links.apx — DO NOT EDIT.
 ...
 * (./p00114-us-states-with-links.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: about-this-page, airports-by-state
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): about-this-page (staticContent), airports-by-state (map).
...
import { UsStatesWithLinksPage } from './p00114-us-states-with-links.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 114: US States with Links [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new UsStatesWithLinksPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new UsStatesWithLinksPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('&APP_NAME. - US States with Links'));
  });
});
