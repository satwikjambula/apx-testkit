 ...
 * Regions present in metadata: airport-pages, info, map-app-pages, map-app-pages-2, sample-maps-application
 ...
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): airport-pages (list), info (staticContent), sample-maps-application (staticContent).
...
test.describe('page 1: Home [requires auth]', () => {
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new HomePage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('"&APP_NAME. "'));
  });

  test('every interactiveReport/cards/facetedSearch region resolves (2 regions)', async ({ page }) => {
    const po = new HomePage(page);
    await po.goto();
    await expectRegionsResolve(page, ['map-app-pages', 'map-app-pages-2']);
  });
});
