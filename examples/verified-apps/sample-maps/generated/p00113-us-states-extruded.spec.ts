/**
 * GENERATED by apx-testgen from pages/p00113-us-states-extruded.apx — DO NOT EDIT.
 ...
 * (./p00113-us-states-extruded.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: about-this-page, airports-by-state, water-area-by-state
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): about-this-page (staticContent), airports-by-state (map), water-area-by-state (map).
...
import { UsStatesExtrudedPage } from './p00113-us-states-extruded.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 113: US States (extruded) [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new UsStatesExtrudedPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new UsStatesExtrudedPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('&APP_NAME. - US States (extruded)'));
  });
});
