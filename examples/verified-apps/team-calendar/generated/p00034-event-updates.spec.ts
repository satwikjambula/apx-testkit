/**
 * GENERATED by apx-testgen from pages/p00034-event-updates.apx — DO NOT EDIT.
 ...
 * (./p00034-event-updates.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: button-bar, event-updates, updates
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): button-bar (staticContent), event-updates (breadcrumb), updates (classicReport).
...
import { EventUpdatesPage } from './p00034-event-updates.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 34: Event Updates [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new EventUpdatesPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new EventUpdatesPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Event Updates'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 0 hidden)', async ({ page }) => {
    const po = new EventUpdatesPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P34_ID']);
  });

  test('every labeled .apx button exists (1 button)', async ({ page }) => {
    const po = new EventUpdatesPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Add Update']);
  });
});
