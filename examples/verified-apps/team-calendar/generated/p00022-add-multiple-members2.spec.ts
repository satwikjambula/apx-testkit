 ...
 * (./p00022-add-multiple-members2.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, buttons, invalid-data, valid-emails
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), buttons (staticContent), invalid-data (classicReport), valid-emails (classicReport).
...
import { AddMultipleMembers2Page } from './p00022-add-multiple-members2.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 22: Add Multiple Members [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new AddMultipleMembers2Page(page);
  ...
  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 0 hidden)', async ({ page }) => {
    const po = new AddMultipleMembers2Page(page);
    await po.goto();
    await expectItemsPresent(page, ['P22_GROUP_ID']);
  });

  test('every labeled .apx button exists (3 buttons)', async ({ page }) => {
    const po = new AddMultipleMembers2Page(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Add Members', 'Previous']);
  });
});
