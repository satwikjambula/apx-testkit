 ...
 * (./p00019-group-member-details.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, group-member-details
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), group-member-details (staticContent).
...
import { GroupMemberDetailsPage } from './p00019-group-member-details.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 19: Group Member Details [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new GroupMemberDetailsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new GroupMemberDetailsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Group Member Details'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (3 items, 1 hidden)', async ({ page }) => {
    const po = new GroupMemberDetailsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P19_EMAIL_ADDRESS', 'P19_GROUP_ID', 'P19_MBR_ID']);
  });

  test('apex.item round-trip on P19_EMAIL_ADDRESS', async ({ page }) => {
    const po = new GroupMemberDetailsPage(page);
    await po.goto();
  ...
  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new GroupMemberDetailsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Add Member', 'Delete', 'Apply Changes']);
  });
});
