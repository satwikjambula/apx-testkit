/**
 * GENERATED by apx-testgen from pages/p00029-access-control-list.apx — DO NOT EDIT.
 ...
 * (./p00029-access-control-list.page.js), not raw testkit calls, so both stay in sync.
...
import { AccessControlListPage } from './p00029-access-control-list.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 29: Access Control List [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new AccessControlListPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new AccessControlListPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Access Control List'));
  });
