 ...
 * (./p00030-user-details.page.js), not raw testkit calls, so both stay in sync.
...
import { UserDetailsPage } from './p00030-user-details.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 30: User Details [requires auth]', () => {
  ...
  test('every .apx pageItem exists in DOM/apex.item registry (5 items, 1 hidden)', async ({ page }) => {
    const po = new UserDetailsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P30_ACCESS_LEVEL_ID', 'P30_ACCOUNT_LOCKED', 'P30_ID', 'P30_RESTRICTED_TO', 'P30_USERNAME']);
  });

  test('apex.item round-trip on P30_USERNAME', async ({ page }) => {
    const po = new UserDetailsPage(page);
    await po.goto();
