/**
 * GENERATED by apx-testgen from pages/p00012-timeline.apx — DO NOT EDIT.
 ...
 * (./p00012-timeline.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumbs, calendars, contact-email, event-types, filters, notifications, timeline-of-future-events, timezone
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumbs (breadcrumb), calendars (classicReport), contact-email (classicReport), event-types (classicReport), filters (staticContent), notifications (classicReport), timeline-of-future-events (plugin/timelineAndStatusList), timezone (staticContent).
...
import { TimelinePage } from './p00012-timeline.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 12: Timeline [requires auth]', () => {
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new TimelinePage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('&APPLICATION_TITLE. - Timeline'));
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new TimelinePage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Add Event', 'Update Filters']);
  });
});
