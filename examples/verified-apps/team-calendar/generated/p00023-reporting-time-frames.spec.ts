/**
 * GENERATED by apx-testgen from pages/p00023-reporting-time-frames.apx — DO NOT EDIT.
 ...
 * (./p00023-reporting-time-frames.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: reporting-time-frames
...
import { ReportingTimeFramesPage } from './p00023-reporting-time-frames.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 23: Reporting Time Frames [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ReportingTimeFramesPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ReportingTimeFramesPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Reporting Time Frames'));
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new ReportingTimeFramesPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Create Time Frame', 'Reset']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new ReportingTimeFramesPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['reporting-time-frames']);
  });
});
