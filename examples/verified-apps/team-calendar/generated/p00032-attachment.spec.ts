 ...
 * (./p00032-attachment.page.js), not raw testkit calls, so both stay in sync.
...
import { AttachmentPage } from './p00032-attachment.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 32: Attachment [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new AttachmentPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new AttachmentPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Attachment'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (7 items, 1 hidden)', async ({ page }) => {
    const po = new AttachmentPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P32_EVENT_ID', 'P32_FILENAME', 'P32_FILE_BLOB', 'P32_FILE_COMMENTS', 'P32_FILE_MIMETYPE', 'P32_ID', 'P32_TAGS']);
  });

  test('apex.item round-trip on P32_FILE_COMMENTS', async ({ page }) => {
    const po = new AttachmentPage(page);
    await po.goto();
