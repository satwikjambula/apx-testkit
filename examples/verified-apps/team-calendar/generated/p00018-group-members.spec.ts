 ...
 * (./p00018-group-members.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, group-members
...
import { GroupMembersPage } from './p00018-group-members.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 18: Group Members [requires auth]', () => {
  ...
  test('every labeled .apx button exists (3 buttons)', async ({ page }) => {
    const po = new GroupMembersPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Add Member', 'Add Multiple Members', 'Reset']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new GroupMembersPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['group-members']);
  });
});
