/**
 * GENERATED by apx-testgen from pages/p00008-event-type-details.apx — DO NOT EDIT.
 ...
 * (./p00008-event-type-details.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, event-type-details, hidden, recommended-colors
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), event-type-details (staticContent), hidden (staticContent), recommended-colors (staticContent).
...
import { EventTypeDetailsPage } from './p00008-event-type-details.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 8: Event Type Details [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new EventTypeDetailsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new EventTypeDetailsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Event Type Details'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (6 items, 3 hidden)', async ({ page }) => {
    const po = new EventTypeDetailsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P8_BORDER_COLOR', 'P8_COLOR_PREF_ID', 'P8_DISPLAY_COLOR', 'P8_INTERNAL_YN', 'P8_TYPE_ID', 'P8_TYPE_NAME']);
  });

  test('apex.item round-trip on P8_TYPE_NAME', async ({ page }) => {
    const po = new EventTypeDetailsPage(page);
    await po.goto();
    await po.typeName.setValue('apx-testgen');
    expect(await po.typeName.getValue()).toBe('apx-testgen');
  });

  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new EventTypeDetailsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Add Event Type', 'Delete', 'Apply Changes']);
  });
});
