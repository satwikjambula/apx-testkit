 ...
 * (./p00031-attachments.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: attachments, bc, button-bar
 ...
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): bc (breadcrumb), button-bar (staticContent).
...
import { AttachmentsPage } from './p00031-attachments.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 31: Attachments [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new AttachmentsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new AttachmentsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Attachments'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (2 items, 1 hidden)', async ({ page }) => {
    const po = new AttachmentsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P31_DELETE_FILE_ID', 'P31_ID']);
  });
  ...
  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new AttachmentsPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['attachments']);
  });
});
