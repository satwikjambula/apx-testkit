 ...
 * (./p00005-filter-options.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, filter-options
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), filter-options (staticContent).
...
import { FilterOptionsPage } from './p00005-filter-options.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 5: Filter Options [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new FilterOptionsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new FilterOptionsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Filter Options'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (5 items, 0 hidden)', async ({ page }) => {
    const po = new FilterOptionsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P5_CALENDARS', 'P5_CAL_ALL', 'P5_CONTACT_EMAIL', 'P5_ET_ALL', 'P5_EVENT_TYPES']);
  });

  test('every labeled .apx button exists (3 buttons)', async ({ page }) => {
    const po = new FilterOptionsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Reset All', 'Set']);
  });
});
