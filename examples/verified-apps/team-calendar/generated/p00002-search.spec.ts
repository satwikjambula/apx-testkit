/**
 * GENERATED by apx-testgen from pages/p00002-search.apx — DO NOT EDIT.
 ...
 * (./p00002-search.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumbs, events, hidden-items, notifications, timezone
 ...
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumbs (breadcrumb), hidden-items (staticContent), notifications (classicReport), timezone (staticContent).
...
import { SearchPage } from './p00002-search.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 2: Search [requires auth]', () => {
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new SearchPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('&APPLICATION_TITLE. - Search'));
  });

  test('every labeled .apx button exists (3 buttons)', async ({ page }) => {
    const po = new SearchPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Add Event', 'Delete Multiple Events', 'Reset']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new SearchPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['events']);
  });
});
