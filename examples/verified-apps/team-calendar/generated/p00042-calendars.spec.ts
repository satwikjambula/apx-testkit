/**
 * GENERATED by apx-testgen from pages/p00042-calendars.apx — DO NOT EDIT.
 ...
 * (./p00042-calendars.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: calendars
...
import { CalendarsPage } from './p00042-calendars.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 42: Calendars [requires auth]', () => {
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new CalendarsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Calendars'));
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new CalendarsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Add Calendar', 'Reset']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new CalendarsPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['calendars']);
  });
});
