 ...
 * (./p00021-add-multiple-members.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: add-multiple-members, breadcrumb, buttons
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): add-multiple-members (staticContent), breadcrumb (breadcrumb), buttons (staticContent).
...
import { AddMultipleMembersPage } from './p00021-add-multiple-members.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 21: Add Multiple Members [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new AddMultipleMembersPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new AddMultipleMembersPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Add Multiple Members'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (3 items, 0 hidden)', async ({ page }) => {
    const po = new AddMultipleMembersPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P21_GROUP_ID', 'P21_LB', 'P21_MBRS']);
  });

  test('apex.item round-trip on P21_MBRS', async ({ page }) => {
    const po = new AddMultipleMembersPage(page);
    await po.goto();
    await po.mbrs.setValue('apx-testgen');
    expect(await po.mbrs.getValue()).toBe('apx-testgen');
  });
