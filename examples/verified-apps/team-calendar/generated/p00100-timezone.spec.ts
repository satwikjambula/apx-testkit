 ...
 * (./p00100-timezone.page.js), not raw testkit calls, so both stay in sync.
...
import { TimezonePage } from './p00100-timezone.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 100: Time Zone Preference [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new TimezonePage(page);
  ...
  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 0 hidden)', async ({ page }) => {
    const po = new TimezonePage(page);
    await po.goto();
    await expectItemsPresent(page, ['P100_TIMEZONE']);
  });
