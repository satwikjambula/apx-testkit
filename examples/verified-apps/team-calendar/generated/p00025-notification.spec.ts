 ...
 * (./p00025-notification.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: about-this-page, buttons, notification, notification-items
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): about-this-page (staticContent), buttons (staticContent), notification (staticContent), notification-items (staticContent).
...
import { NotificationPage } from './p00025-notification.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 25: Notification [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new NotificationPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new NotificationPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Notification'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (7 items, 1 hidden)', async ({ page }) => {
    const po = new NotificationPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P25_DISPLAY_FROM', 'P25_DISPLAY_SEQUENCE', 'P25_DISPLAY_UNTIL', 'P25_ID', 'P25_NOTIFICATION_DESCRIPTION', 'P25_NOTIFICATION_NAME', 'P25_NOTIFICATION_TYPE']);
  });

  test('apex.item round-trip on P25_DISPLAY_SEQUENCE', async ({ page }) => {
    const po = new NotificationPage(page);
    await po.goto();
