/**
 * GENERATED by apx-testgen from pages/p00053-add-multiple-users.apx — DO NOT EDIT.
 ...
 * (./p00053-add-multiple-users.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, hidden-items, wizard, wizard-items
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), hidden-items (staticContent), wizard (staticContent), wizard-items (staticContent).
...
import { AddMultipleUsersPage } from './p00053-add-multiple-users.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 53: Add Multiple Users [requires auth]', () => {
  ...
  test('every .apx pageItem exists in DOM/apex.item registry (4 items, 1 hidden)', async ({ page }) => {
    const po = new AddMultipleUsersPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P53_PRELIM_USERS', 'P53_RESTRICTED_TO', 'P53_ROLE', 'P53_USERNAME_FORMAT1']);
  });

  test('apex.item round-trip on P53_PRELIM_USERS', async ({ page }) => {
    const po = new AddMultipleUsersPage(page);
    await po.goto();
