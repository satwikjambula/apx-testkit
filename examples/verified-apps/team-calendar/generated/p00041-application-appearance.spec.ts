 ...
 * (./p00041-application-appearance.page.js), not raw testkit calls, so both stay in sync.
...
import { ApplicationAppearancePage } from './p00041-application-appearance.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 41: Application Appearance [requires auth]', () => {
  ...
  test('every .apx pageItem exists in DOM/apex.item registry (2 items, 0 hidden)', async ({ page }) => {
    const po = new ApplicationAppearancePage(page);
    await po.goto();
    await expectItemsPresent(page, ['P41_DESKTOP_THEME_STYLE_ID', 'P41_END_USER_STYLE']);
  });
