 ...
 * (./p00007-event-types.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: event-types
...
import { EventTypesPage } from './p00007-event-types.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 7: Event Types [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new EventTypesPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new EventTypesPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Event Types'));
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new EventTypesPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Add Event Type', 'Reset']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new EventTypesPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['event-types']);
  });
});
