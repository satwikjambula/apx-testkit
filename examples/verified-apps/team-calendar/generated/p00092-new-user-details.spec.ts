 ...
 * (./p00092-new-user-details.page.js), not raw testkit calls, so both stay in sync.
...
import { NewUserDetailsPage } from './p00092-new-user-details.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 92: New User Details [requires auth]', () => {
  ...
  test('every .apx pageItem exists in DOM/apex.item registry (3 items, 1 hidden)', async ({ page }) => {
    const po = new NewUserDetailsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P92_ACCESS_LEVEL_ID', 'P92_SEQUENCE', 'P92_USERNAME']);
  });

  test('apex.item round-trip on P92_USERNAME', async ({ page }) => {
    const po = new NewUserDetailsPage(page);
    await po.goto();
