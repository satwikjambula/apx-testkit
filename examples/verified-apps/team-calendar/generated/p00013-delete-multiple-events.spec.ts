 ...
 * (./p00013-delete-multiple-events.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, directions, events-to-be-deleted, filter
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), directions (staticContent), events-to-be-deleted (classicReport), filter (staticContent).
...
import { DeleteMultipleEventsPage } from './p00013-delete-multiple-events.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 13: Delete Multiple Events [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new DeleteMultipleEventsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new DeleteMultipleEventsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Delete Multiple Events'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (5 items, 0 hidden)', async ({ page }) => {
    const po = new DeleteMultipleEventsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P13_CALENDAR_ID', 'P13_END_DATE', 'P13_EVENT_TYPE', 'P13_START_DATE', 'P13_TIMEFRAME']);
  });

  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new DeleteMultipleEventsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Delete Events Listed Below', 'Clear', 'Go']);
  });
});
