/**
 * GENERATED by apx-testgen from pages/p00037-rename-application.apx — DO NOT EDIT.
 ...
 * (./p00037-rename-application.page.js), not raw testkit calls, so both stay in sync.
...
import { RenameApplicationPage } from './p00037-rename-application.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 37: Rename Application [requires auth]', () => {
  ...
  test('every .apx pageItem exists in DOM/apex.item registry (2 items, 0 hidden)', async ({ page }) => {
    const po = new RenameApplicationPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P37_APPLICATION_SUBTITLE', 'P37_APPLICATION_TITLE']);
  });

  test('apex.item round-trip on P37_APPLICATION_SUBTITLE', async ({ page }) => {
    const po = new RenameApplicationPage(page);
    await po.goto();
    await po.applicationSubtitle.setValue('apx-testgen');
    expect(await po.applicationSubtitle.getValue()).toBe('apx-testgen');
  });
