 ...
 * (./p00033-event-update.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, event-update
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), event-update (staticContent).
...
import { EventUpdatePage } from './p00033-event-update.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 33: Event Update [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new EventUpdatePage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new EventUpdatePage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Event Update'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (3 items, 1 hidden)', async ({ page }) => {
    const po = new EventUpdatePage(page);
    await po.goto();
    await expectItemsPresent(page, ['P33_EVENT_ID', 'P33_ID', 'P33_NOTE']);
  });

  test('apex.item round-trip on P33_NOTE', async ({ page }) => {
    const po = new EventUpdatePage(page);
    await po.goto();
  ...
  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new EventUpdatePage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Create Event', 'Delete', 'Apply Changes']);
  });
});
