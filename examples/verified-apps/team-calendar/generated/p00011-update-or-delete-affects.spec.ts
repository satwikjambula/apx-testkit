 ...
 * (./p00011-update-or-delete-affects.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: associated-events, breadcrumb, remove-event-s, scope, update-event-s
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): associated-events (classicReport), breadcrumb (breadcrumb), remove-event-s (staticContent), scope (staticContent), update-event-s (staticContent).
...
import { UpdateOrDeleteAffectsPage } from './p00011-update-or-delete-affects.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 11: Update or Delete Affects [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new UpdateOrDeleteAffectsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new UpdateOrDeleteAffectsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Update or Delete Affects'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (3 items, 1 hidden)', async ({ page }) => {
    const po = new UpdateOrDeleteAffectsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P11_DEL_REQUEST', 'P11_REQUEST_TYPE', 'P11_UPD_REQUEST']);
  });
