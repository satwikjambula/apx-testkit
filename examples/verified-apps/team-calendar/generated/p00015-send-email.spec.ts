 ...
 * (./p00015-send-email.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, email-details, email-functionality-disabled, events-to-be-included, filter
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), email-details (staticContent), email-functionality-disabled (staticContent), events-to-be-included (classicReport), filter (staticContent).
...
import { SendEmailPage } from './p00015-send-email.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 15: Send Email [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new SendEmailPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new SendEmailPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Send Email'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (9 items, 0 hidden)', async ({ page }) => {
    const po = new SendEmailPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P15_END_DATE', 'P15_EVENT_TYPE', 'P15_FROM', 'P15_GROUP_ID', 'P15_MSG_BODY', 'P15_START_DATE', 'P15_SUBJECT', 'P15_TIMEFRAME', 'P15_TO']);
  });

  test('apex.item round-trip on P15_FROM', async ({ page }) => {
    const po = new SendEmailPage(page);
    await po.goto();
  ...
  test('every labeled .apx button exists (5 buttons)', async ({ page }) => {
    const po = new SendEmailPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'View Members', 'Clear', 'Go', 'Send Email']);
  });
});
