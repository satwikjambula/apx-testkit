 ...
 * Regions present in metadata: acl-warning, application-title, calendars, contact-email, event-types, events-calendar, events-calendar-2, filters, notifications, timezone
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): acl-warning (plugin/aclWarning), application-title (plSqlDynamicContent), calendars (classicReport), contact-email (classicReport), event-types (classicReport), events-calendar (calendar), events-calendar-2 (calendar), filters (staticContent), notifications (classicReport), timezone (staticContent).
...
test.describe('page 1: Home [requires auth]', () => {
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new HomePage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('&APPLICATION_TITLE. - Home'));
  });
