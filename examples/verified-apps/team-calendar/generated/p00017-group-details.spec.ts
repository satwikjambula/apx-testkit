 ...
 * (./p00017-group-details.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: audit-information, buttons, group-details
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): audit-information (staticContent), buttons (staticContent), group-details (staticContent).
...
import { GroupDetailsPage } from './p00017-group-details.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 17: Group Details [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new GroupDetailsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new GroupDetailsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Group Details'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (6 items, 1 hidden)', async ({ page }) => {
    const po = new GroupDetailsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P17_CREATED_BY', 'P17_CREATED_ON', 'P17_GROUP_ID', 'P17_GROUP_NAME', 'P17_LAST_UPDATED_BY', 'P17_LAST_UPDATED_ON']);
  });

  test('apex.item round-trip on P17_GROUP_NAME', async ({ page }) => {
    const po = new GroupDetailsPage(page);
    await po.goto();
    await po.groupName.setValue('apx-testgen');
    expect(await po.groupName.getValue()).toBe('apx-testgen');
  });

  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new GroupDetailsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Add Group', 'Delete', 'Apply Changes']);
  });
});
