 ...
 * (./p00024-reporting-time-frame-details.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: audit-information, buttons, reporting-time-frame-details
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): audit-information (staticContent), buttons (staticContent), reporting-time-frame-details (staticContent).
...
import { ReportingTimeFrameDetailsPage } from './p00024-reporting-time-frame-details.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 24: Reporting Time Frame Details [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ReportingTimeFrameDetailsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ReportingTimeFrameDetailsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Reporting Time Frame Details'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (8 items, 1 hidden)', async ({ page }) => {
    const po = new ReportingTimeFrameDetailsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P24_CREATED_BY', 'P24_CREATED_ON', 'P24_END_DATE', 'P24_LAST_UPDATED_BY', 'P24_LAST_UPDATED_ON', 'P24_START_DATE', 'P24_TF_ID', 'P24_TF_NAME']);
  });

  test('apex.item round-trip on P24_TF_NAME', async ({ page }) => {
    const po = new ReportingTimeFrameDetailsPage(page);
    await po.goto();
    await po.tfName.setValue('apx-testgen');
    expect(await po.tfName.getValue()).toBe('apx-testgen');
  });

  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new ReportingTimeFrameDetailsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Create Time Frame', 'Delete', 'Apply Changes']);
  });
});
