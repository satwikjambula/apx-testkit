/**
 * GENERATED by apx-testgen from pages/p00047-projected-close-dates.apx — DO NOT EDIT.
 ...
 * (./p00047-projected-close-dates.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, projected-close-dates-displays-rep-title-and-deal-amount
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), projected-close-dates-displays-rep-title-and-deal-amount (calendar).
...
import { ProjectedCloseDatesPage } from './p00047-projected-close-dates.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 47: Projected Close Dates [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ProjectedCloseDatesPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ProjectedCloseDatesPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Projected Close Dates'));
  });
});
