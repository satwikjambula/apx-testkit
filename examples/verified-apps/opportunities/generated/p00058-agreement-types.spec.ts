/**
 * GENERATED by apx-testgen from pages/p00058-agreement-types.apx — DO NOT EDIT.
 ...
 * (./p00058-agreement-types.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: agreement-types
...
import { AgreementTypesPage } from './p00058-agreement-types.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 58: Agreement Types [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new AgreementTypesPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new AgreementTypesPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Agreement Types'));
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new AgreementTypesPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Add Agreement Type', 'Reset']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new AgreementTypesPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['agreement-types']);
  });
});
