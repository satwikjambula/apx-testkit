/**
 * GENERATED by apx-testgen from pages/p00075-agreement-type.apx — DO NOT EDIT.
 ...
 * (./p00075-agreement-type.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: agreement-type, buttons
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): agreement-type (staticContent), buttons (staticContent).
...
import { AgreementTypePage } from './p00075-agreement-type.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 75: Agreement Type [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new AgreementTypePage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new AgreementTypePage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Agreement Type'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (3 items, 1 hidden)', async ({ page }) => {
    const po = new AgreementTypePage(page);
    await po.goto();
    await expectItemsPresent(page, ['P75_DESCRIPTION', 'P75_ID', 'P75_NAME']);
  });

  test('apex.item round-trip on P75_DESCRIPTION', async ({ page }) => {
    const po = new AgreementTypePage(page);
    await po.goto();
  ...
  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new AgreementTypePage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Add Agreement Type', 'Delete', 'Apply Changes']);
  });
});
