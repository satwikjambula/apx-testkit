/**
 * GENERATED by apx-testgen from pages/p00109-territory-acl.apx — DO NOT EDIT.
 ...
 * (./p00109-territory-acl.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, territory-acl
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), territory-acl (staticContent).
...
import { TerritoryAclPage } from './p00109-territory-acl.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 109: Territory ACL [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new TerritoryAclPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new TerritoryAclPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Territory ACL'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (4 items, 1 hidden)', async ({ page }) => {
    const po = new TerritoryAclPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P109_ACL_COMMENTS', 'P109_ID', 'P109_TERRITORY_ID', 'P109_USERID']);
  });

  test('apex.item round-trip on P109_ACL_COMMENTS', async ({ page }) => {
    const po = new TerritoryAclPage(page);
    await po.goto();
    await po.aclComments.setValue('apx-testgen');
    expect(await po.aclComments.getValue()).toBe('apx-testgen');
  });

  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new TerritoryAclPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Add Territory Assignment', 'Delete', 'Apply Changes']);
  });
});
