 ...
 * (./p00114-link-details.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, opportunity-link
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), opportunity-link (staticContent).
...
import { LinkDetailsPage } from './p00114-link-details.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 114: Link Details [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new LinkDetailsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new LinkDetailsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Link Details'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (13 items, 9 hidden)', async ({ page }) => {
    const po = new LinkDetailsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P114_ACCOUNT_ID', 'P114_CONTACT_ID', 'P114_ENTITY_ID', 'P114_ENTITY_TYPE', 'P114_ID', 'P114_LEAD_ID', 'P114_LINK_COMMENTS', 'P114_LINK_TARGET', 'P114_LINK_TEXT', 'P114_OPPORTUNITY_ID', 'P114_PRODUCT_ID', 'P114_TAGS', 'P114_TERRITORY_ID']);
  });

  test('apex.item round-trip on P114_LINK_COMMENTS', async ({ page }) => {
    const po = new LinkDetailsPage(page);
    await po.goto();
    await po.linkComments.setValue('apx-testgen');
    expect(await po.linkComments.getValue()).toBe('apx-testgen');
  });

  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new LinkDetailsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Add Link', 'Delete', 'Apply Changes']);
  });
});
