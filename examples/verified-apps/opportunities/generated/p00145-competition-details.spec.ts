/**
 * GENERATED by apx-testgen from pages/p00145-competition-details.apx — DO NOT EDIT.
 ...
 * (./p00145-competition-details.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, competition-details
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), competition-details (staticContent).
...
import { CompetitionDetailsPage } from './p00145-competition-details.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 145: Competition Details [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new CompetitionDetailsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new CompetitionDetailsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Competition Details'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (5 items, 2 hidden)', async ({ page }) => {
    const po = new CompetitionDetailsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P145_COMPETITION_DESC', 'P145_COMPETITOR_ID', 'P145_COMPETITOR_THREAT_ID', 'P145_CUSTOMER_ID', 'P145_ID']);
  });

  test('apex.item round-trip on P145_COMPETITION_DESC', async ({ page }) => {
    const po = new CompetitionDetailsPage(page);
    await po.goto();
    await po.competitionDesc.setValue('apx-testgen');
    expect(await po.competitionDesc.getValue()).toBe('apx-testgen');
  });

  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new CompetitionDetailsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Add Competition', 'Delete', 'Apply Changes']);
  });
});
