 ...
 * (./p00035-application-preferences.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, preferences
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), preferences (staticContent).
...
import { ApplicationPreferencesPage } from './p00035-application-preferences.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 35: Application Preferences [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ApplicationPreferencesPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ApplicationPreferencesPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Application Preferences'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (2 items, 0 hidden)', async ({ page }) => {
    const po = new ApplicationPreferencesPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P35_REP_TITLE', 'P35_SALES_LDR_TITLE']);
  });

  test('apex.item round-trip on P35_REP_TITLE', async ({ page }) => {
    const po = new ApplicationPreferencesPage(page);
    await po.goto();
    await po.repTitle.setValue('apx-testgen');
    expect(await po.repTitle.getValue()).toBe('apx-testgen');
  });
