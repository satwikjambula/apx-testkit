 ...
 * (./p00027-competition-details2.page.js), not raw testkit calls, so both stay in sync.
...
import { CompetitionDetails2Page } from './p00027-competition-details2.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 27: Competition Details [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new CompetitionDetails2Page(page);
  ...
  test('every .apx pageItem exists in DOM/apex.item registry (5 items, 2 hidden)', async ({ page }) => {
    const po = new CompetitionDetails2Page(page);
    await po.goto();
    await expectItemsPresent(page, ['P27_COMPETITION_DESC', 'P27_COMPETITOR_ID', 'P27_COMPETITOR_THREAT_ID', 'P27_DEAL_ID', 'P27_ID']);
  });

  test('apex.item round-trip on P27_COMPETITION_DESC', async ({ page }) => {
    const po = new CompetitionDetails2Page(page);
    await po.goto();
