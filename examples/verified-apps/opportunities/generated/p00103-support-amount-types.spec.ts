/**
 * GENERATED by apx-testgen from pages/p00103-support-amount-types.apx — DO NOT EDIT.
 ...
 * (./p00103-support-amount-types.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: support-amount-types
...
import { SupportAmountTypesPage } from './p00103-support-amount-types.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 103: Support Amount Types [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new SupportAmountTypesPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new SupportAmountTypesPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Support Amount Types'));
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new SupportAmountTypesPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Add Support Amount Type', 'Reset']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new SupportAmountTypesPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['support-amount-types']);
  });
});
