/**
 * GENERATED by apx-testgen from pages/p00008-territories2.apx — DO NOT EDIT.
 ...
 * (./p00008-territories2.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: territories
...
import { Territories2Page } from './p00008-territories2.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 8: Territories [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new Territories2Page(page);
  ...
  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new Territories2Page(page);
    await po.goto();
    await expectButtonsPresent(page, ['Create Territory', 'Reset']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new Territories2Page(page);
    await po.goto();
    await expectRegionsResolve(page, ['territories']);
  });
});
