 ...
 * (./p00112-currency-details.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, currency-details
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), currency-details (staticContent).
...
import { CurrencyDetailsPage } from './p00112-currency-details.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 112: Currency Details [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new CurrencyDetailsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new CurrencyDetailsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Currency Details'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (4 items, 1 hidden)', async ({ page }) => {
    const po = new CurrencyDetailsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P112_CURRENCY_CODE', 'P112_CURRENCY_DESCRIPTION', 'P112_ID', 'P112_USD_EX_RATE']);
  });

  test('apex.item round-trip on P112_CURRENCY_CODE', async ({ page }) => {
    const po = new CurrencyDetailsPage(page);
    await po.goto();
    await po.currencyCode.setValue('apx-testgen');
    expect(await po.currencyCode.getValue()).toBe('apx-testgen');
  });

  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new CurrencyDetailsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Create Currency', 'Delete', 'Apply Changes']);
  });
});
