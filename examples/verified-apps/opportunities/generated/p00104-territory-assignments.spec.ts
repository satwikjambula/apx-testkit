/**
 * GENERATED by apx-testgen from pages/p00104-territory-assignments.apx — DO NOT EDIT.
 ...
 * (./p00104-territory-assignments.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, territory-acl
...
import { TerritoryAssignmentsPage } from './p00104-territory-assignments.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 104: Territory Assignments [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new TerritoryAssignmentsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new TerritoryAssignmentsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Territory Assignments'));
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new TerritoryAssignmentsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Add Territory Assignment', 'Reset']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new TerritoryAssignmentsPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['territory-acl']);
  });
});
