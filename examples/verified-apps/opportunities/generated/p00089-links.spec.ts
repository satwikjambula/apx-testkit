 ...
 * (./p00089-links.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: links
...
import { LinksPage } from './p00089-links.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 89: Links [requires auth]', () => {
  ...
  test('every .apx pageItem exists in DOM/apex.item registry (3 items, 3 hidden)', async ({ page }) => {
    const po = new LinksPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P89_DELETE_ID', 'P89_ENTITY_ID', 'P89_ENTITY_TYPE']);
  });
  ...
  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new LinksPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['links']);
  });
});
