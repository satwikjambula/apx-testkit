/**
 * GENERATED by apx-testgen from pages/p00033-settings.apx — DO NOT EDIT.
 ...
 * (./p00033-settings.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: access-control, access-control-2, acl-info, acl-status, additional-administration-tasks, application-authentication-and-authorization, breadcrumb, code-tables, confirm-enabling-access-control, primary-administration-tasks, upload-data, user-counts
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): access-control (staticContent), access-control-2 (list), acl-info (plSqlDynamicContent), acl-status (plugin/aclStatus), additional-administration-tasks (list), application-authentication-and-authorization (plugin/authorizationAdministration), breadcrumb (breadcrumb), code-tables (list), confirm-enabling-access-control (staticContent), primary-administration-tasks (list), upload-data (list), user-counts (classicReport).
...
import { SettingsPage } from './p00033-settings.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 33: Administration [requires auth]', () => {
  ...
  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new SettingsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Enable Access Control']);
  });
});
