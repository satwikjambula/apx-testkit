 ...
 * (./p00067-svp-details.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, svp-details
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), svp-details (staticContent).
...
import { SvpDetailsPage } from './p00067-svp-details.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 67: SVP Details [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new SvpDetailsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new SvpDetailsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('SVP Details'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (3 items, 1 hidden)', async ({ page }) => {
    const po = new SvpDetailsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P67_ID', 'P67_SVP_NAME', 'P67_SVP_ORG_DESCRIPTION']);
  });

  test('apex.item round-trip on P67_SVP_NAME', async ({ page }) => {
    const po = new SvpDetailsPage(page);
    await po.goto();
    await po.svpName.setValue('apx-testgen');
    expect(await po.svpName.getValue()).toBe('apx-testgen');
  });

  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new SvpDetailsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Create SVP', 'Delete', 'Apply Changes']);
  });
});
