 ...
 * (./p00070-lead-source-details.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, lead-source-details
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), lead-source-details (staticContent).
...
import { LeadSourceDetailsPage } from './p00070-lead-source-details.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 70: Lead Source Details [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new LeadSourceDetailsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new LeadSourceDetailsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Lead Source Details'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (3 items, 1 hidden)', async ({ page }) => {
    const po = new LeadSourceDetailsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P70_ID', 'P70_LEAD_SOURCE', 'P70_LEAD_SOURCE_DESCRIPTION']);
  });

  test('apex.item round-trip on P70_LEAD_SOURCE', async ({ page }) => {
    const po = new LeadSourceDetailsPage(page);
    await po.goto();
    await po.leadSource.setValue('apx-testgen');
    expect(await po.leadSource.getValue()).toBe('apx-testgen');
  });

  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new LeadSourceDetailsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Create Lead Source', 'Delete', 'Apply Changes']);
  });
});
