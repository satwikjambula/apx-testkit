 ...
 * (./p00050-territories.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: filters, page-settings, territories, territories-cards, territories-grid, territories-report
 ...
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): filters (staticContent), page-settings (staticContent), territories (staticContent), territories-cards (classicReport), territories-report (classicReport).
...
import { TerritoriesPage } from './p00050-territories.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 50: Territories [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new TerritoriesPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new TerritoriesPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Territories'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (3 items, 0 hidden)', async ({ page }) => {
    const po = new TerritoriesPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P50_DISPLAY_AS', 'P50_SEARCH', 'P50_SORT']);
  });

  test('apex.item round-trip on P50_SEARCH', async ({ page }) => {
    const po = new TerritoriesPage(page);
    await po.goto();
  ...
  test('every labeled .apx button exists (3 buttons)', async ({ page }) => {
    const po = new TerritoriesPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Manage Territories', 'Reset', 'Reset']);
  });
