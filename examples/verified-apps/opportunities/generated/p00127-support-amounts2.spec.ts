/**
 * GENERATED by apx-testgen from pages/p00127-support-amounts2.apx — DO NOT EDIT.
 ...
 * (./p00127-support-amounts2.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: support-amounts
...
import { SupportAmounts2Page } from './p00127-support-amounts2.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 127: Support Amounts [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new SupportAmounts2Page(page);
  ...
  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 1 hidden)', async ({ page }) => {
    const po = new SupportAmounts2Page(page);
    await po.goto();
    await expectItemsPresent(page, ['P127_ID']);
  });
