/**
 * GENERATED by apx-testgen from pages/p00043-time-zone-preference.apx — DO NOT EDIT.
 ...
 * (./p00043-time-zone-preference.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, preference
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), preference (staticContent).
...
import { TimeZonePreferencePage } from './p00043-time-zone-preference.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 43: Time Zone Preference [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new TimeZonePreferencePage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new TimeZonePreferencePage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Time Zone Preference'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 0 hidden)', async ({ page }) => {
    const po = new TimeZonePreferencePage(page);
    await po.goto();
    await expectItemsPresent(page, ['P43_TIMEZONE']);
  });
