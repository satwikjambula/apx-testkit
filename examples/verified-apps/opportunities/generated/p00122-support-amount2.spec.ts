 ...
 * (./p00122-support-amount2.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, support-amount
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), support-amount (staticContent).
...
import { SupportAmount2Page } from './p00122-support-amount2.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 122: Support Amount [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new SupportAmount2Page(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new SupportAmount2Page(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Support Amount'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (8 items, 2 hidden)', async ({ page }) => {
    const po = new SupportAmount2Page(page);
    await po.goto();
    await expectItemsPresent(page, ['P122_AMOUNT_ID', 'P122_CLOSE_DATE', 'P122_CUSTOMER_ID', 'P122_DESCRIPTION', 'P122_ID', 'P122_NAME', 'P122_QUOTE_PRICE', 'P122_TERM_ID']);
  });

  test('apex.item round-trip on P122_DESCRIPTION', async ({ page }) => {
    const po = new SupportAmount2Page(page);
    await po.goto();
  ...
  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new SupportAmount2Page(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Add Support Amount', 'Delete', 'Apply Changes']);
  });
});
