/**
 * GENERATED by apx-testgen from pages/p00118-support-amounts.apx — DO NOT EDIT.
 ...
 * (./p00118-support-amounts.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, support-amounts
...
import { SupportAmountsPage } from './p00118-support-amounts.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 118: Support Amounts [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new SupportAmountsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new SupportAmountsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Support Amounts'));
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new SupportAmountsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Add Support Amount', 'Reset']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new SupportAmountsPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['support-amounts']);
  });
});
