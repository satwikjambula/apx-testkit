 ...
 * (./p00012-fiscal-quarters.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: fiscal-quarters
...
import { FiscalQuartersPage } from './p00012-fiscal-quarters.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 12: Fiscal Quarters [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new FiscalQuartersPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new FiscalQuartersPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Fiscal Quarters'));
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new FiscalQuartersPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Create Fiscal Quarter', 'Reset']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new FiscalQuartersPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['fiscal-quarters']);
  });
});
