 ...
 * (./p00063-convert-to-opportunity.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: button-bar, convert
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): button-bar (staticContent), convert (staticContent).
...
import { ConvertToOpportunityPage } from './p00063-convert-to-opportunity.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 63: Convert to Opportunity [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ConvertToOpportunityPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ConvertToOpportunityPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Convert to Opportunity'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (7 items, 1 hidden)', async ({ page }) => {
    const po = new ConvertToOpportunityPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P63_ACCOUNT', 'P63_AMOUNT', 'P63_CLOSE_DATE', 'P63_LEAD_ID', 'P63_OPPORTUNITY', 'P63_SALES_REP', 'P63_STATUS']);
  });

  test('apex.item round-trip on P63_OPPORTUNITY', async ({ page }) => {
    const po = new ConvertToOpportunityPage(page);
    await po.goto();
    await po.opportunity.setValue('apx-testgen');
    expect(await po.opportunity.getValue()).toBe('apx-testgen');
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new ConvertToOpportunityPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Convert to Opportunity']);
  });
});
