 ...
 * (./p00146-validation-details.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: button-bar, form-items, introduction-text, validation-details
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): button-bar (staticContent), form-items (staticContent), introduction-text (staticContent), validation-details (staticContent).
...
import { ValidationDetailsPage } from './p00146-validation-details.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 146: Validation Details [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ValidationDetailsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ValidationDetailsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Validation Details'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (5 items, 3 hidden)', async ({ page }) => {
    const po = new ValidationDetailsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P146_ENTITY_ID', 'P146_ENTITY_TYPE', 'P146_ENTITY_TYPE_LOWER', 'P146_I_VALIDATE', 'P146_VERIFICATION_COMMENT']);
  });

  test('apex.item round-trip on P146_VERIFICATION_COMMENT', async ({ page }) => {
    const po = new ValidationDetailsPage(page);
    await po.goto();
    await po.verificationComment.setValue('apx-testgen');
    expect(await po.verificationComment.getValue()).toBe('apx-testgen');
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new ValidationDetailsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Validate']);
  });
});
