 ...
 * (./p00026-team-role-details.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, role
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), role (staticContent).
...
import { TeamRoleDetailsPage } from './p00026-team-role-details.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 26: Team Role Details [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new TeamRoleDetailsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new TeamRoleDetailsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Team Role Details'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (3 items, 1 hidden)', async ({ page }) => {
    const po = new TeamRoleDetailsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P26_ID', 'P26_IS_SALES_REP', 'P26_ROLE_NAME']);
  });

  test('apex.item round-trip on P26_ROLE_NAME', async ({ page }) => {
    const po = new TeamRoleDetailsPage(page);
    await po.goto();
    await po.roleName.setValue('apx-testgen');
    expect(await po.roleName.getValue()).toBe('apx-testgen');
  });

  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new TeamRoleDetailsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Create Team Role', 'Delete', 'Apply Changes']);
  });
});
