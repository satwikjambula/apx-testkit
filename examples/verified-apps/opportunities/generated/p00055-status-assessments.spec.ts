 ...
 * (./p00055-status-assessments.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: status-assessments
...
import { StatusAssessmentsPage } from './p00055-status-assessments.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 55: Status Assessments [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new StatusAssessmentsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new StatusAssessmentsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Status Assessments'));
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new StatusAssessmentsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Create Status Assessment', 'Reset']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new StatusAssessmentsPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['status-assessments']);
  });
});
