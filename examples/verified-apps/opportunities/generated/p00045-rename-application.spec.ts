 ...
 * (./p00045-rename-application.page.js), not raw testkit calls, so both stay in sync.
...
import { RenameApplicationPage } from './p00045-rename-application.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 45: Rename Application [requires auth]', () => {
  ...
  test('every .apx pageItem exists in DOM/apex.item registry (2 items, 0 hidden)', async ({ page }) => {
    const po = new RenameApplicationPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P45_APPLICATION_SUBTITLE', 'P45_APPLICATION_TITLE']);
  });

  test('apex.item round-trip on P45_APPLICATION_SUBTITLE', async ({ page }) => {
    const po = new RenameApplicationPage(page);
    await po.goto();
