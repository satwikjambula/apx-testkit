/**
 * GENERATED by apx-testgen from pages/p00054-risk-assessment-details.apx — DO NOT EDIT.
 ...
 * (./p00054-risk-assessment-details.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, risk-assessment-details
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), risk-assessment-details (staticContent).
...
import { RiskAssessmentDetailsPage } from './p00054-risk-assessment-details.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 54: Risk Assessment Details [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new RiskAssessmentDetailsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new RiskAssessmentDetailsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Risk Assessment Details'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (4 items, 1 hidden)', async ({ page }) => {
    const po = new RiskAssessmentDetailsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P54_ASSESSMENT_DESCRIPTION', 'P54_ASSESSMENT_TEXT', 'P54_DISPLAY_ORDER', 'P54_ID']);
  });

  test('apex.item round-trip on P54_ASSESSMENT_DESCRIPTION', async ({ page }) => {
    const po = new RiskAssessmentDetailsPage(page);
    await po.goto();
  ...
  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new RiskAssessmentDetailsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Create Risk Assessment', 'Delete', 'Apply Changes']);
  });
});
