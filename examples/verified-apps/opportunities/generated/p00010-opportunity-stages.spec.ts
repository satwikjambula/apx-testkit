 ...
 * (./p00010-opportunity-stages.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: opportunity-stages
...
import { OpportunityStagesPage } from './p00010-opportunity-stages.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 10: Opportunity Stages [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new OpportunityStagesPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new OpportunityStagesPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Opportunity Stages'));
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new OpportunityStagesPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Create Opportunity Stage', 'Reset']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new OpportunityStagesPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['opportunity-stages']);
  });
});
