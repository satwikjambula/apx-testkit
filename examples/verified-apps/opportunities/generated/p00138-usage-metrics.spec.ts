 ...
 * (./p00138-usage-metrics.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: calendar, region-toggle, report
 ...
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): calendar (calendar), region-toggle (staticContent).
...
import { UsageMetricsPage } from './p00138-usage-metrics.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 138: Usage Metrics [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new UsageMetricsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new UsageMetricsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Usage Metrics'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (3 items, 2 hidden)', async ({ page }) => {
    const po = new UsageMetricsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P138_DISPLAY_AS', 'P138_ENTITY_ID', 'P138_ENTITY_TYPE']);
  });
