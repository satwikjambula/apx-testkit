 ...
 * (./p00002-accounts.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: account-cards, accounts, accounts-grid, accounts-report, filters, page-settings
 ...
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): account-cards (classicReport), accounts (staticContent), accounts-report (classicReport), filters (staticContent), page-settings (staticContent).
...
import { AccountsPage } from './p00002-accounts.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 2: Accounts [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new AccountsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new AccountsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Accounts'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (5 items, 0 hidden)', async ({ page }) => {
    const po = new AccountsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P2_DISPLAY_AS', 'P2_KEY', 'P2_SEARCH', 'P2_SORT', 'P2_TERRITORY_ID']);
  });

  test('apex.item round-trip on P2_SEARCH', async ({ page }) => {
    const po = new AccountsPage(page);
    await po.goto();
  ...
  test('every labeled .apx button exists (3 buttons)', async ({ page }) => {
    const po = new AccountsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Create Account', 'Reset', 'Reset']);
  });
