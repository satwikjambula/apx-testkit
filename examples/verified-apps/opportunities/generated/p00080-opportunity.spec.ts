 ...
 * (./p00080-opportunity.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, competition, history, opportunity-actions, opportunity-details, products, region-display-selector, team-members, usage-metrics-90-days, validations
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), competition (classicReport), history (plSqlDynamicContent), opportunity-actions (list), opportunity-details (classicReport), products (classicReport), region-display-selector (regionDisplaySelector), team-members (classicReport), usage-metrics-90-days (plugin/badgeList), validations (plugin/contentValidator).
...
import { OpportunityPage } from './p00080-opportunity.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 80: Opportunity [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new OpportunityPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new OpportunityPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Opportunity'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 1 hidden)', async ({ page }) => {
    const po = new OpportunityPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P80_ID']);
  });

  test('every labeled .apx button exists (14 buttons)', async ({ page }) => {
    const po = new OpportunityPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Add Competition', 'Add Link', 'Add Product', 'Add Team Member', 'Add Attachment', 'Add Comment', 'Edit Opportunity', 'Close Won / Lost', 'View Attachments', 'View Comments', 'View Competition', 'View Links', 'View Products', 'View Team']);
  });
});
