/**
 * GENERATED by apx-testgen from pages/p00020-past-due-opportunities.apx — DO NOT EDIT.
 ...
 * (./p00020-past-due-opportunities.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, filters, past-due-opportunities
...
import { PastDueOpportunitiesPage } from './p00020-past-due-opportunities.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 20: Past Due Opportunities [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new PastDueOpportunitiesPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new PastDueOpportunitiesPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Past Due Opportunities'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 0 hidden)', async ({ page }) => {
    const po = new PastDueOpportunitiesPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P20_CLOSING_TIMEFRAME']);
  });
  ...
  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new PastDueOpportunitiesPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['past-due-opportunities']);
  });
});
