 ...
 * (./p00153-add-multiple-users.page.js), not raw testkit calls, so both stay in sync.
...
import { AddMultipleUsersPage } from './p00153-add-multiple-users.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 153: Add Multiple Users [requires auth]', () => {
  ...
  test('every .apx pageItem exists in DOM/apex.item registry (3 items, 1 hidden)', async ({ page }) => {
    const po = new AddMultipleUsersPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P153_PRELIM_USERS', 'P153_ROLE', 'P153_USERNAME_FORMAT1']);
  });

  test('apex.item round-trip on P153_PRELIM_USERS', async ({ page }) => {
    const po = new AddMultipleUsersPage(page);
    await po.goto();
