 ...
 * (./p00001-opportunities.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: filters, opportunities, opportunities-cards, opportunities-grid, opportunities-report, page-settings
 ...
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): filters (staticContent), opportunities (staticContent), opportunities-cards (classicReport), opportunities-report (classicReport), page-settings (staticContent).
...
import { OpportunitiesPage } from './p00001-opportunities.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 1: Opportunities [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new OpportunitiesPage(page);
  ...
  test('every .apx pageItem exists in DOM/apex.item registry (7 items, 0 hidden)', async ({ page }) => {
    const po = new OpportunitiesPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P1_ACCOUNT_ID', 'P1_DISPLAY_AS', 'P1_QUARTER', 'P1_SEARCH', 'P1_SHOW', 'P1_SORT', 'P1_TERRITORY_ID']);
  });
  ...
  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new OpportunitiesPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Create Opportunity', 'Reset', 'Reset', 'Upload Data']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new OpportunitiesPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['grid_region']);
  });
});
