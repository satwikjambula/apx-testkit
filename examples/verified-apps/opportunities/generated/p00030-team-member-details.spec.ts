 ...
 * (./p00030-team-member-details.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, team-member-details
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), team-member-details (staticContent).
...
import { TeamMemberDetailsPage } from './p00030-team-member-details.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 30: Team Member Details [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new TeamMemberDetailsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new TeamMemberDetailsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Team Member Details'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (4 items, 3 hidden)', async ({ page }) => {
    const po = new TeamMemberDetailsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P30_DEAL', 'P30_DEAL_ID', 'P30_ID', 'P30_REP_ID']);
  });

  test('every labeled .apx button exists (3 buttons)', async ({ page }) => {
    const po = new TeamMemberDetailsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Add Team Member', 'Delete']);
  });
});
