 ...
 * (./p00213-data-load-results2.page.js), not raw testkit calls, so both stay in sync.
...
import { DataLoadResults2Page } from './p00213-data-load-results2.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 213: Data Load Results [requires auth]', () => {
  ...
  test('every .apx pageItem exists in DOM/apex.item registry (3 items, 0 hidden)', async ({ page }) => {
    const po = new DataLoadResults2Page(page);
    await po.goto();
    await expectItemsPresent(page, ['P213_ERROR_COUNT', 'P213_INSERT_COUNT', 'P213_UPDATE_COUNT']);
  });
