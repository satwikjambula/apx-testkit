/**
 * GENERATED by apx-testgen from pages/p00064-industries.apx — DO NOT EDIT.
 ...
 * (./p00064-industries.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: industries
...
import { IndustriesPage } from './p00064-industries.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 64: Industries [requires auth]', () => {
  ...
  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new IndustriesPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Create Industry', 'Reset']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new IndustriesPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['industries']);
  });
});
