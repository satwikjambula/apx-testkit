/**
 * GENERATED by apx-testgen from pages/p00006-sales-reps.apx — DO NOT EDIT.
 ...
 * (./p00006-sales-reps.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: rep-title-s
...
import { SalesRepsPage } from './p00006-sales-reps.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 6: &REP_TITLE.s [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new SalesRepsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new SalesRepsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('&REP_TITLE_RAW.s'));
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new SalesRepsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Create &REP_TITLE.', 'Reset']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new SalesRepsPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['rep-title-s']);
  });
});
