 ...
 * (./p00032-location-details.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, location
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), location (staticContent).
...
import { LocationDetailsPage } from './p00032-location-details.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 32: Location Details [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new LocationDetailsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new LocationDetailsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Location Details'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (11 items, 3 hidden)', async ({ page }) => {
    const po = new LocationDetailsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P32_CUSTOMER_ADDRESS1', 'P32_CUSTOMER_ADDRESS2', 'P32_CUSTOMER_CITY', 'P32_CUSTOMER_COUNTRY', 'P32_CUSTOMER_DESCRIPTION', 'P32_CUSTOMER_ID', 'P32_CUSTOMER_LOCATION_NAME', 'P32_CUSTOMER_POSTAL_CODE', 'P32_CUSTOMER_STATE', 'P32_CUSTOMER_TERRITORY_ID', 'P32_ID']);
  });

  test('apex.item round-trip on P32_CUSTOMER_ADDRESS1', async ({ page }) => {
    const po = new LocationDetailsPage(page);
    await po.goto();
    await po.customerAddress1.setValue('apx-testgen');
    expect(await po.customerAddress1.getValue()).toBe('apx-testgen');
  });

  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new LocationDetailsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Create Location', 'Delete', 'Apply Changes']);
  });
});
