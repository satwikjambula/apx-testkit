 ...
 * (./p00022-comment-details.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, comment
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), comment (staticContent).
...
import { CommentDetailsPage } from './p00022-comment-details.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 22: Comment Details [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new CommentDetailsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new CommentDetailsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Comment Details'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (10 items, 9 hidden)', async ({ page }) => {
    const po = new CommentDetailsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P22_ACCOUNT_ID', 'P22_CONTACT_ID', 'P22_ENTITY_ID', 'P22_ENTITY_TYPE', 'P22_ID', 'P22_LEAD_ID', 'P22_NOTE', 'P22_OPPORTUNITY_ID', 'P22_PRODUCT_ID', 'P22_TERRITORY_ID']);
  });

  test('apex.item round-trip on P22_NOTE', async ({ page }) => {
    const po = new CommentDetailsPage(page);
    await po.goto();
  ...
  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new CommentDetailsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Add Comment', 'Delete', 'Apply Changes']);
  });
});
