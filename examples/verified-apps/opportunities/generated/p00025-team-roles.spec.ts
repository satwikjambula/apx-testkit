 ...
 * (./p00025-team-roles.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: rep-title-roles
...
import { TeamRolesPage } from './p00025-team-roles.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 25: Team Roles [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new TeamRolesPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new TeamRolesPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Team Roles'));
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new TeamRolesPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Create Team Role', 'Reset']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new TeamRolesPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['rep-title-roles']);
  });
});
