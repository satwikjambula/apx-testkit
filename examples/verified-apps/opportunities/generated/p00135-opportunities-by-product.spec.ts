 ...
 * (./p00135-opportunities-by-product.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, hidden-items, opportunities-by-product
 ...
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), hidden-items (staticContent).
...
import { OpportunitiesByProductPage } from './p00135-opportunities-by-product.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 135: Opportunities by Product [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new OpportunitiesByProductPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new OpportunitiesByProductPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Opportunities by Product'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 1 hidden)', async ({ page }) => {
    const po = new OpportunitiesByProductPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P135_HELP']);
  });
