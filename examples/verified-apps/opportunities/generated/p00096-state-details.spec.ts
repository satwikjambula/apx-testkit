 ...
 * (./p00096-state-details.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, state-details
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), state-details (staticContent).
...
import { StateDetailsPage } from './p00096-state-details.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 96: State Details [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new StateDetailsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new StateDetailsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('State Details'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (4 items, 1 hidden)', async ({ page }) => {
    const po = new StateDetailsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P96_CODE', 'P96_COUNTRY', 'P96_ID', 'P96_STATE']);
  });

  test('apex.item round-trip on P96_CODE', async ({ page }) => {
    const po = new StateDetailsPage(page);
    await po.goto();
  ...
  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new StateDetailsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Create State', 'Delete', 'Apply Changes']);
  });
});
