 ...
 * (./p00003-account-details.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: account, buttons, classification
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): account (staticContent), buttons (staticContent), classification (staticContent).
...
import { AccountDetailsPage } from './p00003-account-details.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 3: Account Details [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new AccountDetailsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new AccountDetailsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Account Details'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (16 items, 1 hidden)', async ({ page }) => {
    const po = new AccountDetailsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P3_CUSTOMER_DESCRIPTION', 'P3_CUSTOMER_DUNS', 'P3_CUSTOMER_FACEBOOK', 'P3_CUSTOMER_IS_KEY_ACCOUNT_YN', 'P3_CUSTOMER_LINKEDIN', 'P3_CUSTOMER_NAME', 'P3_CUSTOMER_SIC', 'P3_CUSTOMER_STOCK_SYMB', 'P3_CUSTOMER_TERRITORY_ID', 'P3_CUSTOMER_TWITTER', 'P3_CUSTOMER_WEB_SITE', 'P3_DEFAULT_REP_ID', 'P3_ID', 'P3_INDUSTRY_ID', 'P3_SALES_LEADER', 'P3_TAGS']);
  });

  test('apex.item round-trip on P3_CUSTOMER_DESCRIPTION', async ({ page }) => {
    const po = new AccountDetailsPage(page);
    await po.goto();
  ...
  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new AccountDetailsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Create Account', 'Delete', 'Apply Changes']);
  });
});
