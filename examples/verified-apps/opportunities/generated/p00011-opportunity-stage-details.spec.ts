 ...
 * (./p00011-opportunity-stage-details.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, opportunity-stage-details
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), opportunity-stage-details (staticContent).
...
import { OpportunityStageDetailsPage } from './p00011-opportunity-stage-details.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 11: Opportunity Stage Details [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new OpportunityStageDetailsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new OpportunityStageDetailsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Opportunity Stage Details'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (4 items, 1 hidden)', async ({ page }) => {
    const po = new OpportunityStageDetailsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P11_CODE_DESCRIPTION', 'P11_CORRESPONDING_PROB_PCT', 'P11_ID', 'P11_STATUS_CODE']);
  });

  test('apex.item round-trip on P11_CODE_DESCRIPTION', async ({ page }) => {
    const po = new OpportunityStageDetailsPage(page);
    await po.goto();
    await po.codeDescription.setValue('apx-testgen');
    expect(await po.codeDescription.getValue()).toBe('apx-testgen');
  });

  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new OpportunityStageDetailsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Create Step to Close', 'Delete', 'Apply Changes']);
  });
});
