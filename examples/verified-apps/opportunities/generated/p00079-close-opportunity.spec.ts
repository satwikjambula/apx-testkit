/**
 * GENERATED by apx-testgen from pages/p00079-close-opportunity.apx — DO NOT EDIT.
 ...
 * (./p00079-close-opportunity.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, button-bar, opportunity-details
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), button-bar (staticContent), opportunity-details (classicReport).
...
import { CloseOpportunityPage } from './p00079-close-opportunity.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 79: Close Opportunity [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new CloseOpportunityPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new CloseOpportunityPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Close Opportunity'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 1 hidden)', async ({ page }) => {
    const po = new CloseOpportunityPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P79_ID']);
  });

  test('every labeled .apx button exists (3 buttons)', async ({ page }) => {
    const po = new CloseOpportunityPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Opportunity Lost', 'Oppotunity Won']);
  });
});
