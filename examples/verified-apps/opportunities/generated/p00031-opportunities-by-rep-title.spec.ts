 ...
 * (./p00031-opportunities-by-rep-title.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, opportunities-by-rep
...
import { OpportunitiesByRepTitlePage } from './p00031-opportunities-by-rep-title.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 31: Opportunities by &REP_TITLE. [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new OpportunitiesByRepTitlePage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new OpportunitiesByRepTitlePage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Opportunities by &REP_TITLE_RAW.'));
  });
  ...
  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new OpportunitiesByRepTitlePage(page);
    await po.goto();
    await expectRegionsResolve(page, ['opportunities-by-rep']);
  });
});
