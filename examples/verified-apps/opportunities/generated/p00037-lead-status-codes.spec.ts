/**
 * GENERATED by apx-testgen from pages/p00037-lead-status-codes.apx — DO NOT EDIT.
 ...
 * (./p00037-lead-status-codes.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: lead-status-codes
...
import { LeadStatusCodesPage } from './p00037-lead-status-codes.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 37: Lead Status Codes [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new LeadStatusCodesPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new LeadStatusCodesPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Lead Status Codes'));
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new LeadStatusCodesPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Create Lead Status Code', 'Reset']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new LeadStatusCodesPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['lead-status-codes']);
  });
});
