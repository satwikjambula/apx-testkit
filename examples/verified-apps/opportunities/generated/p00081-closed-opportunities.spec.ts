/**
 * GENERATED by apx-testgen from pages/p00081-closed-opportunities.apx — DO NOT EDIT.
 ...
 * (./p00081-closed-opportunities.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: closed-opportunities
...
import { ClosedOpportunitiesPage } from './p00081-closed-opportunities.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 81: Closed Opportunities [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ClosedOpportunitiesPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ClosedOpportunitiesPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Closed Opportunities'));
  });
  ...
  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new ClosedOpportunitiesPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['closed-opportunities']);
  });
});
