 ...
 * (./p00005-product-details2.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, product
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), product (staticContent).
...
import { ProductDetails2Page } from './p00005-product-details2.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 5: Product Details [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ProductDetails2Page(page);
  ...
  test('every .apx pageItem exists in DOM/apex.item registry (7 items, 1 hidden)', async ({ page }) => {
    const po = new ProductDetails2Page(page);
    await po.goto();
    await expectItemsPresent(page, ['P5_ID', 'P5_PRODUCT_DESCRIPTION', 'P5_PRODUCT_FAMILY_ID', 'P5_PRODUCT_NAME', 'P5_PRODUCT_SKU', 'P5_SOLD_AS', 'P5_TAGS']);
  });

  test('apex.item round-trip on P5_PRODUCT_DESCRIPTION', async ({ page }) => {
    const po = new ProductDetails2Page(page);
    await po.goto();
    await po.productDescription.setValue('apx-testgen');
    expect(await po.productDescription.getValue()).toBe('apx-testgen');
  });

  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new ProductDetails2Page(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Create Product', 'Delete', 'Apply Changes']);
  });
});
