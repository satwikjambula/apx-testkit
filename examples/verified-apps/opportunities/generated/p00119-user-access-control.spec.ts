 ...
 * (./p00119-user-access-control.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, form-items, user-details
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), form-items (staticContent), user-details (staticContent).
...
import { UserAccessControlPage } from './p00119-user-access-control.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 119: "User Access Control  " [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new UserAccessControlPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new UserAccessControlPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('"User Access Control  "'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (4 items, 1 hidden)', async ({ page }) => {
    const po = new UserAccessControlPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P119_ACCESS_LEVEL_ID', 'P119_ACCOUNT_LOCKED', 'P119_ID', 'P119_USERNAME']);
  });

  test('apex.item round-trip on P119_USERNAME', async ({ page }) => {
    const po = new UserAccessControlPage(page);
    await po.goto();
  ...
  test('every labeled .apx button exists (5 buttons)', async ({ page }) => {
    const po = new UserAccessControlPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Add User', 'Add and Add Another', 'Delete', 'Apply Changes']);
  });
});
