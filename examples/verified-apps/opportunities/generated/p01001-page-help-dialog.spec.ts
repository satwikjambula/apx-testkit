 ...
 * (./p01001-page-help-dialog.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: about-this-page, buttons
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): about-this-page (plSqlDynamicContent), buttons (staticContent).
...
import { PageHelpDialogPage } from './p01001-page-help-dialog.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 1001: Page Help Dialog [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new PageHelpDialogPage(page);
  ...
  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 1 hidden)', async ({ page }) => {
    const po = new PageHelpDialogPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P1001_PAGE_ID']);
  });

  test('every labeled .apx button exists (1 button)', async ({ page }) => {
    const po = new PageHelpDialogPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Learn More about &APP_TITLE.']);
  });
});
