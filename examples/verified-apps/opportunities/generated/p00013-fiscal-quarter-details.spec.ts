 ...
 * (./p00013-fiscal-quarter-details.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, fiscal-quarter-details
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), fiscal-quarter-details (staticContent).
...
import { FiscalQuarterDetailsPage } from './p00013-fiscal-quarter-details.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 13: Fiscal Quarter Details [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new FiscalQuarterDetailsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new FiscalQuarterDetailsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Fiscal Quarter Details'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (5 items, 1 hidden)', async ({ page }) => {
    const po = new FiscalQuarterDetailsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P13_FIRST_DAY', 'P13_FISCAL_YEAR', 'P13_ID', 'P13_LAST_DAY', 'P13_PERIOD_NAME']);
  });

  test('apex.item round-trip on P13_FISCAL_YEAR', async ({ page }) => {
    const po = new FiscalQuarterDetailsPage(page);
    await po.goto();
    await po.fiscalYear.setValue('apx-testgen');
    expect(await po.fiscalYear.getValue()).toBe('apx-testgen');
  });

  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new FiscalQuarterDetailsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Create Fiscal Quarter', 'Delete', 'Apply Changes']);
  });
});
