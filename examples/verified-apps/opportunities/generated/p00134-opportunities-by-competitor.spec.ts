/**
 * GENERATED by apx-testgen from pages/p00134-opportunities-by-competitor.apx — DO NOT EDIT.
 ...
 * (./p00134-opportunities-by-competitor.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, opportunities-by-competitor
...
import { OpportunitiesByCompetitorPage } from './p00134-opportunities-by-competitor.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 134: Opportunities by Competitor [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new OpportunitiesByCompetitorPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new OpportunitiesByCompetitorPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Opportunities by Competitor'));
  });
