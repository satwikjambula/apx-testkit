/**
 * GENERATED by apx-testgen from pages/p00083-states.apx — DO NOT EDIT.
 ...
 * (./p00083-states.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: states
...
import { StatesPage } from './p00083-states.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 83: States [requires auth]', () => {
  ...
  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new StatesPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Create State', 'Reset']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new StatesPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['states']);
  });
});
