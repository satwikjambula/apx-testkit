/**
 * GENERATED by apx-testgen from pages/p00116-search-results.apx — DO NOT EDIT.
 ...
 * (./p00116-search-results.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, filters, search-results
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), filters (staticContent), search-results (classicReport).
...
import { SearchResultsPage } from './p00116-search-results.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 116: Search Results [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new SearchResultsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new SearchResultsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Search Results'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (2 items, 0 hidden)', async ({ page }) => {
    const po = new SearchResultsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P116_INCLUDE', 'P116_SEARCH']);
  });

  test('apex.item round-trip on P116_SEARCH', async ({ page }) => {
    const po = new SearchResultsPage(page);
    await po.goto();
  ...
  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new SearchResultsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Reset', 'Search']);
  });
});
