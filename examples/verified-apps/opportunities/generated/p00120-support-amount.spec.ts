 ...
 * (./p00120-support-amount.page.js), not raw testkit calls, so both stay in sync.
...
import { SupportAmountPage } from './p00120-support-amount.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 120: Support Amount [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new SupportAmountPage(page);
  ...
  test('every .apx pageItem exists in DOM/apex.item registry (4 items, 1 hidden)', async ({ page }) => {
    const po = new SupportAmountPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P120_AMOUNT_TYPE_ID', 'P120_DESCRIPTION', 'P120_ID', 'P120_NAME']);
  });

  test('apex.item round-trip on P120_DESCRIPTION', async ({ page }) => {
    const po = new SupportAmountPage(page);
    await po.goto();
