/**
 * GENERATED by apx-testgen from pages/p00111-currencies.apx — DO NOT EDIT.
 ...
 * (./p00111-currencies.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: currencies
...
import { CurrenciesPage } from './p00111-currencies.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 111: Currencies [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new CurrenciesPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new CurrenciesPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Currencies'));
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new CurrenciesPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Create Currency', 'Reset']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new CurrenciesPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['currencies']);
  });
});
