 ...
 * (./p00057-financial-assessments.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: financial-assessments
...
import { FinancialAssessmentsPage } from './p00057-financial-assessments.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 57: Financial Assessments [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new FinancialAssessmentsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new FinancialAssessmentsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Financial Assessments'));
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new FinancialAssessmentsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Create Financial Assessment', 'Reset']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new FinancialAssessmentsPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['financial-assessments']);
  });
});
