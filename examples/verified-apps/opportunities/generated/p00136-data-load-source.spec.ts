 ...
 * (./p00136-data-load-source.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, data-load-source, data-load-wizard-progress
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), data-load-source (staticContent), data-load-wizard-progress (list).
...
import { DataLoadSourcePage } from './p00136-data-load-source.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 136: Data Load Source [requires auth]', () => {
  ...
  test('every .apx pageItem exists in DOM/apex.item registry (12 items, 0 hidden)', async ({ page }) => {
    const po = new DataLoadSourcePage(page);
    await po.goto();
    await expectItemsPresent(page, ['P136_APP_DATE_FORMAT', 'P136_CHAR_SET', 'P136_COPY_PASTE', 'P136_CURRENCY', 'P136_DECIMAL_CHARACTER', 'P136_ENCLOSED_BY', 'P136_FILE_NAME', 'P136_FIRST_ROW', 'P136_GROUP_SEPARATOR', 'P136_IMPORT_FROM', 'P136_SEPARATOR', 'P136_SHOW_ADV_SETTINGS']);
  });

  test('apex.item round-trip on P136_COPY_PASTE', async ({ page }) => {
    const po = new DataLoadSourcePage(page);
    await po.goto();
