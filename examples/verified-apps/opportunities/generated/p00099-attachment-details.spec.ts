 ...
 * (./p00099-attachment-details.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: attachment, buttons
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): attachment (staticContent), buttons (staticContent).
...
import { AttachmentDetailsPage } from './p00099-attachment-details.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 99: Attachment Details [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new AttachmentDetailsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new AttachmentDetailsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Attachment Details'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (14 items, 10 hidden)', async ({ page }) => {
    const po = new AttachmentDetailsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P99_ACCOUNT_ID', 'P99_CONTACT_ID', 'P99_ENTITY_ID', 'P99_ENTITY_TYPE', 'P99_FILENAME', 'P99_FILE_BLOB', 'P99_FILE_COMMENTS', 'P99_FILE_MIMETYPE', 'P99_ID', 'P99_LEAD_ID', 'P99_OPPORTUNITY_ID', 'P99_PRODUCT_ID', 'P99_TAGS', 'P99_TERRITORY_ID']);
  });

  test('apex.item round-trip on P99_FILE_COMMENTS', async ({ page }) => {
    const po = new AttachmentDetailsPage(page);
    await po.goto();
    await po.fileComments.setValue('apx-testgen');
    expect(await po.fileComments.getValue()).toBe('apx-testgen');
  });

  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new AttachmentDetailsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Add Attachment', 'Delete', 'Apply Changes']);
  });
});
