/**
 * GENERATED by apx-testgen from pages/p00024-contact.apx — DO NOT EDIT.
 ...
 * (./p00024-contact.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, contact-actions, contact-details, rds, usage-metrics-90-days, validations
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), contact-actions (list), contact-details (classicReport), rds (regionDisplaySelector), usage-metrics-90-days (plugin/badgeList), validations (plugin/contentValidator).
...
import { ContactPage } from './p00024-contact.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 24: Contact [requires auth]', () => {
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ContactPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Contact'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 1 hidden)', async ({ page }) => {
    const po = new ContactPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P24_ID']);
  });

  test('every labeled .apx button exists (7 buttons)', async ({ page }) => {
    const po = new ContactPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Edit Contact', 'Add Link', 'Add Attachment', 'Add Comment', 'View Attachments', 'View Comments', 'View Links']);
  });
});
