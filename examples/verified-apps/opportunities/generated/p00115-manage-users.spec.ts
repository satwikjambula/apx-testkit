 ...
 * (./p00115-manage-users.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: manage-users
...
import { ManageUsersPage } from './p00115-manage-users.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 115: Manage Users [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ManageUsersPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ManageUsersPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Manage Users'));
  });
  ...
  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new ManageUsersPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['manage-users']);
  });
});
