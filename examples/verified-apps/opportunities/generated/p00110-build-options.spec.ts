 ...
 * (./p00110-build-options.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, buttons-2, feature-configuration
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), buttons-2 (staticContent), feature-configuration (classicReport).
...
import { BuildOptionsPage } from './p00110-build-options.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 110: Build Options [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new BuildOptionsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new BuildOptionsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Build Options'));
  });
