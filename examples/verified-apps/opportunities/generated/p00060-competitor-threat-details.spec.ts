 ...
 * (./p00060-competitor-threat-details.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, competitor-threat-details
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), competitor-threat-details (staticContent).
...
import { CompetitorThreatDetailsPage } from './p00060-competitor-threat-details.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 60: Competitor Threat Details [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new CompetitorThreatDetailsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new CompetitorThreatDetailsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Competitor Threat Details'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (3 items, 1 hidden)', async ({ page }) => {
    const po = new CompetitorThreatDetailsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P60_COMPETITOR_THREAT', 'P60_DISPLAY_ORDER', 'P60_ID']);
  });

  test('apex.item round-trip on P60_COMPETITOR_THREAT', async ({ page }) => {
    const po = new CompetitorThreatDetailsPage(page);
    await po.goto();
    await po.competitorThreat.setValue('apx-testgen');
    expect(await po.competitorThreat.getValue()).toBe('apx-testgen');
  });

  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new CompetitorThreatDetailsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Create Competitor Threat', 'Delete', 'Apply Changes']);
  });
});
