/**
 * GENERATED by apx-testgen from pages/p00086-competition.apx — DO NOT EDIT.
 ...
 * (./p00086-competition.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, competition
...
import { CompetitionPage } from './p00086-competition.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 86: Competition [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new CompetitionPage(page);
  ...
  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 1 hidden)', async ({ page }) => {
    const po = new CompetitionPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P86_ID']);
  });
