/**
 * GENERATED by apx-testgen from pages/p00021-new-user-details.apx — DO NOT EDIT.
 ...
 * (./p00021-new-user-details.page.js), not raw testkit calls, so both stay in sync.
...
import { NewUserDetailsPage } from './p00021-new-user-details.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 21: New User Details [requires auth]', () => {
  ...
  test('every .apx pageItem exists in DOM/apex.item registry (3 items, 1 hidden)', async ({ page }) => {
    const po = new NewUserDetailsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P21_ACCESS_LEVEL_ID', 'P21_SEQUENCE', 'P21_USERNAME']);
  });

  test('apex.item round-trip on P21_USERNAME', async ({ page }) => {
    const po = new NewUserDetailsPage(page);
    await po.goto();
