 ...
 * (./p00019-leads.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: filters, hidden, leads, leads-cards, leads-grid, leads-report, page-settings
 ...
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): filters (staticContent), hidden (staticContent), leads (staticContent), leads-cards (classicReport), leads-report (classicReport), page-settings (staticContent).
...
import { LeadsPage } from './p00019-leads.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 19: Leads [requires auth]', () => {
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new LeadsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Leads'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (7 items, 0 hidden)', async ({ page }) => {
    const po = new LeadsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P19_ACCOUNT_ID', 'P19_DISPLAY_AS', 'P19_SEARCH', 'P19_SORT', 'P19_SOURCE_ID', 'P19_STATUS_ID', 'P19_TERRITORY_ID']);
  });

  test('apex.item round-trip on P19_SEARCH', async ({ page }) => {
    const po = new LeadsPage(page);
    await po.goto();
  ...
  test('every labeled .apx button exists (3 buttons)', async ({ page }) => {
    const po = new LeadsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Create Lead', 'Reset', 'Reset']);
  });
