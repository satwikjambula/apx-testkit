 ...
 * (./p00062-pipeline.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, button-bar, weighted-pipeline-by-opportunity, weighted-pipeline-by-territory
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), button-bar (staticContent), weighted-pipeline-by-opportunity (plugin/html5BarChart), weighted-pipeline-by-territory (plugin/html5BarChart).
...
import { PipelinePage } from './p00062-pipeline.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 62: Pipeline [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new PipelinePage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new PipelinePage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Pipeline'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (2 items, 0 hidden)', async ({ page }) => {
    const po = new PipelinePage(page);
    await po.goto();
    await expectItemsPresent(page, ['P62_SEARCH', 'P62_TERRITORY']);
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new PipelinePage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Go', 'Reset']);
  });
});
