/**
 * GENERATED by apx-testgen from pages/p00068-products.apx — DO NOT EDIT.
 ...
 * (./p00068-products.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: filters, page-settings, product-cards, products, products-grid, products-report
 ...
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): filters (staticContent), page-settings (staticContent), product-cards (classicReport), products (staticContent), products-report (classicReport).
...
import { ProductsPage } from './p00068-products.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 68: Products [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ProductsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ProductsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Products'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (5 items, 0 hidden)', async ({ page }) => {
    const po = new ProductsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P68_ACCOUNT_ID', 'P68_DISPLAY_AS', 'P68_QUARTER', 'P68_SEARCH', 'P68_SORT']);
  });

  test('apex.item round-trip on P68_SEARCH', async ({ page }) => {
    const po = new ProductsPage(page);
    await po.goto();
  ...
  test('every labeled .apx button exists (3 buttons)', async ({ page }) => {
    const po = new ProductsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Manage Products', 'Reset', 'Reset']);
  });
