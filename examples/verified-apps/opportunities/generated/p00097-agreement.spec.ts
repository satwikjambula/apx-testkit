/**
 * GENERATED by apx-testgen from pages/p00097-agreement.apx — DO NOT EDIT.
 ...
 * (./p00097-agreement.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: agreement, buttons
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): agreement (staticContent), buttons (staticContent).
...
import { AgreementPage } from './p00097-agreement.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 97: Agreement [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new AgreementPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new AgreementPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Agreement'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (4 items, 1 hidden)', async ({ page }) => {
    const po = new AgreementPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P97_AGREEMENT_TYPE_ID', 'P97_DESCRIPTION', 'P97_ID', 'P97_NAME']);
  });

  test('apex.item round-trip on P97_DESCRIPTION', async ({ page }) => {
    const po = new AgreementPage(page);
    await po.goto();
