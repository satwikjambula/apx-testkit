 ...
 * (./p00150-line-of-business.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, line-of-business
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), line-of-business (staticContent).
...
import { LineOfBusinessPage } from './p00150-line-of-business.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 150: Line of Business [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new LineOfBusinessPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new LineOfBusinessPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Line of Business'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (3 items, 1 hidden)', async ({ page }) => {
    const po = new LineOfBusinessPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P150_DESCRIPTION', 'P150_ID', 'P150_NAME']);
  });

  test('apex.item round-trip on P150_DESCRIPTION', async ({ page }) => {
    const po = new LineOfBusinessPage(page);
    await po.goto();
  ...
  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new LineOfBusinessPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Add Line of Business', 'Delete', 'Apply Changes']);
  });
});
