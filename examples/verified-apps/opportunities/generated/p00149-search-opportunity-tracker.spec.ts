 ...
 * (./p00149-search-opportunity-tracker.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: search
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): search (staticContent).
...
import { SearchOpportunityTrackerPage } from './p00149-search-opportunity-tracker.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 149: Search Opportunities [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new SearchOpportunityTrackerPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new SearchOpportunityTrackerPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Search Opportunities'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 0 hidden)', async ({ page }) => {
    const po = new SearchOpportunityTrackerPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P149_SEARCH']);
  });

  test('apex.item round-trip on P149_SEARCH', async ({ page }) => {
    const po = new SearchOpportunityTrackerPage(page);
    await po.goto();
  ...
  test('every labeled .apx button exists (1 button)', async ({ page }) => {
    const po = new SearchOpportunityTrackerPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Search']);
  });
});
