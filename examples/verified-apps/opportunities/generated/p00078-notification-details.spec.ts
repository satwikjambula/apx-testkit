 ...
 * (./p00078-notification-details.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, form-items, notification-details
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), form-items (staticContent), notification-details (staticContent).
...
import { NotificationDetailsPage } from './p00078-notification-details.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 78: Notification Details [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new NotificationDetailsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new NotificationDetailsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Notification Details'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (7 items, 1 hidden)', async ({ page }) => {
    const po = new NotificationDetailsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P78_DISPLAY_FROM', 'P78_DISPLAY_SEQUENCE', 'P78_DISPLAY_UNTIL', 'P78_ID', 'P78_NOTIFICATION_DESCRIPTION', 'P78_NOTIFICATION_NAME', 'P78_NOTIFICATION_TYPE']);
  });

  test('apex.item round-trip on P78_DISPLAY_SEQUENCE', async ({ page }) => {
    const po = new NotificationDetailsPage(page);
    await po.goto();
    await po.displaySequence.setValue('apx-testgen');
    expect(await po.displaySequence.getValue()).toBe('apx-testgen');
  });

  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new NotificationDetailsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Create Notification', 'Delete', 'Apply Changes']);
  });
});
