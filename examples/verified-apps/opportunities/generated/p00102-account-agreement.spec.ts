 ...
 * (./p00102-account-agreement.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: account-agreement, buttons
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): account-agreement (staticContent), buttons (staticContent).
...
import { AccountAgreementPage } from './p00102-account-agreement.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 102: Account Agreement [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new AccountAgreementPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new AccountAgreementPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Account Agreement'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (8 items, 2 hidden)', async ({ page }) => {
    const po = new AccountAgreementPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P102_AGREEMENT_ID', 'P102_CLOSE_DATE', 'P102_CUSTOMER_ID', 'P102_DESCRIPTION', 'P102_ID', 'P102_NAME', 'P102_QUOTE_PRICE', 'P102_TERM_ID']);
  });

  test('apex.item round-trip on P102_DESCRIPTION', async ({ page }) => {
    const po = new AccountAgreementPage(page);
    await po.goto();
  ...
  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new AccountAgreementPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Add Agreement', 'Delete', 'Apply Changes']);
  });
});
