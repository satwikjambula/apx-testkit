/**
 * GENERATED by apx-testgen from pages/p00133-lead.apx — DO NOT EDIT.
 ...
 * (./p00133-lead.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, lead-actions, lead-details, region-display-selector, usage-metrics-90-days, validations
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), lead-actions (list), lead-details (classicReport), region-display-selector (regionDisplaySelector), usage-metrics-90-days (plugin/badgeList), validations (plugin/contentValidator).
...
import { LeadPage } from './p00133-lead.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 133: Lead [requires auth]', () => {
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new LeadPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Lead'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 1 hidden)', async ({ page }) => {
    const po = new LeadPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P133_ID']);
  });

  test('every labeled .apx button exists (8 buttons)', async ({ page }) => {
    const po = new LeadPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Convert Lead', 'Edit Lead', 'Add Link', 'Add Attachment', 'Add Comment', 'View Attachments', 'View Comments', 'View Links']);
  });
});
