 ...
 * (./p00088-comments.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: comments
...
import { CommentsPage } from './p00088-comments.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 88: Comments [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new CommentsPage(page);
  ...
  test('every .apx pageItem exists in DOM/apex.item registry (3 items, 3 hidden)', async ({ page }) => {
    const po = new CommentsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P88_DELETE_ID', 'P88_ENTITY_ID', 'P88_ENTITY_TYPE']);
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new CommentsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Add Comment', 'Reset']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new CommentsPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['comments']);
  });
});
