/**
 * GENERATED by apx-testgen from pages/p00034-product-family.apx — DO NOT EDIT.
 ...
 * (./p00034-product-family.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, product-family
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), product-family (staticContent).
...
import { ProductFamilyPage } from './p00034-product-family.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 34: Product Family [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ProductFamilyPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ProductFamilyPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Product Family'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (4 items, 1 hidden)', async ({ page }) => {
    const po = new ProductFamilyPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P34_DESCRIPTION', 'P34_ID', 'P34_LOB_ID', 'P34_PRODUCT_FAMILY']);
  });

  test('apex.item round-trip on P34_DESCRIPTION', async ({ page }) => {
    const po = new ProductFamilyPage(page);
    await po.goto();
