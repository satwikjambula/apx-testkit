 ...
 * (./p00052-competitor-details.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, competitor-details
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), competitor-details (staticContent).
...
import { CompetitorDetailsPage } from './p00052-competitor-details.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 52: Competitor Details [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new CompetitorDetailsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new CompetitorDetailsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Competitor Details'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (12 items, 1 hidden)', async ({ page }) => {
    const po = new CompetitorDetailsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P52_COMPETITOR_DESC', 'P52_COMPETITOR_NAME', 'P52_ID', 'P52_TAGS', 'P52_URL_1', 'P52_URL_1_NAME', 'P52_URL_2', 'P52_URL_2_NAME', 'P52_URL_3', 'P52_URL_3_NAME', 'P52_URL_4', 'P52_URL_4_NAME']);
  });

  test('apex.item round-trip on P52_COMPETITOR_DESC', async ({ page }) => {
    const po = new CompetitorDetailsPage(page);
    await po.goto();
    await po.competitorDesc.setValue('apx-testgen');
    expect(await po.competitorDesc.getValue()).toBe('apx-testgen');
  });

  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new CompetitorDetailsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Create Competitor', 'Delete', 'Apply Changes']);
  });
});
