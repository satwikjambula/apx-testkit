/**
 * GENERATED by apx-testgen from pages/p00066-svps.apx — DO NOT EDIT.
 ...
 * (./p00066-svps.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: svps
...
import { SvpsPage } from './p00066-svps.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 66: SVPs [requires auth]', () => {
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new SvpsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('SVPs'));
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new SvpsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Create SVP', 'Reset']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new SvpsPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['svps']);
  });
});
