 ...
 * (./p00094-account.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: account, account-actions, agreements, breadcrumb, competition, contacts, locations, open-opportunities, rds, support-amounts, usage-metrics-90-days, validations
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): account (classicReport), account-actions (list), agreements (classicReport), breadcrumb (breadcrumb), competition (classicReport), contacts (classicReport), locations (classicReport), open-opportunities (classicReport), rds (regionDisplaySelector), support-amounts (classicReport), usage-metrics-90-days (plugin/badgeList), validations (plugin/contentValidator).
...
import { AccountPage } from './p00094-account.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 94: Account [requires auth]', () => {
  ...
  test('every .apx pageItem exists in DOM/apex.item registry (3 items, 3 hidden)', async ({ page }) => {
    const po = new AccountPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P94_ID', 'P94_ROW_KEY', 'P94_TERRITORY_ID']);
  });

  test('every labeled .apx button exists (18 buttons)', async ({ page }) => {
    const po = new AccountPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Add Agreement', 'Add Contact', 'Add Support Amount', 'Edit Account', 'Add Competition', 'Add Location', 'Add Link', 'Add Attachment', 'Add Comment', 'View Agreements', 'View Support Amounts', 'View Attachments', 'View Comments', 'View Competitors', 'View Contacts', 'View Links', 'View Locations', 'View Open Opportunities']);
  });
});
