 ...
 * (./p00095-financial-assessment-details.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, financial-assessment-details
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), financial-assessment-details (staticContent).
...
import { FinancialAssessmentDetailsPage } from './p00095-financial-assessment-details.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 95: Financial Assessment Details [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new FinancialAssessmentDetailsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new FinancialAssessmentDetailsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Financial Assessment Details'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (4 items, 1 hidden)', async ({ page }) => {
    const po = new FinancialAssessmentDetailsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P95_ASSESSMENT_DESCRIPTION', 'P95_ASSESSMENT_TEXT', 'P95_DISPLAY_ORDER', 'P95_ID']);
  });

  test('apex.item round-trip on P95_ASSESSMENT_DESCRIPTION', async ({ page }) => {
    const po = new FinancialAssessmentDetailsPage(page);
    await po.goto();
    await po.assessmentDescription.setValue('apx-testgen');
    expect(await po.assessmentDescription.getValue()).toBe('apx-testgen');
  });

  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new FinancialAssessmentDetailsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Create Financial Assessment', 'Delete', 'Apply Changes']);
  });
});
