 ...
 * (./p00123-territory-countries.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, countries
...
import { TerritoryCountriesPage } from './p00123-territory-countries.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 123: Territory Countries [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new TerritoryCountriesPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new TerritoryCountriesPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Countries'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (2 items, 2 hidden)', async ({ page }) => {
    const po = new TerritoryCountriesPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P123_TERRITORY_COUNTRY_ID', 'P123_TERRITORY_ID']);
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new TerritoryCountriesPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Add Country', 'Reset']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new TerritoryCountriesPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['countries']);
  });
});
