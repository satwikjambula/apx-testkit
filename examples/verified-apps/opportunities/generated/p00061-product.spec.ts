 ...
 * (./p00061-product.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, product, product-actions, rds, summary, usage-metrics-90-days, validations
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), product (classicReport), product-actions (list), rds (regionDisplaySelector), summary (plugin/badgeList), usage-metrics-90-days (plugin/badgeList), validations (plugin/contentValidator).
...
import { ProductPage } from './p00061-product.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 61: Product [requires auth]', () => {
  ...
  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 1 hidden)', async ({ page }) => {
    const po = new ProductPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P61_ID']);
  });

  test('every labeled .apx button exists (6 buttons)', async ({ page }) => {
    const po = new ProductPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Add Link', 'Add Attachment', 'Add Comment', 'View Attachments', 'View Comments', 'View Links']);
  });
});
