/**
 * GENERATED by apx-testgen from pages/p00044-page-privileges.apx — DO NOT EDIT.
 ...
 * (./p00044-page-privileges.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: application-page-privileges
...
import { PagePrivilegesPage } from './p00044-page-privileges.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 44: Page Privileges [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new PagePrivilegesPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new PagePrivilegesPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Page Privileges'));
  });
  ...
  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new PagePrivilegesPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['application-page-privileges']);
  });
});
