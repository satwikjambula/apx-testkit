/**
 * GENERATED by apx-testgen from pages/p00144-lines-of-business.apx — DO NOT EDIT.
 ...
 * (./p00144-lines-of-business.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: lines-of-business
...
import { LinesOfBusinessPage } from './p00144-lines-of-business.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 144: Lines of Business [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new LinesOfBusinessPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new LinesOfBusinessPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Lines of Business'));
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new LinesOfBusinessPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Add Line of Business', 'Reset Report']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new LinesOfBusinessPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['lines-of-business']);
  });
});
