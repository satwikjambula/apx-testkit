 ...
 * (./p00038-lead-status-code-details.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, lead-status-code
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), lead-status-code (staticContent).
...
import { LeadStatusCodeDetailsPage } from './p00038-lead-status-code-details.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 38: Lead Status Code Details [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new LeadStatusCodeDetailsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new LeadStatusCodeDetailsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Lead Status Code Details'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (4 items, 1 hidden)', async ({ page }) => {
    const po = new LeadStatusCodeDetailsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P38_DISPLAY_ORDER', 'P38_ID', 'P38_STATUS_CODE', 'P38_STATUS_DESCRIPTION']);
  });

  test('apex.item round-trip on P38_STATUS_CODE', async ({ page }) => {
    const po = new LeadStatusCodeDetailsPage(page);
    await po.goto();
    await po.statusCode.setValue('apx-testgen');
    expect(await po.statusCode.getValue()).toBe('apx-testgen');
  });

  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new LeadStatusCodeDetailsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Create Lead Status Code', 'Delete', 'Apply Changes']);
  });
});
