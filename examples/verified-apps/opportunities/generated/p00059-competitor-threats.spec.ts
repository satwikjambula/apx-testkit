/**
 * GENERATED by apx-testgen from pages/p00059-competitor-threats.apx — DO NOT EDIT.
 ...
 * (./p00059-competitor-threats.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: competitor-threats
...
import { CompetitorThreatsPage } from './p00059-competitor-threats.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 59: Competitor Threats [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new CompetitorThreatsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new CompetitorThreatsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Competitor Threats'));
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new CompetitorThreatsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Create Competitor Threat', 'Reset']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new CompetitorThreatsPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['competitor-threats']);
  });
});
