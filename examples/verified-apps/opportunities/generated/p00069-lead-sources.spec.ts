 ...
 * (./p00069-lead-sources.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: lead-sources
...
import { LeadSourcesPage } from './p00069-lead-sources.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 69: Lead Sources [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new LeadSourcesPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new LeadSourcesPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Lead Sources'));
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new LeadSourcesPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Create Lead Source', 'Reset']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new LeadSourcesPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['lead-sources']);
  });
});
