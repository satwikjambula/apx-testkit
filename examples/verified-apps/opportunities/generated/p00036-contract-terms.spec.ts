/**
 * GENERATED by apx-testgen from pages/p00036-contract-terms.apx — DO NOT EDIT.
 ...
 * (./p00036-contract-terms.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: contract-terms
...
import { ContractTermsPage } from './p00036-contract-terms.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 36: Contract Terms [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ContractTermsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ContractTermsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Contract Terms'));
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new ContractTermsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Add Contract Term', 'Reset']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new ContractTermsPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['contract-terms']);
  });
});
