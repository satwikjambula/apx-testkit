 ...
 * (./p00056-status-assessment-details.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, status-assessment-details
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), status-assessment-details (staticContent).
...
import { StatusAssessmentDetailsPage } from './p00056-status-assessment-details.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 56: Status Assessment Details [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new StatusAssessmentDetailsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new StatusAssessmentDetailsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Status Assessment Details'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (4 items, 1 hidden)', async ({ page }) => {
    const po = new StatusAssessmentDetailsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P56_ASSESSMENT_DESCRIPTION', 'P56_ASSESSMENT_TEXT', 'P56_DISPLAY_ORDER', 'P56_ID']);
  });

  test('apex.item round-trip on P56_ASSESSMENT_DESCRIPTION', async ({ page }) => {
    const po = new StatusAssessmentDetailsPage(page);
    await po.goto();
  ...
  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new StatusAssessmentDetailsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Create Status Assessment', 'Delete', 'Apply Changes']);
  });
});
