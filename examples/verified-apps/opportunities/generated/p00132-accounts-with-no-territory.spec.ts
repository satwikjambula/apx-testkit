 ...
 * (./p00132-accounts-with-no-territory.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: accounts-with-no-territory, breadcrumb
...
import { AccountsWithNoTerritoryPage } from './p00132-accounts-with-no-territory.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 132: Accounts with No Territory [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new AccountsWithNoTerritoryPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new AccountsWithNoTerritoryPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Accounts with No Territory'));
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new AccountsWithNoTerritoryPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['accounts-with-no-territory']);
  });
});
