/**
 * GENERATED by apx-testgen from pages/p00053-risk-assessments.apx — DO NOT EDIT.
 ...
 * (./p00053-risk-assessments.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: risk-assessments
...
import { RiskAssessmentsPage } from './p00053-risk-assessments.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 53: Risk Assessments [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new RiskAssessmentsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new RiskAssessmentsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Risk Assessments'));
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new RiskAssessmentsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Create Risk Assessment', 'Reset']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new RiskAssessmentsPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['risk-assessments']);
  });
});
