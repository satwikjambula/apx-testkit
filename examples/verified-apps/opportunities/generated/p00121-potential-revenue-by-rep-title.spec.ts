 ...
 * (./p00121-potential-revenue-by-rep-title.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, button-bar, potential-revenue-by-rep-title
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * 1 chart region(s) SKIPPED -- no htmlDomId set, runtime id genuinely unconstructible from static data (ADR-003 layer 3): potential-revenue-by-rep-title.
...
import { PotentialRevenueByRepTitlePage } from './p00121-potential-revenue-by-rep-title.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 121: Potential Revenue by &REP_TITLE. [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new PotentialRevenueByRepTitlePage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new PotentialRevenueByRepTitlePage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Potential Revenue by &REP_TITLE_RAW.'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (2 items, 0 hidden)', async ({ page }) => {
    const po = new PotentialRevenueByRepTitlePage(page);
    await po.goto();
    await expectItemsPresent(page, ['P121_QUARTER', 'P121_TERRITORY']);
  });
