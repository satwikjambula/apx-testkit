 ...
 * (./p00029-lead-details.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, lead, lead-classification, lead-details
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), lead (staticContent), lead-classification (staticContent), lead-details (staticContent).
...
import { LeadDetailsPage } from './p00029-lead-details.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 29: Lead Details [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new LeadDetailsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new LeadDetailsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Lead Details'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (10 items, 2 hidden)', async ({ page }) => {
    const po = new LeadDetailsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P29_ACCOUNT_ID', 'P29_ID', 'P29_LEAD_DETAILS', 'P29_LEAD_EMAIL_01', 'P29_LEAD_NAME_01', 'P29_LEAD_PRIORITY', 'P29_LEAD_SOURCE_ID', 'P29_LEAD_STATUS_ID', 'P29_OPPORTUNITY_ID', 'P29_TAGS']);
  });

  test('apex.item round-trip on P29_LEAD_DETAILS', async ({ page }) => {
    const po = new LeadDetailsPage(page);
    await po.goto();
    await po.leadDetails.setValue('apx-testgen');
    expect(await po.leadDetails.getValue()).toBe('apx-testgen');
  });

  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new LeadDetailsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Create Lead', 'Delete', 'Apply Changes']);
  });
});
