 ...
 * (./p00023-contacts2.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: contacts, contacts-cards, contacts-grid, contacts-report, filters, page-settings
 ...
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): contacts (staticContent), contacts-cards (classicReport), contacts-report (classicReport), filters (staticContent), page-settings (staticContent).
...
import { Contacts2Page } from './p00023-contacts2.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 23: Contacts [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new Contacts2Page(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new Contacts2Page(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Contacts'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (5 items, 0 hidden)', async ({ page }) => {
    const po = new Contacts2Page(page);
    await po.goto();
    await expectItemsPresent(page, ['P23_ACCOUNT_ID', 'P23_DISPLAY_AS', 'P23_SEARCH', 'P23_SORT', 'P23_TERRITORY_ID']);
  });

  test('apex.item round-trip on P23_SEARCH', async ({ page }) => {
    const po = new Contacts2Page(page);
    await po.goto();
  ...
  test('every labeled .apx button exists (3 buttons)', async ({ page }) => {
    const po = new Contacts2Page(page);
    await po.goto();
    await expectButtonsPresent(page, ['Create Contact', 'Reset', 'Reset']);
  });
