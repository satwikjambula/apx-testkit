 ...
 * (./p00018-country-details2.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, country-details
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), country-details (staticContent).
...
import { CountryDetails2Page } from './p00018-country-details2.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 18: Country Details [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new CountryDetails2Page(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new CountryDetails2Page(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Country Details'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (3 items, 1 hidden)', async ({ page }) => {
    const po = new CountryDetails2Page(page);
    await po.goto();
    await expectItemsPresent(page, ['P18_COUNTRY_CODE', 'P18_COUNTRY_NAME', 'P18_ID']);
  });

  test('apex.item round-trip on P18_COUNTRY_CODE', async ({ page }) => {
    const po = new CountryDetails2Page(page);
    await po.goto();
  ...
  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new CountryDetails2Page(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Create Country', 'Delete', 'Apply Changes']);
  });
});
