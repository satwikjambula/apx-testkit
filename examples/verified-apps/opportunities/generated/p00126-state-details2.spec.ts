 ...
 * (./p00126-state-details2.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: button-bar, state-details
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): button-bar (staticContent), state-details (staticContent).
...
import { StateDetails2Page } from './p00126-state-details2.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 126: State Details [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new StateDetails2Page(page);
  ...
  test('every .apx pageItem exists in DOM/apex.item registry (2 items, 1 hidden)', async ({ page }) => {
    const po = new StateDetails2Page(page);
    await po.goto();
    await expectItemsPresent(page, ['P126_STATE', 'P126_TERRITORY_ID']);
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new StateDetails2Page(page);
    await po.goto();
    await expectButtonsPresent(page, ['Add State', 'Cancel']);
  });
});
