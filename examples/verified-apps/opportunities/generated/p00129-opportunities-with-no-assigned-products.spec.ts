/**
 * GENERATED by apx-testgen from pages/p00129-opportunities-with-no-assigned-products.apx — DO NOT EDIT.
 ...
 * (./p00129-opportunities-with-no-assigned-products.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, opportunities-with-no-assigned-products
...
import { OpportunitiesWithNoAssignedProductsPage } from './p00129-opportunities-with-no-assigned-products.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 129: Opportunities with No Assigned Products [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new OpportunitiesWithNoAssignedProductsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new OpportunitiesWithNoAssignedProductsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Opportunities with No Assigned Products'));
  });
  ...
  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new OpportunitiesWithNoAssignedProductsPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['opportunities-with-no-assigned-products']);
  });
});
