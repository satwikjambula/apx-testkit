/**
 * GENERATED by apx-testgen from pages/p00049-competitor-analysis.apx — DO NOT EDIT.
 ...
 * (./p00049-competitor-analysis.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, competitor-analysis
...
import { CompetitorAnalysisPage } from './p00049-competitor-analysis.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 49: Competitor Analysis [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new CompetitorAnalysisPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new CompetitorAnalysisPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Competitor Analysis'));
  });
  ...
  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new CompetitorAnalysisPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['competitor-analysis']);
  });
});
