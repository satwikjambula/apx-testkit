 ...
 * (./p00065-industry-details.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, industry-details
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), industry-details (staticContent).
...
import { IndustryDetailsPage } from './p00065-industry-details.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 65: Industry Details [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new IndustryDetailsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new IndustryDetailsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Industry Details'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (2 items, 1 hidden)', async ({ page }) => {
    const po = new IndustryDetailsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P65_ID', 'P65_INDUSTRY_NAME']);
  });

  test('apex.item round-trip on P65_INDUSTRY_NAME', async ({ page }) => {
    const po = new IndustryDetailsPage(page);
    await po.goto();
    await po.industryName.setValue('apx-testgen');
    expect(await po.industryName.getValue()).toBe('apx-testgen');
  });

  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new IndustryDetailsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Create Industry', 'Delete', 'Apply Changes']);
  });
});
