 ...
 * (./p00009-territory-details.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: button-bar, territory-details
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): button-bar (staticContent), territory-details (staticContent).
...
import { TerritoryDetailsPage } from './p00009-territory-details.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 9: Territory Details [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new TerritoryDetailsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new TerritoryDetailsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Territory Details'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (6 items, 1 hidden)', async ({ page }) => {
    const po = new TerritoryDetailsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P9_ID', 'P9_SVP_ID', 'P9_TAGS', 'P9_TERRITORY_DESCRIPTION', 'P9_TERRITORY_NAME', 'P9_TERRITORY_TYPE']);
  });

  test('apex.item round-trip on P9_TERRITORY_DESCRIPTION', async ({ page }) => {
    const po = new TerritoryDetailsPage(page);
    await po.goto();
    await po.territoryDescription.setValue('apx-testgen');
    expect(await po.territoryDescription.getValue()).toBe('apx-testgen');
  });

  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new TerritoryDetailsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Create Territory', 'Delete', 'Apply Changes']);
  });
});
