 ...
 * (./p00093-territory.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: accounts, breadcrumb, countries, rds, states, summary, territory, territory-actions, usage-metrics-90-days, validations
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): accounts (classicReport), breadcrumb (breadcrumb), countries (classicReport), rds (regionDisplaySelector), states (classicReport), summary (plugin/badgeList), territory (classicReport), territory-actions (list), usage-metrics-90-days (plugin/badgeList), validations (plugin/contentValidator).
...
import { TerritoryPage } from './p00093-territory.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 93: Territory [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new TerritoryPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new TerritoryPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Territory'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 1 hidden)', async ({ page }) => {
    const po = new TerritoryPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P93_ID']);
  });

  test('every labeled .apx button exists (11 buttons)', async ({ page }) => {
    const po = new TerritoryPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Add Country', 'Add State', 'Edit Territory', 'Add Link', 'Add Attachment', 'Add Comment', 'View Attachments', 'View Comments', 'View Countries', 'View Links', 'View States']);
  });
});
