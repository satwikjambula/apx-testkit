/**
 * GENERATED by apx-testgen from pages/p00014-revenue-by-quarter.apx — DO NOT EDIT.
 ...
 * (./p00014-revenue-by-quarter.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, revenue-by-quarter, search
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * 1 chart region(s) SKIPPED -- no htmlDomId set, runtime id genuinely unconstructible from static data (ADR-003 layer 3): revenue-by-quarter.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), search (staticContent).
...
import { RevenueByQuarterPage } from './p00014-revenue-by-quarter.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 14: Revenue by Quarter [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new RevenueByQuarterPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new RevenueByQuarterPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Revenue by Quarter'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (2 items, 0 hidden)', async ({ page }) => {
    const po = new RevenueByQuarterPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P14_BEGIN_QTR', 'P14_END_QTR']);
  });
});
