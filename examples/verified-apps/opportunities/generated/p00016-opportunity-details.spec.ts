 ...
 * (./p00016-opportunity-details.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, opportunity
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), opportunity (staticContent).
...
import { OpportunityDetailsPage } from './p00016-opportunity-details.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 16: Opportunity Details [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new OpportunityDetailsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new OpportunityDetailsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Opportunity Details'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (18 items, 1 hidden)', async ({ page }) => {
    const po = new OpportunityDetailsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P16_ACCOUNT_STANDING_ID', 'P16_CUSTOMER_ID', 'P16_DEAL_AMOUNT', 'P16_DEAL_CLOSE_DATE', 'P16_DEAL_CUSTOMER_LOCATION_ID', 'P16_DEAL_NAME', 'P16_DEAL_PROBABILITY', 'P16_DEAL_STATUS_CODE_ID', 'P16_DEAL_SUMMARY', 'P16_FINANCIAL_ASSESSMENT_ID', 'P16_ID', 'P16_PRO_RE_ACTIVE', 'P16_RISK_ASSESSMENT_ID', 'P16_SALESREP_ID_01', 'P16_SPONSOR_CONTACT_ID', 'P16_STATUS_ASSESSMENT_ID', 'P16_SVP', 'P16_TAGS']);
  });

  test('apex.item round-trip on P16_DEAL_NAME', async ({ page }) => {
    const po = new OpportunityDetailsPage(page);
    await po.goto();
  ...
  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new OpportunityDetailsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Create Opportunity', 'Delete', 'Apply Changes']);
  });
});
