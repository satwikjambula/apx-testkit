/**
 * GENERATED by apx-testgen from pages/p00147-previous-validations.apx — DO NOT EDIT.
 ...
 * (./p00147-previous-validations.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: previous-validations
...
import { PreviousValidationsPage } from './p00147-previous-validations.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 147: Previous Validations [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new PreviousValidationsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new PreviousValidationsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Previous Validations'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (2 items, 2 hidden)', async ({ page }) => {
    const po = new PreviousValidationsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P147_ENTITY_ID', 'P147_ENTITY_TYPE']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new PreviousValidationsPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['previous-validations']);
  });
});
