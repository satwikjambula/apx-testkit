 ...
 * (./p00076-contract-term.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, contract-term
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), contract-term (staticContent).
...
import { ContractTermPage } from './p00076-contract-term.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 76: Contract Term [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ContractTermPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ContractTermPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Contract Term'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (3 items, 1 hidden)', async ({ page }) => {
    const po = new ContractTermPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P76_ID', 'P76_MONTH_EQUIV', 'P76_NAME']);
  });

  test('apex.item round-trip on P76_NAME', async ({ page }) => {
    const po = new ContractTermPage(page);
    await po.goto();
  ...
  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new ContractTermPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Add Contract Term', 'Delete', 'Apply Changes']);
  });
});
