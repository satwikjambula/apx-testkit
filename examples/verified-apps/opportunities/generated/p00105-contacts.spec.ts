 ...
 * (./p00105-contacts.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: button-bar, contacts
...
import { ContactsPage } from './p00105-contacts.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 105: Contacts [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ContactsPage(page);
  ...
  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 1 hidden)', async ({ page }) => {
    const po = new ContactsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P105_ID']);
  });
