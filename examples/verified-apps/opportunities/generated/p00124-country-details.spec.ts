 ...
 * (./p00124-country-details.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: button-bar, country-details
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): button-bar (staticContent), country-details (staticContent).
...
import { CountryDetailsPage } from './p00124-country-details.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 124: Country Details [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new CountryDetailsPage(page);
  ...
  test('every .apx pageItem exists in DOM/apex.item registry (2 items, 1 hidden)', async ({ page }) => {
    const po = new CountryDetailsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P124_COUNTRY', 'P124_TERRITORY_ID']);
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new CountryDetailsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Add Country', 'Cancel']);
  });
});
