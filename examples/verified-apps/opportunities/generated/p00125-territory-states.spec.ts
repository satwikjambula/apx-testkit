 ...
 * (./p00125-territory-states.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, territory-states
...
import { TerritoryStatesPage } from './p00125-territory-states.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 125: Territory States [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new TerritoryStatesPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new TerritoryStatesPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('States'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (2 items, 2 hidden)', async ({ page }) => {
    const po = new TerritoryStatesPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P125_TERRITORY_ID', 'P125_TERRITORY_STATE_ID']);
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new TerritoryStatesPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Add State', 'Reset']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new TerritoryStatesPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['territory-states']);
  });
});
