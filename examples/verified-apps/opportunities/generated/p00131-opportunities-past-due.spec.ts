/**
 * GENERATED by apx-testgen from pages/p00131-opportunities-past-due.apx — DO NOT EDIT.
 ...
 * (./p00131-opportunities-past-due.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, opportunities-past-due
...
import { OpportunitiesPastDuePage } from './p00131-opportunities-past-due.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 131: Opportunities Past Due [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new OpportunitiesPastDuePage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new OpportunitiesPastDuePage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Opportunities Past Due'));
  });
  ...
  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new OpportunitiesPastDuePage(page);
    await po.goto();
    await expectRegionsResolve(page, ['opportunities-past-due']);
  });
});
