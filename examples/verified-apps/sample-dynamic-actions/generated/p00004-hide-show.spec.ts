/**
 * GENERATED by apx-testgen from pages/p00004-hide-show.apx — DO NOT EDIT.
 ...
 * (./p00004-hide-show.page.js), not raw testkit calls, so both stay in sync.
...
import { HideShowPage } from './p00004-hide-show.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 4: Hide/Show [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new HideShowPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new HideShowPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Hide/Show'));
  });
