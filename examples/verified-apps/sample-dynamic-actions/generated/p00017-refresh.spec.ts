/**
 * GENERATED by apx-testgen from pages/p00017-refresh.apx — DO NOT EDIT.
 ...
 * (./p00017-refresh.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: employees, overview, saved-interactive-reports, server-side-refresh
 ...
 * Region types NOT covered by an auto-generated assertion (no verified DOM convention, or a runtime id genuinely unconstructible from static data -- see docs/grammar-assumptions.md "Still open" and ADR-003): overview (staticContent), saved-interactive-reports (classicReport), server-side-refresh (breadcrumb).
...
import { RefreshPage } from './p00017-refresh.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 17: Refresh [requires auth]', () => {
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new RefreshPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Refresh'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 1 hidden)', async ({ page }) => {
    const po = new RefreshPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P17_REPORT_ID']);
  });
