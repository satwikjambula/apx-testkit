 ...
 * (./p00007-edit3.page.js), not raw testkit calls, so both stay in sync.
...
import { Edit3Page } from './p00007-edit3.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 7: Edit [requires auth]', () => {
  ...
  test('every .apx pageItem exists in DOM/apex.item registry (9 items, 1 hidden)', async ({ page }) => {
    const po = new Edit3Page(page);
    await po.goto();
    await expectItemsPresent(page, ['P7_COMM', 'P7_DEPTNO', 'P7_EMPNO', 'P7_ENAME', 'P7_HIREDATE', 'P7_JOB', 'P7_MGR', 'P7_ROWID', 'P7_SAL']);
  });

  test('apex.item round-trip on P7_EMPNO', async ({ page }) => {
    const po = new Edit3Page(page);
    await po.goto();
