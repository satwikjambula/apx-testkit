 ...
 * (./p00005-edit2.page.js), not raw testkit calls, so both stay in sync.
...
import { Edit2Page } from './p00005-edit2.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 5: Edit [requires auth]', () => {
  ...
  test('every .apx pageItem exists in DOM/apex.item registry (9 items, 1 hidden)', async ({ page }) => {
    const po = new Edit2Page(page);
    await po.goto();
    await expectItemsPresent(page, ['P5_COMM', 'P5_DEPTNO', 'P5_EMPNO', 'P5_ENAME', 'P5_HIREDATE', 'P5_JOB', 'P5_MGR', 'P5_ROWID', 'P5_SAL']);
  });

  test('apex.item round-trip on P5_EMPNO', async ({ page }) => {
    const po = new Edit2Page(page);
    await po.goto();
