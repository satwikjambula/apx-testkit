/**
 * GENERATED by apx-testgen from pages/p00014-set-values-pl-sql.apx — DO NOT EDIT.
 ...
 * (./p00014-set-values-pl-sql.page.js), not raw testkit calls, so both stay in sync.
...
import { SetValuesPlSqlPage } from './p00014-set-values-pl-sql.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 14: Set Values (PL/SQL) [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new SetValuesPlSqlPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new SetValuesPlSqlPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Set Values (PL/SQL)'));
  });
