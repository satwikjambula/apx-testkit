 ...
 * (./p00015-edit6.page.js), not raw testkit calls, so both stay in sync.
...
import { Edit6Page } from './p00015-edit6.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 15: Edit [requires auth]', () => {
  ...
  test('every .apx pageItem exists in DOM/apex.item registry (9 items, 1 hidden)', async ({ page }) => {
    const po = new Edit6Page(page);
    await po.goto();
    await expectItemsPresent(page, ['P15_COMM', 'P15_DEPTNO', 'P15_EMPNO', 'P15_ENAME', 'P15_HIREDATE', 'P15_JOB', 'P15_MGR', 'P15_ROWID', 'P15_SAL']);
  });

  test('apex.item round-trip on P15_EMPNO', async ({ page }) => {
    const po = new Edit6Page(page);
    await po.goto();
