/**
 * GENERATED by apx-testgen from pages/p00010-stripe-report.apx — DO NOT EDIT.
 ...
 * (./p00010-stripe-report.page.js), not raw testkit calls, so both stay in sync.
...
import { StripeReportPage } from './p00010-stripe-report.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 10: Stripe Report [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new StripeReportPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new StripeReportPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Stripe Report'));
  });
