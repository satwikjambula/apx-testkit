/**
 * GENERATED by apx-testgen from pages/p00006-add-remove-class-error.apx — DO NOT EDIT.
 ...
 * (./p00006-add-remove-class-error.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, employees, overview, sql-source
 ...
 * Region types NOT covered by an auto-generated assertion (no verified DOM convention, or a runtime id genuinely unconstructible from static data -- see docs/grammar-assumptions.md "Still open" and ADR-003): breadcrumb (breadcrumb), overview (staticContent), sql-source (plugin/sourceDisplay).
...
import { AddRemoveClassErrorPage } from './p00006-add-remove-class-error.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 6: Add/Remove Class (Error) [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new AddRemoveClassErrorPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new AddRemoveClassErrorPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Add/Remove Class (Error)'));
  });
  ...
  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new AddRemoveClassErrorPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['employees_report']);
  });
});
