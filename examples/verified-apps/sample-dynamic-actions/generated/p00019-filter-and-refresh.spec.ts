/**
 * GENERATED by apx-testgen from pages/p00019-filter-and-refresh.apx — DO NOT EDIT.
 ...
 * (./p00019-filter-and-refresh.page.js), not raw testkit calls, so both stay in sync.
...
import { FilterAndRefreshPage } from './p00019-filter-and-refresh.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 19: Filter and Refresh [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new FilterAndRefreshPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new FilterAndRefreshPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Filter and Refresh'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (2 items, 0 hidden)', async ({ page }) => {
    const po = new FilterAndRefreshPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P19_DEPTNO', 'P19_JOB']);
  });
