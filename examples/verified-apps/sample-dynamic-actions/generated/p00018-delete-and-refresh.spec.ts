 ...
 * (./p00018-delete-and-refresh.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: employees, overview, server-side-refresh
 ...
 * Region types NOT covered by an auto-generated assertion (no verified DOM convention, or a runtime id genuinely unconstructible from static data -- see docs/grammar-assumptions.md "Still open" and ADR-003): overview (staticContent), server-side-refresh (breadcrumb).
...
import { DeleteAndRefreshPage } from './p00018-delete-and-refresh.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 18: Delete and Refresh [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new DeleteAndRefreshPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new DeleteAndRefreshPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Delete and Refresh'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 1 hidden)', async ({ page }) => {
    const po = new DeleteAndRefreshPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P18_EMPNO']);
  });
