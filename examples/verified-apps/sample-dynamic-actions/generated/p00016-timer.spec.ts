 ...
 * (./p00016-timer.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: auto-refresh, breadcrumb, employees, overview
 ...
 * Region types NOT covered by an auto-generated assertion (no verified DOM convention, or a runtime id genuinely unconstructible from static data -- see docs/grammar-assumptions.md "Still open" and ADR-003): auto-refresh (staticContent), breadcrumb (breadcrumb), overview (staticContent).
...
import { TimerPage } from './p00016-timer.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 16: Timer [requires auth]', () => {
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new TimerPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Timer'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 0 hidden)', async ({ page }) => {
    const po = new TimerPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P16_REFRESH']);
  });
