/**
 * GENERATED by apx-testgen from pages/p00002-disable-enable.apx — DO NOT EDIT.
 ...
 * (./p00002-disable-enable.page.js), not raw testkit calls, so both stay in sync.
...
import { DisableEnablePage } from './p00002-disable-enable.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 2: Disable/Enable [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new DisableEnablePage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new DisableEnablePage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Disable/Enable'));
  });
