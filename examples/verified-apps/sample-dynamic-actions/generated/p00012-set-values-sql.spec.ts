/**
 * GENERATED by apx-testgen from pages/p00012-set-values-sql.apx — DO NOT EDIT.
 ...
 * (./p00012-set-values-sql.page.js), not raw testkit calls, so both stay in sync.
...
import { SetValuesSqlPage } from './p00012-set-values-sql.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 12: Set Values (SQL) [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new SetValuesSqlPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new SetValuesSqlPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Set Values (SQL)'));
  });
