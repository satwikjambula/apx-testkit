/**
 * GENERATED by apx-testgen from pages/p00011-execute-pl-sql-code.apx — DO NOT EDIT.
 ...
 * (./p00011-execute-pl-sql-code.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: employees, overview, standard-button
 ...
 * Region types NOT covered by an auto-generated assertion (no verified DOM convention, or a runtime id genuinely unconstructible from static data -- see docs/grammar-assumptions.md "Still open" and ADR-003): overview (staticContent), standard-button (breadcrumb).
...
import { ExecutePlSqlCodePage } from './p00011-execute-pl-sql-code.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 11: Execute PL/SQL Code [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ExecutePlSqlCodePage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ExecutePlSqlCodePage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Execute PL/SQL Code'));
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new ExecutePlSqlCodePage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Reset', 'Update Salary by 10%']);
  });
