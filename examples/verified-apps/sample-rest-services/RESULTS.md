# sample-rest-services — verification results
...
- `staticContent`: 57
- `breadcrumb`: 29
- `interactiveReport`: 14
- `list`: 6
- `themeTemplateComponent/contentRow`: 5
- `chart`: 3
- `classicReport`: 2
...
in `raw` bags, nothing lost (ADR-001): `action`, `axis`, `branch`, `column`, `computation`, `facet`, `filter`, `layer`, `pageGroup`, `parameter`, `process`, `savedReport`, `series`, `validation`

## Generation

`@apx/testgen` produced **36** page object + smoke spec pairs
(35 marked skip: auth required). See `generated/` for
the actual files.
...
**0 added, 0 removed, 0 changed, 36 unchanged** — confirms zero spurious differences.
