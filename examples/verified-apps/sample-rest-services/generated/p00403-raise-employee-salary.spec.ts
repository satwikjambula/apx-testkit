 ...
 * (./p00403-raise-employee-salary.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: button
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): button (staticContent).
...
import { RaiseEmployeeSalaryPage } from './p00403-raise-employee-salary.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 403: Raise Salary [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new RaiseEmployeeSalaryPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new RaiseEmployeeSalaryPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Raise Salary'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (5 items, 3 hidden)', async ({ page }) => {
    const po = new RaiseEmployeeSalaryPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P403_MESSAGE', 'P403_RESPONSE', 'P403_STATUS', 'P_DIALOG_EMP_NO', 'P_DIALOG_NEW_SALARY']);
  });
