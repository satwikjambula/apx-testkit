/**
 * GENERATED by apx-testgen from pages/p00101-simple-http.apx — DO NOT EDIT.
 ...
 * (./p00101-simple-http.page.js), not raw testkit calls, so both stay in sync.
...
import { SimpleHttpPage } from './p00101-simple-http.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 101: Simple HTTP [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new SimpleHttpPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new SimpleHttpPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Simple HTTP'));
  });
