/**
 * GENERATED by apx-testgen from pages/p00408-workflow-console.apx — DO NOT EDIT.
 ...
 * (./p00408-workflow-console.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, my-workflows-report, my-workflows-smart-filters
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), my-workflows-report (themeTemplateComponent/contentRow), my-workflows-smart-filters (smartFilters).
...
import { WorkflowConsolePage } from './p00408-workflow-console.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 408: Workflow Console [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new WorkflowConsolePage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new WorkflowConsolePage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Workflow Console'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (2 items, 1 hidden)', async ({ page }) => {
    const po = new WorkflowConsolePage(page);
    await po.goto();
    await expectItemsPresent(page, ['P408_SORT_BY', 'P408_WORKFLOW_ID']);
  });
