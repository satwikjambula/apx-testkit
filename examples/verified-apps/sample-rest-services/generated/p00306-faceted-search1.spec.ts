/**
 * GENERATED by apx-testgen from pages/p00306-faceted-search1.apx — DO NOT EDIT.
 ...
 * (./p00306-faceted-search1.page.js), not raw testkit calls, so both stay in sync.
...
import { FacetedSearch1Page } from './p00306-faceted-search1.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 306: Faceted Search [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new FacetedSearch1Page(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new FacetedSearch1Page(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Faceted Search'));
  });

  test('every labeled .apx button exists (3 buttons)', async ({ page }) => {
    const po = new FacetedSearch1Page(page);
    await po.goto();
    await expectButtonsPresent(page, ['Next', 'Previous', 'Reset']);
  });
