 ...
 * (./p00406-rest-data-source-plugins1.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, overview, rest-data-source-plugins-results, web-credentials-not-provided
 ...
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), overview (staticContent), web-credentials-not-provided (staticContent).
...
import { RestDataSourcePlugins1Page } from './p00406-rest-data-source-plugins1.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 406: REST Data Source Plugins [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new RestDataSourcePlugins1Page(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new RestDataSourcePlugins1Page(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('REST Data Source Plugins'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 1 hidden)', async ({ page }) => {
    const po = new RestDataSourcePlugins1Page(page);
    await po.goto();
    await expectItemsPresent(page, ['P406_MOVIE']);
  });

  test('every labeled .apx button exists (3 buttons)', async ({ page }) => {
    const po = new RestDataSourcePlugins1Page(page);
    await po.goto();
    await expectButtonsPresent(page, ['Next', 'Previous', 'Set API Key']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new RestDataSourcePlugins1Page(page);
    await po.goto();
    await expectRegionsResolve(page, ['rest-data-source-plugins-results']);
  });
});
