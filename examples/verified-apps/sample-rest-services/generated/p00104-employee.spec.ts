 ...
 * (./p00104-employee.page.js), not raw testkit calls, so both stay in sync.
 ...
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), employee (form).
...
import { EmployeePage } from './p00104-employee.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 104: Employee [requires auth]', () => {
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new EmployeePage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Create / Edit Employee'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (9 items, 2 hidden)', async ({ page }) => {
    const po = new EmployeePage(page);
    await po.goto();
    await expectItemsPresent(page, ['P104_COMM', 'P104_DEPTNO', 'P104_EMPNO', 'P104_ENAME', 'P104_HIREDATE', 'P104_JOB', 'P104_MGR', 'P104_REQ_LC', 'P104_SAL']);
  });

  test('apex.item round-trip on P104_ENAME', async ({ page }) => {
    const po = new EmployeePage(page);
    await po.goto();
