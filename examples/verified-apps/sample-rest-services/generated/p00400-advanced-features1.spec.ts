/**
 * GENERATED by apx-testgen from pages/p00400-advanced-features1.apx — DO NOT EDIT.
 ...
 * (./p00400-advanced-features1.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: advanced-features-list, breadcrumb
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): advanced-features-list (list), breadcrumb (breadcrumb).
...
import { AdvancedFeatures1Page } from './p00400-advanced-features1.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 400: Advanced Features [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new AdvancedFeatures1Page(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new AdvancedFeatures1Page(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Advanced Features'));
  });
