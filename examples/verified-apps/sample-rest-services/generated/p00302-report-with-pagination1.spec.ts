/**
 * GENERATED by apx-testgen from pages/p00302-report-with-pagination1.apx — DO NOT EDIT.
 ...
 * (./p00302-report-with-pagination1.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, overview, pagination-results
...
import { ReportWithPagination1Page } from './p00302-report-with-pagination1.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 302: Report with Pagination [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ReportWithPagination1Page(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ReportWithPagination1Page(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Report with Pagination'));
  });
  ...
  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new ReportWithPagination1Page(page);
    await po.goto();
    await expectRegionsResolve(page, ['pagination-results']);
  });
});
