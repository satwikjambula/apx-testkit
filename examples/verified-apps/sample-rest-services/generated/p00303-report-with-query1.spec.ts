 ...
 * (./p00303-report-with-query1.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, overview, query-filter-results
...
import { ReportWithQuery1Page } from './p00303-report-with-query1.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 303: Report with Query [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ReportWithQuery1Page(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ReportWithQuery1Page(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Report with Query'));
  });

  test('every labeled .apx button exists (3 buttons)', async ({ page }) => {
    const po = new ReportWithQuery1Page(page);
    await po.goto();
    await expectButtonsPresent(page, ['Enable IR Debug', 'Next', 'Previous']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new ReportWithQuery1Page(page);
    await po.goto();
    await expectRegionsResolve(page, ['query-filter-results']);
  });
});
