/**
 * GENERATED by apx-testgen from pages/p00504-network-setup1.apx — DO NOT EDIT.
 ...
 * (./p00504-network-setup1.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, general-guide, update-odata-base-url, update-or-set-proxy-server, update-remote-server
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), general-guide (staticContent), update-odata-base-url (staticContent), update-or-set-proxy-server (staticContent), update-remote-server (staticContent).
...
import { NetworkSetup1Page } from './p00504-network-setup1.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 504: Network Setup [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new NetworkSetup1Page(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new NetworkSetup1Page(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Network Setup'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (4 items, 0 hidden)', async ({ page }) => {
    const po = new NetworkSetup1Page(page);
    await po.goto();
    await expectItemsPresent(page, ['P504_BASE_URL', 'P504_ODATA', 'P504_PROXY_SERVER', 'P504_SCHEMA']);
  });

  test('apex.item round-trip on P504_BASE_URL', async ({ page }) => {
    const po = new NetworkSetup1Page(page);
    await po.goto();
