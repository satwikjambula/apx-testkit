/**
 * GENERATED by apx-testgen from pages/p00501-application-appearance1.apx — DO NOT EDIT.
 ...
 * (./p00501-application-appearance1.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, configure-appearance
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), configure-appearance (staticContent).
...
import { ApplicationAppearance1Page } from './p00501-application-appearance1.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 501: Application Appearance [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ApplicationAppearance1Page(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ApplicationAppearance1Page(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Application Appearance'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 0 hidden)', async ({ page }) => {
    const po = new ApplicationAppearance1Page(page);
    await po.goto();
    await expectItemsPresent(page, ['P501_DESKTOP_THEME_STYLE_ID']);
  });
