 ...
 * (./p00407-rest-and-workflow.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, button, overview, start-workflow, workflow-console
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), button (staticContent), overview (staticContent), start-workflow (staticContent), workflow-console (staticContent).
...
import { RestAndWorkflowPage } from './p00407-rest-and-workflow.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 407: REST and Workflow [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new RestAndWorkflowPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new RestAndWorkflowPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('REST and Workflow'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (2 items, 1 hidden)', async ({ page }) => {
    const po = new RestAndWorkflowPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P407_PRODUCT_ID', 'P407_WF_ID']);
  });

  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new RestAndWorkflowPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Next', 'Open Workflow Console', 'Previous', 'Start Workflow']);
  });
});
