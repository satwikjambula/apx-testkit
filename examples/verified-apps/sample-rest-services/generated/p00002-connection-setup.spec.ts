 ...
 * (./p00002-connection-setup.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: 1-remote-server-setup, 2-odata-url-setup, 3-proxy-setup, button-remote-server, guide, remote-server-setup-guide, welcome-to-sample-rest-services
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): 1-remote-server-setup (staticContent), 2-odata-url-setup (staticContent), 3-proxy-setup (staticContent), button-remote-server (staticContent), guide (staticContent), remote-server-setup-guide (staticContent), welcome-to-sample-rest-services (staticContent).
...
import { ConnectionSetupPage } from './p00002-connection-setup.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 2: Connection Setup [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ConnectionSetupPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ConnectionSetupPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Connection Setup'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (4 items, 0 hidden)', async ({ page }) => {
    const po = new ConnectionSetupPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P2_BASE_URL', 'P2_ODATA_URL', 'P2_PROXY', 'P2_SCHEMA']);
  });

  test('apex.item round-trip on P2_BASE_URL', async ({ page }) => {
    const po = new ConnectionSetupPage(page);
    await po.goto();
    await po.baseUrl.setValue('apx-testgen');
    expect(await po.baseUrl.getValue()).toBe('apx-testgen');
  });

  test('every labeled .apx button exists (1 button)', async ({ page }) => {
    const po = new ConnectionSetupPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Complete Setup']);
  });
});
