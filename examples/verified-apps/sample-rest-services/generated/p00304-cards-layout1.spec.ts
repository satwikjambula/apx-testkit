/**
 * GENERATED by apx-testgen from pages/p00304-cards-layout1.apx — DO NOT EDIT.
 ...
 * (./p00304-cards-layout1.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, movies, overview, web-credentials-not-provided
...
import { CardsLayout1Page } from './p00304-cards-layout1.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 304: Cards Layout [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new CardsLayout1Page(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new CardsLayout1Page(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Cards Layout'));
  });

  test('every labeled .apx button exists (3 buttons)', async ({ page }) => {
    const po = new CardsLayout1Page(page);
    await po.goto();
    await expectButtonsPresent(page, ['Next', 'Previous', 'Set Credentials']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new CardsLayout1Page(page);
    await po.goto();
    await expectRegionsResolve(page, ['movies']);
  });
});
