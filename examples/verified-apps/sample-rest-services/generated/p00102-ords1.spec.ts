/**
 * GENERATED by apx-testgen from pages/p00102-ords1.apx — DO NOT EDIT.
 ...
 * (./p00102-ords1.page.js), not raw testkit calls, so both stay in sync.
...
import { Ords1Page } from './p00102-ords1.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 102: ORDS [requires auth]', () => {
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new Ords1Page(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('ORDS'));
  });

  test('every labeled .apx button exists (3 buttons)', async ({ page }) => {
    const po = new Ords1Page(page);
    await po.goto();
    await expectButtonsPresent(page, ['Create', 'Next', 'Previous']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new Ords1Page(page);
    await po.goto();
    await expectRegionsResolve(page, ['employees-data']);
  });
});
