 ...
 * (./p00201-post-processing-sql.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, overview, post-processing-sql-query, post-processing-sql-results
 ...
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), overview (staticContent), post-processing-sql-query (staticContent).
...
import { PostProcessingSqlPage } from './p00201-post-processing-sql.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 201: Post Processing SQL [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new PostProcessingSqlPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new PostProcessingSqlPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Post Processing SQL'));
  });
  ...
  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new PostProcessingSqlPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['post-processing-sql-results']);
  });
});
