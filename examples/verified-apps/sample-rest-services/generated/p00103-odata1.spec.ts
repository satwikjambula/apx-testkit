/**
 * GENERATED by apx-testgen from pages/p00103-odata1.apx — DO NOT EDIT.
 ...
 * (./p00103-odata1.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, northwind-odata-products, odata-url-not-provided, overview
 ...
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), odata-url-not-provided (staticContent), overview (staticContent).
...
import { Odata1Page } from './p00103-odata1.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 103: OData [requires auth]', () => {
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new Odata1Page(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('OData'));
  });

  test('every labeled .apx button exists (3 buttons)', async ({ page }) => {
    const po = new Odata1Page(page);
    await po.goto();
    await expectButtonsPresent(page, ['Next', 'Previous', 'Set OData URL']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new Odata1Page(page);
    await po.goto();
    await expectRegionsResolve(page, ['northwind-odata-products']);
  });
});
