/**
 * GENERATED by apx-testgen from pages/p00202-derived-columns1.apx — DO NOT EDIT.
 ...
 * (./p00202-derived-columns1.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, derived-column-results, overview
...
import { DerivedColumns1Page } from './p00202-derived-columns1.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 202: Derived Columns [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new DerivedColumns1Page(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new DerivedColumns1Page(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Derived Columns'));
  });
  ...
  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new DerivedColumns1Page(page);
    await po.goto();
    await expectRegionsResolve(page, ['derived-column-results']);
  });
});
