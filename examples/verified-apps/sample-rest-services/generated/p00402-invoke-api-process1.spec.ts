/**
 * GENERATED by apx-testgen from pages/p00402-invoke-api-process1.apx — DO NOT EDIT.
 ...
 * (./p00402-invoke-api-process1.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, employees-data, overview
...
import { InvokeApiProcess1Page } from './p00402-invoke-api-process1.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 402: Invoke API Process [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new InvokeApiProcess1Page(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new InvokeApiProcess1Page(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Invoke API Process'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (2 items, 2 hidden)', async ({ page }) => {
    const po = new InvokeApiProcess1Page(page);
    await po.goto();
    await expectItemsPresent(page, ['P402_MESSAGE', 'P402_STATUS']);
  });

  test('every labeled .apx button exists (3 buttons)', async ({ page }) => {
    const po = new InvokeApiProcess1Page(page);
    await po.goto();
    await expectButtonsPresent(page, ['Next', 'Previous', 'Raise Salary']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new InvokeApiProcess1Page(page);
    await po.goto();
    await expectRegionsResolve(page, ['empIR']);
  });
});
