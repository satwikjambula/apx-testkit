 ...
 * (./p00305-oracle-jet-charts1.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, order-status-distribution, overview, project-task-count
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * 2 chart region(s) SKIPPED -- no htmlDomId set, runtime id genuinely unconstructible from static data (ADR-003 layer 3): order-status-distribution, project-task-count.
...
import { OracleJetCharts1Page } from './p00305-oracle-jet-charts1.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 305: Oracle JET Charts [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new OracleJetCharts1Page(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new OracleJetCharts1Page(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Oracle JET Charts'));
  });
