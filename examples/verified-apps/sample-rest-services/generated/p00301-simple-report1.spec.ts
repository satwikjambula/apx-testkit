/**
 * GENERATED by apx-testgen from pages/p00301-simple-report1.apx — DO NOT EDIT.
 ...
 * (./p00301-simple-report1.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, overview, simple-employees-report
...
import { SimpleReport1Page } from './p00301-simple-report1.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 301: Simple Report [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new SimpleReport1Page(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new SimpleReport1Page(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Simple Report'));
  });
  ...
  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new SimpleReport1Page(page);
    await po.goto();
    await expectRegionsResolve(page, ['simple-employees-report']);
  });
});
