/**
 * GENERATED by apx-testgen from pages/p00200-combine-with-relational-data.apx — DO NOT EDIT.
 ...
 * (./p00200-combine-with-relational-data.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, data-enrichment-list
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), data-enrichment-list (list).
...
import { CombineWithRelationalDataPage } from './p00200-combine-with-relational-data.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 200: Combine with Relational Data [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new CombineWithRelationalDataPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new CombineWithRelationalDataPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Combine with Relational Data'));
  });
