 ...
 * (./p00502-reset-data1.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, reset-data
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), reset-data (staticContent).
...
import { ResetData1Page } from './p00502-reset-data1.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 502: Reset Data [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ResetData1Page(page);
  ...
  test('every labeled .apx button exists (1 button)', async ({ page }) => {
    const po = new ResetData1Page(page);
    await po.goto();
    await expectButtonsPresent(page, ['Reset Data']);
  });
});
