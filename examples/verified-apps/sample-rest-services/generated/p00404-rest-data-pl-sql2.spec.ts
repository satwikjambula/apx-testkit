 ...
 * (./p00404-rest-data-pl-sql2.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: actions, breadcrumb, etl-order-summary-results, overview
 ...
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): actions (staticContent), breadcrumb (breadcrumb), overview (staticContent).
...
import { RestDataPlSql2Page } from './p00404-rest-data-pl-sql2.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 404: REST Data & PL/SQL [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new RestDataPlSql2Page(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new RestDataPlSql2Page(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('REST Data & PL/SQL'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 0 hidden)', async ({ page }) => {
    const po = new RestDataPlSql2Page(page);
    await po.goto();
    await expectItemsPresent(page, ['P404_EMAIL']);
  });

  test('apex.item round-trip on P404_EMAIL', async ({ page }) => {
    const po = new RestDataPlSql2Page(page);
    await po.goto();
  ...
  test('every labeled .apx button exists (3 buttons)', async ({ page }) => {
    const po = new RestDataPlSql2Page(page);
    await po.goto();
    await expectButtonsPresent(page, ['Process Data & Send report', 'Next', 'Previous']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new RestDataPlSql2Page(page);
    await po.goto();
    await expectRegionsResolve(page, ['etl-order-summary-results']);
  });
});
