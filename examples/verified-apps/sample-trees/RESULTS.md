# sample-trees — verification results
...
in `raw` bags, nothing lost (ADR-001): `axis`, `branch`, `column`, `computation`, `process`, `series`, `validation`

## Generation

`@apx/testgen` produced **11** page object + smoke spec pairs
(10 marked skip: auth required). See `generated/` for
the actual files.
...
**0 added, 0 removed, 0 changed, 11 unchanged** — confirms zero spurious differences.
