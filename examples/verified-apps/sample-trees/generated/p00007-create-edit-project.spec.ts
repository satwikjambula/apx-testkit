 ...
 * (./p00007-create-edit-project.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, create-edit-project
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), create-edit-project (staticContent).
...
import { CreateEditProjectPage } from './p00007-create-edit-project.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 7: Create/Edit Project [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new CreateEditProjectPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new CreateEditProjectPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Create/Edit Project'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (7 items, 2 hidden)', async ({ page }) => {
    const po = new CreateEditProjectPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P7_COMPLETION_DATE', 'P7_ESTIMATED_COMPLETION', 'P7_PROJECT_NAME', 'P7_PROJ_ID', 'P7_START_DATE', 'P7_STATUS', 'P7_UPDATED']);
  });

  test('apex.item round-trip on P7_PROJECT_NAME', async ({ page }) => {
    const po = new CreateEditProjectPage(page);
    await po.goto();
    await po.projectName.setValue('apx-testgen');
    expect(await po.projectName.getValue()).toBe('apx-testgen');
  });
