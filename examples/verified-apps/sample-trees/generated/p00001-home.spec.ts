 ...
 * Regions present in metadata: about-this-application, additional-communities-and-resources, app-name, example-trees, footer
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): about-this-application (staticContent), additional-communities-and-resources (plugin/sampleAppsFooter), app-name (staticContent), example-trees (list), footer (plugin/sampleAppsFooter).
...
test.describe('page 1: Home [requires auth]', () => {
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new HomePage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Sample Trees'));
  });
});
