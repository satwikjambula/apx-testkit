 ...
 * (./p00003-project-tracking.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, sql-source, task-tracking-tabs, task-tree
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), sql-source (plugin/sourceDisplay), task-tracking-tabs (list), task-tree (tree).
...
import { ProjectTrackingPage } from './p00003-project-tracking.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 3: Project Tracking [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ProjectTrackingPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ProjectTrackingPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Project Tracking'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 1 hidden)', async ({ page }) => {
    const po = new ProjectTrackingPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P3_SELECTED_NODE']);
  });

  test('every labeled .apx button exists (3 buttons)', async ({ page }) => {
    const po = new ProjectTrackingPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Collapse All', 'Expand All', 'Reset Tree']);
  });
});
