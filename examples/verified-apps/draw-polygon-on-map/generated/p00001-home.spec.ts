 ...
 * Regions present in metadata: chat, draw-polygon-on-map, map
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): chat (staticContent), draw-polygon-on-map (staticContent), map (map).
...
test.describe('page 1: ホーム [requires auth]', () => {
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new HomePage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Draw Polygon On Map'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (2 items, 0 hidden)', async ({ page }) => {
    const po = new HomePage(page);
    await po.goto();
    await expectItemsPresent(page, ['P1_POLYGON', 'P1_PROMPT']);
  });

  test('apex.item round-trip on P1_POLYGON', async ({ page }) => {
    const po = new HomePage(page);
    await po.goto();
    await po.polygon.setValue('apx-testgen');
    expect(await po.polygon.getValue()).toBe('apx-testgen');
  });

  test('every labeled .apx button exists (1 button)', async ({ page }) => {
    const po = new HomePage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Draw']);
  });
});
