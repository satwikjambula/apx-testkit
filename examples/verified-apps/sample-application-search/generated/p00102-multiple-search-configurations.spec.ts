/**
 * GENERATED by apx-testgen from pages/p00102-multiple-search-configurations.apx — DO NOT EDIT.
 ...
 * (./p00102-multiple-search-configurations.page.js), not raw testkit calls, so both stay in sync.
 ...
 * Region types NOT covered by an auto-generated assertion (no verified DOM convention, or a runtime id genuinely unconstructible from static data -- see docs/grammar-assumptions.md "Still open" and ADR-003): breadcrumb (breadcrumb), overview (staticContent), search (staticContent), search-results (search).
...
import { MultipleSearchConfigurationsPage } from './p00102-multiple-search-configurations.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 102: Multiple Search Configurations [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new MultipleSearchConfigurationsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new MultipleSearchConfigurationsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Multiple Search Configurations'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 0 hidden)', async ({ page }) => {
    const po = new MultipleSearchConfigurationsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P102_SEARCH']);
  });

  test('apex.item round-trip on P102_SEARCH', async ({ page }) => {
    const po = new MultipleSearchConfigurationsPage(page);
    await po.goto();
