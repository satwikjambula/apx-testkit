...
import { CardRegionDetailsPage } from './p00304-card-region-details.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 304: Card Region Details [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new CardRegionDetailsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new CardRegionDetailsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Card Region Details'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (11 items, 4 hidden)', async ({ page }) => {
    const po = new CardRegionDetailsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P304_CONFIG', 'P304_CUSTOMER_ID', 'P304_EMAIL_ADDRESS', 'P304_FULL_NAME', 'P304_PHYSICAL_ADDRESS', 'P304_PRODUCT_ID', 'P304_PRODUCT_NAME', 'P304_STORE_ID', 'P304_STORE_NAME', 'P304_UNIT_PRICE', 'P304_WEB_ADDRESS']);
  });

  test('apex.item round-trip on P304_EMAIL_ADDRESS', async ({ page }) => {
    const po = new CardRegionDetailsPage(page);
    await po.goto();
