/**
 * GENERATED by apx-testgen from pages/p00303-highlight-results.apx — DO NOT EDIT.
 ...
 * (./p00303-highlight-results.page.js), not raw testkit calls, so both stay in sync.
 ...
 * Region types NOT covered by an auto-generated assertion (no verified DOM convention, or a runtime id genuinely unconstructible from static data -- see docs/grammar-assumptions.md "Still open" and ADR-003): breadcrumb (breadcrumb), overview (staticContent), search (staticContent).
...
import { HighlightResultsPage } from './p00303-highlight-results.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 303: Highlight Results [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new HighlightResultsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new HighlightResultsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Highlight Results'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 0 hidden)', async ({ page }) => {
    const po = new HighlightResultsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P303_SEARCH']);
  });

  test('apex.item round-trip on P303_SEARCH', async ({ page }) => {
    const po = new HighlightResultsPage(page);
    await po.goto();
  ...
  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new HighlightResultsPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['search-results']);
  });
});
