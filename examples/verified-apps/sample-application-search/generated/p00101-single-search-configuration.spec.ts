/**
 * GENERATED by apx-testgen from pages/p00101-single-search-configuration.apx — DO NOT EDIT.
 ...
 * (./p00101-single-search-configuration.page.js), not raw testkit calls, so both stay in sync.
...
import { SingleSearchConfigurationPage } from './p00101-single-search-configuration.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 101: Single Search Configuration [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new SingleSearchConfigurationPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new SingleSearchConfigurationPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Single Search Configuration'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 0 hidden)', async ({ page }) => {
    const po = new SingleSearchConfigurationPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P101_SEARCH']);
  });

  test('apex.item round-trip on P101_SEARCH', async ({ page }) => {
    const po = new SingleSearchConfigurationPage(page);
    await po.goto();
