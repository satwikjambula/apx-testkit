 ...
 * (./p00503-web-credentials-settings.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: about-this-page, breadcrumb, credentials-form
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Region types NOT covered by an auto-generated assertion (no verified DOM convention, or a runtime id genuinely unconstructible from static data -- see docs/grammar-assumptions.md "Still open" and ADR-003): about-this-page (staticContent), breadcrumb (breadcrumb), credentials-form (staticContent).
...
import { WebCredentialsSettingsPage } from './p00503-web-credentials-settings.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 503: Web Credentials Settings [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new WebCredentialsSettingsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new WebCredentialsSettingsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Web Credentials Settings'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (2 items, 0 hidden)', async ({ page }) => {
    const po = new WebCredentialsSettingsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P503_API_KEY', 'P503_NAME']);
  });
