/**
 * GENERATED by apx-testgen from pages/p00103-rest-data-source.apx — DO NOT EDIT.
 ...
 * (./p00103-rest-data-source.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, overview, search, search-results, web-credentials-not-provided
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), overview (staticContent), search (staticContent), search-results (search), web-credentials-not-provided (staticContent).
...
import { RestDataSourcePage } from './p00103-rest-data-source.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 103: REST Data Source [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new RestDataSourcePage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new RestDataSourcePage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('REST Data Source'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 0 hidden)', async ({ page }) => {
    const po = new RestDataSourcePage(page);
    await po.goto();
    await expectItemsPresent(page, ['P103_SEARCH']);
  });

  test('apex.item round-trip on P103_SEARCH', async ({ page }) => {
    const po = new RestDataSourcePage(page);
    await po.goto();
  ...
  test('every labeled .apx button exists (3 buttons)', async ({ page }) => {
    const po = new RestDataSourcePage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Next', 'Previous', 'Provide Credentials']);
  });
});
