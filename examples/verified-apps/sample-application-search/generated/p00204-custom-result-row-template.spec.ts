/**
 * GENERATED by apx-testgen from pages/p00204-custom-result-row-template.apx — DO NOT EDIT.
 ...
 * (./p00204-custom-result-row-template.page.js), not raw testkit calls, so both stay in sync.
...
import { CustomResultRowTemplatePage } from './p00204-custom-result-row-template.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 204: Custom Result Row Template [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new CustomResultRowTemplatePage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new CustomResultRowTemplatePage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Custom Result Row Template'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 0 hidden)', async ({ page }) => {
    const po = new CustomResultRowTemplatePage(page);
    await po.goto();
    await expectItemsPresent(page, ['P204_SEARCH']);
  });

  test('apex.item round-trip on P204_SEARCH', async ({ page }) => {
    const po = new CustomResultRowTemplatePage(page);
    await po.goto();
