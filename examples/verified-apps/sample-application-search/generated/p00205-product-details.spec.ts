 ...
 * (./p00205-product-details.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: left, product-details, reviews, right
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Region types NOT covered by an auto-generated assertion (no verified DOM convention, or a runtime id genuinely unconstructible from static data -- see docs/grammar-assumptions.md "Still open" and ADR-003): left (staticContent), product-details (form), reviews (listView), right (staticContent).
...
import { ProductDetailsPage } from './p00205-product-details.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 205: Product Details [requires auth]', () => {
  ...
  test('every .apx pageItem exists in DOM/apex.item registry (9 items, 5 hidden)', async ({ page }) => {
    const po = new ProductDetailsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P205_DESCRIPTION', 'P205_IMAGE_CHARSET', 'P205_IMAGE_FILENAME', 'P205_IMAGE_LAST_UPDATED', 'P205_IMAGE_MIME_TYPE', 'P205_PRODUCT_ID', 'P205_PRODUCT_IMAGE', 'P205_PRODUCT_NAME', 'P205_UNIT_PRICE']);
  });
