/**
 * GENERATED by apx-testgen from pages/p00202-link-search-result-to-form.apx — DO NOT EDIT.
 ...
 * (./p00202-link-search-result-to-form.page.js), not raw testkit calls, so both stay in sync.
...
import { LinkSearchResultToFormPage } from './p00202-link-search-result-to-form.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 202: Link Search Result to Form [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new LinkSearchResultToFormPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new LinkSearchResultToFormPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Link Search Result to Form'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 0 hidden)', async ({ page }) => {
    const po = new LinkSearchResultToFormPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P202_SEARCH']);
  });

  test('apex.item round-trip on P202_SEARCH', async ({ page }) => {
    const po = new LinkSearchResultToFormPage(page);
    await po.goto();
