/**
 * GENERATED by apx-testgen from pages/p00402-text-index-function.apx — DO NOT EDIT.
 ...
 * (./p00402-text-index-function.page.js), not raw testkit calls, so both stay in sync.
...
import { TextIndexFunctionPage } from './p00402-text-index-function.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 402: Text Index Function [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new TextIndexFunctionPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new TextIndexFunctionPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Text Index Function'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 0 hidden)', async ({ page }) => {
    const po = new TextIndexFunctionPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P402_SEARCH']);
  });

  test('apex.item round-trip on P402_SEARCH', async ({ page }) => {
    const po = new TextIndexFunctionPage(page);
    await po.goto();
