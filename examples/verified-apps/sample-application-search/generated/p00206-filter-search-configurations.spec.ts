/**
 * GENERATED by apx-testgen from pages/p00206-filter-search-configurations.apx — DO NOT EDIT.
 ...
 * (./p00206-filter-search-configurations.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, overview, search, search-results, search-results-container
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), overview (staticContent), search (staticContent), search-results (search), search-results-container (staticContent).
...
import { FilterSearchConfigurationsPage } from './p00206-filter-search-configurations.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 206: Filter Search Configurations [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new FilterSearchConfigurationsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new FilterSearchConfigurationsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Filter Search Configurations'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (2 items, 0 hidden)', async ({ page }) => {
    const po = new FilterSearchConfigurationsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P206_CHECK_GRP', 'P206_SEARCH']);
  });

  test('apex.item round-trip on P206_SEARCH', async ({ page }) => {
    const po = new FilterSearchConfigurationsPage(page);
    await po.goto();
