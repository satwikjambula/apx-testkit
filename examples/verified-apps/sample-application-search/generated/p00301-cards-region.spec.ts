 ...
 * (./p00301-cards-region.page.js), not raw testkit calls, so both stay in sync.
...
import { CardsRegionPage } from './p00301-cards-region.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 301: Cards Region [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new CardsRegionPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new CardsRegionPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Cards Region'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 0 hidden)', async ({ page }) => {
    const po = new CardsRegionPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P301_SEARCH']);
  });

  test('apex.item round-trip on P301_SEARCH', async ({ page }) => {
    const po = new CardsRegionPage(page);
    await po.goto();
