 ...
 * (./p00302-map-region.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, overview, search, search-region, search-results-map, store-details
 ...
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), overview (staticContent), search (staticContent), search-region (staticContent), search-results-map (map).
...
import { MapRegionPage } from './p00302-map-region.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 302: Map Region [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new MapRegionPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new MapRegionPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Map Region'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (2 items, 1 hidden)', async ({ page }) => {
    const po = new MapRegionPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P302_SEARCH', 'P302_STORE_ID']);
  });

  test('apex.item round-trip on P302_SEARCH', async ({ page }) => {
    const po = new MapRegionPage(page);
    await po.goto();
  ...
  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new MapRegionPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['store-details']);
  });
});
