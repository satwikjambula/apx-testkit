 ...
 * (./p00105-company-details.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, company-details, details, overview
 ...
 * Region types NOT covered by an auto-generated assertion (no verified DOM convention, or a runtime id genuinely unconstructible from static data -- see docs/grammar-assumptions.md "Still open" and ADR-003): buttons (staticContent), company-details (form), overview (staticContent).
...
import { CompanyDetailsPage } from './p00105-company-details.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 105: Company Details [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new CompanyDetailsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new CompanyDetailsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Company Details'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (8 items, 8 hidden)', async ({ page }) => {
    const po = new CompanyDetailsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P105_DESCRIPTION', 'P105_HEADQUARTERS', 'P105_HOMEPAGE', 'P105_ID', 'P105_LOGO_PATH', 'P105_NAME', 'P105_ORIGIN_COUNTRY', 'P105_PARENT_COMPANY']);
  });
