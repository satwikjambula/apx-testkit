/**
 * GENERATED by apx-testgen from pages/p00207-multiple-search-regions.apx — DO NOT EDIT.
 ...
 * (./p00207-multiple-search-regions.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, cancelled-orders, complete-orders, overview, search, search-results, search-results-2, search-results-container
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Region types NOT covered by an auto-generated assertion (no verified DOM convention, or a runtime id genuinely unconstructible from static data -- see docs/grammar-assumptions.md "Still open" and ADR-003): breadcrumb (breadcrumb), cancelled-orders (staticContent), complete-orders (staticContent), overview (staticContent), search (staticContent), search-results (search), search-results-2 (search), search-results-container (staticContent).
...
import { MultipleSearchRegionsPage } from './p00207-multiple-search-regions.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 207: Multiple Search Regions [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new MultipleSearchRegionsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new MultipleSearchRegionsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Multiple Search Regions'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 0 hidden)', async ({ page }) => {
    const po = new MultipleSearchRegionsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P207_SEARCH']);
  });

  test('apex.item round-trip on P207_SEARCH', async ({ page }) => {
    const po = new MultipleSearchRegionsPage(page);
    await po.goto();
