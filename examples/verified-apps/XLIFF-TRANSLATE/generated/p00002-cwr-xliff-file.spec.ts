 ...
 * (./p00002-cwr-xliff-file.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, cwr-xliff-file
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), cwr-xliff-file (form).
...
import { CwrXliffFilePage } from './p00002-cwr-xliff-file.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 2: Cwr Xliff File [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new CwrXliffFilePage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new CwrXliffFilePage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Cwr Xliff File'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (14 items, 9 hidden)', async ({ page }) => {
    const po = new CwrXliffFilePage(page);
    await po.goto();
    await expectItemsPresent(page, ['P2_ID', 'P2_NAME', 'P2_SOURCE_LANG', 'P2_TARGET_LANG', 'P2_XLIFF_RESULT', 'P2_XLIFF_RESULT_CHARSET', 'P2_XLIFF_RESULT_FILENAME', 'P2_XLIFF_RESULT_LASTUPD', 'P2_XLIFF_RESULT_MIMETYPE', 'P2_XLIFF_SOURCE', 'P2_XLIFF_SOURCE_CHARSET', 'P2_XLIFF_SOURCE_FILENAME', 'P2_XLIFF_SOURCE_LASTUPD', 'P2_XLIFF_SOURCE_MIMETYPE']);
  });

  test('apex.item round-trip on P2_NAME', async ({ page }) => {
    const po = new CwrXliffFilePage(page);
    await po.goto();
    await po.name.setValue('apx-testgen');
    expect(await po.name.getValue()).toBe('apx-testgen');
  });
