 ...
 * (./p00003-translate.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: ai-translate, breadcrumb, translate
 ...
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): ai-translate (staticContent), breadcrumb (breadcrumb).
...
import { TranslatePage } from './p00003-translate.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 3: Translate [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new TranslatePage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new TranslatePage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Translate'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (6 items, 1 hidden)', async ({ page }) => {
    const po = new TranslatePage(page);
    await po.goto();
    await expectItemsPresent(page, ['P3_GID', 'P3_ID', 'P3_SOURCE_LANG', 'P3_SOURCE_TEXT', 'P3_TARGET_LANG', 'P3_TARGET_TEXT']);
  });

  test('apex.item round-trip on P3_SOURCE_TEXT', async ({ page }) => {
    const po = new TranslatePage(page);
    await po.goto();
    await po.sourceText.setValue('apx-testgen');
    expect(await po.sourceText.getValue()).toBe('apx-testgen');
  });

  test('every labeled .apx button exists (7 buttons)', async ({ page }) => {
    const po = new TranslatePage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Apply', 'Batch Translate', 'Cancel', 'Load From Result', 'Load From Source', 'Store As Result', 'Translate']);
  });

  test('every Interactive Grid region with a known runtime id resolves a current view (1 region)', async ({ page }) => {
    const po = new TranslatePage(page);
    await po.goto();
    for (const id of ['IGTRANS']) {
