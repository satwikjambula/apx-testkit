/**
 * GENERATED by apx-testgen from pages/p00004-messages.apx — DO NOT EDIT.
 ...
 * (./p00004-messages.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, messages
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * 1 Interactive Grid region(s) SKIPPED -- no htmlDomId set, runtime id genuinely unconstructible from static data (ADR-003 layer 3): messages.
...
import { MessagesPage } from './p00004-messages.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 4: Messages [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new MessagesPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new MessagesPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Messages'));
  });
});
