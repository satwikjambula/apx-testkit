 ...
 * Regions present in metadata: xliff-files, xliff-translate
...
test.describe('page 1: XLIFF Files [requires auth]', () => {
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new HomePage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('XLIFF Translate'));
  });
  ...
  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new HomePage(page);
    await po.goto();
    await expectRegionsResolve(page, ['xliff-files']);
  });
});
