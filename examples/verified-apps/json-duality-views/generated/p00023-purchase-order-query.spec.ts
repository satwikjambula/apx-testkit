/**
 * GENERATED by apx-testgen from pages/p00023-purchase-order-query.apx — DO NOT EDIT.
 ...
 * (./p00023-purchase-order-query.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: about, display-query, duality-view-ddl, json-schema, query, region-display-selector
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): about (staticContent), display-query (staticContent), duality-view-ddl (staticContent), json-schema (staticContent), query (classicReport), region-display-selector (regionDisplaySelector).
...
import { PurchaseOrderQueryPage } from './p00023-purchase-order-query.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 23: Purchase Order Query [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new PurchaseOrderQueryPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new PurchaseOrderQueryPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Purchase Order Query'));
  });
});
