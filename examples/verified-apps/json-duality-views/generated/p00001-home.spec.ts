 ...
 * Regions present in metadata: about, app-navigation, duality-view-demo-region
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): about (staticContent), app-navigation (list), duality-view-demo-region (staticContent).
...
test.describe('page 1: Home [requires auth]', () => {
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new HomePage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Sample Duality Views'));
  });
});
