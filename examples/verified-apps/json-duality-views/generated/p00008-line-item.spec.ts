 ...
 * (./p00008-line-item.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: line-item-buttons, line-item-form
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): line-item-buttons (staticContent), line-item-form (form).
...
import { LineItemPage } from './p00008-line-item.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 8: Line Item [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new LineItemPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new LineItemPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Line Item'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (8 items, 3 hidden)', async ({ page }) => {
    const po = new LineItemPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P8_LINE_ID', 'P8_PO_ID', 'P8_PRODUCT_DESC', 'P8_PRODUCT_PRODUCT_CODE', 'P8_PRODUCT_UNIT_PRICE', 'P8_QUANTITY', 'P8_TOTAL', 'P8_UNIT_PRICE']);
  });

  test('apex.item round-trip on P8_PRODUCT_DESC', async ({ page }) => {
    const po = new LineItemPage(page);
    await po.goto();
    await po.productDesc.setValue('apx-testgen');
    expect(await po.productDesc.getValue()).toBe('apx-testgen');
  });

  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new LineItemPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Create', 'Delete', 'Apply']);
  });
});
