 ...
 * (./p00002-create-purchase-order.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: create-purchase-order-buttons, create-purchase-order-form, line-items-container, line-items-grid
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * 1 Interactive Grid region(s) SKIPPED -- no htmlDomId set, runtime id genuinely unconstructible from static data (ADR-003 layer 3): line-items-grid.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): create-purchase-order-buttons (staticContent), create-purchase-order-form (form), line-items-container (staticContent).
...
import { CreatePurchaseOrderPage } from './p00002-create-purchase-order.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 2: Create Purchase Order [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new CreatePurchaseOrderPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new CreatePurchaseOrderPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Create Purchase Order'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (3 items, 2 hidden)', async ({ page }) => {
    const po = new CreatePurchaseOrderPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P2_CUSTOMER_ID', 'P2_PO_ID', 'P2_PO_NUMBER']);
  });

  test('every labeled .apx button exists (1 button)', async ({ page }) => {
    const po = new CreatePurchaseOrderPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Create']);
  });
});
