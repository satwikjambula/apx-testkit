 ...
 * (./p00009-add-line-item.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: add-line-item-buttons, add-line-item-form
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): add-line-item-buttons (staticContent), add-line-item-form (form).
...
import { AddLineItemPage } from './p00009-add-line-item.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 9: Add Line Item [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new AddLineItemPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new AddLineItemPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Add Line Item'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (6 items, 2 hidden)', async ({ page }) => {
    const po = new AddLineItemPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P9_LINE_ID', 'P9_PO_ID', 'P9_PRODUCT_ID', 'P9_QUANTITY', 'P9_TOTAL', 'P9_UNIT_PRICE']);
  });
