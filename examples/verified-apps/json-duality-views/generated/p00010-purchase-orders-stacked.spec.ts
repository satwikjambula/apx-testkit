 ...
 * (./p00010-purchase-orders-stacked.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: duality-view-demo-region, purchase-orders-report
 ...
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): duality-view-demo-region (breadcrumb).
...
import { PurchaseOrdersStackedPage } from './p00010-purchase-orders-stacked.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 10: Purchase Orders - Stacked [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new PurchaseOrdersStackedPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new PurchaseOrdersStackedPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Purchase Orders - Stacked'));
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new PurchaseOrdersStackedPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Create Purchase Order', 'Reset']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new PurchaseOrdersStackedPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['purchase-orders-report']);
  });
});
