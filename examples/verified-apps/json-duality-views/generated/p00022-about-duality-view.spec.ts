/**
 * GENERATED by apx-testgen from pages/p00022-about-duality-view.apx — DO NOT EDIT.
 ...
 * (./p00022-about-duality-view.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: apex-usage, breadcrumb, duality-view-ddl, json-schema, region-display-selector, sample-json-document
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): apex-usage (staticContent), breadcrumb (breadcrumb), duality-view-ddl (staticContent), json-schema (staticContent), region-display-selector (regionDisplaySelector), sample-json-document (staticContent).
...
import { AboutDualityViewPage } from './p00022-about-duality-view.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 22: About JSON Relational Duality View [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new AboutDualityViewPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new AboutDualityViewPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('About JSON Relational Duality View'));
  });
});
