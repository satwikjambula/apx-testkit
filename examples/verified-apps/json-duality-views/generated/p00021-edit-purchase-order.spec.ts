 ...
 * (./p00021-edit-purchase-order.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, dv-purchase-orders
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), dv-purchase-orders (form).
...
import { EditPurchaseOrderPage } from './p00021-edit-purchase-order.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 21: Edit Purchase Order [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new EditPurchaseOrderPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new EditPurchaseOrderPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Edit Purchase Order'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (17 items, 5 hidden)', async ({ page }) => {
    const po = new EditPurchaseOrderPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P21_CUSTOMER_ADDRESS', 'P21_CUSTOMER_CITY', 'P21_CUSTOMER_COMPANY_NAME', 'P21_CUSTOMER_EMAIL', 'P21_CUSTOMER_ID', 'P21_CUSTOMER_NAME', 'P21_CUSTOMER_PHONE', 'P21_CUSTOMER_STATE', 'P21_CUSTOMER_ZIP_CODE', 'P21_DATE', 'P21_DELIVERY_INSTRUCTIONS', 'P21_LAST_UPDATED', 'P21_LINE_ITEMS', 'P21_PO_ID', 'P21_PO_NUMBER', 'P21_STATUS', 'P21_TOTAL_AMOUNT']);
  });

  test('apex.item round-trip on P21_CUSTOMER_ADDRESS', async ({ page }) => {
    const po = new EditPurchaseOrderPage(page);
    await po.goto();
