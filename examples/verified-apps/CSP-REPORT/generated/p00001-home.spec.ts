 ...
 * Regions present in metadata: csp-report, policy-violations
...
test.describe('page 1: Policy Violations [requires auth]', () => {
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new HomePage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('CSP Report'));
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new HomePage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Reset', 'リセット']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new HomePage(page);
    await po.goto();
    await expectRegionsResolve(page, ['policy-violations']);
  });
});
