/**
 * GENERATED by apx-testgen from pages/p00004-store-inventory.apx — DO NOT EDIT.
 ...
 * (./p00004-store-inventory.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: inventory-api, inventory-summary
 ...
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): inventory-api (staticContent).
...
import { StoreInventoryPage } from './p00004-store-inventory.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 4: Store Inventory [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new StoreInventoryPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new StoreInventoryPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Store Inventory'));
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new StoreInventoryPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['inventory-summary']);
  });
});
