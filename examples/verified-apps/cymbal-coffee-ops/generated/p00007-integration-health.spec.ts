/**
 * GENERATED by apx-testgen from pages/p00007-integration-health.apx — DO NOT EDIT.
 ...
 * (./p00007-integration-health.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: catalog-status-api, integration-health, vector-status-api
 ...
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): integration-health (staticContent).
...
import { IntegrationHealthPage } from './p00007-integration-health.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 7: Integration Health [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new IntegrationHealthPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new IntegrationHealthPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Integration Health'));
  });

  test('every interactiveReport/cards/facetedSearch region resolves (2 regions)', async ({ page }) => {
    const po = new IntegrationHealthPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['catalog-status-api', 'vector-status-api']);
  });
});
