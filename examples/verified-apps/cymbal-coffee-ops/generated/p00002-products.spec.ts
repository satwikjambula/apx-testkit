/**
 * GENERATED by apx-testgen from pages/p00002-products.apx — DO NOT EDIT.
 ...
 * (./p00002-products.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: product-catalog, products-api
 ...
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): products-api (staticContent).
...
import { ProductsPage } from './p00002-products.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 2: Product Catalog [requires auth]', () => {
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ProductsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Product Catalog'));
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new ProductsPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['product-catalog']);
  });
});
