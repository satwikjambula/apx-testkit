/**
 * GENERATED by apx-testgen from pages/p00003-stores.apx — DO NOT EDIT.
 ...
 * (./p00003-stores.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: stores, stores-api
 ...
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): stores-api (staticContent).
...
import { StoresPage } from './p00003-stores.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 3: Stores [requires auth]', () => {
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new StoresPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Stores'));
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new StoresPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['stores']);
  });
});
