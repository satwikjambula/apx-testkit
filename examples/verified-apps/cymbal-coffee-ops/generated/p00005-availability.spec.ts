/**
 * GENERATED by apx-testgen from pages/p00005-availability.apx — DO NOT EDIT.
 ...
 * (./p00005-availability.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: availability-api, availability-lookup
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): availability-api (staticContent), availability-lookup (staticContent).
...
import { AvailabilityPage } from './p00005-availability.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 5: Availability Lookup [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new AvailabilityPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new AvailabilityPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Availability Lookup'));
  });
});
