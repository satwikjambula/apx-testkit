/**
 * GENERATED by apx-testgen from pages/p00006-vector-recommendations.apx — DO NOT EDIT.
 ...
 * (./p00006-vector-recommendations.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: recommendations-api, vector-recommendations
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): recommendations-api (staticContent), vector-recommendations (staticContent).
...
import { VectorRecommendationsPage } from './p00006-vector-recommendations.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 6: Vector Recommendations [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new VectorRecommendationsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new VectorRecommendationsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Vector Recommendations'));
  });
});
