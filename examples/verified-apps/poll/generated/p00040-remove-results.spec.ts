/**
 * GENERATED by apx-testgen from pages/p00040-remove-results.apx — DO NOT EDIT.
 ...
 * (./p00040-remove-results.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: button, delete-results
...
import { RemoveResultsPage } from './p00040-remove-results.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 40: Remove Results [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new RemoveResultsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new RemoveResultsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Remove Results'));
  });
  ...
  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new RemoveResultsPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['delete-results']);
  });
});
