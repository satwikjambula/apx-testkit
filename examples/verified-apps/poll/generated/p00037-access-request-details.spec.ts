 ...
 * (./p00037-access-request-details.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: access-request-details, buttons
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): access-request-details (staticContent), buttons (staticContent).
...
import { AccessRequestDetailsPage } from './p00037-access-request-details.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 37: Access Request Details [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new AccessRequestDetailsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new AccessRequestDetailsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Access Request Details'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (9 items, 1 hidden)', async ({ page }) => {
    const po = new AccessRequestDetailsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P37_ACTIONED_BY', 'P37_ACTIONED_REASON', 'P37_ALREADY_APP_USER', 'P37_CREATED', 'P37_EMAIL_ADDRESS', 'P37_ID', 'P37_JUSTIFICATION', 'P37_STATUS', 'P37_USERNAME']);
  });

  test('apex.item round-trip on P37_ACTIONED_REASON', async ({ page }) => {
    const po = new AccessRequestDetailsPage(page);
    await po.goto();
    await po.actionedReason.setValue('apx-testgen');
    expect(await po.actionedReason.getValue()).toBe('apx-testgen');
  });

  test('every labeled .apx button exists (3 buttons)', async ({ page }) => {
    const po = new AccessRequestDetailsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Approve Request for Access', 'Cancel', 'Decline']);
  });
});
