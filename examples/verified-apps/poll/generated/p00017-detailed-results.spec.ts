 ...
 * (./p00017-detailed-results.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, current-poll, result-details
 ...
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), current-poll (staticContent).
...
import { DetailedResultsPage } from './p00017-detailed-results.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 17: Detailed Results [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new DetailedResultsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new DetailedResultsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Detailed Results'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (21 items, 20 hidden)', async ({ page }) => {
    const po = new DetailedResultsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P17_POLL_NAME', 'P17_Q1', 'P17_Q10', 'P17_Q11', 'P17_Q12', 'P17_Q13', 'P17_Q14', 'P17_Q15', 'P17_Q16', 'P17_Q17', 'P17_Q18', 'P17_Q19', 'P17_Q2', 'P17_Q20', 'P17_Q3', 'P17_Q4', 'P17_Q5', 'P17_Q6', 'P17_Q7', 'P17_Q8', 'P17_Q9']);
  });

  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new DetailedResultsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Back', 'Alternate View', 'Remove Results', 'Reset']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new DetailedResultsPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['result-details']);
  });
});
