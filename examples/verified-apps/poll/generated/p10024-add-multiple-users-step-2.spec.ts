/**
 * GENERATED by apx-testgen from pages/p10024-add-multiple-users-step-2.apx — DO NOT EDIT.
 ...
 * (./p10024-add-multiple-users-step-2.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: add-multiple-users-step, button-bar, exceptions, hidden-items, no-valid-users-exist-page-info, p10024-valid-count-users-to-add, valid-users-exist-page-info
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): add-multiple-users-step (staticContent), button-bar (staticContent), exceptions (classicReport), hidden-items (staticContent), no-valid-users-exist-page-info (staticContent), p10024-valid-count-users-to-add (listView), valid-users-exist-page-info (plSqlDynamicContent).
...
import { AddMultipleUsersStep2Page } from './p10024-add-multiple-users-step-2.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 10024: Add Multiple Users - Step 2 [requires auth]', () => {
  ...
  test('every .apx pageItem exists in DOM/apex.item registry (3 items, 3 hidden)', async ({ page }) => {
    const po = new AddMultipleUsersStep2Page(page);
    await po.goto();
    await expectItemsPresent(page, ['P10024_INVALID_COUNT', 'P10024_ROLE', 'P10024_VALID_COUNT']);
  });
