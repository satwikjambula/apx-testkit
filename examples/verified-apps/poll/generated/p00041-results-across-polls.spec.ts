/**
 * GENERATED by apx-testgen from pages/p00041-results-across-polls.apx — DO NOT EDIT.
 ...
 * (./p00041-results-across-polls.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, result-details
...
import { ResultsAcrossPollsPage } from './p00041-results-across-polls.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 41: Results Across Polls [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ResultsAcrossPollsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ResultsAcrossPollsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Results Across Polls'));
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new ResultsAcrossPollsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Back', 'Reset']);
  });
