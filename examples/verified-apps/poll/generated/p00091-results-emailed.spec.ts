/**
 * GENERATED by apx-testgen from pages/p00091-results-emailed.apx — DO NOT EDIT.
 ...
 * (./p00091-results-emailed.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, button, emails-not-sent, emails-sent, results-emailed-for-poll-name
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), button (staticContent), emails-not-sent (classicReport), emails-sent (classicReport), results-emailed-for-poll-name (staticContent).
...
import { ResultsEmailedPage } from './p00091-results-emailed.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 91: Results Emailed [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ResultsEmailedPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ResultsEmailedPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Results Emailed'));
  });
