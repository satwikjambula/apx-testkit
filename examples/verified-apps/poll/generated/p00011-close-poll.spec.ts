 ...
 * (./p00011-close-poll.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, close-poll
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), close-poll (staticContent).
...
import { ClosePollPage } from './p00011-close-poll.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 11: Close Poll [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ClosePollPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ClosePollPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Close Poll'));
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new ClosePollPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Close Poll']);
  });
});
