/**
 * GENERATED by apx-testgen from pages/p00022-access-requests.apx — DO NOT EDIT.
 ...
 * (./p00022-access-requests.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: access-requests
...
import { AccessRequestsPage } from './p00022-access-requests.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 22: Access Requests [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new AccessRequestsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new AccessRequestsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Access Requests'));
  });
  ...
  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new AccessRequestsPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['access-requests']);
  });
});
