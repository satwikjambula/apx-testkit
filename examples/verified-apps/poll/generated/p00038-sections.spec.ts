 ...
 * (./p00038-sections.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, question-image-optional, sections
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), question-image-optional (plSqlDynamicContent), sections (staticContent).
...
import { SectionsPage } from './p00038-sections.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 38: Sections [requires auth]', () => {
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new SectionsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Sections'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (9 items, 2 hidden)', async ({ page }) => {
    const po = new SectionsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P38_DISPLAY_SEQUENCE', 'P38_ID', 'P38_IMAGE_BLOB', 'P38_IMAGE_HEIGHT', 'P38_IMAGE_WIDTH', 'P38_POLL_ID', 'P38_POLL_NAME', 'P38_SECTION_TEXT', 'P38_TITLE']);
  });

  test('apex.item round-trip on P38_SECTION_TEXT', async ({ page }) => {
    const po = new SectionsPage(page);
    await po.goto();
    await po.sectionText.setValue('apx-testgen');
    expect(await po.sectionText.getValue()).toBe('apx-testgen');
  });

  test('every labeled .apx button exists (5 buttons)', async ({ page }) => {
    const po = new SectionsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Add Section', 'Delete', 'Remove Image', 'Apply Changes']);
  });
});
