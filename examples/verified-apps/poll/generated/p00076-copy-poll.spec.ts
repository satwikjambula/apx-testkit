 ...
 * (./p00076-copy-poll.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: select-poll-to-copy
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): select-poll-to-copy (staticContent).
...
import { CopyPollPage } from './p00076-copy-poll.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 76: Copy Poll [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new CopyPollPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new CopyPollPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Copy Poll'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (2 items, 1 hidden)', async ({ page }) => {
    const po = new CopyPollPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P76_NEW_POLL_ID', 'P76_POLL_ID']);
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new CopyPollPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Copy Poll']);
  });
});
