/**
 * GENERATED by apx-testgen from pages/p00060-application-user-log.apx — DO NOT EDIT.
 ...
 * (./p00060-application-user-log.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: application-user-log
...
import { ApplicationUserLogPage } from './p00060-application-user-log.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 60: Application User Log [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ApplicationUserLogPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ApplicationUserLogPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Application User Log'));
  });
  ...
  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new ApplicationUserLogPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['application-user-log']);
  });
});
