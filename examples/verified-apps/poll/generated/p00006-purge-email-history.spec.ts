/**
 * GENERATED by apx-testgen from pages/p00006-purge-email-history.apx — DO NOT EDIT.
 ...
 * (./p00006-purge-email-history.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, purge-email-history
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), purge-email-history (staticContent).
...
import { PurgeEmailHistoryPage } from './p00006-purge-email-history.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 6: Purge Email History [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new PurgeEmailHistoryPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new PurgeEmailHistoryPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Purge Email History'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 0 hidden)', async ({ page }) => {
    const po = new PurgeEmailHistoryPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P6_NBR_DAYS_TO_KEEP']);
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new PurgeEmailHistoryPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Purge Email History']);
  });
});
