 ...
 * (./p10021-manage-user-access.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: manage-user-access
...
import { ManageUserAccessPage } from './p10021-manage-user-access.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 10021: Manage User Access [requires auth]', () => {
  ...
  test('every labeled .apx button exists (3 buttons)', async ({ page }) => {
    const po = new ManageUserAccessPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Add Multiple Users', 'Add User', 'Reset']);
  });
