 ...
 * (./p00073-view-response.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, response
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), response (plSqlDynamicContent).
...
import { ViewResponsePage } from './p00073-view-response.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 73: View Response [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ViewResponsePage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ViewResponsePage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('View Response'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (2 items, 2 hidden)', async ({ page }) => {
    const po = new ViewResponsePage(page);
    await po.goto();
    await expectItemsPresent(page, ['P73_POLL_NAME', 'P73_RESULT_ID']);
  });
