 ...
 * (./p00090-email-results.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, community-members, email-results-for-poll-name, header, items, items-2
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), community-members (classicReport), email-results-for-poll-name (staticContent), header (staticContent), items (staticContent), items-2 (staticContent).
...
import { EmailResultsPage } from './p00090-email-results.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 90: Email Results [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new EmailResultsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new EmailResultsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Email Results'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (4 items, 0 hidden)', async ({ page }) => {
    const po = new EmailResultsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P90_COMMUNITY_ID', 'P90_EMAIL_FROM', 'P90_EMAIL_TO', 'P90_INTRO_TEXT']);
  });

  test('apex.item round-trip on P90_EMAIL_TO', async ({ page }) => {
    const po = new EmailResultsPage(page);
    await po.goto();
    await po.emailTo.setValue('apx-testgen');
    expect(await po.emailTo.getValue()).toBe('apx-testgen');
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new EmailResultsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Email Results']);
  });
});
