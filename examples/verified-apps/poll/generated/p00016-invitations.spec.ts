 ...
 * (./p00016-invitations.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, invitations
...
import { InvitationsPage } from './p00016-invitations.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 16: Invitations [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new InvitationsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new InvitationsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Invitations'));
  });
  ...
  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new InvitationsPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['invitations']);
  });
});
