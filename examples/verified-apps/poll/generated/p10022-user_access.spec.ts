 ...
 * (./p10022-user_access.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: button-bar, form-items-region
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): button-bar (staticContent), form-items-region (staticContent).
...
import { UserAccessPage } from './p10022-user_access.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 10022: Manage User Access [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new UserAccessPage(page);
  ...
  test('every .apx pageItem exists in DOM/apex.item registry (4 items, 2 hidden)', async ({ page }) => {
    const po = new UserAccessPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P10022_APPLICATION_ID', 'P10022_ID', 'P10022_ROLE_ID', 'P10022_USERNAME']);
  });

  test('apex.item round-trip on P10022_USERNAME', async ({ page }) => {
    const po = new UserAccessPage(page);
    await po.goto();
  ...
  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new UserAccessPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Add User', 'Delete', 'Apply Changes']);
  });
});
