 ...
 * (./p00023-invitations-sent.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, community-invites, invitations-sent
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), community-invites (classicReport), invitations-sent (staticContent).
...
import { InvitationsSentPage } from './p00023-invitations-sent.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 23: Invitations Sent [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new InvitationsSentPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new InvitationsSentPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Invitations Sent'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 0 hidden)', async ({ page }) => {
    const po = new InvitationsSentPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P23_OTHER_INVITEES_MSG']);
  });

  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new InvitationsSentPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Invite More', 'View Emails', 'View Invitations', 'Manage Poll']);
  });
});
