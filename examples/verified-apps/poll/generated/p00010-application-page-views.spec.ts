/**
 * GENERATED by apx-testgen from pages/p00010-application-page-views.apx — DO NOT EDIT.
 ...
 * (./p00010-application-page-views.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: application-page-views, button-bar
 ...
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): button-bar (staticContent).
...
import { ApplicationPageViewsPage } from './p00010-application-page-views.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 10: Application Page Views [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ApplicationPageViewsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ApplicationPageViewsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Application Page Views'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 0 hidden)', async ({ page }) => {
    const po = new ApplicationPageViewsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P10_SINCE']);
  });
  ...
  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new ApplicationPageViewsPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['application-page-views']);
  });
});
