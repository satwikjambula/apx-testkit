 ...
 * (./p00075-delete-member-from-community.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, community-member
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), community-member (staticContent).
...
import { DeleteMemberFromCommunityPage } from './p00075-delete-member-from-community.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 75: Delete Member from Community [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new DeleteMemberFromCommunityPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new DeleteMemberFromCommunityPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Delete Member from Community'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (5 items, 3 hidden)', async ({ page }) => {
    const po = new DeleteMemberFromCommunityPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P75_COMMUNITY_ID', 'P75_COMMUNITY_NAME', 'P75_ID', 'P75_MEMBER_NAME', 'P75_RESPONDENT_ID']);
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new DeleteMemberFromCommunityPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Remove Member']);
  });
});
