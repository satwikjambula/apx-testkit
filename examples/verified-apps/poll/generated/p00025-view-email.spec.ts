/**
 * GENERATED by apx-testgen from pages/p00025-view-email.apx — DO NOT EDIT.
 ...
 * (./p00025-view-email.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: preview-email, view-email
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): preview-email (dynamicContent), view-email (staticContent).
...
import { ViewEmailPage } from './p00025-view-email.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 25: View Email [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ViewEmailPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ViewEmailPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('View Email'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 1 hidden)', async ({ page }) => {
    const po = new ViewEmailPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P25_EMAIL_ID']);
  });
});
