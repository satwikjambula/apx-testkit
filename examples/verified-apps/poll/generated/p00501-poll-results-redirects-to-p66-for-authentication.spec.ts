 ...
 * (./p00501-poll-results-redirects-to-p66-for-authentication.page.js), not raw testkit calls, so both stay in sync.
...
import { PollResultsRedirectsToP66ForAuthenticationPage } from './p00501-poll-results-redirects-to-p66-for-authentication.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 501: Poll Results - redirects to p66 for authentication [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new PollResultsRedirectsToP66ForAuthenticationPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new PollResultsRedirectsToP66ForAuthenticationPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Poll Results'));
  });
});
