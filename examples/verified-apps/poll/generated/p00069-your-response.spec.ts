/**
 * GENERATED by apx-testgen from pages/p00069-your-response.apx — DO NOT EDIT.
 ...
 * (./p00069-your-response.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: hidden-items, your-response
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): hidden-items (staticContent), your-response (plSqlDynamicContent).
...
import { YourResponsePage } from './p00069-your-response.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 69: Your Response [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new YourResponsePage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new YourResponsePage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Your Response'));
  });
});
