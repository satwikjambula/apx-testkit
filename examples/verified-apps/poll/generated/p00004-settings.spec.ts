 ...
 * (./p00004-settings.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: access-control, access-control-actions, acl-information, administration, administrative-reports, authorization-and-authentication, breadcrumb, confirm-enabling-access-control, email-address-not-set, email-administration, hidden-items, user-counts-report
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): access-control (staticContent), access-control-actions (list), acl-information (plSqlDynamicContent), administration (list), administrative-reports (list), authorization-and-authentication (plugin/authorizationAdministration), breadcrumb (breadcrumb), confirm-enabling-access-control (staticContent), email-address-not-set (staticContent), email-administration (list), hidden-items (staticContent), user-counts-report (classicReport).
...
import { SettingsPage } from './p00004-settings.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 4: Administration [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new SettingsPage(page);
  ...
  test('every .apx pageItem exists in DOM/apex.item registry (6 items, 6 hidden)', async ({ page }) => {
    const po = new SettingsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P4_APP_ERROR_LOG_CNT', 'P4_APP_LOG_CNT', 'P4_APP_USER_LOG_CNT', 'P4_BUILD_OPTION_CNT', 'P4_NOTIFICATION_CNT', 'P4_SELF_SRVC_REQ_CNT']);
  });

  test('every labeled .apx button exists (3 buttons)', async ({ page }) => {
    const po = new SettingsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Add', 'Cancel', 'Enable Access Control']);
  });
});
