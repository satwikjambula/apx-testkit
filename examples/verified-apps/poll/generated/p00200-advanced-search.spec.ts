/**
 * GENERATED by apx-testgen from pages/p00200-advanced-search.apx — DO NOT EDIT.
 ...
 * (./p00200-advanced-search.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, polls-contributors
...
import { AdvancedSearchPage } from './p00200-advanced-search.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 200: Advanced Search [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new AdvancedSearchPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new AdvancedSearchPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Advanced Search'));
  });
  ...
  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new AdvancedSearchPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['polls-contributors']);
  });
});
