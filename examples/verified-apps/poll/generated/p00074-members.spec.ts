 ...
 * (./p00074-members.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, members
...
import { MembersPage } from './p00074-members.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 74: Members [requires auth]', () => {
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new MembersPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Members'));
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new MembersPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Add Members', 'Reset']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new MembersPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['members']);
  });
});
