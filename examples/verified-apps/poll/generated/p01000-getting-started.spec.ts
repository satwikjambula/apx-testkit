/**
 * GENERATED by apx-testgen from pages/p01000-getting-started.apx — DO NOT EDIT.
 ...
 * (./p01000-getting-started.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: access-control, acl-level, application-options, buttons, default-email-address, feature-configuration, first-time-setup-completed, introduction-wizard, sample-data, users, users-2, welcome-to-application-title
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): access-control (staticContent), acl-level (staticContent), application-options (staticContent), buttons (staticContent), default-email-address (staticContent), feature-configuration (classicReport), first-time-setup-completed (staticContent), introduction-wizard (staticContent), sample-data (staticContent), users (staticContent), users-2 (classicReport), welcome-to-application-title (staticContent).
...
import { GettingStartedPage } from './p01000-getting-started.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 1000: Getting Started [requires auth]', () => {
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new GettingStartedPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Getting Started'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (4 items, 0 hidden)', async ({ page }) => {
    const po = new GettingStartedPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P1000_ACL_CONFIG', 'P1000_FROM', 'P1000_LOAD_SAMPLE_YN', 'P1000_USERNAME_FORMAT']);
  });

  test('apex.item round-trip on P1000_FROM', async ({ page }) => {
    const po = new GettingStartedPage(page);
    await po.goto();
    await po.from.setValue('apx-testgen');
    expect(await po.from.getValue()).toBe('apx-testgen');
  });

  test('every labeled .apx button exists (3 buttons)', async ({ page }) => {
    const po = new GettingStartedPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Add User', 'Complete Setup', 'Continue']);
  });
});
