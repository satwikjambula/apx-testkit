/**
 * GENERATED by apx-testgen from pages/p00031-notification.apx — DO NOT EDIT.
 ...
 * (./p00031-notification.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, existing-notifications, form-items
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), existing-notifications (classicReport), form-items (staticContent).
...
import { NotificationPage } from './p00031-notification.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 31: Notification [requires auth]', () => {
  ...
  test('every .apx pageItem exists in DOM/apex.item registry (7 items, 1 hidden)', async ({ page }) => {
    const po = new NotificationPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P31_DISPLAY_FROM', 'P31_DISPLAY_SEQUENCE', 'P31_DISPLAY_UNTIL', 'P31_ID', 'P31_NOTIFICATION_DESCRIPTION', 'P31_NOTIFICATION_NAME', 'P31_NOTIFICATION_TYPE']);
  });

  test('apex.item round-trip on P31_DISPLAY_SEQUENCE', async ({ page }) => {
    const po = new NotificationPage(page);
    await po.goto();
