 ...
 * (./p00057-email-preview.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: button-bar, show-template
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): button-bar (staticContent), show-template (plSqlDynamicContent).
...
import { EmailPreviewPage } from './p00057-email-preview.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 57: Email Preview [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new EmailPreviewPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new EmailPreviewPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Email Preview'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 1 hidden)', async ({ page }) => {
    const po = new EmailPreviewPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P57_METHOD']);
  });

  test('every labeled .apx button exists (1 button)', async ({ page }) => {
    const po = new EmailPreviewPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Close Preview']);
  });
});
