/**
 * GENERATED by apx-testgen from pages/p00018-poll-history.apx — DO NOT EDIT.
 ...
 * (./p00018-poll-history.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: change-history, history
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): change-history (classicReport), history (classicReport).
...
import { PollHistoryPage } from './p00018-poll-history.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 18: Poll History [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new PollHistoryPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new PollHistoryPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Poll History'));
  });
});
