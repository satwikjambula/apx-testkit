 ...
 * (./p00072-invite.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: advanced, already-invited, buttons, community-members, invite-community-and-or-individuals, warning-email-from-address-undefined, warning-poll-in-testing-phase
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): advanced (staticContent), already-invited (classicReport), buttons (staticContent), community-members (classicReport), invite-community-and-or-individuals (staticContent), warning-email-from-address-undefined (staticContent), warning-poll-in-testing-phase (staticContent).
...
import { InvitePage } from './p00072-invite.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 72: Invite [requires auth]', () => {
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new InvitePage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Invite'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (12 items, 6 hidden)', async ({ page }) => {
    const po = new InvitePage(page);
    await po.goto();
    await expectItemsPresent(page, ['P72_COMMUNITY_ID', 'P72_COMM_INVITE_ID', 'P72_COMM_INVITE_ID2', 'P72_EMAIL_FROM', 'P72_INVITE_BODY', 'P72_INVITE_METHOD', 'P72_INVITE_SUBJECT', 'P72_OTHER_INVITEES', 'P72_OTHER_INVITEES_MSG', 'P72_POLL_ID', 'P72_POLL_NAME', 'P72_PREVIEW_URL']);
  });

  test('apex.item round-trip on P72_INVITE_BODY', async ({ page }) => {
    const po = new InvitePage(page);
    await po.goto();
    await po.inviteBody.setValue('apx-testgen');
    expect(await po.inviteBody.getValue()).toBe('apx-testgen');
  });

  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new InvitePage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Add New Community', 'Cancel', 'Preview', 'Invite']);
  });
});
