/**
 * GENERATED by apx-testgen from pages/p00058-default-email-addresses.apx — DO NOT EDIT.
 ...
 * (./p00058-default-email-addresses.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, default-email-addresses
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), default-email-addresses (staticContent).
...
import { DefaultEmailAddressesPage } from './p00058-default-email-addresses.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 58: Default Email Addresses [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new DefaultEmailAddressesPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new DefaultEmailAddressesPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Default Email Addresses'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (2 items, 0 hidden)', async ({ page }) => {
    const po = new DefaultEmailAddressesPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P58_FROM', 'P58_REPLY_TO']);
  });

  test('apex.item round-trip on P58_FROM', async ({ page }) => {
    const po = new DefaultEmailAddressesPage(page);
    await po.goto();
