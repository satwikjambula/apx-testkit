 ...
 * (./p00071-community.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: create-community, email-addresses-to-be-added-to-this-community, exceptions, footer, hidden-items, invalid-count, no-valid-emails-exist-page-info, valid-count
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): create-community (staticContent), email-addresses-to-be-added-to-this-community (listView), exceptions (classicReport), footer (staticContent), hidden-items (staticContent), invalid-count (plugin/translatedMessage), no-valid-emails-exist-page-info (staticContent), valid-count (plugin/translatedMessage).
...
import { CommunityPage } from './p00071-community.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 71: Community [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new CommunityPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new CommunityPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Community'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (5 items, 2 hidden)', async ({ page }) => {
    const po = new CommunityPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P71_COMMUNITY_FROM', 'P71_COMMUNITY_NAME', 'P71_COMMUNITY_TO', 'P71_INVALID_COUNT', 'P71_VALID_COUNT']);
  });

  test('apex.item round-trip on P71_COMMUNITY_FROM', async ({ page }) => {
    const po = new CommunityPage(page);
    await po.goto();
    await po.communityFrom.setValue('apx-testgen');
    expect(await po.communityFrom.getValue()).toBe('apx-testgen');
  });

  test('every labeled .apx button exists (3 buttons)', async ({ page }) => {
    const po = new CommunityPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Add Community / Members', 'Previous']);
  });
});
