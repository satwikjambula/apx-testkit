/**
 * GENERATED by apx-testgen from pages/p00008-rename-application.apx — DO NOT EDIT.
 ...
 * (./p00008-rename-application.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, form-items
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), form-items (staticContent).
...
import { RenameApplicationPage } from './p00008-rename-application.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 8: Rename Application [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new RenameApplicationPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new RenameApplicationPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Rename Application'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 0 hidden)', async ({ page }) => {
    const po = new RenameApplicationPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P8_APPLICATION_TITLE']);
  });

  test('apex.item round-trip on P8_APPLICATION_TITLE', async ({ page }) => {
    const po = new RenameApplicationPage(page);
    await po.goto();
    await po.applicationTitle.setValue('apx-testgen');
    expect(await po.applicationTitle.getValue()).toBe('apx-testgen');
  });
