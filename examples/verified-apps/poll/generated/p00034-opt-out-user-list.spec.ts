/**
 * GENERATED by apx-testgen from pages/p00034-opt-out-user-list.apx — DO NOT EDIT.
 ...
 * (./p00034-opt-out-user-list.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: opt-out-user-list
...
import { OptOutUserListPage } from './p00034-opt-out-user-list.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 34: Opt Out User List [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new OptOutUserListPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new OptOutUserListPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Opt Out User List'));
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new OptOutUserListPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Add User', 'Reset']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new OptOutUserListPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['opt-out-user-list']);
  });
});
