 ...
 * (./p00082-delete-test-data.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, delete-test-data
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), delete-test-data (staticContent).
...
import { DeleteTestDataPage } from './p00082-delete-test-data.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 82: Delete Test Data [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new DeleteTestDataPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new DeleteTestDataPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Delete Test Data'));
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new DeleteTestDataPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Delete Test Data']);
  });
});
