/**
 * GENERATED by apx-testgen from pages/p10023-add-multiple-users-step-1.apx — DO NOT EDIT.
 ...
 * (./p10023-add-multiple-users-step-1.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: button-bar, wizard-container
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): button-bar (staticContent), wizard-container (staticContent).
...
import { AddMultipleUsersStep1Page } from './p10023-add-multiple-users-step-1.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 10023: Add Multiple Users - Step 1 [requires auth]', () => {
  ...
  test('every .apx pageItem exists in DOM/apex.item registry (3 items, 0 hidden)', async ({ page }) => {
    const po = new AddMultipleUsersStep1Page(page);
    await po.goto();
    await expectItemsPresent(page, ['P10023_PRELIM_USERS', 'P10023_ROLE', 'P10023_USERNAME_FORMAT']);
  });

  test('apex.item round-trip on P10023_PRELIM_USERS', async ({ page }) => {
    const po = new AddMultipleUsersStep1Page(page);
    await po.goto();
