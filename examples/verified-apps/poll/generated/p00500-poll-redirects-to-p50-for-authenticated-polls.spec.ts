/**
 * GENERATED by apx-testgen from pages/p00500-poll-redirects-to-p50-for-authenticated-polls.apx — DO NOT EDIT.
 ...
 * (./p00500-poll-redirects-to-p50-for-authenticated-polls.page.js), not raw testkit calls, so both stay in sync.
...
import { PollRedirectsToP50ForAuthenticatedPollsPage } from './p00500-poll-redirects-to-p50-for-authenticated-polls.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 500: Poll - redirects to p50 for authenticated polls [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new PollRedirectsToP50ForAuthenticatedPollsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new PollRedirectsToP50ForAuthenticatedPollsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Poll'));
  });
});
