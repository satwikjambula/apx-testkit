/**
 * GENERATED by apx-testgen from pages/p00067-preview-poll.apx — DO NOT EDIT.
 ...
 * (./p00067-preview-poll.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, hidden-items, poll-preview
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), hidden-items (staticContent), poll-preview (plSqlDynamicContent).
...
import { PreviewPollPage } from './p00067-preview-poll.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 67: Preview Poll [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new PreviewPollPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new PreviewPollPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Preview Poll'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 1 hidden)', async ({ page }) => {
    const po = new PreviewPollPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P67_POLL_NAME']);
  });
