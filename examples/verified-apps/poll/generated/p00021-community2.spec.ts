 ...
 * (./p00021-community2.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, details, members
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), details (staticContent), members (classicReport).
...
import { Community2Page } from './p00021-community2.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 21: Community [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new Community2Page(page);
  ...
  test('every .apx pageItem exists in DOM/apex.item registry (3 items, 1 hidden)', async ({ page }) => {
    const po = new Community2Page(page);
    await po.goto();
    await expectItemsPresent(page, ['P21_ID', 'P21_NAME', 'P21_PUBLISH_YN']);
  });

  test('apex.item round-trip on P21_NAME', async ({ page }) => {
    const po = new Community2Page(page);
    await po.goto();
  ...
  test('every labeled .apx button exists (6 buttons)', async ({ page }) => {
    const po = new Community2Page(page);
    await po.goto();
    await expectButtonsPresent(page, ['Add Members', 'Cancel', 'Add Respondent Community', 'Delete Community', 'Manage Members', 'Apply Changes']);
  });
});
