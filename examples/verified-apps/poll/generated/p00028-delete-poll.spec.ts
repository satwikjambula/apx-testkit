 ...
 * (./p00028-delete-poll.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: delete-poll, new
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): delete-poll (staticContent), new (staticContent).
...
import { DeletePollPage } from './p00028-delete-poll.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 28: Delete Poll [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new DeletePollPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new DeletePollPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Delete Poll'));
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new DeletePollPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Delete Poll']);
  });
});
