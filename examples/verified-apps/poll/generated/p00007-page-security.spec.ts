/**
 * GENERATED by apx-testgen from pages/p00007-page-security.apx — DO NOT EDIT.
 ...
 * (./p00007-page-security.page.js), not raw testkit calls, so both stay in sync.
...
import { PageSecurityPage } from './p00007-page-security.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 7: Page Security [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new PageSecurityPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new PageSecurityPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Page Security'));
  });
