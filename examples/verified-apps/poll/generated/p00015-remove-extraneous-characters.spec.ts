 ...
 * (./p00015-remove-extraneous-characters.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, character-to-scrub, matching-data, remove-extraneous-characters, updates-done
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): breadcrumb (breadcrumb), character-to-scrub (staticContent), matching-data (classicReport), remove-extraneous-characters (staticContent), updates-done (staticContent).
...
import { RemoveExtraneousCharactersPage } from './p00015-remove-extraneous-characters.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 15: Remove Extraneous Characters [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new RemoveExtraneousCharactersPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new RemoveExtraneousCharactersPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Remove Extraneous Characters'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (5 items, 0 hidden)', async ({ page }) => {
    const po = new RemoveExtraneousCharactersPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P15_CHAR_TO_SCRUB', 'P15_COMM_REF_CNT', 'P15_INVITE_CNT', 'P15_MATCHING_RESP_CNT', 'P15_NON_MATCH_RESP_CNT']);
  });

  test('apex.item round-trip on P15_CHAR_TO_SCRUB', async ({ page }) => {
    const po = new RemoveExtraneousCharactersPage(page);
    await po.goto();
    await po.charToScrub.setValue('apx-testgen');
    expect(await po.charToScrub.getValue()).toBe('apx-testgen');
  });

  test('every labeled .apx button exists (4 buttons)', async ({ page }) => {
    const po = new RemoveExtraneousCharactersPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'OK', 'Go', 'Scrub Data']);
  });
});
