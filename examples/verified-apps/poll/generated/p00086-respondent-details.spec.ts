 ...
 * (./p00086-respondent-details.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: polls-not-taken-but-invited-to-take, polls-taken, selection
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): polls-not-taken-but-invited-to-take (classicReport), polls-taken (classicReport), selection (staticContent).
...
import { RespondentDetailsPage } from './p00086-respondent-details.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 86: Respondent Details [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new RespondentDetailsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new RespondentDetailsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Respondent Details'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (2 items, 0 hidden)', async ({ page }) => {
    const po = new RespondentDetailsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P86_RESPONDENT_ID', 'P86_ROLE']);
  });
});
