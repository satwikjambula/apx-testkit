/**
 * GENERATED by apx-testgen from pages/p00084-all-respondents.apx — DO NOT EDIT.
 ...
 * (./p00084-all-respondents.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: poll-responses
...
import { AllRespondentsPage } from './p00084-all-respondents.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 84: All Respondents [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new AllRespondentsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new AllRespondentsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('All Respondents'));
  });
  ...
  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new AllRespondentsPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['poll-responses']);
  });
});
