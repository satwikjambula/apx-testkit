 ...
 * (./p00068-poll-name.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, chart, details, hidden-items, hide-nobody, question, region-display-selector
 ...
 * 1 chart region(s) SKIPPED -- no htmlDomId set, runtime id genuinely unconstructible from static data (ADR-003 layer 3): chart.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), hidden-items (staticContent), hide-nobody (staticContent), question (plSqlDynamicContent), region-display-selector (regionDisplaySelector).
...
import { PollNamePage } from './p00068-poll-name.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 68: &POLL_NAME. [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new PollNamePage(page);
  ...
  test('every .apx pageItem exists in DOM/apex.item registry (5 items, 5 hidden)', async ({ page }) => {
    const po = new PollNamePage(page);
    await po.goto();
    await expectItemsPresent(page, ['P68_ANONYMOUS_YN', 'P68_NEXT_QID', 'P68_PREV_QID', 'P68_QUESTION_ID', 'P68_QUESTION_NBR']);
  });

  test('every labeled .apx button exists (5 buttons)', async ({ page }) => {
    const po = new PollNamePage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Next', 'Next', 'Previous', 'Previous', 'Refresh Results']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new PollNamePage(page);
    await po.goto();
    await expectRegionsResolve(page, ['details']);
  });
});
