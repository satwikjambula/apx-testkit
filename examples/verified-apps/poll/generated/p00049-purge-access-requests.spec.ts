/**
 * GENERATED by apx-testgen from pages/p00049-purge-access-requests.apx — DO NOT EDIT.
 ...
 * (./p00049-purge-access-requests.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, purge-access-requests
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), purge-access-requests (staticContent).
...
import { PurgeAccessRequestsPage } from './p00049-purge-access-requests.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 49: Purge Access Requests [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new PurgeAccessRequestsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new PurgeAccessRequestsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Purge Access Requests'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 0 hidden)', async ({ page }) => {
    const po = new PurgeAccessRequestsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P49_NBR_DAYS_TO_KEEP']);
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new PurgeAccessRequestsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Purge Access Requests']);
  });
});
