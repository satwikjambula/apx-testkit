 ...
 * (./p00046-opt-out-user-details.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, opt-out-user-details
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), opt-out-user-details (staticContent).
...
import { OptOutUserDetailsPage } from './p00046-opt-out-user-details.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 46: Opt Out User Details [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new OptOutUserDetailsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new OptOutUserDetailsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Opt Out User Details'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (4 items, 1 hidden)', async ({ page }) => {
    const po = new OptOutUserDetailsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P46_CREATED', 'P46_CREATED_BY', 'P46_ID', 'P46_RESPONDENT_ID']);
  });

  test('every labeled .apx button exists (3 buttons)', async ({ page }) => {
    const po = new OptOutUserDetailsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Add User', 'Cancel', 'Delete']);
  });
});
