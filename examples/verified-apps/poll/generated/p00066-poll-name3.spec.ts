 ...
 * (./p00066-poll-name3.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: average-score, breadcrumb, button-region, hidden-items, hide-nobody, invites, no-access, outstanding-invitations, poll-attributes, quiz-average, refresh, results, side-region, summarized-answers, summary
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): average-score (classicReport), breadcrumb (breadcrumb), button-region (staticContent), hidden-items (staticContent), hide-nobody (staticContent), invites (classicReport), no-access (staticContent), outstanding-invitations (plSqlDynamicContent), poll-attributes (classicReport), quiz-average (classicReport), refresh (staticContent), results (classicReport), side-region (staticContent), summarized-answers (plSqlDynamicContent), summary (staticContent).
...
import { PollName3Page } from './p00066-poll-name3.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 66: &POLL_NAME. [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new PollName3Page(page);
  ...
  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 0 hidden)', async ({ page }) => {
    const po = new PollName3Page(page);
    await po.goto();
    await expectItemsPresent(page, ['P66_AUTO_REFRESH']);
  });

  test('every labeled .apx button exists (9 buttons)', async ({ page }) => {
    const po = new PollName3Page(page);
    await po.goto();
    await expectButtonsPresent(page, ['Email Results', 'Manage Poll', 'Permalink', 'Refresh Results', 'Send Reminders', 'Take this Poll', 'Update Your Response', 'View Detailed Results', 'View Your Response']);
  });
});
