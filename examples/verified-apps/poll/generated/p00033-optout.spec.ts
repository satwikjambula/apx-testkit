/**
 * GENERATED by apx-testgen from pages/p00033-optout.apx — DO NOT EDIT.
 ...
 * (./p00033-optout.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: hidden-items, hide-nobody, opt-in, opt-out
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): hidden-items (staticContent), hide-nobody (staticContent), opt-in (staticContent), opt-out (staticContent).
...
import { OptoutPage } from './p00033-optout.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 33: Opt Out of Email [requires auth]', () => {
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new OptoutPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Opt Out of Email'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 1 hidden)', async ({ page }) => {
    const po = new OptoutPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P33_PERSON_ID']);
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new OptoutPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Opt Back In', 'Opt Out']);
  });
});
