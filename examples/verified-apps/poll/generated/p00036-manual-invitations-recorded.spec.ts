 ...
 * (./p00036-manual-invitations-recorded.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, community-invitations, invitations-recorded, other
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), community-invitations (classicReport), invitations-recorded (staticContent), other (staticContent).
...
import { ManualInvitationsRecordedPage } from './p00036-manual-invitations-recorded.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 36: Manual Invitations Recorded [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ManualInvitationsRecordedPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ManualInvitationsRecordedPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Manual Invitations Recorded'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 0 hidden)', async ({ page }) => {
    const po = new ManualInvitationsRecordedPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P36_OTHER_INVITEES_MSG']);
  });

  test('every labeled .apx button exists (3 buttons)', async ({ page }) => {
    const po = new ManualInvitationsRecordedPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Invite More', 'View Invitations', 'Manage Poll']);
  });
});
