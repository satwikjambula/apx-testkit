/**
 * GENERATED by apx-testgen from pages/p00009-communities.apx — DO NOT EDIT.
 ...
 * (./p00009-communities.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, communities
...
import { CommunitiesPage } from './p00009-communities.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 9: Communities [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new CommunitiesPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new CommunitiesPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Communities'));
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new CommunitiesPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Add Community / Members', 'Reset']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new CommunitiesPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['communities']);
  });
});
