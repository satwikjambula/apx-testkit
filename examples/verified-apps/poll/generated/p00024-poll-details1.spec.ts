 ...
 * (./p00024-poll-details1.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: details
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): details (classicReport).
...
import { PollDetails1Page } from './p00024-poll-details1.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 24: Details [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new PollDetails1Page(page);
  ...
  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 1 hidden)', async ({ page }) => {
    const po = new PollDetails1Page(page);
    await po.goto();
    await expectItemsPresent(page, ['P24_TAKE_URL']);
  });
});
