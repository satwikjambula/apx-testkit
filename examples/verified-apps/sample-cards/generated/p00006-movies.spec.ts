/**
 * GENERATED by apx-testgen from pages/p00006-movies.apx — DO NOT EDIT.
 ...
 * (./p00006-movies.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: about-this-page, apex-youtube-channel, breadcrumb
...
import { MoviesPage } from './p00006-movies.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 6: Embedded Video [requires auth]', () => {
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new MoviesPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('&APP_NAME. - Embedded Video'));
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new MoviesPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['apex-youtube-channel']);
  });
});
