/**
 * GENERATED by apx-testgen from pages/p00004-image-url.apx — DO NOT EDIT.
 ...
 * (./p00004-image-url.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: about-this-page, breadcrumb, image-url
...
import { ImageUrlPage } from './p00004-image-url.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 4: Image URL [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ImageUrlPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ImageUrlPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('&APP_NAME. - Image URL'));
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new ImageUrlPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['image-url']);
  });
});
