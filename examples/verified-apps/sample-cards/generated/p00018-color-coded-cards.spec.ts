/**
 * GENERATED by apx-testgen from pages/p00018-color-coded-cards.apx — DO NOT EDIT.
 ...
 * (./p00018-color-coded-cards.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: about-this-page, breadcrumb, cards, icons, rds
...
import { ColorCodedCardsPage } from './p00018-color-coded-cards.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 18: Color Coded Cards [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ColorCodedCardsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ColorCodedCardsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('&APP_NAME. - Color Coded Cards'));
  });

  test('every interactiveReport/cards/facetedSearch region resolves (2 regions)', async ({ page }) => {
    const po = new ColorCodedCardsPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['cards', 'icons']);
  });
});
