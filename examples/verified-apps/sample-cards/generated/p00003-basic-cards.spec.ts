/**
 * GENERATED by apx-testgen from pages/p00003-basic-cards.apx — DO NOT EDIT.
 ...
 * (./p00003-basic-cards.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: about-this-page, breadcrumb, rds, style-a, style-b, style-c
...
import { BasicCardsPage } from './p00003-basic-cards.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 3: Basic Cards [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new BasicCardsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new BasicCardsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('&APP_NAME. - Styles'));
  });

  test('every interactiveReport/cards/facetedSearch region resolves (3 regions)', async ({ page }) => {
    const po = new BasicCardsPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['style-a', 'style-b', 'style-c']);
  });
});
