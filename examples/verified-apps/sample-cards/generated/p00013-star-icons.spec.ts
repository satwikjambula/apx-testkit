/**
 * GENERATED by apx-testgen from pages/p00013-star-icons.apx — DO NOT EDIT.
 ...
 * (./p00013-star-icons.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: about-this-page, breadcrumb, star-icons
...
import { StarIconsPage } from './p00013-star-icons.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 13: Star Icons [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new StarIconsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new StarIconsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('&APP_NAME. - Star Icons'));
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new StarIconsPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['star-icons']);
  });
});
