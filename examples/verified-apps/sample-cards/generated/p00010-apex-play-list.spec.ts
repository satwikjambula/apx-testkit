/**
 * GENERATED by apx-testgen from pages/p00010-apex-play-list.apx — DO NOT EDIT.
 ...
 * (./p00010-apex-play-list.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: about-this-page, apex-play-list, breadcrumb
...
import { ApexPlayListPage } from './p00010-apex-play-list.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 10: APEX Play List [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ApexPlayListPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ApexPlayListPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('&APP_NAME. - Video Images with Durations'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 1 hidden)', async ({ page }) => {
    const po = new ApexPlayListPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P10_ID']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new ApexPlayListPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['apex-play-list']);
  });
});
