 ...
 * (./p00014-employee-detail.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, employee-detail
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Region types NOT covered by an auto-generated assertion (no verified DOM convention, or a runtime id genuinely unconstructible from static data -- see docs/grammar-assumptions.md "Still open" and ADR-003): buttons (staticContent), employee-detail (form).
...
import { EmployeeDetailPage } from './p00014-employee-detail.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 14: Employee Detail [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new EmployeeDetailPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new EmployeeDetailPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Employee Detail'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (13 items, 4 hidden)', async ({ page }) => {
    const po = new EmployeeDetailPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P14_COMM', 'P14_DEPTNO', 'P14_EMPNO', 'P14_ENAME', 'P14_FILENAME', 'P14_HIREDATE', 'P14_IMAGE_LAST_UPDATE', 'P14_JOB', 'P14_MGR', 'P14_MIMETYPE', 'P14_PROFILE_IMAGE', 'P14_SAL', 'P14_TAGS']);
  });

  test('apex.item round-trip on P14_ENAME', async ({ page }) => {
    const po = new EmployeeDetailPage(page);
    await po.goto();
