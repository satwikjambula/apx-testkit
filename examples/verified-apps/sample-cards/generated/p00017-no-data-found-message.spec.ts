/**
 * GENERATED by apx-testgen from pages/p00017-no-data-found-message.apx — DO NOT EDIT.
 ...
 * (./p00017-no-data-found-message.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: about-this-page, breadcrumb, default-no-data-found, no-data-found-with-icon, rds
...
import { NoDataFoundMessagePage } from './p00017-no-data-found-message.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 17: No Data Found Message [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new NoDataFoundMessagePage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new NoDataFoundMessagePage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('&APP_NAME. - No Data Found'));
  });

  test('every interactiveReport/cards/facetedSearch region resolves (2 regions)', async ({ page }) => {
    const po = new NoDataFoundMessagePage(page);
    await po.goto();
    await expectRegionsResolve(page, ['default-no-data-found', 'no-data-found-with-icon']);
  });
});
