/**
 * GENERATED by apx-testgen from pages/p00007-background-image.apx — DO NOT EDIT.
 ...
 * (./p00007-background-image.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: about-this-page, background-image, breadcrumb
...
import { BackgroundImagePage } from './p00007-background-image.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 7: Background Image [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new BackgroundImagePage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new BackgroundImagePage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('&APP_NAME. - Background Image'));
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new BackgroundImagePage(page);
    await po.goto();
    await expectRegionsResolve(page, ['background-image']);
  });
});
