/**
 * GENERATED by apx-testgen from pages/p00002-blob-column.apx — DO NOT EDIT.
 ...
 * (./p00002-blob-column.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: about-this-page, breadcrumb, icon-image, media-image, rds
...
import { BlobColumnPage } from './p00002-blob-column.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 2: BLOB column [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new BlobColumnPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new BlobColumnPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('&APP_NAME. - BLOB column'));
  });

  test('every interactiveReport/cards/facetedSearch region resolves (2 regions)', async ({ page }) => {
    const po = new BlobColumnPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['icon-image', 'media-image']);
  });
});
