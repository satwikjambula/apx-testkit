/**
 * GENERATED by apx-testgen from pages/p00016-conditional-actions.apx — DO NOT EDIT.
 ...
 * (./p00016-conditional-actions.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: about-this-page, breadcrumb, conditional-card-actions
 ...
 * Region types NOT covered by an auto-generated assertion (no verified DOM convention, or a runtime id genuinely unconstructible from static data -- see docs/grammar-assumptions.md "Still open" and ADR-003): about-this-page (staticContent), breadcrumb (breadcrumb).
...
import { ConditionalActionsPage } from './p00016-conditional-actions.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 16: Conditional Actions [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ConditionalActionsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ConditionalActionsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('&APP_NAME. - Conditional Actions'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 0 hidden)', async ({ page }) => {
    const po = new ConditionalActionsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P16_ORDER_BY']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new ConditionalActionsPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['conditional-card-actions']);
  });
});
