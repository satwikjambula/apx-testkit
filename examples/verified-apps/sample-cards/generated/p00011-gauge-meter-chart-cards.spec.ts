/**
 * GENERATED by apx-testgen from pages/p00011-gauge-meter-chart-cards.apx — DO NOT EDIT.
 ...
 * (./p00011-gauge-meter-chart-cards.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: about-this-page, breadcrumb, gauge-meter-chart-cards
...
import { GaugeMeterChartCardsPage } from './p00011-gauge-meter-chart-cards.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 11: Gauge Meter Chart Cards [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new GaugeMeterChartCardsPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new GaugeMeterChartCardsPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('&APP_NAME. - Charts'));
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new GaugeMeterChartCardsPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['gauge-container']);
  });
});
