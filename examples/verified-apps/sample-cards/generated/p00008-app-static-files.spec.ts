/**
 * GENERATED by apx-testgen from pages/p00008-app-static-files.apx — DO NOT EDIT.
...
import { AppStaticFilesPage } from './p00008-app-static-files.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 8: Application Static Files [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new AppStaticFilesPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new AppStaticFilesPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('&APP_NAME. - Application Static Files'));
  });

  test('every interactiveReport/cards/facetedSearch region resolves (3 regions)', async ({ page }) => {
    const po = new AppStaticFilesPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['float-with-custom-css', 'horizontal-media-background', 'horizontal-media-first']);
  });
});
