 ...
 * (./p00021-images-and-media.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, image-and-media-examples
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Region types NOT covered by an auto-generated assertion (no verified DOM convention, or a runtime id genuinely unconstructible from static data -- see docs/grammar-assumptions.md "Still open" and ADR-003): breadcrumb (breadcrumb), image-and-media-examples (list).
...
import { ImagesAndMediaPage } from './p00021-images-and-media.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 21: Images and Media [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new ImagesAndMediaPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ImagesAndMediaPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('&APP_NAME. - Images and Media'));
  });
});
