...
import { MixedLanguagesPage } from './p00042-mixed-languages.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 42: Mixed Languages [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new MixedLanguagesPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new MixedLanguagesPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Mixed Languages'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 0 hidden)', async ({ page }) => {
    const po = new MixedLanguagesPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P42_SEARCH']);
  });

  test('apex.item round-trip on P42_SEARCH', async ({ page }) => {
    const po = new MixedLanguagesPage(page);
    await po.goto();
