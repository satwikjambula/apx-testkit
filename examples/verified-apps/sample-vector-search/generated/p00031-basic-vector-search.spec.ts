 ...
 * (./p00031-basic-vector-search.page.js), not raw testkit calls, so both stay in sync.
...
import { BasicVectorSearchPage } from './p00031-basic-vector-search.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 31: Basic Vector Search [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new BasicVectorSearchPage(page);
  ...
  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 0 hidden)', async ({ page }) => {
    const po = new BasicVectorSearchPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P31_SEARCH']);
  });

  test('apex.item round-trip on P31_SEARCH', async ({ page }) => {
    const po = new BasicVectorSearchPage(page);
    await po.goto();
