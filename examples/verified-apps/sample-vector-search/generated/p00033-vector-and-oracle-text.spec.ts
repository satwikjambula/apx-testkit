 ...
 * (./p00033-vector-and-oracle-text.page.js), not raw testkit calls, so both stay in sync.
...
import { VectorAndOracleTextPage } from './p00033-vector-and-oracle-text.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 33: Vector and Oracle Text [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new VectorAndOracleTextPage(page);
  ...
  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 0 hidden)', async ({ page }) => {
    const po = new VectorAndOracleTextPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P33_SEARCH']);
  });

  test('apex.item round-trip on P33_SEARCH', async ({ page }) => {
    const po = new VectorAndOracleTextPage(page);
    await po.goto();
