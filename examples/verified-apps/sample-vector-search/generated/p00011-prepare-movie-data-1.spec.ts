 ...
 * (./p00011-prepare-movie-data-1.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: button-bar, data-generation, progress
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Region types NOT covered by an auto-generated assertion (no verified DOM convention, or a runtime id genuinely unconstructible from static data -- see docs/grammar-assumptions.md "Still open" and ADR-003): button-bar (staticContent), data-generation (staticContent), progress (list).
...
import { PrepareMovieData1Page } from './p00011-prepare-movie-data-1.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 11: Prepare Movie Data [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new PrepareMovieData1Page(page);
  ...
  test('every .apx pageItem exists in DOM/apex.item registry (2 items, 1 hidden)', async ({ page }) => {
    const po = new PrepareMovieData1Page(page);
    await po.goto();
    await expectItemsPresent(page, ['P11_EXE_ID', 'P11_PROMPT_INPUT']);
  });

  test('apex.item round-trip on P11_PROMPT_INPUT', async ({ page }) => {
    const po = new PrepareMovieData1Page(page);
    await po.goto();
    await po.promptInput.setValue('apx-testgen');
    expect(await po.promptInput.getValue()).toBe('apx-testgen');
  });

  test('every labeled .apx button exists (1 button)', async ({ page }) => {
    const po = new PrepareMovieData1Page(page);
    await po.goto();
    await expectButtonsPresent(page, ['Generate Data']);
  });
});
