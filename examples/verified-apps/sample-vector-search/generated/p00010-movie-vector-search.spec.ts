/**
 * GENERATED by apx-testgen from pages/p00010-movie-vector-search.apx — DO NOT EDIT.
 ...
 * (./p00010-movie-vector-search.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, movie-vector-search-navigation
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Region types NOT covered by an auto-generated assertion (no verified DOM convention, or a runtime id genuinely unconstructible from static data -- see docs/grammar-assumptions.md "Still open" and ADR-003): breadcrumb (breadcrumb), movie-vector-search-navigation (list).
...
import { MovieVectorSearchPage } from './p00010-movie-vector-search.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 10: Movie Vector Search [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new MovieVectorSearchPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new MovieVectorSearchPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Movie Vector Search'));
  });
});
