 ...
 * (./p00054-set-ai-provider.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, button-oci, button-openai, enter-oci-web-credentials, enter-openai-web-credentials, select-provider, web-credentials
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Region types NOT covered by an auto-generated assertion (no verified DOM convention, or a runtime id genuinely unconstructible from static data -- see docs/grammar-assumptions.md "Still open" and ADR-003): breadcrumb (breadcrumb), button-oci (staticContent), button-openai (staticContent), enter-oci-web-credentials (staticContent), enter-openai-web-credentials (staticContent), select-provider (staticContent), web-credentials (staticContent).
...
import { SetAiProviderPage } from './p00054-set-ai-provider.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 54: Set AI Provider [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new SetAiProviderPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new SetAiProviderPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Set AI Provider'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (11 items, 0 hidden)', async ({ page }) => {
    const po = new SetAiProviderPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P54_AI_PROVIDER', 'P54_OCI_COMPARTMENT_ID', 'P54_OCI_FINGERPRINT', 'P54_OCI_PRIVATE_KEY', 'P54_OCI_REGION', 'P54_OCI_TENANCY_ID', 'P54_OCI_USER_ID', 'P54_OCI_WEB_CREDENTIALS_HEADER', 'P54_OPENAI_API_KEY', 'P54_OPENAI_WEB_CRED_HEADER', 'P54_SHOW_WEB_CREDENTIALS']);
  });

  test('apex.item round-trip on P54_OCI_COMPARTMENT_ID', async ({ page }) => {
    const po = new SetAiProviderPage(page);
    await po.goto();
    await po.ociCompartmentId.setValue('apx-testgen');
    expect(await po.ociCompartmentId.getValue()).toBe('apx-testgen');
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new SetAiProviderPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Apply Changes', 'Apply Changes']);
  });
});
