 ...
 * (./p00026-prepare-custom-data-2.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, loaded-file, preview, progress, rds, upload-file
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): buttons (staticContent), loaded-file (staticContent), preview (classicReport), progress (list), rds (regionDisplaySelector), upload-file (staticContent).
...
import { PrepareCustomData2Page } from './p00026-prepare-custom-data-2.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 26: Prepare Custom Data (Step 2: Upload Data) [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new PrepareCustomData2Page(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new PrepareCustomData2Page(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Prepare Custom Data'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 1 hidden)', async ({ page }) => {
    const po = new PrepareCustomData2Page(page);
    await po.goto();
    await expectItemsPresent(page, ['P26_FILE_NAME']);
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new PrepareCustomData2Page(page);
    await po.goto();
    await expectButtonsPresent(page, ['Load Data', 'Back']);
  });
});
