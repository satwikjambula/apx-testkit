/**
 * GENERATED by apx-testgen from pages/p00041-movie-recommendation.apx — DO NOT EDIT.
 ...
 * (./p00041-movie-recommendation.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: about-movie-recommendation, breadcrumb, content, search, search-results-oci, search-results-openai
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): about-movie-recommendation (staticContent), breadcrumb (breadcrumb), content (staticContent), search (staticContent), search-results-oci (search), search-results-openai (search).
...
import { MovieRecommendationPage } from './p00041-movie-recommendation.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 41: Movie Recommendation [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new MovieRecommendationPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new MovieRecommendationPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Movie Recommendation'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 0 hidden)', async ({ page }) => {
    const po = new MovieRecommendationPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P41_SEARCH']);
  });
});
