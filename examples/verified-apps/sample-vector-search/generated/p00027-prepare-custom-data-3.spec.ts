/**
 * GENERATED by apx-testgen from pages/p00027-prepare-custom-data-3.apx — DO NOT EDIT.
 ...
 * (./p00027-prepare-custom-data-3.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, data-uploaded-successfully, progress, summary
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Region types NOT covered by an auto-generated assertion (no verified DOM convention, or a runtime id genuinely unconstructible from static data -- see docs/grammar-assumptions.md "Still open" and ADR-003): buttons (staticContent), data-uploaded-successfully (staticContent), progress (list), summary (classicReport).
...
import { PrepareCustomData3Page } from './p00027-prepare-custom-data-3.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 27: Prepare Custom Data (Step 3: Vector Embeddings) [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new PrepareCustomData3Page(page);
  ...
  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new PrepareCustomData3Page(page);
    await po.goto();
    await expectButtonsPresent(page, ['Generate Vector Embeddings', 'Back']);
  });
});
