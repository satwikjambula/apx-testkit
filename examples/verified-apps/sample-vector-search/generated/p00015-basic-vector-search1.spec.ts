 ...
 * (./p00015-basic-vector-search1.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: about-basic-vector-search, breadcrumb, content, search, search-results-oci, search-results-openai
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): about-basic-vector-search (staticContent), breadcrumb (breadcrumb), content (staticContent), search (staticContent), search-results-oci (search), search-results-openai (search).
...
import { BasicVectorSearch1Page } from './p00015-basic-vector-search1.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 15: Basic Vector Search [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new BasicVectorSearch1Page(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new BasicVectorSearch1Page(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Basic Vector Search'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 0 hidden)', async ({ page }) => {
    const po = new BasicVectorSearch1Page(page);
    await po.goto();
    await expectItemsPresent(page, ['P15_SEARCH']);
  });

  test('apex.item round-trip on P15_SEARCH', async ({ page }) => {
    const po = new BasicVectorSearch1Page(page);
    await po.goto();
