/**
 * GENERATED by apx-testgen from pages/p00016-vector-vs-oracle-text1.apx — DO NOT EDIT.
 ...
 * (./p00016-vector-vs-oracle-text1.page.js), not raw testkit calls, so both stay in sync.
...
import { VectorVsOracleText1Page } from './p00016-vector-vs-oracle-text1.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 16: Vector vs. Oracle Text [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new VectorVsOracleText1Page(page);
  ...
  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 0 hidden)', async ({ page }) => {
    const po = new VectorVsOracleText1Page(page);
    await po.goto();
    await expectItemsPresent(page, ['P16_SEARCH']);
  });

  test('apex.item round-trip on P16_SEARCH', async ({ page }) => {
    const po = new VectorVsOracleText1Page(page);
    await po.goto();
