 ...
 * (./p00025-prepare-custom-data-1.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: about-this-page, buttons, loaded-file, progress, rds, upload-a-file, upload-file
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Region types NOT covered by an auto-generated assertion (no verified DOM convention, or a runtime id genuinely unconstructible from static data -- see docs/grammar-assumptions.md "Still open" and ADR-003): about-this-page (staticContent), buttons (staticContent), loaded-file (staticContent), progress (list), rds (regionDisplaySelector), upload-a-file (staticContent), upload-file (staticContent).
...
import { PrepareCustomData1Page } from './p00025-prepare-custom-data-1.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 25: Prepare Custom Data [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new PrepareCustomData1Page(page);
  ...
  test('every .apx pageItem exists in DOM/apex.item registry (3 items, 2 hidden)', async ({ page }) => {
    const po = new PrepareCustomData1Page(page);
    await po.goto();
    await expectItemsPresent(page, ['P25_FILE', 'P25_FILE_ERROR_ROW_COUNT', 'P25_FILE_NAME']);
  });
