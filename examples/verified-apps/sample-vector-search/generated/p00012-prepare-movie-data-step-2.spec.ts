 ...
 * (./p00012-prepare-movie-data-step-2.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: button-bar, data-generated-successfully, generated-csv, generating-data, progress
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): button-bar (staticContent), data-generated-successfully (staticContent), generated-csv (staticContent), generating-data (staticContent), progress (list).
...
import { PrepareMovieDataStep2Page } from './p00012-prepare-movie-data-step-2.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 12: Prepare Movie Data (Step 2: Preview) [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new PrepareMovieDataStep2Page(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new PrepareMovieDataStep2Page(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Prepare Movie Data'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (3 items, 2 hidden)', async ({ page }) => {
    const po = new PrepareMovieDataStep2Page(page);
    await po.goto();
    await expectItemsPresent(page, ['P12_AI_OUTPUT', 'P12_SOFAR', 'P12_TOTALWORK']);
  });

  test('apex.item round-trip on P12_AI_OUTPUT', async ({ page }) => {
    const po = new PrepareMovieDataStep2Page(page);
    await po.goto();
    await po.aiOutput.setValue('apx-testgen');
    expect(await po.aiOutput.getValue()).toBe('apx-testgen');
  });

  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new PrepareMovieDataStep2Page(page);
    await po.goto();
    await expectButtonsPresent(page, ['Back', 'Upload Data']);
  });
});
