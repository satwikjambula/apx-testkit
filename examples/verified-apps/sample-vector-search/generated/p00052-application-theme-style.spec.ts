 ...
 * (./p00052-application-theme-style.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: breadcrumb, set-theme-style
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Region types NOT covered by an auto-generated assertion (no verified DOM convention, or a runtime id genuinely unconstructible from static data -- see docs/grammar-assumptions.md "Still open" and ADR-003): breadcrumb (breadcrumb), set-theme-style (staticContent).
...
import { ApplicationThemeStylePage } from './p00052-application-theme-style.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 52: Application Theme Style [requires auth]', () => {
  ...
  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 0 hidden)', async ({ page }) => {
    const po = new ApplicationThemeStylePage(page);
    await po.goto();
    await expectItemsPresent(page, ['P52_DESKTOP_THEME_STYLE_ID']);
  });
