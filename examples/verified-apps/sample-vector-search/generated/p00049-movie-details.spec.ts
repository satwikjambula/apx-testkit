 ...
 * (./p00049-movie-details.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: buttons, movie-details-report
 ...
 * Region types NOT covered by an auto-generated assertion (no verified DOM convention, or a runtime id genuinely unconstructible from static data -- see docs/grammar-assumptions.md "Still open" and ADR-003): buttons (staticContent).
...
import { MovieDetailsPage } from './p00049-movie-details.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 49: Movie Details [requires auth]', () => {
  ...
  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 1 hidden)', async ({ page }) => {
    const po = new MovieDetailsPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P49_SEARCH_ID']);
  });

  test('every labeled .apx button exists (1 button)', async ({ page }) => {
    const po = new MovieDetailsPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Find Similar Movies']);
  });

  test('every interactiveReport/cards/facetedSearch region resolves (1 region)', async ({ page }) => {
    const po = new MovieDetailsPage(page);
    await po.goto();
    await expectRegionsResolve(page, ['movie-details-report']);
  });
});
