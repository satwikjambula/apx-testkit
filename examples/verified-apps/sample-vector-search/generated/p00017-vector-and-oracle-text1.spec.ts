 ...
 * (./p00017-vector-and-oracle-text1.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: about-vector-and-oracle-text-search, breadcrumb, content, oracle-text-not-available, search, search-results
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Region types NOT covered by an auto-generated assertion (no verified DOM convention, or a runtime id genuinely unconstructible from static data -- see docs/grammar-assumptions.md "Still open" and ADR-003): about-vector-and-oracle-text-search (staticContent), breadcrumb (breadcrumb), content (staticContent), oracle-text-not-available (staticContent), search (staticContent), search-results (search).
...
import { VectorAndOracleText1Page } from './p00017-vector-and-oracle-text1.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 17: Vector and Oracle Text [requires auth]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new VectorAndOracleText1Page(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new VectorAndOracleText1Page(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Vector and Oracle Text'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (1 items, 0 hidden)', async ({ page }) => {
    const po = new VectorAndOracleText1Page(page);
    await po.goto();
    await expectItemsPresent(page, ['P17_SEARCH']);
  });

  test('apex.item round-trip on P17_SEARCH', async ({ page }) => {
    const po = new VectorAndOracleText1Page(page);
    await po.goto();
