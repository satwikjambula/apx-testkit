 ...
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): button-oci (staticContent), button-openai (staticContent), enter-oci-web-credentials (staticContent), enter-openai-web-credentials (staticContent), introduction-wizard (staticContent), introduction-wizard-container (staticContent), select-provider (staticContent), unsupported-database-version (staticContent), web-credentials (staticContent), welcome-to-sample-vector-search (staticContent).
...
import { GettingStartedPage } from './p00100-getting-started.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 100: Getting Started [requires auth]', () => {
  ...
  test('every .apx pageItem exists in DOM/apex.item registry (11 items, 0 hidden)', async ({ page }) => {
    const po = new GettingStartedPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P100_AI_PROVIDER', 'P100_OCI_COMPARTMENT_ID', 'P100_OCI_FINGERPRINT', 'P100_OCI_PRIVATE_KEY', 'P100_OCI_REGION', 'P100_OCI_TENANCY_ID', 'P100_OCI_USER_ID', 'P100_OCI_WEB_CRED_HEADER', 'P100_OPENAI_API_KEY', 'P100_OPENAI_WEB_CRED_HEADER', 'P100_SHOW_WEB_CREDENTIALS']);
  });

  test('apex.item round-trip on P100_OCI_COMPARTMENT_ID', async ({ page }) => {
    const po = new GettingStartedPage(page);
    await po.goto();
  ...
  test('every labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new GettingStartedPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Complete Setup', 'Complete Setup']);
  });
});
