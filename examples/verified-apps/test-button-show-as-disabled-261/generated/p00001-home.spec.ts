...
test.describe('page 1: ホーム [requires auth]', () => {
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new HomePage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('ボタンの無効化'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (2 items, 0 hidden)', async ({ page }) => {
    const po = new HomePage(page);
    await po.goto();
    await expectItemsPresent(page, ['P1_ITEM1', 'P1_ITEM2']);
  });

  test('apex.item round-trip on P1_ITEM1', async ({ page }) => {
    const po = new HomePage(page);
    await po.goto();
    await po.item1.setValue('apx-testgen');
    expect(await po.item1.getValue()).toBe('apx-testgen');
  });

  test('every labeled .apx button exists (3 buttons)', async ({ page }) => {
    const po = new HomePage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Button', 'Disable Button', 'Enable Button']);
  });
});
