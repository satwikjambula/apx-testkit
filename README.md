...
|---|---|---|---|
| Page (alias/name/title) | ✅ | ✅ `gotoApexPage`/`normalizeTitle`; `assessNavigationSafety`/`navigateViaUiPath`/`gotoApexPageAuto` (runtime-review P0 item 2) detect `security.pageAccessProtection: argumentsMustHaveChecksum` on non-public pages — confirmed live to redirect a bare goto to `/login` — and provide a confirmed-working UI-click-path alternative instead of guessing | ✅ load + title; pages flagged navigation-unsafe (item 2) or `appearance.pageMode: modalDialog` (item 3, confirmed live to return HTTP 400 on direct GET — `isModalDialogUnroutable()`) get an unconditional, clearly-reasoned `test.skip()` instead of a normal test guaranteed to fail; the two reasons combine into a single skip when both apply (confirmed on 22+ real pages in the local corpus) |
| pageItem (text/number/select/date/hidden) | ✅ | ✅ `ApexItem` | ✅ presence + round-trip |
| Button | ✅ (label/action) + `ApexButton.htmlDomId` (typed; CORRECTED runtime-review P0 item 4 — an earlier claim that it was unset on every real button in the corpus was an overclaim based on grepping one app; a full-corpus sweep found 4 real buttons across 4 apps that DO set it — see below) + `ApexButton.target`/`ApexButton.url` (redirect destination: `target` for `redirectThisApp`/`redirectOtherApp` page/app redirects, `url` for the flat `redirectUrl` variant — `url`/`redirectUrl` DIRECTLY confirmed live, 17 real buttons in `ux-pattern-catalog`; `target`/`redirectThisApp` DIRECTLY confirmed via real parsed export data, 17 occurrences across 12 pages in `concurrent-manager`, `page`/`items`/`clearCache` all witnessed — `redirectOtherApp` specifically still unwitnessed — see `docs/grammar-assumptions.md`'s "Navigation Graph prerequisite pass" entry and `ApexButtonTarget`'s doc comment in `packages/parser/src/ast.ts` for the Fourteenth-round correction) | 🚧 accessible-role locator, no verified id convention; ✅ `expectButtonsPresent()`/`buttonsPresent()` confirmed live against 9 real buttons (Sample Charts, Area page); 🚧 `buttonByHtmlDomId()` exists (runtime-review P0 item 4) but is NOT YET LIVE-VERIFIED for buttons (unlike regions' identical mechanism, ADR-003); ✅ label collisions are detected, not silently trusted — `buttonByLabel()`'s optional `{ pageId, identifier }` identity carries through to coverage tracking (`recordButtonCoverageTouch`) so two different, same-labeled buttons are never collapsed into one entry | ✅ click methods (each carrying its button's `{ pageId, identifier }` semantic identity) + an auto-generated non-mutating presence assertion per page, for every labeled button; ✅ buttons whose label collides with another button's on the same page get NO generated click method at all — a clear comment instead, never a guessed/ambiguous one (confirmed real on UX Pattern Catalog's "Dashboard Advanced" page, 5 buttons sharing one label) |
| Region (generic) | ✅ (type/name/source) | ✅ `ApexRegion` — confirmed on 3 widget types; `expectRegionsResolve()` confirmed live for Interactive Report/Cards/Faceted Search (ADR-003 htmlDomId-resolved where set); `resolveRegion()`/`regionCandidatesFromAst()` (runtime-review P0 item 1) resolve a region's runtime id LIVE by trying each evidence-backed candidate against `apex.region()` in order and reporting which one worked, replacing a prior static `htmlDomId ?? identifier` guess with no runtime confirmation | ✅ auto-generated resolve-check per page for Interactive Report/Cards/Faceted Search regions (via `resolveRegion()`, not a baked-in id), with explicit skip notes for other types |
