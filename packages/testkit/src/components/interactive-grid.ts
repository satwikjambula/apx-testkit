 ...
 * Grids" gallery app, Basic Editing page, DOM static id `emp`).
 *
 ...
 * identifier IS the runtime static id apex.region() expects -- confirmed),
 * Interactive Grid's runtime static id can DIFFER from the .apx export's
 * region identifier. Confirmed live: a region declared as
 * `region basic-editing (type: interactiveGrid ...)` in the export resolved
 * at runtime to static id "emp" (DOM widget container `#emp_ig`) --
 ...
 * alone; you must discover the real static id yourself (e.g. inspect the
 * DOM for a container whose id is `<static id>_ig`) and pass it in by hand.
 *
  ...
  const result = await page.evaluate(
    ([id, method, args]: [string, string, unknown[]]) => {
      ...
      if (!region) {
        throw new Error(
          `apex.region('${id}') did not resolve -- not a recognized widget region. Interactive Grid's runtime ` +
            `static id often differs from its .apx export identifier -- see docs/quirks/26.1.json ` +
      ...
      if (typeof widget?.interactiveGrid !== 'function') {
        throw new Error(
          `apex.region('${id}').widget() did not expose an interactiveGrid() widget-factory method -- is '${id}' really an Interactive Grid region's static id?`,
        );
      }
