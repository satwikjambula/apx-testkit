 ...
 * (./p00020-my-account.page.js), not raw testkit calls, so both stay in sync.
 * Regions present in metadata: account-form
 * No interactiveReport/cards/facetedSearch regions on this page -- no region resolve-check to emit.
 * Other region types NOT covered by an auto-generated assertion (no verified DOM convention -- see docs/grammar-assumptions.md "Still open"): account-form (form).
...
import { MyAccountPage } from './p00020-my-account.page.js';
import { APP_BASE } from '../playwright.config.js';

test.describe('page 20: My Account [requires auth]', () => {
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new MyAccountPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('My Account'));
  });
  ...
  test('apex.item round-trip on P20_DISPLAY_NAME', async ({ page }) => {
    const po = new MyAccountPage(page);
    await po.goto();
    await po.displayName.setValue('apx-testgen');
