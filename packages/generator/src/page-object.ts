 ...
 * contract in lib.ts).
 */
import type { ApexApplication, ApexButton, ApexItem, ApexPage } from '@apx/parser';
...
/** Keep arbitrary metadata from terminating or spanning a generated comment. */
...
export function pageObjectBaseName(page: ApexPage): string {
  const path = safeFilePart(page.alias ?? '');
  return `p${String(page.id).padStart(5, '0')}-${path}.page`;
}

function safeFilePart(raw: string): string {
  return raw.toLowerCase().replace(/[^a-z0-9._-]+/g, '-').replace(/\.{2,}/g, '.');
}
...
export function specFileName(page: ApexPage): string {
  return `p${String(page.id).padStart(5, '0')}-${safeFilePart(page.alias ?? '')}.spec.ts`;
}
...
export function docsFileName(page: ApexPage): string {
  return `p${String(page.id).padStart(5, '0')}-${safeFilePart(page.alias ?? '')}.docs.md`;
}

function itemDoc(item: ApexItem): string {
  const bits = [item.type ?? 'unknown'];
  if (item.label) bits.push(`"${commentText(item.label)}"`);
  if (item.required) bits.push('required');
  return bits.join(', ');
}

export function pageObjectFor(page: ApexPage, application: ApexApplication | null = null): string {
  ...
  const friendlyUrls = application?.runtime.friendlyUrls ?? true;
  ...
  for (const button of page.buttons) {
    ...
    if (duplicateGroup) {
      ...
      const htmlDomIdNote = button.htmlDomId
        ? ` This button DOES declare advanced { htmlDomId: ${commentText(button.htmlDomId)} }, which COULD disambiguate it once a live-verified id-based locator strategy is confirmed for buttons (buttonByHtmlDomId() exists in @apx/testkit, marked NOT YET LIVE-VERIFIED -- see docs/quirks/26.1.json 'button-id-not-static-id') -- not auto-wired here until then.`
        : ' No advanced { htmlDomId } is set on this button, so there is currently no way to disambiguate it from a generated test alone.';
      buttonMethods.push(`  // Cannot generate a deterministic click method for '${commentText(button.identifier)}' ("${commentText(button.label!)}") --
  // its label is shared with ${others.map((id) => `'${commentText(id)}'`).join(', ')} on this same page, and buttonByLabel()'s
  // locator (page.getByRole('button', { name: '${commentText(button.label!)}', exact: true })) cannot tell them apart.${htmlDomIdNote}`);
      continue;
    }

    usesButtonByLabel = true;
    buttonMethods.push(`  /** "${commentText(button.label!)}" */
  ...
  return `/**
 * GENERATED by apx-testgen from pages/p${String(page.id).padStart(5, '0')}-${commentText(alias)}.apx — DO NOT EDIT.
...
import { ${page.items.length > 0 ? 'ApexItem, ' : ''}apexPageUrl${pageMode === 'modalDialog' ? '' : ', assessNavigationSafety'}${usesButtonByLabel ? ', clickButton' : ''}${pageMode === 'modalDialog' ? '' : ', gotoApexPageAuto'} } from '@apx/testkit';
...
export class ${className} {
  ...
  url(): string {
    return apexPageUrl(APP_BASE, ${className}.alias, ${friendlyUrls});
  }
...
${pageMode === 'modalDialog'
    ...
    : `    return gotoApexPageAuto(
      this.page,
      this.url(),
      assessNavigationSafety({ pageAccessProtection: ${pageAccessProtection === null ? 'null' : `'${esc(pageAccessProtection)}'`}, isPublic: ${isPublic} }),
    );`}
  }
