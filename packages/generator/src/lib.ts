...
export interface StaticTextResolution {
  value: string;
  /** APEX substitution tokens whose value still depends on runtime context. */
  unresolvedTokens: string[];
}

/**
 * Resolve only application substitutions whose static values are present in
 * the export. Oracle documents these as centrally defined static strings and
 * the raw 26.1 APEXlang EBNF exposes them as substitution-a/value.staticValue.
 *
 * This deliberately does NOT emulate APEX's full substitution engine. Built-in
 * values, page/application items, report columns, escape filters, template
 * placeholders, and nested substitutions remain unresolved and force the
 * caller to omit any exact assertion rather than guess a runtime value.
 */
export function resolveStaticApplicationSubstitutions(
  text: string,
  application: ApexApplication | null,
): StaticTextResolution {
  const values = new Map<string, string>();
  for (const substitution of application?.staticSubstitutions ?? []) {
    if (substitution.staticValue !== null) {
      values.set(substitution.name.toUpperCase(), substitution.staticValue);
    }
  }

  const value = text.replace(/&([A-Z][A-Z0-9_$#]*)\./gi, (token, name: string) => {
    return values.get(name.toUpperCase()) ?? token;
  });
  const unresolvedTokens = [
    ...new Set(
      value.match(/&(?:"[^"\r\n]+"|[A-Z0-9_$#]+)(?:![A-Z]+)?\.|#[A-Z0-9_$#]+#/gi) ?? [],
    ),
  ].sort();
  return { value, unresolvedTokens };
}
...
function specFor(page: ApexPage, application: ApexApplication | null): SpecForResult {
  ...
  const resolvedTitle = page.title
    ? resolveStaticApplicationSubstitutions(page.title, application)
    : null;
...
`;
  ...
  if (page.title && resolvedTitle && resolvedTitle.unresolvedTokens.length === 0) {
    bodyParts.push(`  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new ${className}(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('${esc(resolvedTitle.value)}'));
  });`);
  } else if (page.title && resolvedTitle) {
    bodyParts.push(
      `  // Exact title assertion omitted: the exported title contains runtime-dependent APEX ` +
        `substitution token(s) that cannot be resolved safely from static export data: ` +
        `${resolvedTitle.unresolvedTokens.map((token) => commentText(token)).join(', ')}.`,
    );
  }
